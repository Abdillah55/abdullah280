...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``nimro setup`` display).
    ...
    # built-ins).  Used by ``nimro gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `nimro`
        # invocation -- including plain `nimro chat`, which never touches any
        ...
        # `nimro setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``nimro setup``/``gateway status``, channel
