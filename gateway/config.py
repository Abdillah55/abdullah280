...
# dashboard's pre-write mutation validation (nimro_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Nimro is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_nimro_home() / "sessions")
    ...
    # gateway behaves exactly as before — single NIMRO_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.nimro/config.yaml (primary user-facing config)
    3. ~/.nimro/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_nimro_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through nimro_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `nimro config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `nimro config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``nimro config set gateway.streaming.*``
