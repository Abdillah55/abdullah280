...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Nimro Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to NIMRO_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/nimro), this function
    mirrors the bridge source to a writable NIMRO_HOME location and returns that
    ...
    # Try NIMRO_HOME location first
    nimro_home = get_nimro_home()
    nimro_home_bridge = nimro_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to NIMRO_HOME if needed
    if nimro_home_bridge.exists():
        return nimro_home_bridge

    # Mirror the bridge source to NIMRO_HOME
    try:
        nimro_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            nimro_home_bridge,
            dirs_exist_ok=False,
