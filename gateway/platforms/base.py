...
# Audio file extensions Nimro recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Nimro but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Nimro' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Nimro-created Telegram private-chat
    ...
    if (
        ...
        and raw_message.get("_nimro_no_thread_response")
    ):
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``NIMRO_SESSION_PLATFORM`` contextvar — that contextvar is
...
from nimro_constants import get_default_nimro_root, get_nimro_dir, get_nimro_home
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.nimro/.env manually."
)
...
IMAGE_CACHE_DIR = get_nimro_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_nimro_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_nimro_dir(new_subpath, old_name)
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        from nimro_cli.config import load_config_readonly as _load_config
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; NimroAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_nimro_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_nimro_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_nimro_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_nimro_dir("cache/screenshots", "browser_screenshots")
...
_NIMRO_ROOT = get_default_nimro_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "NIMRO_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "NIMRO_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "NIMRO_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "NIMRO_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _NIMRO_HOME / "image_cache",
    _NIMRO_HOME / "audio_cache",
    _NIMRO_HOME / "video_cache",
    _NIMRO_HOME / "document_cache",
    _NIMRO_HOME / "browser_screenshots",
    ...
    _NIMRO_HOME / "cache" / "images",
    _NIMRO_HOME / "cache" / "audio",
    _NIMRO_HOME / "cache" / "videos",
    _NIMRO_HOME / "cache" / "documents",
    _NIMRO_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Nimro root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* NIMRO_HOME's cache, so a gateway running at
    the root (e.g. ``NIMRO_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when NIMRO_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _NIMRO_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("NIMRO_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _NIMRO_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Nimro profile and shared Nimro root both contain control
    ...
    # NIMRO_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.nimro
    ...
    for nimro_root in (_NIMRO_HOME, _NIMRO_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(nimro_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Nimro
    secrets (``~/.nimro/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``NIMRO_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Nimro-managed cache, under an operator-allowlisted root
    (``NIMRO_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Nimro root (~/.nimro/.env,
    ...
    # ``MEDIA:~/.nimro/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?nimro\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+nimro[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``NIMRO_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Nimro commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-NIMRO_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("NIMRO_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "NIMRO_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "NIMRO_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Nimro sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-NIMRO_HOME holder may be replaced only when the runner
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Nimro home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Nimro home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.nimro/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from nimro_cli.commands import (
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          NIMRO_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          NIMRO_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          NIMRO_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("NIMRO_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("NIMRO_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("NIMRO_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_nimro_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # NIMRO_SESSION_PLATFORM contextvar the tool would
            ...
            # not at the top of this task.  _nimro_run_generation is set on
    ...
    def build_source(
        ...
        per-profile NIMRO_HOME isolation.
        """
