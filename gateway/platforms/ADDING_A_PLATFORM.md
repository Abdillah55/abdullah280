...
There are two ways to add a platform to the Nimro gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.nimro/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Nimro code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `nimro gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `nimro_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Nimro core.
...
And add it to the `nimro-gateway` composite:

```python
"nimro-gateway": {
    "includes": [..., "nimro-your-platform"]
}
```
...
## 12. Status Display (`nimro_cli/status.py`)
...
## 13. Gateway Setup Wizard (`nimro_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ nimro_cli/ toolsets.py \
  --include="*.py" -l | sort -u
