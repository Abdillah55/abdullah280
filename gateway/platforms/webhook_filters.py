...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.nimro to the active profile home."""
    ...
    if raw == "~/.nimro":
        return nimro_home
    if raw.startswith("~/.nimro/"):
        return nimro_home / raw.removeprefix("~/.nimro/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return nimro_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under NIMRO_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from nimro_constants import get_nimro_home

    scripts_root = (get_nimro_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.nimro" or raw_text.startswith("~/.nimro/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__nimro_ignore__") is True
        ):
            return False, None
