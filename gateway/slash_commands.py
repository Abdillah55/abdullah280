...
Module-level run.py helpers a handler needs (``_nimro_home``,
...
from nimro_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: nimro gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Nimro commands.
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`nimro gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _nimro_home
        ...
        try:
            ...
            atomic_json_write(
                _nimro_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _nimro_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Nimro Agent version."""
        from nimro_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _nimro_home, _load_gateway_config
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _nimro_home) / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from nimro_cli.context_switch_guard import (
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from nimro_cli.route_identity import should_clear_context_pin_async
                        ...
                        from nimro_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Nimro default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        config_path = _nimro_home / 'config.yaml'
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_nimro_home())
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Nimro itself changed. ``--stat`` limits output to the summary.
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.nimro/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``nimro skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _nimro_home
        from nimro_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `nimro skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.nimro/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _nimro_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _nimro_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from nimro_cli.session_listing import (
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `nimro bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``nimro debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Nimro Agent to the latest version.

        Spawns ``nimro update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``nimro update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _nimro_home, _resolve_nimro_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Nimro Agent')}"
        ...
        nimro_cmd = _resolve_nimro_bin()
        if not nimro_cmd:
            return t("gateway.update.nimro_cmd_not_found")

        pending_path = _nimro_home / ".update_pending.json"
        output_path = _nimro_home / ".update_output.txt"
        exit_code_path = _nimro_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `nimro update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `nimro update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # nimro_cmd is a list of argv parts we can pass directly
                ...
                nimro_cmd_str = " ".join(shlex.quote(part) for part in nimro_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {nimro_cmd_str} update --gateway"
