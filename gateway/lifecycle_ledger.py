...
``<NIMRO_HOME>/state/gateway.lifecycle.json``:
...
def _process_nimro_home() -> Path:
    """NIMRO_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<NIMRO_HOME>/state/gateway.lifecycle.json``."""
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``nimro_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
