...
class GatewayKanbanWatchersMixin:
        ...
        while self._running:
            try:
                def _collect():
                    ...
                    # NIMRO_KANBAN_DB pins the board path; without this guard
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        no separate `nimro kanban daemon` process needed. When false, the
        ...
        # watcher here. Honours NIMRO_KANBAN_DISPATCH_IN_GATEWAY env var
        ...
        try:
            ...
            return
        env_override = os.environ.get("NIMRO_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via NIMRO_KANBAN_DISPATCH_IN_GATEWAY env")
            return
            ...
            try:
                ...
                if _is_corrupt_board_db_error(exc):
                    ...
                    logger.error(
                        ...
                        "then run `nimro kanban init` if you need a fresh board.",
                        slug,
                        fingerprint[0],
                    )
                    return None
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Nimro profile
            ...
            PATH, missing venv, credential loss for a real Nimro profile).
            """
            try:
                boards = _kb.list_boards(include_archived=False)
            except Exception:
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            """
            try:
                from nimro_cli import kanban_decompose as _decomp
            ...
            for b in boards:
                ...
                prev_env = os.environ.get("NIMRO_KANBAN_BOARD")
                try:
                    os.environ["NIMRO_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        os.environ.pop("NIMRO_KANBAN_BOARD", None)
                    else:
                        os.environ["NIMRO_KANBAN_BOARD"] = prev_env
            return successes
                ...
                if bad_ticks >= HEALTH_WINDOW:
                    ...
                    if now - last_warn_at >= 300:
                        logger.warning(
                            ...
                            "`nimro kanban list --status ready`.",
                            bad_ticks,
                        )
                        last_warn_at = now
