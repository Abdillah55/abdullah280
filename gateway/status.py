...
The PID file lives at ``{NIMRO_HOME}/gateway.pid``.  NIMRO_HOME defaults to
``~/.nimro`` but can be overridden via the environment variable.  This means
separate NIMRO_HOME directories naturally get separate PID files — a property
...
from nimro_constants import get_nimro_home, _get_platform_default_nimro_home
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_nimro_home() / "gateway-starts.log"
...
def _get_process_nimro_home() -> Path:
    """Return the process-level NIMRO_HOME, skipping context-local overrides.
    ...
    ``get_nimro_home()`` honors ``_NIMRO_HOME_OVERRIDE`` contextvar used for
...
def _canonical_nimro_home(path: Path | str) -> Path:
    """Return a stable absolute NIMRO_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_nimro_home(left: Path | str, right: Path | str) -> bool:
    """Compare NIMRO_HOME paths with the host platform's case semantics."""
    return os.path.normcase(str(_canonical_nimro_home(left))) == os.path.normcase(
        str(_canonical_nimro_home(right))
    )


def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting NIMRO_HOME."""
    home = _get_process_nimro_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("NIMRO_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "nimro" / _LOCKS_DIRNAME
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Nimro' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Nimro gateway lifecycle subcommand from a command line.
    ...
    test also matched ``nimro_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\nimro-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Nimro's
    ...
    for token in tokens:
        ...
        if basename in ("nimro-gateway", "nimro-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "nimro_cli.main" in joined
        or "nimro_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("nimro", "nimro.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `nimro gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Nimro-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Nimro gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a NIMRO_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.nimro`` or ``$NIMRO_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``nimro_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``NIMRO_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a NIMRO_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"nimro_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit NIMRO_HOME= on the argv. NIMRO_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "nimro_home=" in command_lc and f"nimro_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # NIMRO_HOME-local.  Persist the owning gateway's process home so an
        # explicit cross-profile --replace can place its planned-takeover
        # marker where the target process will actually read it.
        "nimro_home": str(_canonical_nimro_home(_get_process_nimro_home())),
    }
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `nimro status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``NIMRO_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``nimro_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    NIMRO_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different NIMRO_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``nimro.service`` + ``nimro-
# gateway.service``).
#
...
def _get_takeover_marker_path(nimro_home: Optional[Path] = None) -> Path:
    """Return the path to the --replace takeover marker file.

    ``nimro_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = nimro_home or _get_process_nimro_home()
    return _canonical_nimro_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-NIMRO_HOME --replace while
    ...
    our_home = _get_process_nimro_home()
    target_home = record.get("target_nimro_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_nimro_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_nimro_home")
        if replacer_home is not None and not _same_nimro_home(
    ...
    # ``nimro gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's NIMRO_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_nimro_home(
            target_home or _get_process_nimro_home()
        )
        ...
        record = {
            ...
            "target_nimro_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_nimro_home": str(
                _canonical_nimro_home(_get_process_nimro_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target NIMRO_HOME's gateway PID record, and the live
    ...
    pid_record_home = pid_record.get("nimro_home")
    if not isinstance(pid_record_home, str) or not _same_nimro_home(
