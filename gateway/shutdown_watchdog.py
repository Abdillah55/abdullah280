...
2. An event-loop heartbeat file at ``<NIMRO_HOME>/state/gateway.heartbeat`` so
...
def _process_nimro_home() -> Path:
    """NIMRO_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<NIMRO_HOME>/state/gateway.heartbeat``."""
    base = home if home is not None else _process_nimro_home()
    ...
    *,
    ...
    def _watchdog() -> None:
        ...
        try:
            ...
            pass
        try:
            from nimro_logging import drain_log_queue
