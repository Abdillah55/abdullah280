...
PAIRING_DIR = get_nimro_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    ``nimro_cli.config.save_env_value`` / ``remove_env_value``, which target
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from nimro_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{NIMRO_HOME}/pairing`` while newer code/docs may
    write ``{NIMRO_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own NIMRO_HOME using the same legacy/consolidated layout rules
    as ``nimro -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current NIMRO_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp NIMRO_HOME
        ...
        if profile:
            ...
            # ``nimro -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `nimro` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> nimro pairing approve ...` as root, "
                    "re-run with `docker exec -u nimro <container> ...` and "
                    "chown the existing file to the nimro user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``nimro
