...
Defenses 1 and 2 (the ``_NIMRO_GATEWAY`` guard on ``nimro gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.nimro.gateway")``,
...
State lives in ``<NIMRO_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_nimro_home() / "gateway" / "restart_loop.json"
