"""Anthropic Messages API adapter for Nimro Agent.

Translates between Nimro's internal OpenAI-style message format and
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in nimro_cli/runtime_provider.py.
    """
...
# Nimro effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to nimro's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to nimro's outer conversation
    ...
    # from ``ANTHROPIC_API_KEY`` (Nimro loads that into the process env from
    # ``~/.nimro/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Nimro notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Nimro historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``nimro models``) never mutates ``~/.nimro/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Nimro)
      ...
      5. Anthropic credential_pool OAuth entry (~/.nimro/auth.json)
    ...
    # 1. Nimro-managed OAuth/setup token env var
    ...
    # 5. Nimro credential_pool OAuth entry.
...
# ── Nimro-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.nimro/.anthropic_oauth.json (our own file).
...
def _get_nimro_oauth_file() -> Path:
    return get_nimro_home() / ".anthropic_oauth.json"
...
def run_nimro_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Nimro-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Nimro with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from nimro_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_nimro_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Nimro-managed OAuth credentials from ~/.nimro/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Nimro OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See nimro-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See nimro-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See nimro-agent#13848 (Kimi) and
    nimro-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Nimro Agent", "Claude Code")
                text = text.replace("Nimro agent", "Claude Code")
                text = text.replace("nimro-agent", "claude-code")
        ...
        #      a) bare Nimro-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Nimro surfaces in its CLI. We
