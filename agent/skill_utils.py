...
from nimro_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.nimro/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``NIMRO_KANBAN_TASK`` / ``NIMRO_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("NIMRO_KANBAN_TASK") or os.getenv("NIMRO_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Nimro Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``nimro_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``NIMRO_PLATFORM`` or
            ``NIMRO_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("NIMRO_PLATFORM")
        or get_session_env("NIMRO_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``nimro`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.nimro/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``nimro`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against NIMRO_HOME, not cwd
        if not p.is_absolute():
            p = (nimro_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.nimro/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.nimro/skills/`` (including via symlinks) or configured
    ...
    # ~/.nimro/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Nimro can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    nimro = metadata.get("nimro") or {}
    if not isinstance(nimro, dict):
        nimro = {}
    return {
        "fallback_for_toolsets": nimro.get("fallback_for_toolsets", []),
        "requires_toolsets": nimro.get("requires_toolsets", []),
        "fallback_for_tools": nimro.get("fallback_for_tools", []),
        "requires_tools": nimro.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    nimro = metadata.get("nimro")
    if not isinstance(nimro, dict):
        return []
    raw = nimro.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Nimro metadata, VCS, virtualenv/dependency, cache, and skill
