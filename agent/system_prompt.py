...
upstream prefix cache warm.  See ``nimro-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``NIMRO_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("NIMRO_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Nimro never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want NIMRO_HOME persona while keeping
    ...
    # Pointer to the nimro-agent skill + docs for user questions about Nimro itself.
    stable_parts.append(NIMRO_AGENT_HELP_GUIDANCE)
    ...
    # NIMRO_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Nimro, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Nimro profile the agent is running
    # under so it doesn't conflate ~/.nimro/skills/ (default profile) with
    # ~/.nimro/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Nimro profile: default. Other profiles (if any) live "
            "under " + str(get_nimro_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        post_workspace_parts.append(
            f"Active Nimro profile: {active_profile}. This session reads "
            f"and writes {get_nimro_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_nimro_home()}/skills/, {get_nimro_home()}/plugins/, "
            f"{get_nimro_home()}/cron/, {get_nimro_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Nimro). Every other surface (desktop chat panel,
