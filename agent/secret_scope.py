...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the NIMRO_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Nimro runtime / deployment
    "NIMRO_HOME", "NIMRO_PROFILE", "NIMRO_GATEWAY_LOCK_DIR",
    "NIMRO_MAX_ITERATIONS", "NIMRO_MAX_TOKENS", "NIMRO_API_TIMEOUT",
    "NIMRO_REDACT_SECRETS", "NIMRO_NOUS_TIMEOUT_SECONDS",
    "_NIMRO_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "NIMRO_KANBAN_DB", "NIMRO_KANBAN_WORKSPACES_ROOT", "NIMRO_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "NIMRO_KANBAN_",
    "NIMRO_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Nimro writes
    ...
    ``nimro_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Nimro parser: save_env_value
    ...
    from nimro_cli.config import _parse_env_value
...
def build_profile_secret_scope(nimro_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(nimro_home)
    secrets = load_env_file(home / ".env")

    try:
        from nimro_cli.env_loader import get_secret_source_values
