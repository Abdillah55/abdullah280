...
MoA-enabled; the normal Nimro agent loop still owns tool calling and turn
...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from nimro_cli.moa_config import coerce_privacy_filter
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Nimro boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Nimro agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Nimro agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Nimro agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
...
class MoAChatCompletions:
    ...
    def create(self, **api_kwargs: Any) -> Any:
        ...
        from nimro_cli.config import get_config_path, load_config
