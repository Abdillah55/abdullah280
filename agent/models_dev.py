...
Data resolution order:
  ...
  2. Disk cache (~/.nimro/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Nimro ↔ models.dev
# ---------------------------------------------------------------------------

# Nimro provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Nimro (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_nimro_home() / "models_dev_cache.json"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``nimro config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Nimro provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Nimro agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Nimro provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Nimro ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Nimro or models.dev provider ID.  Tries exact match then
