"""Upload a Nimro session transcript to Hugging Face as an agent trace.

Nimro stores sessions in its own SQLite store (``nimro_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``nimro trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Nimro' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "nimro-traces"
_NIMRO_VERSION = "nimro-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Nimro OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Nimro' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Nimro conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.nimro/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `nimro trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/nimro-traces`` dataset. Returns a
