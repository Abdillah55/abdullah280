...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Nimro process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    raw = os.getenv("NIMRO_CONCURRENT_TOOL_TIMEOUT_S", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "invalid NIMRO_CONCURRENT_TOOL_TIMEOUT_S=%r; using %.0fs",
            raw,
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Nimro process before the normal turn-end persistence path runs.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Nimro policy and dispatch exactly once."""
    ...
    def _nimro_pipeline(relay_args: dict[str, Any]) -> Any:
