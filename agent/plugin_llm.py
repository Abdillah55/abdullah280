...
Plugins built on Nimro Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Nimro
...
:class:`~nimro_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Nimro' main config
...
handles every provider, fallback chain, and per-task override Nimro
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`nimro_cli.plugins.PluginContext`
