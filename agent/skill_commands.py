...
#
# When a user invokes a /skill (or /bundle), Nimro expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_nimro_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``NIMRO_PLATFORM`` env var and
    ``NIMRO_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("NIMRO_PLATFORM")
            or get_session_env("NIMRO_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("NIMRO_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.nimro.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_nimro_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.nimro/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
                ...
                try:
                    ...
                    # with a core Nimro slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Nimro command; skipping "
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.nimro/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``nimro -s <skill>`` or
    a deployment's ``NIMRO_TUI_SKILLS`` env var could force-load a skill an
