...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_NIMRO_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Nimro expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``nimro_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_nimro_home()`` themselves.
        """
        if "nimro_home" not in kwargs:
            from nimro_constants import get_nimro_home
            kwargs["nimro_home"] = str(get_nimro_home())
        for provider in self._providers:
            try:
