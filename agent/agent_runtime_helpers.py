...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Nimro-Session-Id header), and an
        ...
        if env_var_enabled("NIMRO_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from nimro_cli.config import load_config as _load_moa_cfg
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to nimro's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    try:
        ...
        from nimro_cli.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from nimro_cli.middleware import apply_tool_request_middleware
    ...
    from nimro_cli.middleware import run_tool_execution_middleware
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: nimro' own dispatch loop intentionally keeps an
...
def _iter_httpx_pool_objects(http_client: Any):
    ...
    Nimro' keepalive client (#10324 / ``_build_keepalive_http_client``) and
