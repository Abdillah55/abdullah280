...
row taxonomy is inferred from the atlas shape so Nimro can render both legacy
...
# rows (1536x1872), while older Nimro/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Nimro' activity state names. They are not always identical to the
...
# Legacy Nimro/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Nimro activity names -> accepted row-name aliases in descending
