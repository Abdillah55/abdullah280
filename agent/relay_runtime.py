"""Profile-scoped NeMo Relay runtimes owned by the Nimro agent core."""
...
SESSION_SCOPE = "nimro.session"
TURN_SCOPE = "nimro.turn"
LOGICAL_LLM_SCOPE = "nimro.logical_llm_call"
RUNTIME_SCHEMA_KEY = "nimro.relay.schema_version"
RUNTIME_SCHEMA_VERSION = "nimro.relay.runtime.v1"
RUNTIME_INSTANCE_KEY = "nimro.relay.runtime_instance"
_PROFILE_KEY_CACHE: dict[str, str] = {}


@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a Nimro session."""
...
class RelayRuntime:
    ...
    def managed_execution_enabled(self) -> bool:
        """Return whether a Nimro-managed consumer needs the Relay pipeline."""
        with self._execution_consumers_lock:
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active Nimro Relay session without creating one."""
        with self._sessions_lock:
    ...
    def get_session_handle(self, session_id: str) -> Any:
        """Return the Relay parent handle for a Nimro session, if active."""
        ...
        **kwargs: Any,
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("Nimro Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("Nimro Relay session context is unavailable")
    ...
    def emit_mark(
        ...
        *,
        ...
        """Emit a mark parented to the Nimro session identified by ``event``."""
    ...
    def apply_tool_request_intercepts(
        ...
        *,
        ...
        """Apply Relay request rewriting before Nimro authorizes a tool call."""
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Nimro Relay session %s closed with errors: %s",
                session_id,
                "; ".join(failures),
            )
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Nimro Relay runtime operation failed", exc_info=True)
            return None
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical Nimro profile."""
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "Nimro Relay runtime initialization failed", exc_info=True
                )
...
class RelayTurnContext:
    """Runtime-only context for one Nimro turn or top-level task."""
...
_CURRENT_TURN: contextvars.ContextVar[RelayTurnContext | None] = contextvars.ContextVar(
    "nimro_relay_turn", default=None
)


class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for Nimro core."""
    ...
    def _prepare_session(
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning(
                    "Nimro Relay session initializer failed: %s",
                    name,
                    exc_info=True,
                )
        ...
        *,
        ...
        if isinstance(host, RelayRuntime):
            try:
                ...
                metadata = {"nimro.execution_surface": platform or "unknown"}
                ...
                logger.warning(
                    "Nimro Relay conversation initialization failed",
                    exc_info=True,
                )
    ...
    def begin_turn(
        ...
        *,
        ...
        if lease.released:
            raise RuntimeError("Nimro Relay conversation lease is released")
            ...
            try:
                turn.handle = lease.host.run_in_session(
                    ...
                    metadata={
                        ...
                        "nimro.execution_surface": lease.platform or "unknown",
                    },
                )
            except Exception:
                logger.warning("Nimro Relay turn initialization failed", exc_info=True)
                ...
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        try:
                            ...
                            logger.warning(
                                "Nimro Relay turn finalization failed", exc_info=True
                            )
            finally:
                try:
                    ...
                    logger.warning(
                        "Nimro Relay child conversation finalization failed",
                        exc_info=True,
                    )
                finally:
        ...
        *,
        ...
        for index in range(len(logical_calls) - 1, -1, -1):
            ...
            try:
                ...
                logger.warning(
                    "Nimro Relay logical LLM finalization failed",
                    exc_info=True,
                )
                break
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a Nimro session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("Nimro Relay mark failed: %s", name, exc_info=True)
        return False
...
def apply_tool_request_intercepts(
    *,
    ...
    """Return Relay-rewritten arguments at Nimro's authorization boundary."""
    if not session_id:
        return args
    runtime = get_runtime(create=False)
    if runtime is None:
...
def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by Nimro core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("Nimro Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared Nimro session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Nimro Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("Nimro Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared Nimro session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Nimro Relay runtime is unavailable")
    session = runtime.get_session(session_id)
...
def get_runtime(
    *,
    ...
    """Return the Relay host for the active Nimro profile."""
