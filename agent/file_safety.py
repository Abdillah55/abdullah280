...
def _nimro_home_path() -> Path:
    """Resolve the active NIMRO_HOME (profile-aware) without circular imports."""
    try:
        from nimro_constants import get_nimro_home  # local import to avoid cycles
        return get_nimro_home()
    except Exception:
        return Path(os.path.expanduser("~/.nimro"))


def _nimro_root_path() -> Path:
    """Resolve the Nimro root dir (always the parent of any profile, never per-profile)."""
    try:
        from nimro_constants import get_default_nimro_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    nimro_home = _nimro_home_path()
    nimro_root = _nimro_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(nimro_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(nimro_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(nimro_home / "cache" / "bws_cache.enc.json"),
            str(nimro_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved NIMRO_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("NIMRO_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    nimro_dirs = []
    for base in (_nimro_home_path(), _nimro_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in nimro_dirs:
                nimro_dirs.append(real)
        except Exception:
            continue

    for base_real in nimro_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside NIMRO_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Nimro path.

    Three categories are blocked:

      * Internal Nimro cache files under ``NIMRO_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under NIMRO_HOME and the global Nimro
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.nimro/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active NIMRO_HOME (profile-aware) AND the global
    # Nimro root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (NIMRO_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    nimro_dirs: list[Path] = []
    for base in (_nimro_home_path(), _nimro_root_path()):
        try:
            real = base.resolve()
            if real not in nimro_dirs:
    ...
    for hd in nimro_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Nimro cache file "
    ...
    for hd in nimro_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Nimro credential store "
    ...
    for hd in nimro_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Nimro MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Nimro MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Nimro read (see
...
#
# Nimro profiles are separate NIMRO_HOME dirs under
...
#
# Reference: May 2026 incident where a nimro-security profile session
# edited skills under both ``~/.nimro/profiles/nimro-security/skills/``
# AND ``~/.nimro/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under NIMRO_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from NIMRO_HOME.
    ...
    """
    try:
        home_real = _nimro_home_path().resolve()
        root_real = _nimro_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Nimro scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Nimro entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Nimro profile {info['target_profile']!r}, but the "
...
#
#   <NIMRO_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.nimro/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.nimro/…`` segment and warns
# regardless of which Nimro profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.nimro/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.nimro`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.nimro/…`` and returns the
    index where the inner Nimro-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .nimro / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".nimro":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Nimro state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.nimro``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.nimro`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Nimro resolver to
    succeed, so it works correctly even when called from contexts where
    NIMRO_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Nimro process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real NIMRO_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.nimro/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.nimro/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host NIMRO_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Nimro state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Nimro "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real NIMRO_HOME. Use the "
