...
projection into Nimro' display, approval bridging, transcript projection into
...
"codex_app_server"`. Nimro' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Nimro secret — gateway
        ...
        spawn_env = nimro_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("NIMRO_KANBAN_TASK"):
            kanban_db = spawn_env.get("NIMRO_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "NIMRO_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("NIMRO_HOME", os.path.expanduser("~/.nimro")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "nimro",
        client_title: str = "Nimro Agent",
