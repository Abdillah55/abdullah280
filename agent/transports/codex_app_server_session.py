...
Owns one Codex thread per Nimro session. Drives `turn/start`, consumes
...
# Nimro' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_NIMRO_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Nimro/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Nimro session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _NIMRO_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("NIMRO_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="nimro",
            client_title="Nimro Agent",
            client_version=_get_nimro_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Nimro' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Nimro' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own nimro-tools callback we
            # auto-accept — the user already approved Nimro' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Nimro
        approval-mode/timeout logic. The Nimro-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Nimro approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Nimro approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Nimro returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Nimro approval mode/timeout resolution
    ...
    # "prompt expired"; the Nimro-side messaging already distinguishes them.
    return "decline"
...
def _get_nimro_version() -> str:
    """Best-effort Nimro version string for codex's userAgent line."""
    try:
