...
from nimro_constants import get_nimro_home, get_skills_dir, is_wsl
...
_NIMRO_MD_NAMES = (".nimro.md", "NIMRO.md")


def _find_nimro_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.nimro.md`` or ``NIMRO.md``.
    ...
    # could pick up a .nimro.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _NIMRO_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Nimro Agent, an intelligent AI assistant created by Nous Research. "
...
NIMRO_AGENT_HELP_GUIDANCE = (
    "You run on Nimro Agent (by Nous Research). When the user needs help with "
    "Nimro itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://nimro-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `nimro-agent` skill with skill_view(name='nimro-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.nimro/kanban.db`. Your task id is in "
    "`$NIMRO_KANBAN_TASK`; your workspace is `$NIMRO_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $NIMRO_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $NIMRO_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${NIMRO_KANBAN_BRANCH:-wt/$NIMRO_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$NIMRO_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`nimro profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `nimro kanban <verb>` for board operations. Use "
...
#
# The nimro-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to nimro-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "when Nimro has none; explicit Nimro YOLO uses a private unrestricted "
        ...
        "run `nimro computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Nimro "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Nimro terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Nimro desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Nimro WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Nimro itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Nimro runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Nimro itself is running. The host OS, home, and cwd "
                f"of the Nimro process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Nimro "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Nimro
    ...
    extra = (os.getenv("NIMRO_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_nimro_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("NIMRO_PLATFORM") or os.environ.get("NIMRO_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.nimro/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Nimro Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `nimro-agent` skill "
            "first. It has the actual commands (e.g. `nimro config set …`, `nimro tools`, "
            "`nimro setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: nimro setup, nimro setup tools, nimro setup terminal, nimro status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from NIMRO_HOME and return its content, or None.
    ...
    """
    try:
        from nimro_cli.config import ensure_nimro_home
        ensure_nimro_home()
    except Exception as e:
        logger.debug("Could not ensure NIMRO_HOME before loading SOUL.md: %s", e)

    soul_path = get_nimro_home() / "SOUL.md"
...
def _load_nimro_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.nimro.md / NIMRO.md — walk to git root."""
    nimro_md_path = _find_nimro_md(cwd_path)
    if not nimro_md_path:
        return ""
    try:
        content = nimro_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = nimro_md_path.name
        try:
            rel = str(nimro_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".nimro.md", context_length=context_length,
            read_path=str(nimro_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", nimro_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .nimro.md / NIMRO.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from NIMRO_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Nimro install/source
    ...
    # explicitly configured cwd is honored verbatim — the Nimro tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Nimro in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Nimro install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_nimro_md(cwd_path, context_length)
    ...
    # SOUL.md from NIMRO_HOME only — skip when already loaded as identity
    if not skip_soul:
