...
Every Nimro request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Nimro Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=nimro-agent",
        "client=nimro-client-v<__version__>",
    ]

The version is sourced live from ``nimro_cli.__version__`` so it auto-aligns
...
``nimro_cli.__version__`` is the canonical source of truth.
"""
...
def _nimro_version() -> str:
    """Return the current Nimro release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``nimro_cli`` cannot be imported (should
...
def nimro_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=nimro-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=nimro-client-v{_nimro_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Nimro session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Nimro conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.nimro/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Nimro
    ...
    """
    tags = ["product=nimro-agent", nimro_client_tag()]
