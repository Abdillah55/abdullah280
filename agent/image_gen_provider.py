...
as ``kind: backend``) or ``~/.nimro/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``nimro tools``
...
def _images_cache_dir() -> Path:
    """Return ``$NIMRO_HOME/cache/images/``, creating parents as needed."""
    from nimro_constants import get_nimro_home

    path = get_nimro_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$NIMRO_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$NIMRO_HOME/cache/images/``.
