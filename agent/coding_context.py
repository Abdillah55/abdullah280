"""Coding-context awareness — base Nimro, every interactive surface.

When the user runs Nimro inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Nimro shifts into a **coding posture**. This module is the
...
from nimro_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("nimro.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "nimro", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _NIMRO_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``NIMRO_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Nimro should operate in its coding posture right now."""
