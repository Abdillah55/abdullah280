...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Nimro with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.nimro/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Nimro
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``nimro tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``nimro setup`` for
