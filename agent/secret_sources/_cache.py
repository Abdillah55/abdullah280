...
cache problem must never block Nimro startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Nimro home used for cache paths.

    ``home_path`` is whatever ``load_nimro_dotenv()`` already resolved;
    falling back to ``$NIMRO_HOME`` / ``~/.nimro`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from nimro_constants import get_nimro_home

        home_path = get_nimro_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<nimro_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if NIMRO_HOME is ever made traversable.
            try:
