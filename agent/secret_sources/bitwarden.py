...
Nimro pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.nimro/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<nimro_home>/bin/bws`` on
  first use.  Nimro pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.nimro/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``nimro`` invocations don't
  hammer the API.
* Failures NEVER block Nimro startup.  Missing binary, no network,
...
# In-process cache so repeated load_nimro_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `nimro chat -q ...`
...
# <nimro_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.nimro/.env (which
...
_ENCRYPTED_CACHE_INFO = b"nimro-bws-encrypted-cache-v1"
...
def _nimro_bin_dir() -> Path:
    """Where Nimro stores its managed binaries.  Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<nimro_home>/bin/bws``  (our managed copy — preferred)
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``nimro secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="nimro-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _summarize_bws_stderr(raw: str) -> str:
    ...
    Everything from ``Location:`` on is diagnostic noise for a Nimro
...
# ---------------------------------------------------------------------------
# Public entry point — called from nimro_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_nimro_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `nimro secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `nimro secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `nimro secrets bitwarden setup` to install."
        )
        return result
    ...
    *fetches* — precedence, override semantics, conflict warnings, and
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            return (
                "Run `nimro secrets bitwarden token` to paste a fresh access "
                "token (create one in the Bitwarden web app: Secrets Manager → "
                "Machine accounts → Access tokens).  Wrong region?  Re-run "
                "`nimro secrets bitwarden setup` and pick EU/self-hosted."
            )
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`nimro secrets bitwarden token`) so the
