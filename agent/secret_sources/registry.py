...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``nimro`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's NIMRO_HOME is ``~/.nimro/profiles/<name>``; the
    default profile (``~/.nimro``) returns "".
    """
    if home_path is not None:
        resolved = Path(home_path)
        if resolved.parent.name == "profiles" and resolved.name:
            return resolved.name
    for env_name in ("NIMRO_PROFILE_NAME", "NIMRO_PROFILE"):
