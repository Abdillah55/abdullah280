...
Storage
-------
Bundles live in ``~/.nimro/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``NIMRO_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``nimro bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under NIMRO_HOME.

    Honors ``NIMRO_BUNDLES_DIR`` for tests; falls back to
    ``<NIMRO_HOME>/skill-bundles``.
    """
    override = os.environ.get("NIMRO_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_nimro_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `nimro bundles` CLI subcommand.
# ---------------------------------------------------------------------------
