...
def _base_attrs(*, profile: str, install_id: str, version: str, supervision_mode: str) -> Dict[str, str]:
    ...
    return {
        ...
        "nimro.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
...
def build_gateway_health_snapshot(
    ...
    *,
    ...
    metrics: list[GatewayMetric] = [
        _metric("nimro.gateway.up", 1 if gateway_running else 0, base),
        _metric("nimro.gateway.active_agents", active_agents, base),
        _metric("nimro.gateway.busy", 1 if busy else 0, base),
        _metric("nimro.gateway.drainable", 1 if drainable else 0, base),
        _metric("nimro.gateway.restart_requested", 1 if runtime.get("restart_requested") else 0, base),
    ]
    if gateway_state:
        metrics.append(_metric("nimro.gateway.state", 1, base, **{"nimro.gateway.state": str(gateway_state)}))
    ...
    for platform, pdata in platforms.items():
        ...
        metrics.append(_metric(
            ...
            1 if is_up else 0,
            base,
            **{"nimro.platform": str(platform), "nimro.platform.state": state},
        ))
        metrics.append(_metric(
            "nimro.platform.degraded",
            1 if is_degraded else 0,
            base,
            **{"nimro.platform": str(platform), "nimro.platform.state": state, "nimro.error_code": error_code},
        ))
        if is_degraded:
