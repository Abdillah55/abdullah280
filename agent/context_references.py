...
_SENSITIVE_NIMRO_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    nimro_home = get_nimro_home().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(nimro_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(nimro_home / rel for rel in _SENSITIVE_NIMRO_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Nimro path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.nimro/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Nimro path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:
