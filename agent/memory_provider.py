...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `nimro backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - nimro_home (str): The active NIMRO_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.nimro``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "nimro").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'nimro memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], nimro_home: str) -> None:
        ...
        Called by 'nimro memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``nimro_home`` is the active NIMRO_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE NIMRO_HOME.

        ``nimro backup`` only walks NIMRO_HOME, so any provider state kept
        ...
        of the archive, and ``nimro import`` restores them to their original
