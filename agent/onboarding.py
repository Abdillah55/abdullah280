...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Nimro starts and finds ``~/.openclaw/``.

    Points users at ``nimro claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``nimro claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Nimro, run "
        "`nimro claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `nimro claw cleanup` (renames it to ~/.openclaw.pre-migration — "
