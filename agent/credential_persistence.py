...
``auth.json``.  They intentionally have no dependency on ``nimro_cli.auth`` so
...
# Sources Nimro owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Nimro-owned OAuth/device-code state)
