...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with nimro-agent)
2. User plugins: ``$NIMRO_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$NIMRO_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$NIMRO_HOME/plugins/model-providers/`` if it exists."""
    try:
        from nimro_constants import get_nimro_home

        d = get_nimro_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple NIMRO_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_nimro_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$NIMRO_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with nimro-agent.
    ...
    # 2. User plugins — under $NIMRO_HOME/plugins/model-providers/<name>/.
