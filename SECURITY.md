# Nimro Agent Security Policy

This document describes Nimro Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/nimro-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Nimro Agent does not operate a bug
bounty program.**
...
- Environment details (`nimro version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Nimro Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Nimro Agent's documentation
...
Nimro Agent supports two OS-level isolation postures. They address
...
Nimro Agent supports this in two ways:

- **Nimro Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Nimro Agent's in-process heuristics
...
Nimro Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Nimro Agent
itself. Bugs in Nimro Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Nimro Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Nimro Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Nimro Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  nimro-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Nimro Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Nimro Agent. Bugs in Nimro
...
- Nimro Agent includes supply-chain guards for MCP server
