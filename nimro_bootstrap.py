"""Windows UTF-8 bootstrap for Nimro entry points.
...
should be imported at the very top of every Nimro entry point
(``nimro``, ``nimro-agent``, ``nimro-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``nimro_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``nimro_bootstrap``, never ``nimro_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Nimro modules.

    Nimro ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Nimro source root
    ...
    """
    root = src_root or os.environ.get("NIMRO_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``NIMRO_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("NIMRO_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import nimro_bootstrap``
# (or ``from nimro_bootstrap import apply_windows_utf8_bootstrap``) at
