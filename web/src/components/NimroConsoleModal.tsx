...
interface NimroConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function NimroConsoleModal({ open, onClose }: NimroConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("nimro> ");
  const inputPromptRef = useRef("nimro> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "nimro> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Nimro Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="nimro-console-title"
    >
      <div
        className={cn(
          themedBody,
