...
const SIDEBAR_COLLAPSED_KEY = "nimro-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="nimro-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="nimro-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateNimro = status?.can_update_nimro === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'nimro update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "nimro update";
    return (
      t.status.updateNimroConfirmMessage ??
      `This will run 'nimro update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateNimroConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateNimro) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateNimro,
      runningLabel: t.status.updatingNimro,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Nimro gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateNimroConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateNimroConfirmTitle ?? `${t.status.updateNimro}?`}
    />
    </>
  );
}
