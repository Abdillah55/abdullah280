...
import { api, NIMRO_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "nimro:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${NIMRO_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${NIMRO_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?nimro_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-nimro-plugin", manifest.name);
