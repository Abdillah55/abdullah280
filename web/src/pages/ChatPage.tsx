/**
 * ChatPage — embeds `nimro --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (nimro_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "nimro.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__NIMRO_SESSION_TOKEN__ &&
    !window.__NIMRO_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `nimro dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__NIMRO_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Nimro TUI has enabled xterm mouse-events
    ...
    //      payloads upload to NIMRO_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to NIMRO_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[nimro-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__NIMRO_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets NIMRO_HOME pointed at the
    ...
    // forwarding them into Nimro. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="nimro-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
