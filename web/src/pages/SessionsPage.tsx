...
const SOURCE_CONFIG: Record<string, { icon: typeof Terminal; color: string }> =
  {
    ...
    nimro_flow: { icon: Play, color: "text-warning" },
...
const NO_MATCHING_SESSION_SOURCE = "__nimro_dashboard_no_matching_source__";
    ...
    async (id: string) => {
      try {
        const res = await fetch(api.exportSessionUrl(id), {
          credentials: "include",
          headers: {
            "X-Nimro-Session-Token":
              (window as unknown as { __NIMRO_SESSION_TOKEN__?: string })
                .__NIMRO_SESSION_TOKEN__ ?? "",
          },
        });
