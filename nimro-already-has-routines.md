# Nimro Agent Has Had "Routines" Since March
...
Nimro equivalent — works today:
```bash
nimro webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Nimro implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Nimro Agent |
|---|---|---|
...
## Things Nimro Does That Routines Can't
...
Nimro has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Nimro Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
nimro gateway setup    # enable webhooks
nimro webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [nimro-agent.nousresearch.com/docs/reference/automation-blueprints-catalog](https://nimro-agent.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [nimro-agent.nousresearch.com](https://nimro-agent.nousresearch.com)

GitHub: [github.com/NousResearch/nimro-agent](https://github.com/NousResearch/nimro-agent)

---

*Nimro Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
