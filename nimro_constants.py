"""Shared constants for Nimro Agent.
...
_UNSET = object()
_NIMRO_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_NIMRO_HOME_OVERRIDE", default=_UNSET
)
...
def set_nimro_home_override(path: str | Path | None) -> Token:
    """Set a context-local Nimro home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _NIMRO_HOME_OVERRIDE.set(value)


def reset_nimro_home_override(token: Token) -> None:
    """Restore the previous context-local Nimro home override."""
    _NIMRO_HOME_OVERRIDE.reset(token)


def get_nimro_home_override() -> str | None:
    """Return the active context-local Nimro home override, if any."""
    override = _NIMRO_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_nimro_home() -> Path:
    """Return the platform-native default Nimro home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "nimro"
    return Path.home() / ".nimro"


def _nimro_home_from_env() -> Path:
    """Resolve NIMRO_HOME from the process environment only.

    Reads the ``NIMRO_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_nimro_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_nimro_home`
    and :func:`get_process_nimro_home` so the two never drift.
    """
    val = os.environ.get("NIMRO_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_nimro_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``NIMRO_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_nimro_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[NIMRO_HOME fallback] NIMRO_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass NIMRO_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_nimro_home() -> Path:
    """Return the Nimro home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_nimro_home_override`) → ``NIMRO_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``NIMRO_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``NIMRO_HOME`` explicitly (see the systemd
    template in ``nimro_cli/gateway.py`` and the kanban dispatcher in
    ``nimro_cli/kanban_db.py``).  See https://github.com/NousResearch/nimro-agent/issues/18594.
    """
    override = get_nimro_home_override()
    if override:
        return Path(override)

    if not os.environ.get("NIMRO_HOME", "").strip():
        _warn_profile_fallback_once()

    return _nimro_home_from_env()


def get_process_nimro_home() -> Path:
    """Return the Nimro home for the running process, ignoring task overrides.

    Unlike :func:`get_nimro_home`, this never follows the context-local
    override set by :func:`set_nimro_home_override`.  It resolves only the
    process ``NIMRO_HOME`` env var (falling back to the platform default),
...
def get_default_nimro_root() -> Path:
    """Return the root Nimro directory for profile-level operations.

    In standard deployments this is the platform-native Nimro home
    (``~/.nimro`` on POSIX, ``%LOCALAPPDATA%\\nimro`` on native Windows).

    In Docker or custom deployments where ``NIMRO_HOME`` points outside
    ``~/.nimro`` (e.g. ``/opt/data``), returns ``NIMRO_HOME`` directly
    — that IS the root.

    In profile mode where ``NIMRO_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.nimro/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_nimro_home()
    env_home = os.environ.get("NIMRO_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # NIMRO_HOME is under ~/.nimro (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — NIMRO_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``NIMRO_OPTIONAL_SKILLS``.
    """
    override = os.getenv("NIMRO_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_nimro_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``NIMRO_OPTIONAL_MCPS``.
    """
    override = os.getenv("NIMRO_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_nimro_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``NIMRO_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<NIMRO_HOME>/skills`` last-resort
    """
    override = os.getenv("NIMRO_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_nimro_home() / "skills"
...
def get_nimro_dir(
    ...
    *,
    ...
    """Resolve a Nimro subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to NIMRO_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to NIMRO_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Nimro home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local NIMRO_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_nimro_home()
...
def iter_nimro_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Nimro-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\nimro\\node``. POSIX installs use
    ``$NIMRO_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_nimro_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with nimroManagedNodePathEntries() in
...
_NIMRO_NODE_TARGET_MAJOR = int(os.environ.get("NIMRO_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Nimro-managed Node trees live under ``$NIMRO_HOME/node`` (or a profile's
    ``NIMRO_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_nimro_node_executable``
    used to trust file presence alone, so ``nimro update`` would pick that
...
def nimro_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Nimro-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_nimro_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%NIMRO_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_nimro_home()
    index_url = f"https://nodejs.org/dist/latest-v{_NIMRO_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_NIMRO_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def _bootstrap_managed_node_posix() -> bool:
    """Install a fresh managed Node under ``$NIMRO_HOME/node`` on POSIX.
    ...
    ``NIMRO_NODE_SKIP_LINKS=1`` so the user's own node/npm on PATH is not
...
def bootstrap_nimro_managed_node() -> str | None:
    """Install a Nimro-managed Node tree and return its npm path.
    ...
    Nimro never modifies a toolchain it does not own, so instead it provisions
    its own tree under ``$NIMRO_HOME/node`` (the same tree a fresh install
    ...
    present.
    """
    existing = find_nimro_node_executable("npm")
    if existing:
        return existing
    ...
    for directory in iter_nimro_node_dirs():
...
def heal_nimro_managed_node() -> bool:
    """Redownload Nimro-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not nimro_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "NIMRO_HOME": str(get_nimro_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def _managed_node_tree_outdated(home: Path | None = None) -> bool:
    ...
    same way a broken one does: :func:`find_nimro_node_executable` triggers
    the once-per-process heal, which redownloads
    ``latest-v{_NIMRO_NODE_TARGET_MAJOR}.x`` — so existing users are upgraded
    ...
    for directory in iter_nimro_node_dirs(home):
        for name in _candidate_node_command_names("node"):
            ...
            return major < _NIMRO_NODE_TARGET_MAJOR
    return False


def find_nimro_node_executable(command: str) -> str | None:
    """Return a Nimro-managed Node/npm executable path, healing broken trees.

    Outdated trees (node major below ``_NIMRO_NODE_TARGET_MAJOR``) heal the
    ...
    if needs_heal and heal_nimro_managed_node():
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Nimro-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Nimro-managed installs.

    This is for Nimro-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_nimro_node_executable(command)
    if managed:
        return managed
    if nimro_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_nimro_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Nimro-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_nimro_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``nimro update`` — leaving a **dangling symlink** that ``which`` still
...
def display_nimro_home() -> str:
    """Return a user-friendly display string for the current NIMRO_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.nimro``
        profile:  ``~/.nimro/profiles/coder``
        custom:   ``/opt/nimro-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.nimro``.  For code that needs a real ``Path``, use
    :func:`get_nimro_home` instead.
    """
    home = get_nimro_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``NIMRO_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/nimro-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{NIMRO_HOME}/home`` when the profile-home directory exists."""
    nimro_home = get_nimro_home_override() or (env or {}).get("NIMRO_HOME") or os.getenv("NIMRO_HOME")
    if not nimro_home:
        return None
    profile_home = os.path.join(nimro_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("NIMRO_REAL_HOME") or os.getenv("NIMRO_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Nimro profile HOME.

    ``NIMRO_HOME`` scopes Nimro state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={NIMRO_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{NIMRO_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{NIMRO_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Nimro' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["NIMRO_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Nimro itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: NousResearch/nimro-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under NIMRO_HOME.

    Replaces the ``get_nimro_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, nimro_logging.py, nimro_time.py, etc.).
    """
    return get_nimro_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under NIMRO_HOME."""
    return get_nimro_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under NIMRO_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_nimro_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._nimro_ipv4_patched = True  # type: ignore[attr-defined]
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    across four ``nimro_cli`` modules using three different Windows
    ...
    A few sites outside ``nimro_cli`` (``tools/code_execution_tool.py``,
    ...
    *windows* lets a caller pass its own platform verdict. Several callers
    ...
    Windows paths on Linux CI (``nimro_cli.main._is_windows`` and friends);
...
# Top-level packages/modules that ship as part of Nimro itself. An ImportError
...
# `nimro_cli.update_cmd`'s post-update probe consumes this same set so the
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* is a module that ships with Nimro.
    ...
    if not root:
        return False
    return root in FIRST_PARTY_MODULE_ROOTS or root.startswith("nimro_")
...
def partial_update_hint(exc: BaseException) -> list[str]:
    ...
    rather than their config, is the problem — and `nimro update` is exactly
    ...
    return [
        ...
        "    nimro update",
        "If that also fails, reinstall: https://nimro-agent.nousresearch.com",
    ]
