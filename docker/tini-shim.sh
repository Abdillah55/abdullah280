...
# Nimro WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: NIMRO_TINI_SHIM_TARGET overrides the `/init` path (and
# NIMRO_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${NIMRO_TINI_SHIM_TARGET:-/init}"
WRAPPER="${NIMRO_TINI_SHIM_WRAPPER:-/opt/nimro/docker/main-wrapper.sh}"
