#!/bin/sh
# shellcheck shell=sh
# /opt/nimro/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching NIMRO_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `nimro` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `nimro <args>` (subcommand passthrough)
#
# Drop to nimro via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${NIMRO_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${NIMRO_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export NIMRO_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset NIMRO_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid nimro "$@"; exec "$@"; }
...
# an arbitrary non-root, non-nimro UID, the bootstrap was skipped and the
# baked image dirs (owned by the nimro build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u nimro)" ]; then
    cat >&2 <<EOF
[nimro] ERROR: container started with --user $cur_uid (an arbitrary, non-nimro UID) — not supported.
...
# to the nimro user's home before dropping privileges so libraries that
...
_nimro_orig_cwd="${NIMRO_ORIG_CWD:-$PWD}"
...
# the user's command so `nimro chat` starts in the Docker -w
