...
#
# Wired into the image as /etc/cont-init.d/01-nimro-setup by the
...
NIMRO_HOME="${NIMRO_HOME:-/opt/data}"
INSTALL_DIR="/opt/nimro"

# Drop to nimro via s6-setuidgid, but skip it when already non-root.
as_nimro() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid nimro "$@"; }
...
# starts non-root. The baked install tree under /opt/nimro is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $NIMRO_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass NIMRO_UID/NIMRO_GID — or the PUID/PGID aliases — which the
...
# `id -u nimro` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# nimro UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u nimro)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-nimro UID).
...
and the baked /opt/nimro install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e NIMRO_UID=\$(id -u) -e NIMRO_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the nimro user to that UID/GID at boot and chowns the data
...
# --- Bootstrap NIMRO_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid nimro mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom NIMRO_HOME paths whose parents only
# root can create (e.g. `NIMRO_HOME=/home/nimro/.nimro` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for NIMRO_UID/NIMRO_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  NIMRO_UID/
# NIMRO_GID still win when both are set.  See #15290, salvages #25872.
NIMRO_UID="${NIMRO_UID:-${PUID:-}}"
NIMRO_GID="${NIMRO_GID:-${PGID:-}}"

if [ -n "${NIMRO_UID:-}" ] && validate_uid_gid "$NIMRO_UID" && [ "$NIMRO_UID" != "$(id -u nimro)" ]; then
    echo "[stage2] Changing nimro UID to $NIMRO_UID"
    usermod -u "$NIMRO_UID" nimro
fi
if [ -n "${NIMRO_GID:-}" ] && validate_uid_gid "$NIMRO_GID" && [ "$NIMRO_GID" != "$(id -g nimro)" ]; then
    echo "[stage2] Changing nimro GID to $NIMRO_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$NIMRO_GID" nimro 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised nimro user (UID 10000)
...
# nimro process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes nimro. Idempotent across container
...
#   - nimro is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G nimro 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] nimro already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" nimro 2>/dev/null; then
        echo "[stage2] Added nimro to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group nimro failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When NIMRO_UID is remapped or the top-level $NIMRO_HOME isn't owned by
# the runtime nimro UID, restore ownership to nimro — but ONLY for the
# directories nimro actually writes to. The full $NIMRO_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of nimro-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_nimro_uid=$(id -u nimro)
...
chown_nimro_tree() {
    ...
    chown -R nimro:nimro "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_nimro_owner() {
    target="$1"
    find "$target" \( ! -user nimro -o ! -group nimro \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$NIMRO_HOME" 2>/dev/null)" != "$actual_nimro_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $NIMRO_HOME (targeted) to nimro ($actual_nimro_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $NIMRO_HOME: chown the directory itself (not its contents)
    # so nimro can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$NIMRO_HOME"; then
        :
    else
        chown nimro:nimro "$NIMRO_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $NIMRO_HOME failed (rootless container?) — continuing"
    fi
    # Nimro-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by nimro (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$NIMRO_HOME/$sub" ] && tree_has_non_nimro_owner "$NIMRO_HOME/$sub"; then
            chown_nimro_tree "$NIMRO_HOME/$sub"
        fi
    done
fi
...
# nimro user. Hosted/container instances keep mutable state under
# $NIMRO_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# NIMRO_DISABLE_LAZY_INSTALLS=1. Keeping /opt/nimro root-owned and
...
# $NIMRO_HOME/lazy-packages dir on the data volume (Dockerfile sets
# NIMRO_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to nimro
...
# Always reset ownership of $NIMRO_HOME/profiles to nimro on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> nimro …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as nimro and walks
...
if [ -d "$NIMRO_HOME/profiles" ] && tree_has_non_nimro_owner "$NIMRO_HOME/profiles"; then
    chown_nimro_tree "$NIMRO_HOME/profiles"
fi

# Always reset ownership of $NIMRO_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged nimro runtime even
...
# gate as profiles/).
if [ -d "$NIMRO_HOME/cron" ] && tree_has_non_nimro_owner "$NIMRO_HOME/cron"; then
    chown_nimro_tree "$NIMRO_HOME/cron"
fi

# Always ensure logs/gateways is nimro-owned (#45258). Formerly healed by
...
# top-level $NIMRO_HOME is mis-owned, so a warm volume with nimro-owned
# NIMRO_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid nimro mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as nimro.
if [ -d "$NIMRO_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$NIMRO_HOME/logs/gateways"; then
        :
    else
        chown nimro:nimro "$NIMRO_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# nimro pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged nimro gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $NIMRO_HOME is
...
# and cron/.
if [ -d "$NIMRO_HOME/platforms/pairing" ] && tree_has_non_nimro_owner "$NIMRO_HOME/platforms/pairing"; then
    chown_nimro_tree "$NIMRO_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$NIMRO_HOME/pairing" ] && tree_has_non_nimro_owner "$NIMRO_HOME/pairing"; then
    chown_nimro_tree "$NIMRO_HOME/pairing"
fi

# Reset ownership of nimro-owned top-level state files on every boot.
# The targeted data-volume chown above only covers nimro-owned
# *subdirectories*; loose state files living directly under $NIMRO_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> nimro …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged nimro runtime then hits
...
# sweep so host-owned files in a bind-mounted $NIMRO_HOME are never
...
# entries of nimro_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$NIMRO_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$NIMRO_HOME/$f"; then
            :
        else
            chown nimro:nimro "$NIMRO_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the nimro runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$NIMRO_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$NIMRO_HOME/config.yaml"; then
        :
    else
        chown nimro:nimro "$NIMRO_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$NIMRO_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as nimro user ---
# Run as nimro via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $NIMRO_HOME values
...
as_nimro mkdir -p \
    ...
    "$NIMRO_HOME/sessions" \
    "$NIMRO_HOME/logs" \
    "$NIMRO_HOME/logs/gateways" \
    "$NIMRO_HOME/hooks" \
    "$NIMRO_HOME/memories" \
    "$NIMRO_HOME/skills" \
    "$NIMRO_HOME/skins" \
    "$NIMRO_HOME/plans" \
    "$NIMRO_HOME/workspace" \
    "$NIMRO_HOME/home" \
    "$NIMRO_HOME/pairing" \
    "$NIMRO_HOME/platforms/pairing" \
    "$NIMRO_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/nimro/.install_method (see Dockerfile), NOT written here into
# $NIMRO_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $NIMRO_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.nimro:/opt/data) and sometimes shared with a
...
# 'nimro update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $NIMRO_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/nimro stamp, so deleting the data-dir copy is safe).
if [ -f "$NIMRO_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$NIMRO_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$NIMRO_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$NIMRO_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$NIMRO_HOME/$dest"; then
            :
        else
            as_nimro cp "$INSTALL_DIR/$src" "$NIMRO_HOME/$dest"
        fi
    fi
}
...
if [ -f "$NIMRO_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$NIMRO_HOME/.env"; then
        :
    else
        chown nimro:nimro "$NIMRO_HOME/.env" 2>/dev/null || true
        chmod 600 "$NIMRO_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $NIMRO_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `nimro update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set NIMRO_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$NIMRO_HOME/config.yaml" ]; then
    s6-setuidgid nimro "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$NIMRO_HOME/auth.json" ] && [ -n "${NIMRO_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$NIMRO_HOME/auth.json"; then
        :
    else
        printf '%s' "$NIMRO_AUTH_JSON_BOOTSTRAP" > "$NIMRO_HOME/auth.json"
        chown nimro:nimro "$NIMRO_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$NIMRO_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via NIMRO_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$NIMRO_HOME/auth.json" ] && [ -n "${NIMRO_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$NIMRO_HOME/auth.json"; then
        :
    else
        s6-setuidgid nimro "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$NIMRO_HOME/auth.json" \
...
# can set NIMRO_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# NIMRO_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$NIMRO_HOME/gateway_state.json" ] && \
        [ "${NIMRO_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$NIMRO_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$NIMRO_HOME/gateway_state.json"
        chown nimro:nimro "$NIMRO_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$NIMRO_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_nimro "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/nimro/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Nimro spawns for the
...
# main-wrapper.sh propagates it into the supervised ``nimro`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-nimro, dashboard, etc.).
