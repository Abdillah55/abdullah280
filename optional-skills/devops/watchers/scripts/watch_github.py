...
Usage (via cron with --no-agent):
    ...
    nimro cron create nimro-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$NIMRO_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name nimro-issues --repo NousResearch/nimro-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Nimro .env file
(``${NIMRO_HOME:-~/.nimro}/.env``) to avoid the 60 req/hr
