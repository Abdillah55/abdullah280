...
Usage (via cron with --no-agent):
    ...
    nimro cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$NIMRO_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Nimro-Watcher/1.0"})
