...
All per-subject data lives under PDD_DATA_DIR (default: $NIMRO_HOME/unbroker),
which is the same trust boundary Nimro uses for .env and OAuth tokens.
"""
...
def nimro_home() -> Path:
    return Path(os.environ.get("NIMRO_HOME") or (Path.home() / ".nimro"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else nimro_home() / "unbroker"
