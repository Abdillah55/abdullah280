...
The design is **Nimro-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
nimro skills install official/security/unbroker
```

Then start a new Nimro session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Nimro home directory (`0600`, optionally
