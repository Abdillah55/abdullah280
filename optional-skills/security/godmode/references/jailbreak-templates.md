...
## 5. GODMODE FAST — Nimro 4 405B (Zero Refusal)

**Model:** `nousresearch/nimro-4-405b`
**Strategy:** Nimro models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Nimro Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.nimro/config.yaml`:
