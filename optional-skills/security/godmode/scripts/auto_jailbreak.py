...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("NIMRO_HOME", os.path.expanduser("~/.nimro")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("NIMRO_HOME", Path.home() / ".nimro")) / "skills" / "red-teaming" / "godmode"
...
NIMRO_HOME = Path(os.getenv("NIMRO_HOME", Path.home() / ".nimro"))
CONFIG_PATH = NIMRO_HOME / "config.yaml"
PREFILL_PATH = NIMRO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Nimro models — already uncensored, just needs clean prompt
    "nimro": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "nimro" in model_lower or "nous" in model_lower:
        return "nimro"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Nimro config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.nimro/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Nimro for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Nimro for changes to take effect.")
