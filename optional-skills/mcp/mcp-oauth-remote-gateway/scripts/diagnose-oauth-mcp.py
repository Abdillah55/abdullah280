...
Usage:
  ...
  <server_name> matches the files in $NIMRO_HOME/mcp-tokens/<server>.json etc.
...
def _nimro_home():
    # Prefer Nimro' own resolver (profile-safe); fall back to env then ~/.nimro.
    try:
        from nimro_constants import get_nimro_home
        return str(get_nimro_home())
    except Exception:
        return os.environ.get("NIMRO_HOME") or os.path.expanduser("~/.nimro")


def _tokens_dir():
    return os.path.join(_nimro_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "nimro-diag", "version": "1.0"}},
        },
        headers={
