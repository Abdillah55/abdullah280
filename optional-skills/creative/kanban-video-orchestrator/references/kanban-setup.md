...
the actual `setup.sh` that creates the project workspace, configures Nimro
...
2. **Create profiles** — `nimro profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.nimro/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `nimro kanban create` assigned to the director
...
```bash
nimro profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.nimro/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.nimro/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Nimro
...
Each profile gets a `SOUL.md` at `~/.nimro/profiles/<name>/SOUL.md` that
...
```bash
nimro kanban create "Direct production of <video title>" \
    --assignee director \
...
the Nimro `.env` (`${NIMRO_HOME:-$HOME/.nimro}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _nimro_env="${NIMRO_HOME:-$HOME/.nimro}/.env"
    if grep -q "^${var}=" "$_nimro_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_nimro_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_nimro_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY nimro ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY nimro OPENROUTER_API_KEY || exit 1
# ...
```
