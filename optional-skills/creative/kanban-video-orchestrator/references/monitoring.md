...
nimro kanban watch --tenant <project-slug>

# Snapshot of the board
nimro kanban list --tenant <project-slug>
nimro kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
nimro kanban stats --tenant <project-slug>

# Visual dashboard (browser)
nimro dashboard

# Inspect a specific task (includes comments + events)
nimro kanban show <task-id>

# Follow a single task's event stream
nimro kanban tail <task-id>
```

Verify available subcommands with `nimro kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `nimro kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `nimro kanban block <id>` then `nimro kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `nimro kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `nimro kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `nimro profile list` (profile exists?), `nimro status` (gateway/dispatcher up?), and verify tenant. |
...
nimro kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
nimro kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(nimro kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
nimro kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
nimro kanban block <task-id>
nimro kanban archive <task-id>

# Diagnose what happened
nimro kanban show <task-id>      # task body, comments, recent events
nimro kanban tail <task-id>      # follow the live event stream
nimro kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `nimro kanban create` for the director
...
while true; do
    clear
    nimro kanban list --tenant <slug>
    echo "---"
    nimro kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `nimro kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `nimro profile list` and start
