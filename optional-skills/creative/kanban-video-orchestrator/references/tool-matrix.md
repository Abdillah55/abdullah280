...
Maps each role archetype to the Nimro skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public nimro-agent
...
## Nimro skills relevant to video production

### Visual / rendering skills (`nimro-agent/skills/creative/`)
...
### Design / pre-production skills (`nimro-agent/skills/creative/`)
...
### Audio / media skills (`nimro-agent/skills/creative/` + `skills/media/`)
...
These are **not** Nimro skills but external CLIs / APIs that profiles invoke.
...
## Built-in Nimro tools for media review

These are native Nimro tools — not invoked via terminal but through their own
...
key is present in `${NIMRO_HOME:-~/.nimro}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Nimro profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${NIMRO_HOME:-~/.nimro}/.env` → environment variable.
...
When a new Nimro-public video skill ships:
