...
### 1. Start order: editor first, Nimro second

Nimro probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Nimro session.
...
restart the editor, restart the Nimro session.
...
server fails to bind (Output Log) or Nimro' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.nimro/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Nimro

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Nimro' connection
lives in `~/.nimro/config.yaml` via `nimro mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Nimro — just don't
mistake it for the Nimro setup step.
...
explicitly warns against overlapping calls. Nimro executes same-turn tool
...
### 9. Timeouts: Nimro gives up before Unreal does

Nimro' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Nimro runs elsewhere (SSH backend,
