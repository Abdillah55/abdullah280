#!/usr/bin/env python3
"""Canvas LMS API CLI for Nimro Agent.
...
def _check_config():
    ...
    if missing:
        nimro_env = os.path.join(
            os.environ.get("NIMRO_HOME", os.path.expanduser("~/.nimro")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {nimro_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Nimro Agent"
    )
