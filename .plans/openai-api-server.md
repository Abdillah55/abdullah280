# OpenAI-Compatible API Server for Nimro Agent
...
SSE streaming. By exposing this endpoint, nimro-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  nimro-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.nimro/.env`
2. Run `nimro gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with nimro-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns nimro-agent as a model) |
...
Authorization: Bearer nimro-api-key-here
...
But nimro-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "nimro-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured nimro-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of nimro-agent's core prompt
...
Once implemented, nimro-agent works as a drop-in backend for:
