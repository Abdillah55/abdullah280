 ...
 *
 * The root ``package.json`` is installed by ``nimro update`` on every user,
 ...
 *   ``nimro tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``nimro update`` for
 ...
 * guidance in the ``nimro-agent-dev`` skill before changing the
 * expectations.
 */
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in nimro_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`nimro tools` → Browser Automation → Camofox.'
  )
})
...
test('agent-browser stays eager (default backend)', () => {
  ...
  assert.ok(
    ...
    'agent-browser is the default browser-tool backend used by every ' +
      ...
      'after `nimro setup` / `nimro update` without an explicit ' +
      'post_setup step.'
  )
})
