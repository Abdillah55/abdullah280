...
React + Ink terminal UI for Nimro. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `NIMRO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `nimro setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.nimro/.nimro_history` or under `NIMRO_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `nimro setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/nimro-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external nimro setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.nimro/tui-parent.log
    ...
    types/
      nimro-ink.d.ts            type declarations for @nimro/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent NimroCLI subprocess for slash commands
```
