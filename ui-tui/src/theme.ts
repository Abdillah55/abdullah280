import type { SkinBranding, SkinColors } from '@nimro/shared/skin'
...
// Mirrors @nimro/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @nimro/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Nimro navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Nimro look was never hand-authored: for years the
 ...
 *  answer when known (cached in NIMRO_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.NIMRO_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit NIMRO_TUI_THEME / COLORFGBG signals above still win,
...
// `NIMRO_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `NIMRO_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `NIMRO_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `NIMRO_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Nimro palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.NIMRO_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.NIMRO_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.NIMRO_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into NIMRO_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into NIMRO_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Nimro navy
  ...
  // the derived value already carries the identity seeds (e.g. Nimro navy).
  const activeRow =
