...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.NIMRO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Nimro on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via NIMRO_TUI_TRUECOLOR=1.
