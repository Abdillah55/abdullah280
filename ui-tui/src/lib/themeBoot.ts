 ...
 * `nimro-boot-background` / `nimro-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports NIMRO_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.nimro matches get_nimro_home()'s default.
const bootFilePath = () => join(process.env.NIMRO_HOME ?? join(homedir(), '.nimro'), 'tui-theme-boot.json')

// Never touch the user's real ~/.nimro from test runs (the TS suite has no
// NIMRO_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into NIMRO_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.NIMRO_TUI_THEME || env.NIMRO_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.NIMRO_TUI_BACKGROUND
  ) {
    env.NIMRO_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.NIMRO_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.NIMRO_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into NIMRO_TUI_THEME.
