...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because NIMRO_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.NIMRO_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.nimro', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `nimro-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.NIMRO_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.NIMRO_HEAPDUMP_MAX_BYTES?.trim()
