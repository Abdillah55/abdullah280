...
const logDir = join(process.env.NIMRO_HOME?.trim() || join(homedir(), '.nimro'), 'logs')
...
// a real ~/.nimro (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('nimro-tui: parent lifecycle log unavailable\n')
    }
  }
}
