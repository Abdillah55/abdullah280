import type { RunExternalProcess } from '@nimro/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`nimro ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching nimro: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`nimro ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
