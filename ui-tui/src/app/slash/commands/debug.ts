...
import { terminalBackgroundHex } from '@nimro/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $NIMRO_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see NIMRO_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['NIMRO_TUI_BACKGROUND', process.env.NIMRO_TUI_BACKGROUND ?? '(unset)'],
            ['NIMRO_TUI_THEME', process.env.NIMRO_TUI_THEME ?? '(unset)'],
