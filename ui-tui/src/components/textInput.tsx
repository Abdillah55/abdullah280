import type { InputEvent, Key } from '@nimro/ink'
import * as Ink from '@nimro/ink'
 ...
 * but that accepts `key.meta` on macOS — and nimro-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.NIMRO_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
