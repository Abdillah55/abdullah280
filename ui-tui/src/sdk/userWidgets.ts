...
import { Box, Text } from '@nimro/ink'
...
/**
 * User widget apps — Nimro authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$NIMRO_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.nimro/plugins/`: files under NIMRO_HOME execute
...
const widgetsDir = () => join(process.env.NIMRO_HOME?.trim() || join(homedir(), '.nimro'), 'tui-widgets')
 ...
 *  every change, so a widget Nimro writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Nimro) ever writes must hot-load too — a 10s poll here read
