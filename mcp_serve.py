"""
Nimro MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Nimro-specific extra)

Usage:
    nimro mcp serve
    nimro mcp serve --verbose
...
logger = logging.getLogger("nimro.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using NIMRO_HOME."""
    try:
        from nimro_constants import get_nimro_home
        return get_nimro_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("NIMRO_HOME", Path.home() / ".nimro")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from nimro_constants import get_nimro_home
        directory_file = get_nimro_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("NIMRO_HOME", Path.home() / ".nimro")
...
class EventBridge:
    ...
    This is the Nimro equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline(self) -> None:
        ...
        try:
            from nimro_constants import get_nimro_home
            db_file = get_nimro_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("NIMRO_HOME", Path.home() / ".nimro")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Nimro MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "nimro",
        instructions=(
            "Nimro Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Nimro MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
