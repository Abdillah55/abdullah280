...
Nimro gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its Nimro-owned subscribers.

## What gets exported

| Signal | OTLP route | Content |
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `nimro.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `nimro.platform.up/degraded` with bounded `error_code` attributes |
...
`nimro.gateway.active_agents`, `nimro.gateway.background_work`, and
`nimro.gateway.background_delegations` are complementary. `active_agents`
...
```bash
nimro monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'nimro-agent[otlp]'`),
...
a plain `nimro gateway run`: the exporter lives in the gateway process, so
...
`nimro.gateway.up`, `nimro.platform.up`, and `nimro.platform.degraded`.
...
absent_over_time(nimro_gateway_up[5m])

# Locally owned bridge is explicitly down.
nimro_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
nimro_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
nimro_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
nimro_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(nimro_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `nimro.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
nimro.status = failed|unknown
nimro.delivery_outcome = failed|not_configured
nimro.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `nimro.job_key`;
...
Nimro Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/nimro_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/nimro_otel_capture.jsonl --wait 8
...
`nimro.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
nimro monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `nimro monitoring` CLI intentionally exposes `status` only. This first
release covers only Nimro Agent-owned service-health and operational-diagnostic
signals, including Nimro Agent-owned Relay transport health. Team Gateway's
