# Nimro Observer Hooks

Nimro observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Nimro' planner, model providers, memory, tool registry,
...
Nimro also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "nimro.observer.v1"
```

Hook callbacks are fail-open. Nimro catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Nimro only builds
