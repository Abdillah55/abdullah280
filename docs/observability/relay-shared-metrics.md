...
Nimro includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Nimro and does not require `nimro plugins enable
observability/nemo_relay`. Nimro remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Nimro execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `nimro-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Nimro requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Nimro installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Nimro process so configured interceptors can
...
Collection remains off unless Nimro policy enables it:
...
Nimro core owns one Relay host and one isolated Relay session scope per Nimro
session. Core lifecycle producers use
`nimro_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Nimro plugin registration. Shared-metrics marks must
...
```text
Nimro turn, API, and tool hooks
  -> Relay session, task, and LLM lifecycle
  -> Nimro shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Nimro sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
Each task run is a Relay `Function` scope named `nimro.task_run`, parented to
the owning Nimro session. The start counter contains only bounded execution
...
provider attempts for the same Nimro API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Nimro tool-call
...
cancellations. Active task ownership follows the task ID if Nimro rotates its
conversation session during context compression.

Local state is written under:

```text
$NIMRO_HOME/telemetry/shared_metrics/metrics.sqlite3
$NIMRO_HOME/telemetry/shared_metrics/outbox/*.json
```
...
schema field name, its current scope is one `NIMRO_HOME`, so it is more
...
Deleting `$NIMRO_HOME/telemetry/shared_metrics` resets the identifier together
...
Run a real Nimro CLI turn against the deterministic local model server:
