...
Nimro middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Nimro'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Nimro calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Nimro retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Nimro guardrails,
...
- `telemetry_schema_version`: currently `nimro.observer.v1`
- `middleware_schema_version`: currently `nimro.middleware.v1`
...
Nimro stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Nimro runs
them as a nested chain in registration order. Middleware failures are fail-open:
Nimro logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Nimro applies middleware in this order:
...
For each tool call, Nimro applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Nimro pre-execution path against the effective arguments:
...
```bash
nimro plugins enable <plugin-name>
```

For isolated local testing, use one `NIMRO_HOME` for plugin enablement and the
agent run:

```bash
export NIMRO_HOME=/tmp/nimro-middleware-test
mkdir -p "$NIMRO_HOME"
nimro plugins enable <plugin-name>
nimro chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run nimro plugins enable <plugin-name>
uv run nimro chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["nimro_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/nimro-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Nimro expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Nimro treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Nimro preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Nimro does not convert downstream
