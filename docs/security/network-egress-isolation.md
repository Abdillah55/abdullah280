...
When running Nimro inside Docker, the default `network_mode: host` gives the
...
The Nimro [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   NIMRO_UID=$(id -u) NIMRO_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=nimro,nimro-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://nimro-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Nimro trust model and vulnerability reporting
