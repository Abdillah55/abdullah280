...
`nimro config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.nimro/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `NIMRO_HOME`, secret scope, and
