...
This document is the formal interface between the **Nimro gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `nimro gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`nimro gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `nimro` CLI.
