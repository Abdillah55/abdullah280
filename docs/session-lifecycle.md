...
The canonical transcript store is SQLite via `SessionDB` (from `nimro_state`). The
...
Counts consecutive restarts via a JSON file (`{NIMRO_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `nimro update`, `nimro gateway restart`,
or `/restart`.
