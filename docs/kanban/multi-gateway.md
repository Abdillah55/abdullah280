...
Nimro supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.nimro/config.yaml`:
...
Or set the env var: `NIMRO_KANBAN_DISPATCH_IN_GATEWAY=false`
