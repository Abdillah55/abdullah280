...
has to be thrown away or summarized. Nimro has always done this in one batch:
...
Nimro folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Nimro's normal session flush is
...
With `micro_compact` unset or `false` Nimro behaves exactly as it always has:
...
Defaults to `$NIMRO_HOME/logs/agent.log`. It reports passes, outcome mix, net
