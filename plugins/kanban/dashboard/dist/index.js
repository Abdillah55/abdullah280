/**
 * Nimro Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.nimro/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__NIMRO_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__NIMRO_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "nimro-kanban-dot-triage",
    todo: "nimro-kanban-dot-todo",
    ready: "nimro-kanban-dot-ready",
    running: "nimro-kanban-dot-running",
    blocked: "nimro-kanban-dot-blocked",
    done: "nimro-kanban-dot-done",
    archived: "nimro-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-nimro-task";
  ...
  const DOCS_URL = "https://nimro-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://nimro-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "nimro.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="nimro-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])nimro-kanban-md-code\1/i.test(attrs)) {
      return ' class="nimro-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "nimro-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "nimro-kanban-md",
  ...
  // standard `drop` event and our `nimro-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("nimro-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("nimro-kanban-column--drop");
          if (target) target.classList.add("nimro-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("nimro-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("nimro-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("nimro-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__NIMRO_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "nimro-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "nimro-kanban-attention",
        "nimro-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "nimro-kanban-attention-bar" },
        h("span", { className: "nimro-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "nimro-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "nimro-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "nimro-kanban-attention-row",
                  "nimro-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "nimro-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "nimro-kanban-attention-row-id" }, task.id),
                h("span", { className: "nimro-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "nimro-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "nimro-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "nimro-kanban-diag-action-btn",
      action.suggested ? "nimro-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " nimro-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".nimro-kanban-drawer-comment-row input, .nimro-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "nimro-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("nimro-kanban-diag", sevClass) },
      h("div", { className: "nimro-kanban-diag-header" },
        h("span", { className: "nimro-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "nimro-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "nimro-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "nimro-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "nimro-kanban-diag-data-row" },
                  h("span", { className: "nimro-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "nimro-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "nimro-kanban-diag-data-row" },
                h("span", { className: "nimro-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "nimro-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "nimro-kanban-diag-reassign-row" },
            h("span", { className: "nimro-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "nimro-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "nimro-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "nimro-kanban-diag-msg",
              msg.ok ? "nimro-kanban-diag-msg--ok" : "nimro-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "nimro-kanban-section" },
      h("div", { className: "nimro-kanban-section-head-row" },
        h("span", { className: "nimro-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "nimro-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "nimro-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "nimro-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "nimro-kanban-docs-link",
      title: "Open Nimro Kanban docs in a new tab",
      "aria-label": "Nimro Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `nimro kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "nimro-kanban-msg-ok" : "nimro-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "nimro-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "nimro-kanban-boardswitcher" },
      h("div", { className: "nimro-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "nimro-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "nimro-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "nimro-kanban-dialog-title" },
        ...
        h("div", { className: "nimro-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Nimro profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "nimro-kanban-bulk" },
      h("span", { className: "nimro-kanban-bulk-count" },
      ...
      h("div", { className: "nimro-kanban-bulk-priority",
      ...
      h("div", { className: "nimro-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Nimro profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "nimro-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("nimro-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("nimro-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "nimro-kanban-trash",
        dragOver ? "nimro-kanban-trash--drop" : "",
        props.draggingTaskId ? "nimro-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "nimro-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "nimro-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".nimro-kanban-card")) return true;
      if (target.closest && target.closest(".nimro-kanban-column-add")) return true;
      if (target.closest && target.closest(".nimro-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("nimro-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("nimro-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".nimro-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "nimro-kanban-columns",
        isScrollable ? "nimro-kanban-columns--scrollable" : "",
        isPanning ? "nimro-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("nimro-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("nimro-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "nimro-kanban-column",
        dragOver ? "nimro-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "nimro-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "nimro-kanban-col-check",
        ...
        h("span", { className: cn("nimro-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "nimro-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "nimro-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "nimro-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "nimro-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "nimro-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "nimro-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "nimro-kanban-lane" },
                  h("div", { className: "nimro-kanban-lane-head" },
                    h("span", { className: "nimro-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "nimro-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "nimro-kanban-card--stale-red";
    if (age >= tier.amber) return "nimro-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".nimro-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "nimro-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "nimro-kanban-card",
        props.selected ? "nimro-kanban-card--selected" : "",
        props.failed ? "nimro-kanban-card--failed" : "",
        props.draggingSource ? "nimro-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "nimro-kanban-card-content" },
          h("div", { className: "nimro-kanban-card-row" },
            h("label", {
              className: "nimro-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "nimro-kanban-card-check",
            ...
            h("span", { className: "nimro-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or nimro kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "nimro-kanban-warning-badge",
                    "nimro-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "nimro-kanban-needs-assignee",
          ...
          h("div", { className: "nimro-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "nimro-kanban-card-row nimro-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "nimro-kanban-assignee",
                            title: `Assigned to Nimro profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "nimro-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "nimro-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "nimro-kanban-ago",
  ...
  // centered modal reusing the nimro-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "nimro-kanban-dialog nimro-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Nimro profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Nimro profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "nimro-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "nimro-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "nimro-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "nimro-kanban-drawer-close",
        ...
        data ? h("div", { className: "nimro-kanban-drawer-comment-foot" },
          h("div", {
            className: "nimro-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "nimro-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "nimro-kanban-section" },
      h("div", { className: "nimro-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "nimro-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "nimro-kanban-drawer-body" },
      h("div", { className: "nimro-kanban-drawer-title" },
        h("span", { className: cn("nimro-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "nimro-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "nimro-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "nimro-kanban-section" },
            h("div", { className: "nimro-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "nimro-kanban-section" },
            h("div", { className: "nimro-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "nimro-kanban-done-no-result nimro-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "nimro-kanban-section" },
            h("div", { className: "nimro-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "nimro-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "nimro-kanban-section" },
        h("div", { className: "nimro-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "nimro-kanban-comment" },
            h("div", { className: "nimro-kanban-comment-head" },
              h("span", { className: "nimro-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "nimro-kanban-diag-action-btn",
      ...
      h("div", { className: "nimro-kanban-section" },
        h("div", { className: "nimro-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "nimro-kanban-comment" },
                h("div", { className: "nimro-kanban-comment-head" },
                  h("span", { className: "nimro-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "nimro-kanban-comment-ago" },
      ...
      h("div", { className: "nimro-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "nimro-kanban-event",
              isDiag ? "nimro-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "nimro-kanban-event-header" },
                  h("span", { className: "nimro-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "nimro-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "nimro-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "nimro-kanban-event-header-plain" },
                  h("span", { className: "nimro-kanban-event-kind" }, e.kind),
                  h("span", { className: "nimro-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "nimro-kanban-event-phantom-row" },
                  h("span", { className: "nimro-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "nimro-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "nimro-kanban-event-payload" },
    ...
    return h("div", { className: "nimro-kanban-section" },
      h("div", { className: "nimro-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "nimro-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `nimro-kanban-run--${r.outcome || r.status || "ended"}`
          : "nimro-kanban-run--active";
        return h("div", { key: r.id, className: cn("nimro-kanban-run", outcomeClass) },
          h("div", { className: "nimro-kanban-run-head" },
            h("span", { className: "nimro-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "nimro-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "nimro-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "nimro-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "nimro-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "nimro-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "nimro-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "nimro-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "nimro-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "nimro-kanban-pre nimro-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "nimro-kanban-meta-row" },
      h("span", { className: "nimro-kanban-meta-label" }, props.label),
      h("span", { className: "nimro-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "nimro-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "nimro-kanban-meta-row" },
        h("span", { className: "nimro-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "nimro-kanban-meta-value nimro-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "nimro-kanban-meta-row" },
        h("span", { className: "nimro-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "nimro-kanban-meta-value nimro-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "nimro-kanban-meta-row" },
        h("span", { className: "nimro-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "nimro-kanban-meta-value nimro-kanban-editable",
    ...
    return h("div", { className: "nimro-kanban-meta-row" },
      h("span", { className: "nimro-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "nimro-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "nimro-kanban-section" },
      h("div", { className: "nimro-kanban-section-head-row" },
        h("span", { className: "nimro-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "nimro-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "nimro-kanban-section" },
      h("div", { className: "nimro-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "nimro-kanban-deps-row" },
        h("span", { className: "nimro-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "nimro-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "nimro-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "nimro-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "nimro-kanban-dep-chip-x",
      ...
      h("div", { className: "nimro-kanban-deps-row" },
        h("span", { className: "nimro-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "nimro-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "nimro-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "nimro-kanban-section" },
      ...
      h("div", { className: "nimro-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "nimro-kanban-home-sub nimro-kanban-home-sub--on"
              : "nimro-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__NIMRO_PLUGINS__ && typeof window.__NIMRO_PLUGINS__.register === "function") {
    window.__NIMRO_PLUGINS__.register("kanban", KanbanPage);
  }
})();
