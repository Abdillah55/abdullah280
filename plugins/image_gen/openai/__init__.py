...
three virtual model IDs so the ``nimro tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `nimro tools` → Image "
                    "Generation → OpenAI to configure, or `nimro setup` "
                    "to add the key."
                ),
