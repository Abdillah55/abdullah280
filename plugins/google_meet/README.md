...
Let the nimro agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where nimro runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `nimro meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `nimro meet` CLI |
...
| `cli.py` | `nimro meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$NIMRO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `nimro meet node {run,list,approve,remove,status,ping}` |
...
nimro meet install                                      # pip + Chromium
nimro meet setup                                        # preflight
nimro meet auth                                         # optional
nimro meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
nimro meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.nimro/.env
nimro meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
nimro meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
nimro meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.nimro/.env
nimro meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, nimro will **not** switch your system audio input automatically — the
...
nimro plugins enable google_meet
nimro meet node run --display-name my-mac --host 0.0.0.0 --port 18789
