...
``{title, url, description, position}`` rows every other Nimro web
provider produces.
...
prefers Nimro-managed xAI Grok OAuth (via ``nimro auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.nimro/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``nimro tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `nimro auth` to sign in with "
