"""disk_cleanup — ephemeral file cleanup for Nimro Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under NIMRO_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly NIMRO_HOME and /tmp/nimro-*
Never touches: ~/.nimro/logs/ or any system directory.
"""
...
try:
    ...
    def get_nimro_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("NIMRO_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".nimro").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$NIMRO_HOME/logs/``."""
    return get_nimro_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$NIMRO_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under NIMRO_HOME or ``/tmp/nimro-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    nimro_home = get_nimro_home()
    try:
        path.resolve().relative_to(nimro_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/nimro-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("nimro-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "nimro-agent", "backups", "profiles", ".worktrees",
...
# Paths under $NIMRO_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so NIMRO_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        nimro_home = get_nimro_home()
        for parent in ("cron", "cronjobs"):
            base = nimro_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside NIMRO_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under NIMRO_HOME, but never recurse into known
    # durable state trees.  Some installs place the Nimro checkout, venv,
    # and desktop build under NIMRO_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    nimro_home = get_nimro_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in nimro_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    nimro_home = get_nimro_home()
    try:
        rel = path.resolve().relative_to(nimro_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "nimro-agent",
        ...
        # Path isn't under NIMRO_HOME (e.g. /tmp/nimro-*) — fall through.
        pass
