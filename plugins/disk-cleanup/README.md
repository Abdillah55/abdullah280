...
Auto-tracks and cleans up ephemeral files created during Nimro Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$NIMRO_HOME` and `/tmp/nimro-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `NIMRO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `NIMRO_HOME` or `/tmp/nimro-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$NIMRO_HOME/disk-cleanup/` is itself excluded
- `$NIMRO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
