"""nemo_relay — optional Nimro plugin for NeMo Relay observability."""
...
_SESSION_INITIALIZER_NAME = "nimro.nemo_relay.rich_observability"
...
class _Settings:
    ...
    atof_filename: str = "nimro-atof.jsonl"
    atof_mode: str = "append"
    atif_enabled: bool = False
    atif_output_directory: str = ""
    atif_filename_template: str = "nimro-atif-{session_id}.json"
    atif_subagent_export_mode: str = "embedded"
    atif_agent_name: str = "Nimro Agent"
        ...
        with self._lock:
            ...
            if self._owners:
                ...
                logger.warning(
                    ...
                    "Nimro profile; keeping the existing process-global configuration "
...
class _Runtime:
    def __init__(
        ...
        self._atof_subscriber_name = f"nimro.nemo_relay.atof.{self.host.runtime_id}"
        self._execution_consumer_name = (
            f"nimro.nemo_relay.rich_observability.{self.host.runtime_id}"
        )
        ...
        with self._sessions_lock:
            ...
            if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
                ...
                state.atif_subscriber_name = (
                    f"nimro.nemo_relay.atif.{self.host.runtime_id}.{session_id}"
                )
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if relay_session is None:
            raise RuntimeError("Nimro core Relay session is unavailable")
    ...
    def mark_subagent_stop(self, kwargs: dict[str, Any]) -> None:
        ...
        self.mark("nimro.subagent.stop", kwargs)
...
def register(ctx) -> None:
    ...
    # Activate dynamic plugins before Nimro installs the managed execution
...
def on_session_end(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: (runtime.mark("nimro.session.end", kwargs), runtime.export_atif(runtime.ensure_session(kwargs))))
...
def on_pre_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("nimro.turn.start", kwargs))


def on_post_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("nimro.turn.end", kwargs))


def on_pre_approval_request(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("nimro.approval.request", kwargs))


def on_post_approval_response(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("nimro.approval.response", kwargs))
    ...
    with _LOCK:
        ...
        try:
            ...
            if resolved_host is None:
                raise RuntimeError("Nimro core Relay runtime is unavailable")
            runtime = _Runtime(
...
def _load_settings() -> _Settings:
    plugins_toml_path = _env("NIMRO_NEMO_RELAY_PLUGINS_TOML")
    ...
    return _Settings(
        ...
        atof_enabled=_env_bool("NIMRO_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("NIMRO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("NIMRO_NEMO_RELAY_ATOF_FILENAME") or "nimro-atof.jsonl",
        atof_mode=_env("NIMRO_NEMO_RELAY_ATOF_MODE") or "append",
        atif_enabled=_env_bool("NIMRO_NEMO_RELAY_ATIF_ENABLED"),
        atif_output_directory=_env("NIMRO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"),
        atif_filename_template=_env("NIMRO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "nimro-atif-{session_id}.json",
        atif_subagent_export_mode=_atif_subagent_export_mode(),
        atif_agent_name=_env("NIMRO_NEMO_RELAY_ATIF_AGENT_NAME") or "Nimro Agent",
        atif_agent_version=_env("NIMRO_NEMO_RELAY_ATIF_AGENT_VERSION") or "unknown",
        atif_model_name=_env("NIMRO_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
    )
...
def _dynamic_plugin_specs(
    ...
    if plugins_section is not None:
        ...
        if plugins_section:
            logger.error(
                "Nimro cannot activate Relay gateway [[plugins.dynamic]] records because "
                "the Python binding does not expose the CLI lifecycle resolver for "
                "enablement, trust policy, and worker environments. Use Nimro-owned "
...
def _atif_subagent_export_mode() -> str:
    mode = _env("NIMRO_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
...
def _resolve_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="nimro-nemo-relay-awaitable",
        daemon=True,
    )
    thread.start()
    thread.join()
