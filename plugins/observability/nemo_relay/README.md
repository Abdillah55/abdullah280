...
Optional Nimro observability plugin that configures exporters and maps
Nimro-specific observer hooks to NeMo Relay marks and ATIF state. Nimro core
...
not replace Nimro Agent's planner, tools, memory, model provider routing, or
CLI UX. Nimro core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, Nimro Agent can:

- Export the Relay scopes and LLM/tool lifecycles emitted by Nimro core.
- Add Nimro session, turn, approval, and subagent mark events.
...
```bash
nimro plugins enable observability/nemo_relay
```

The `NIMRO_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `NIMRO_HOME` that the
agent run will use:

```bash
env NIMRO_HOME=/tmp/nimro-nemo-relay-test \
  nimro plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`NIMRO_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`NIMRO_HOME` is the Nimro profile/config home used by both
`nimro plugins enable ...` and the later `nimro chat ...` run. If unset,
Nimro uses the user's default home, usually `~/.nimro`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export NIMRO_HOME=/tmp/nimro-nemo-relay-test
nimro plugins enable observability/nemo_relay
nimro chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `nimro` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run nimro plugins enable observability/nemo_relay
uv run nimro chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, Nimro installs its supported NeMo Relay runtime as a normal dependency:

```bash
uv build --wheel
python -m pip install --force-reinstall dist/nimro_agent-*.whl
nimro plugins enable observability/nemo_relay
```
...
behavior; it does not control Nimro shared client metrics.

## Export Configuration

The plugin can configure exporters directly from `NIMRO_NEMO_RELAY_*`
...
```bash
export NIMRO_NEMO_RELAY_ATOF_ENABLED=1
export NIMRO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export NIMRO_NEMO_RELAY_ATIF_ENABLED=1
export NIMRO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `NIMRO_NEMO_RELAY_ATOF_FILENAME`
- `NIMRO_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `NIMRO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `NIMRO_NEMO_RELAY_ATIF_AGENT_NAME`
- `NIMRO_NEMO_RELAY_ATIF_AGENT_VERSION`
- `NIMRO_NEMO_RELAY_ATIF_MODEL_NAME`
- `NIMRO_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Nimro at it:

```bash
export NIMRO_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Nimro Agent"
agent_version = "local"
```

When `NIMRO_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`NIMRO_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`NIMRO_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Nimro does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Nimro keeps the direct env-var fallbacks active for that run.

Nimro core routes provider and tool execution through NeMo Relay managed APIs
...
Nimro uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Nimro-owned
...
with this Nimro-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Nimro rejects
...
Nimro activates these plugins before registering its managed LLM and tool
...
For the full generic Nimro middleware contract, see
...
with Nimro and a local Ollama model served through the OpenAI-compatible API.

```bash
export NIMRO_HOME=/tmp/nimro-nemo-relay-docs/nimro-home
mkdir -p "$NIMRO_HOME"

cat > "$NIMRO_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Nimro session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export NIMRO_NEMO_RELAY_ATOF_ENABLED=1
export NIMRO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/nimro-nemo-relay-docs/subagent/atof
export NIMRO_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export NIMRO_NEMO_RELAY_ATOF_MODE=overwrite
export NIMRO_NEMO_RELAY_ATIF_ENABLED=1
export NIMRO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/nimro-nemo-relay-docs/subagent/atif
export NIMRO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export NIMRO_NEMO_RELAY_ATIF_AGENT_NAME='Nimro Agent E2E'
export NIMRO_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export NIMRO_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"nimro.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Nimro Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Nimro dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/nimro-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/nimro-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/nimro-nemo-relay-docs/workdir/beta.txt
cd /tmp/nimro-nemo-relay-docs/workdir

export NIMRO_NEMO_RELAY_ATOF_ENABLED=1
export NIMRO_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/nimro-nemo-relay-docs/parallel/atof
export NIMRO_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export NIMRO_NEMO_RELAY_ATOF_MODE=overwrite
export NIMRO_NEMO_RELAY_ATIF_ENABLED=1
export NIMRO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/nimro-nemo-relay-docs/parallel/atif
export NIMRO_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export NIMRO_NEMO_RELAY_ATIF_AGENT_NAME='Nimro Agent E2E'
export NIMRO_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Nimro sessions map to `agent` scopes.
- Nimro core managed provider calls map to `llm` scope start/end events.
- Nimro core managed tool calls map to `tool` scope start/end events.
...
Nimro core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Nimro-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Nimro calls the
...
```bash
export NIMRO_NEMO_RELAY_PLUGINS_TOML=/tmp/nimro-middleware-test/plugins.toml
```
...
Nimro provider call
  -> nemo_relay.llm.execute(...)
    -> Nimro provider adapter callback(...)

Nimro tool call
  -> nemo_relay.tools.execute(...)
    -> Nimro authorization and dispatch callback(...)
```
...
middleware for a local Nimro run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export NIMRO_HOME=/tmp/nimro-middleware-test/nimro-home
mkdir -p "$NIMRO_HOME" /tmp/nimro-middleware-test/nemo-relay
...
cat > /tmp/nimro-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/nimro-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/nimro-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Nimro Middleware E2E"
agent_version = "local"
...
export NIMRO_NEMO_RELAY_PLUGINS_TOML=/tmp/nimro-middleware-test/nemo-relay/plugins.toml
