...
``_check_spotify_available()`` — when the user has not run ``nimro auth
spotify``, the tools remain registered (so they appear in ``nimro
...
The Spotify auth flow (``nimro auth spotify``), CLI plumbing, and docs
