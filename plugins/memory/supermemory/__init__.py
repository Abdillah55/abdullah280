...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=nimro"
...
def _load_supermemory_config(nimro_home: str) -> dict:
    config = _default_config()
    config_path = Path(nimro_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, nimro_home: str) -> None:
    config_path = Path(nimro_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "nimro"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Nimro writes into the "Nimro" Space in the Supermemory
        ...
        merged = {"sm_source": "nimro", **(metadata or {})}
...
def _resolve_container_tag_for_setup(nimro_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(nimro_home)
...
def _probe_supermemory_connection(api_key: str, nimro_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(nimro_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(nimro_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `nimro memory setup`.
        # All other options are documented for $NIMRO_HOME/supermemory.json
    ...
    def save_config(self, values, nimro_home):
        ...
        _save_supermemory_config(sanitized, nimro_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, nimro_home)
    ...
    def post_setup(self, nimro_home: str, config: dict) -> None:
        ...
        from nimro_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(nimro_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from nimro_constants import get_nimro_home
        self._nimro_home = kwargs.get("nimro_home") or str(get_nimro_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._nimro_home)
