...
- Hosted: API key from [app.supermemory.ai/integrations?connect=nimro](http://app.supermemory.ai/integrations?connect=nimro)
...
Before running `nimro memory setup`, add the local endpoint to
...
Then run `nimro memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$NIMRO_HOME/supermemory.json`
...
| `container_tag` | `nimro` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `nimro-{identity}` → `nimro-coder`). |
...
`https://api.supermemory.ai`. Nimro resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: nimro`, and document writes stamp
`metadata.sm_source: nimro`. This is a **functional routing key, not telemetry**:
it groups Nimro-written memories into a dedicated "Nimro" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Nimro profile:

```json
{
  "container_tag": "nimro-{identity}"
}
```

For a profile named `coder`, this resolves to `nimro-coder`. The default profile resolves to `nimro-default`. Without `{identity}`, all profiles share the same container.
