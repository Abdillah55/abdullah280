...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Nimro peer ID in OpenViking (default: nimro)
...
_DEFAULT_AGENT = "nimro"
_AGENT_PROMPT_LABEL = "Nimro peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "NIMRO_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Nimro slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Nimro to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Nimro refused the connection."
        ) from exc
...
def _load_nimro_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("NIMRO_HOME", "")).expanduser() if os.environ.get("NIMRO_HOME") else Path.home() / ".nimro"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Nimro did not start "
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Nimro will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Nimro session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Nimro only", "write values only to Nimro .env"),
    ...
    _save_nimro_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Nimro .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See nimro-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_nimro_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Nimro peer ID in OpenViking, sent as the actor peer and "
    ...
    def save_config(self, values: Dict[str, Any], nimro_home: str) -> None:
    ...
    def post_setup(self, nimro_home: str, config: dict) -> None:
        ...
        from nimro_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        nimro_home_path = Path(nimro_home)
        env_path = nimro_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        connection_error = ""
        try:
            settings = _resolve_connection_settings(_load_nimro_openviking_config())
        except _OpenVikingEndpointError as exc:
        ...
        self._turn_count = 0
        nimro_home = str(kwargs.get("nimro_home") or "").strip()
        if not nimro_home:
            try:
                from nimro_constants import get_nimro_home
                nimro_home = str(get_nimro_home())
            except Exception:
                nimro_home = str(Path.home() / ".nimro")
        self._nimro_home = nimro_home
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Nimro will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.nimro/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts nimro (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Nimro will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._nimro_home:
            return None
        return Path(self._nimro_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._nimro_home:
            return None
        return Path(self._nimro_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _profile_token_budget(self) -> int:
        cfg = _load_nimro_openviking_config()
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Nimro' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Nimro canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See nimro-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/nimro/.overview.md).
