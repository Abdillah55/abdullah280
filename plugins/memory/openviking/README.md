...
- OpenViking server running and reachable from Nimro

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Nimro can identify older servers that expose the legacy status-only health
...
connection values into Nimro, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.nimro/.env`; for a named profile use
...
OpenViking's server config is separate from Nimro:
...
Nimro-side provider config is read from environment variables in the active
profile's `.env`:
...
| `OPENVIKING_AGENT` | `nimro` | Nimro peer ID in OpenViking, used for peer-scoped memories |

When `OPENVIKING_API_KEY` is set, Nimro lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Nimro sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
...
Nimro built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Nimro action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under the configured peer memory namespace |

Built-in `replace` and `remove` operations are not mirrored because Nimro
...
file URIs, such as
`viking://user/peers/nimro/memories/preferences/mem_abc123.md` or the canonical
`viking://user/default/peers/nimro/memories/preferences/mem_abc123.md`. Files
