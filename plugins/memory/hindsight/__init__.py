...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: nimro)
...
Or via $NIMRO_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Nimro can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``nimro memory status`` would stay green while the daemon
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $NIMRO_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_nimro_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "nimro": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "nimro"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Nimro config."""
    profile = config.get("profile", "nimro")
    return str(profile or "nimro")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Nimro .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Nimro profile name (from agent_identity)
      {workspace} — Nimro workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``nimro-{user}`` with no user becomes ``nimro``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Nimro Agent and the User"
    ...
    def save_config(self, values, nimro_home):
        """Write config to $NIMRO_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(nimro_home) / "hindsight"
    ...
    def post_setup(self, nimro_home: str, config: dict) -> None:
        ...
        from nimro_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "nimro")
        ...
        self.save_config(provider_config, nimro_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(nimro_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(nimro_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "nimro"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: nimro-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Nimro Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "nimro"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "nimro"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/nimro (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "nimro", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "nimro")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Nimro Agent and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Nimro as a non-root user, or switch "
                    "to cloud / local_external mode via 'nimro memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Nimro get sluggish. (issue #13125)
                try:
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Nimro created/used that client on the shared async loop,
