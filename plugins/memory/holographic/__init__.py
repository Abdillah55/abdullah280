"""nimro-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $NIMRO_HOME/config.yaml (profile-scoped):
  plugins:
    nimro-memory-store:
      db_path: $NIMRO_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "nimro-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, nimro_home):
        """Write config to config.yaml under plugins.nimro-memory-store."""
        from pathlib import Path
        config_path = Path(nimro_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["nimro-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_nimro_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _nimro_home = str(get_nimro_home())
        _default_db = _nimro_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $NIMRO_HOME in user-supplied paths so config values like
        # "$NIMRO_HOME/memory_store.db" or "~/.nimro/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$NIMRO_HOME", _nimro_home)
            db_path = db_path.replace("${NIMRO_HOME}", _nimro_home)
