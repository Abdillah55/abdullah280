...
```bash
nimro memory setup    # select "mem0"
```
...
Behavioral settings live in `$NIMRO_HOME/mem0.json` (set them via `nimro memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.nimro/.env`.
...
| `user_id` | `nimro-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Nimro session and call `mem0_search` — it connects to your server.
...
```bash
nimro memory setup mem0 --mode oss \
...
```bash
nimro memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$NIMRO_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.nimro/mem0_qdrant"}}
  }
}
```
...
```bash
nimro memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$NIMRO_HOME/mem0.json`).
