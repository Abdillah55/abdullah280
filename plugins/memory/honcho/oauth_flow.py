...
``nimro://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Nimro OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "nimro-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``nimro-desktop`` / ``nimro-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Nimro.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local NIMRO_HOME override can't reach.
