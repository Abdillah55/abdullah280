...
Resolution order for config file:
  1. $NIMRO_HOME/honcho.json  (instance-local, enables isolated Nimro instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Nimro profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Nimro profile.

    Resolution order:
      1. NIMRO_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``nimro_<profile>``
      3. defaultHost from the active config, but only for the default profile
      4. Fallback: ``"nimro"`` (default profile)
    """
    explicit = os.environ.get("NIMRO_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
    ...
    # but applying it before profile resolution makes every named Nimro
    # profile share that same host.  Keep named profiles isolated; only the
    # default Nimro profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $NIMRO_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.nimro/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_nimro_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_nimro_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.nimro
    ...
    def from_global_config(
        ...
        Resolution: $NIMRO_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Nimro profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.nimro block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Nimro session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Nimro session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'nimro honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `nimro honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                nimro_cfg = load_config()
                honcho_cfg = nimro_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
