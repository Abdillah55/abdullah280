...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/nimro>
...
```bash
nimro memory setup honcho   # configure Honcho directly (works on a fresh install)
nimro memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
nimro config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.nimro/.env
```

> `nimro honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `nimro memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `nimro model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$NIMRO_HOME/honcho.json` | Profile-local (isolated Nimro instances) |
| 2 | `~/.nimro/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Nimro profile: `nimro` (default) or `nimro_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `nimro honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Nimro session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`nimro-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`nimro`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-nimro-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening nimro in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Nimro session ID (timestamp + hex). Every `nimro` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Nimro profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`nimro`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Nimro profile: `nimro` (default) or `nimro_<profile>` (e.g. `nimro -p coder` -> host key `nimro_coder`). Older `nimro.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `NIMRO_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `nimro-agent`) |
...
|---------|-------------|
| `nimro memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `nimro honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `nimro memory setup`) |
| `nimro honcho status` | Show resolved config for active profile |
| `nimro honcho enable` / `disable` | Toggle Honcho for active profile |
| `nimro honcho mode <mode>` | Change recall or observation mode |
| `nimro honcho peer --user <name>` | Update user peer name |
| `nimro honcho peer --ai <name>` | Update AI peer name |
| `nimro honcho tokens --context <N>` | Set context token budget |
| `nimro honcho tokens --dialectic <N>` | Set dialectic max chars |
| `nimro honcho map <name>` | Map current directory to a session name |
| `nimro honcho sync` | Create host blocks for all Nimro profiles |
