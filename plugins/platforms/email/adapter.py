"""
Email platform adapter for the Nimro gateway.

Allows users to interact with Nimro by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "nimro-agent" "version" "{_nimro_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `nimro config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `nimro gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Nimro Agent")
        ...
        msg_id = f"<nimro-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# nimro_cli/gateway.py, and the _send_email dispatch in
