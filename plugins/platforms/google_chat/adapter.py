...
# ``model_tools`` import time, so ``nimro status``, ``nimro chat``, etc.
...
# ``nimro_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Nimro is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a nimro MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "nimro-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${NIMRO_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _nimro_home = _get_nimro_home()
        except (ModuleNotFoundError, ImportError):
            _nimro_home = _Path.home() / ".nimro"
        self._thread_count_store = _ThreadCountStore(
            _nimro_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Nimro is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("NIMRO_HOME", str(_Path.home() / ".nimro"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${NIMRO_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a nimro MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Nimro is thinking…' marker message.
        ...
        "Nimro is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Nimro is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Nimro is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Nimro is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``nimro setup``.

    The setup wizard at ``nimro_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.nimro/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. nimro-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.nimro/.env")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Nimro plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `nimro setup` to install Google Chat support.",
        ...
        # the "Nimro is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Nimro is thinking…' marker message "
