"""
LINE Messaging API platform adapter for Nimro Agent.
...
to Nimro Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``NIMRO_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            nimro_home = Path(get_nimro_home()).resolve()
        except Exception:
            nimro_home = Path.home().joinpath(".nimro").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``nimro status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``nimro status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nimro setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.nimro/.env`` via ``nimro_cli.config``.
    """
    print()
    ...
    try:
        from nimro_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("nimro_cli.config not available; set LINE_* vars manually in ~/.nimro/.env")
        return
