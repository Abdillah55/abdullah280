...
# identify NimroAgent traffic — matching other Nimro outbound surfaces
# that already set ``NimroAgent/<version>`` for platform-partner attribution.
try:
    from nimro_cli import __version__ as _NIMRO_VERSION
except Exception:
    _NIMRO_VERSION = "unknown"
_NIMRO_SLACK_USER_AGENT_PREFIX = f"NimroAgent/{_NIMRO_VERSION}"
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from nimro_cli.commands import is_gateway_known_command
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `nimro slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `nimro gateway setup` "
                "or in the active profile's ~/.nimro/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `nimro gateway setup` "
                "or add it to your active profile's ~/.nimro/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `nimro gateway setup` "
                "or in the active profile's ~/.nimro/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `nimro gateway setup` "
                "or add it to your active profile's ~/.nimro/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_nimro_home() / "slack_tokens.json"
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_NIMRO_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # Nimro has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Nimro can "
                    "be removed from the Slack app manifest via "
                    "`nimro slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /nimro prefix). A single regex
            ...
            # manifest (see `nimro slack manifest`). In Socket Mode, Slack
            ...
            from nimro_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/nimro$")

            @self._app.command(_slash_pattern)
            async def handle_nimro_command(ack, command):
            ...
            for _action_id in (
                "nimro_approve_once",
                "nimro_approve_session",
                "nimro_approve_always",
                "nimro_deny",
            ):
            ...
            for _action_id in (
                "nimro_confirm_once",
                "nimro_confirm_always",
                "nimro_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("nimro_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^nimro_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("nimro_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'nimro slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Nimro handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Nimro session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Nimro owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_nimro_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_nimro_no_thread_response"] = True
    ...
    async def _handle_slack_message(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_nimro_no_thread_response"):
        ...
        force_process = bool(event.get("_nimro_force_process"))
        ...
        # the shape produced by peer Nimro agents in Socket Mode on some
        ...
        try:
            ...
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "nimro_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "nimro_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "nimro_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "nimro_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "nimro_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``nimro_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``nimro_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"nimro_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "nimro_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "nimro_confirm_once": "once",
            "nimro_confirm_always": "always",
            "nimro_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "nimro_approve_once": "once",
            "nimro_approve_session": "session",
            "nimro_approve_always": "always",
            "nimro_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "nimro_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/nimro <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/nimro
        ...
        if slash_name in {"nimro", ""}:
            # Legacy /nimro <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from nimro_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/nimro   ``).
        ...
        # "Running /cmd…" ack shown by handle_nimro_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/nimro <question>" must produce public replies so
...
# ``nimro_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_nimro_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``nimro_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from nimro_cli.config import get_env_value, remove_env_value, save_env_value
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under NIMRO_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from nimro_cli.slack_cli import _build_full_manifest
            from nimro_constants import get_nimro_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Nimro",
                bot_description="Your Nimro agent on Slack",
            )
            target = Path(get_nimro_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `nimro slack manifest --write` anytime to refresh after "
                "Nimro adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `nimro update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://nimro-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``nimro_cli.gateway.get_env_value`` at call time (not via the
    ...
    """
    import nimro_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `nimro setup` to install Slack support.",
        # Interactive setup wizard — replaces nimro_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in nimro_cli/gateway.py.
