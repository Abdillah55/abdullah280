"""
IRC Platform Adapter for Nimro Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Nimro agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "nimro-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Nimro Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Nimro Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: nimro_, nimro_1, nimro_2...
...
def check_requirements() -> bool:
    ...
    # nimro setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `nimro gateway setup` flow for the IRC platform.

    Lazy-imports ``nimro_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Nimro to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #nimro — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.nimro/.env")
    print_info("Restart the gateway for changes to take effect: nimro gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "nimro-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "nimro-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Nimro Agent (cron)")
