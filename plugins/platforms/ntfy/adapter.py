"""ntfy platform adapter (Nimro plugin).
...
``plugins/platforms/ntfy/``. The Nimro plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "nimro-in"              # subscribe topic (incoming)
          publish_topic: "nimro-out"     # optional — defaults to topic
...
_ECHO_TAG = "nimro-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``nimro cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Nimro dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `nimro gateway status` without
