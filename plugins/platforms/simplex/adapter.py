"""SimpleX Chat platform adapter (Nimro plugin).
...
This adapter ships as a Nimro platform plugin under
``plugins/platforms/simplex/``. The Nimro plugin loader scans the
...
Optional environment variables:
    ...
    NIMRO_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``nimro setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Nimro — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("NIMRO_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nimro setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.nimro/.env`` via
    ``nimro_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "nimro_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.nimro/.env"
        )
        return
