...
This plugin connects Nimro Agent to iMessage (and other Spectrum
...
TypeScript-only, Nimro runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Nimro channel,
...
Run `nimro photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.nimro/.env` (the same place every other
...
Management metadata lives in `~/.nimro/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `nimro photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Nimro
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Nimro coalesces
...
- **Native polls are supported.** Nimro posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `nimro send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<nimro-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `nimro photon status`, a DM and a group roundtrip,
