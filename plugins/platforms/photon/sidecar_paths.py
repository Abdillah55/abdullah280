...
under an immutable ``/opt/nimro`` (read-only for the nimro user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$NIMRO_HOME/photon/sidecar`` (the durable data volume,
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_nimro_home() / "photon" / "sidecar"
    try:
