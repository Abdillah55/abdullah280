// Nimro Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Nimro): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Nimro -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Nimro-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `nimro photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Nimro can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "nimro-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Nimro'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Nimro allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Nimro, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-nimro-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Nimro supplied them so a known-good
