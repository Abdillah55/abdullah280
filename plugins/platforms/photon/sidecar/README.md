...
Small Node helper that bridges Nimro Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Nimro is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Nimro-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Nimro can receive inbound messages
...
The Nimro plugin's `nimro photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Nimro is Python, so replies go through this sidecar
