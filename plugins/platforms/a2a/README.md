# A2A — Agent-to-Agent protocol for Nimro
...
peer (another Nimro, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
nimro gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Nimro serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.nimro/a2a_audit.jsonl`.
- Conversations persist to `~/.nimro/a2a_conversations/` — they survive context
