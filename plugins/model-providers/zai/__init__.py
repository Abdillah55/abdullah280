...
request omits ``thinking``.  Nimro' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Nimro reasoning
...
(per Z.AI / BigModel docs).  Nimro' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map Nimro reasoning effort onto GLM-5.2's native ``high``/``max``.
