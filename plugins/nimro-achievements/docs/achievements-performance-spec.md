# Nimro Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: nimro-achievements plugin
...
### `GET /api/plugins/nimro-achievements/achievements`
...
### `POST /api/plugins/nimro-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/nimro-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/nimro-achievements/overview`
...
- Persist plugin data under:
  - `~/.nimro/plugins/nimro-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.nimro/plugins/nimro-achievements/scan_checkpoint.json`
...
- `~/.nimro/plugins/nimro-achievements/`
