"""Runtime smoke test for Docker $NIMRO_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the nimro user after container boot.
...
registered later runs its log service as the dropped nimro user and
...
def test_logs_gateways_seeded_and_nimro_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by nimro after boot."""
    ...
    assert "logs=nimro" in r.stdout, (
        f"logs/ not owned by nimro: {r.stdout}"
    )
    assert "gateways=nimro" in r.stdout, (
        f"logs/gateways/ not owned by nimro: {r.stdout}"
    )
