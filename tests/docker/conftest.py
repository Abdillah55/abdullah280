...
Override the image with ``NIMRO_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("NIMRO_TEST_IMAGE", "nimro-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``NIMRO_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("NIMRO_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged nimro user
# ---------------------------------------------------------------------------
#
# Background: every Nimro runtime path inside the container drops to UID
# 10000 (the ``nimro`` user) via ``s6-setuidgid nimro``. ``docker exec``
...
# fails (PermissionError swallowed) for nimro, so a test that ran as root
...
# helpers below run every probe under ``-u nimro`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: nimro).
