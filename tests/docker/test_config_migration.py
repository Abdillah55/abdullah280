...
Build the real image and verify: a config.yaml present in $NIMRO_HOME
is migrated by docker_config_migrate.py on boot, running as the nimro
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $NIMRO_HOME must be migrated on boot by
    docker_config_migrate.py, running as the nimro user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $NIMRO_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/nimro/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by nimro (migration ran as nimro)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "nimro", (
        f"config.yaml not owned by nimro (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
