...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.nimro/nimro-agent/utils.py)
...
2. ``nimro update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``nimro_cli/main.py`` (the ``nimro update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``nimro_constants`` after the
