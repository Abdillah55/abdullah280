...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("nimro.gateway.up"), _M("nimro.gateway.active_agents"),
        _M("nimro.gateway.busy"), _M("nimro.gateway.drainable"),
        _M("nimro.gateway.restart_requested"),
        _M("nimro.platform.up"), _M("nimro.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("nimro.cron.scheduler.heartbeat_age_seconds"),
        _M("nimro.cron.scheduler.last_success_age_seconds"),
        _M("nimro.cron.scheduler.catch_up_occurrences"),
        _M("nimro.cron.jobs.enabled"), _M("nimro.cron.jobs.running"),
        _M("nimro.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Nimro Agent-owned Relay transport health" in text
