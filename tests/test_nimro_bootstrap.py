"""Tests for nimro_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Nimro entry point
(nimro, nimro-agent, nimro-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Nimro entry point imports nimro_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported nimro_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("nimro_bootstrap", None)
    import nimro_bootstrap  # noqa: WPS433
    return nimro_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Nimro CLI
...
class TestEntryPointsImportBootstrap:
    """Every Nimro entry point must import nimro_bootstrap as its
    ...
    # Entry points that invoke Nimro as a process.  Each one must
    # import nimro_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "nimro_cli/main.py",   # nimro CLI (console_script)
        "run_agent.py",          # nimro-agent (console_script)
        "acp_adapter/entry.py",  # nimro-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import nimro_bootstrap' and that
        ...
        half-finished ``nimro update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``nimro_bootstrap`` as a top-level module) leaves nimro
        ...
        """
        # Resolve relative to the nimro-agent repo root.  Tests live
        # at tests/test_nimro_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # nimro start even when nimro_bootstrap hasn't been
        ...
        assert first_import_name == "nimro_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'nimro_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import nimro_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Nimro's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("NIMRO_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["NIMRO_PYTHON_SRC_ROOT"] = env
            elif "NIMRO_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["NIMRO_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/nimro")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("NIMRO_PYTHON_SRC_ROOT", None)
            else:
                os.environ["NIMRO_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/nimro", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Nimro root.  The guard must relocate Nimro to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/nimro"])
        assert result[0] == "/opt/nimro"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Nimro root.
        assert result.index("/opt/nimro") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["NIMRO_PYTHON_SRC_ROOT"] = "/env/nimro"
            hb.harden_import_path()
            assert sys.path[0] == "/env/nimro"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("NIMRO_PYTHON_SRC_ROOT", None)
