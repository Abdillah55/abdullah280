...
def test_root_session_no_compression(db):
    ...
    assert prov["currentNimroSessionId"] == "root1"
    assert prov["rootNimroSessionId"] == "root1"
    assert prov["parentNimroSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_nimro_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentNimroSessionId"] == "old"
    assert prov["rootNimroSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousNimroSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootNimroSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"nimro"}
    assert "sessionProvenance" in meta["nimro"]
    assert meta["nimro"]["sessionProvenance"]["currentNimroSessionId"] == "root1"
