...
Nimro state stays profile-scoped through NIMRO_HOME. Host subprocesses should
...
See: https://github.com/NousResearch/nimro-agent/issues/25114
See: https://github.com/NousResearch/nimro-agent/issues/36144
See: https://github.com/NousResearch/nimro-agent/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for nimro_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(nimro_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("NIMRO_REAL_HOME", raising=False)

    def _container_mode(self, monkeypatch):
        monkeypatch.setattr(nimro_constants, "is_container", lambda: True)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("NIMRO_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        nimro_home = real_home / ".nimro" / "profiles" / "coder"
        profile_home = nimro_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIMRO_REAL_HOME", str(real_home))

        from nimro_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIMRO_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("NIMRO_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["NIMRO_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(nimro_home / "home")
        assert result["NIMRO_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_nimro_home_unset(self, monkeypatch):
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from nimro_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from nimro_cli.profiles import create_profile
