...
def test_non_expired_lock_from_dead_pid_is_reclaimed(
    ...
    monkeypatch.setattr(nimro_state.os, "name", "posix")
    ...
    monkeypatch.setattr(
        nimro_state, "psutil", SimpleNamespace(pid_exists=process_is_gone)
    )
...
def test_probe_doubt_keeps_lease_until_ttl(
    ...
    monkeypatch.setattr(
        nimro_state, "psutil", SimpleNamespace(pid_exists=probe_blows_up)
    )
