...
class TestBackendSelection:
    ...
    ``nimro tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from nimro_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.nimro/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.nimro/.env), not bare os.getenv. Parametrized over the four
