...
class TestWriteDenyExactPaths:
    ...
    def test_nimro_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.nimro/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "nimro_root"
        ...
        # Sanity check: NIMRO_HOME does point to the profile dir, not the root.
        from nimro_constants import get_nimro_home, get_default_nimro_root
        assert get_nimro_home() == profile_home
        assert get_default_nimro_root() == root
...
class TestWriteAllowed:
    ...
    def test_nimro_control_files_requested_writable(self):
