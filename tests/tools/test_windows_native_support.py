...
code paths Nimro takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``nimro_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect NIMRO_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("nimro_cli.stdio", None)
        # Fresh import now; tests import from nimro_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import nimro_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("nimro_cli.stdio", None)

    def test_no_op_on_posix(self, monkeypatch):
        from nimro_cli import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("nimro_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "nimro_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, nimro_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "nimro_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call nimro_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """nimro_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from nimro_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from nimro_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self, monkeypatch):
        ...
        Without it, the gateway-respawn watcher spawned by ``nimro update``
        (which runs under nimro-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# nimro_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "nimro_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the NIMRO_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / nimro_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "nimro_cli/tools_config.py",
            "nimro_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_nimro_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_nimro_home" in source
        assert 'cache_dir = get_nimro_home() / "cache" / "terminal"' in source
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_nimro_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "nimro_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "nimro_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        nimro_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "nimro_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from nimro_cli._subprocess_compat import" in block
    ...
    def test_watcher_threads_hidden_console_spec_into_respawn(self):
        ...
        The spec supplies the stable cwd + env overlay (NIMRO_HOME,
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "NIMRO_HOME) from the restart spec."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import nimro_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "nimro_cli.main", "gateway", "run"]
    ...
    def test_windows_keeps_console_python_and_preserves_tail(self):
        ...
        # ``from nimro_cli.gateway import PROJECT_ROOT`` resolves from
        ...
        import nimro_cli.config  # noqa: F401
        import nimro_cli.gateway  # noqa: F401
        ...
        # Mock get_nimro_home too: the real one calls Path.resolve(), which
        ...
        with mock.patch.object(gw.sys, "platform", "win32"), mock.patch.object(
            gw, "_stable_gateway_working_dir", return_value="C:/nimro"
        ), mock.patch(
            "nimro_cli.config.get_nimro_home", return_value="C:/nimro"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
...
class TestGatewayRunRestartWatcherOuterPopenFallback:
    ...
    def test_outer_watcher_retries_without_breakaway_on_oserror(self, monkeypatch):
        ...
        monkeypatch.setattr(gr, "_resolve_nimro_bin", lambda: ["nimro"])
        ...
        assert "_NIMRO_GATEWAY" not in kw1["env"]
    ...
    def test_outer_watcher_dual_failure_warns_without_leaking_secrets(
        ...
        monkeypatch.setenv("NIMRO_TEST_SECRET", secret)
        ...
        assert secret in (kwargs_used.get("env") or {}).get("NIMRO_TEST_SECRET", "")
