...
class GetSpillConfigTests(unittest.TestCase):
    def test_defaults_when_no_config(self):
        with patch.object(hos, "load_config", create=True, return_value={}):
            ...
            pass
        with patch("nimro_cli.config.load_config", return_value={}):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("nimro_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="nimro-spill-test-")
    ...
    def test_default_directory_uses_nimro_home(self):
        """When no directory override, spill under NIMRO_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="nimro-home-")
        try:
            with patch.dict(os.environ, {"NIMRO_HOME": test_home}):
                # Also patch get_nimro_home to the env var to mirror production.
            ...
            # ~/.nimro/hook_outputs depending on get_nimro_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.nimro/hook_outputs/sess")),
            ]
