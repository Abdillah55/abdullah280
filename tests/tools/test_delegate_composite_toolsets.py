...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_nimro_cli_expands_web(self):
        """nimro-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"nimro-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("nimro-cli", expanded)


    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['nimro-cli'] yields ['web']."""
        parent_toolsets = {"nimro-cli"}
