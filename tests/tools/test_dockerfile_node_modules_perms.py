...
Older images made /opt/nimro/ui-tui and /opt/nimro/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV NIMRO_TUI_DIR=/opt/nimro/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R nimro:nimro /opt/nimro/ui-tui" not in text
    assert "chown -R nimro:nimro /opt/nimro/node_modules" not in text
