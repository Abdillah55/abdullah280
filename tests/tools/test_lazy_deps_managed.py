...
A package-manager install (NixOS, and anything else shipping Nimro from a
...
def _missing_and_installable(monkeypatch):
    ...
    depend on the host's ~/.nimro/config.yaml (a local
...
def test_nixos_install_fails_fast_without_touching_the_installer(monkeypatch):
    monkeypatch.setattr("nimro_cli.config.get_managed_system", lambda: "nixos")
...
def test_unmanaged_install_is_not_blocked_by_the_guard(monkeypatch):
    ...
    monkeypatch.setattr("nimro_cli.config.get_managed_system", lambda: None)
...
def test_durable_install_target_overrides_the_guard(monkeypatch, tmp_path):
    """The container deployment sets NIMRO_MANAGED *and* a writable target.

    Dockerfile sets NIMRO_LAZY_INSTALL_TARGET and the NixOS container module
    passes NIMRO_MANAGED=true; blocking there would break that deployment.
    """
    monkeypatch.setattr("nimro_cli.config.get_managed_system", lambda: "nixos")
...
def test_unreadable_config_fails_open(monkeypatch):
    ...
    monkeypatch.setattr("nimro_cli.config.get_managed_system", _raise)
