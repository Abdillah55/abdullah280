...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("nimro_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("NIMRO_SESSION_KEY", session_key)
        monkeypatch.setenv("NIMRO_EXEC_ASK", "1")
        monkeypatch.delenv("NIMRO_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("nimro-verify-", "nimro-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "nimro-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/nimro-verify-example.py",
            "rm -f /tmp/nimro-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../nimro-verify-example.py",
            "rm -f /tmp/a/../../tmp/nimro-verify-example.py",
            "rm -f /var/tmp/nimro-verify-example.py",
            "rm -f /tmp/nimro-verify-*",
            "rm -f /tmp/nimro-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/nimro-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/nimro-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\nimro-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\nimro-victim", "Windows cmd destructive delete"),
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"NIMRO_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.nimro/.env",
            "echo x | tee $NIMRO_HOME/.env",
            'echo x | tee "$NIMRO_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestNimroConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.nimro/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.nimro/config.yaml",
            "echo '  mode: off' >> ~/.nimro/config.yaml",
            "echo x | tee ~/.nimro/config.yaml",
            "echo x | tee $NIMRO_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.nimro/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Nimro absolute config.yaml is
        # handled by the project patterns, not the Nimro-home rule.
        for cmd in (
            "cat ~/.nimro/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.nimro/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Nimro-home prefixes must fold to ~/ and
    ~/.nimro/ in dangerous-command detection.
    ...
    SSH, and Nimro config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_nimro_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/NIMRO_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("NIMRO_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.nimro/nimro-agent && source venv/bin/activate && python -m nimro_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m nimro_cli.main gateway run --replace &",
            "nohup python -m nimro_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart nimro-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Nimro security config via IFS.
            "sed${IFS}-i ~/.nimro/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep nimro) bypasses the pkill/killall name-matching
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof nimro)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof nimro_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof nimro`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Nimro service
    label achieves the same effect as `nimro gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_nimro_label_detected(self):
        for cmd in (
            "launchctl stop ai.nimro.gateway",
            "launchctl kickstart -k system/ai.nimro.gateway",
            "launchctl bootout system/ai.nimro.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.nimro.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Nimro labels, are
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("NIMRO_GATEWAY_SESSION", "NIMRO_CRON_SESSION",
                      "NIMRO_YOLO_MODE",
                      "NIMRO_SESSION_KEY", "NIMRO_INTERACTIVE")
        }
        os.environ.pop("NIMRO_YOLO_MODE", None)
        os.environ.pop("NIMRO_INTERACTIVE", None)
        # NIMRO_CRON_SESSION takes priority over NIMRO_GATEWAY_SESSION in
        ...
        os.environ.pop("NIMRO_CRON_SESSION", None)
        os.environ["NIMRO_GATEWAY_SESSION"] = "1"
        os.environ["NIMRO_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("nimro_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"NIMRO_INTERACTIVE": "1"}, clear=False):
