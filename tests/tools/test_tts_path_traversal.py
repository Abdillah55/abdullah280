...
def test_output_path_rejects_nimro_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    nimro_home = tmp_path / "nimro-home"
    nimro_home.mkdir()
    monkeypatch.setattr(file_safety, "_nimro_home_path", lambda: nimro_home)
    monkeypatch.setattr(file_safety, "_nimro_root_path", lambda: nimro_home)

    target = nimro_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    nimro_home = tmp_path / "nimro-home"
    token_dir = nimro_home / "mcp-tokens"
