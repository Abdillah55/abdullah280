...
Verifies:
  - Tools are gated on NIMRO_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `nimro chat` sessions (no NIMRO_KANBAN_TASK) must have
    ...
    schema = registry.get_definitions(set(resolve_toolset("nimro-cli")), quiet=True)
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: NIMRO_HOME isolated, NIMRO_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".nimro"
    home.mkdir()
    monkeypatch.setenv("NIMRO_HOME", str(home))
    monkeypatch.setenv("NIMRO_PROFILE", "test-worker")
    monkeypatch.delenv("NIMRO_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("NIMRO_KANBAN_TASK", tid)
    return tid
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("NIMRO_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated NIMRO_HOME with one claimed goal_mode task,
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to NIMRO_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``NIMRO_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``nimro-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "nimro-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from NIMRO_PROFILE in the fixture, not the
        # caller-supplied "nimro-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_unblock_with_pending_parents_returns_todo(monkeypatch, tmp_path):
    ...
    home.mkdir()
    monkeypatch.setenv("NIMRO_HOME", str(home))
    monkeypatch.setenv("NIMRO_PROFILE", "orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# ---------------------------------------------------------------------------
#
# A worker process has NIMRO_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no NIMRO_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # NIMRO_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no NIMRO_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via NIMRO_KANBAN_BOARD env var,
...
# restarting Nimro. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Nimro home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    NIMRO_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".nimro"
    home.mkdir()
    monkeypatch.setenv("NIMRO_HOME", str(home))
    # Make sure neither NIMRO_KANBAN_DB nor NIMRO_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("NIMRO_KANBAN_DB", raising=False)
    monkeypatch.delenv("NIMRO_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("NIMRO_KANBAN_TASK", raising=False)
    monkeypatch.setenv("NIMRO_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# - Gateway sessions: NIMRO_SESSION_PLATFORM + NIMRO_SESSION_CHAT_ID set.
# - TUI sessions: NIMRO_SESSION_KEY (or NIMRO_SESSION_ID) set, with
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    # worker_env already created <tmp>/.nimro; use a fresh sibling
    # home to avoid mkdir() colliding with the worker's directory.
    home = tmp_path / "gate-home" / ".nimro"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n  auto_subscribe_on_create: false\n"
    )
    monkeypatch.setenv("NIMRO_HOME", str(home))
    monkeypatch.setenv("NIMRO_SESSION_PLATFORM", "discord")
    monkeypatch.setenv("NIMRO_SESSION_CHAT_ID", "channel-1")
...
def test_maybe_auto_subscribe_swallows_add_notify_sub_failure(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("NIMRO_SESSION_PLATFORM", "telegram")
    monkeypatch.setenv("NIMRO_SESSION_CHAT_ID", "chat-42")
...
def allow_private_urls(monkeypatch):
    ...
    Mirrors a user setting NIMRO_ALLOW_PRIVATE_URLS on a private network.
    Resets the url_safety process-lifetime cache on both sides so the
    override neither leaks in nor out of the test.
    """
    from tools import url_safety

    monkeypatch.setenv("NIMRO_ALLOW_PRIVATE_URLS", "true")
...
def default_url_guard(monkeypatch):
    ...
    Clears NIMRO_ALLOW_PRIVATE_URLS and resets url_safety's process-lifetime
    cache on both sides so a prior test's opt-in can't leak in.
    """
    from tools import url_safety

    monkeypatch.delenv("NIMRO_ALLOW_PRIVATE_URLS", raising=False)
    url_safety._reset_allow_private_cache()
    yield
    url_safety._reset_allow_private_cache()
