...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        nimro_home.mkdir()
        (nimro_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"NIMRO_HOME": str(nimro_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(nimro_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.nimro/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        nimro_home = tmp_path / ".nimro"
        nimro_home.mkdir()
        (nimro_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.nimro/skills"


    def test_custom_container_base(self, tmp_path):
        nimro_home = tmp_path / ".nimro"
        (nimro_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"NIMRO_HOME": str(nimro_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.nimro")

        assert mounts[0]["container_path"] == "/home/user/.nimro/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.nimro/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.nimro/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape NIMRO_HOME."""
        monkeypatch.setenv("NIMRO_HOME", str(tmp_path / ".nimro"))
        (tmp_path / ".nimro").mkdir()

        # Create a sensitive file one level above nimro_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside nimro_home
    ...
    def test_nested_subdir_inside_nimro_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of NIMRO_HOME must be allowed."""
        nimro_home = tmp_path / ".nimro"
        nimro_home.mkdir()
        subdir = nimro_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside NIMRO_HOME pointing outside must be rejected."""
        nimro_home = tmp_path / ".nimro"
        nimro_home.mkdir()
        monkeypatch.setenv("NIMRO_HOME", str(nimro_home))

        # Create a sensitive file outside nimro_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside nimro_home pointing outside
        symlink = nimro_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes NIMRO_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, nimro_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape NIMRO_HOME."""
        nimro_home = tmp_path / ".nimro"
        nimro_home.mkdir()
        monkeypatch.setenv("NIMRO_HOME", str(nimro_home))
        ...
        sensitive.write_text("{}")
        self._write_config(nimro_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(nimro_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside NIMRO_HOME via config must still mount."""
        nimro_home = tmp_path / ".nimro"
        nimro_home.mkdir()
        monkeypatch.setenv("NIMRO_HOME", str(nimro_home))

        (nimro_home / "oauth.json").write_text("{}")
        self._write_config(nimro_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        nimro_home = tmp_path / ".nimro"
        nimro_home.mkdir()
        (nimro_home / "cache" / "documents").mkdir(parents=True)
        (nimro_home / "cache" / "audio").mkdir(parents=True)
        (nimro_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("NIMRO_HOME", str(nimro_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.nimro/cache/documents" in paths
        assert "/root/.nimro/cache/audio" in paths
        assert "/root/.nimro/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_nimro_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        nimro_home = tmp_path / ".nimro"
        nimro_home.mkdir()
        # Use legacy dir name with content — get_nimro_dir prefers
        # populated old over new.
        legacy_doc = nimro_home / "document_cache"
        legacy_img = nimro_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(nimro_home / "document_cache") in host_paths
        assert str(nimro_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.nimro/cache/documents" in container_paths
        assert "/root/.nimro/cache/images" in container_paths
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``NIMRO_HOME/images``, not
        ...
        """
        nimro_home = tmp_path / ".nimro"
        (nimro_home / "images").mkdir(parents=True)
        monkeypatch.setenv("NIMRO_HOME", str(nimro_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.nimro/images" in by_container
        assert by_container["/root/.nimro/images"] == str(nimro_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        nimro_home = tmp_path / ".nimro"
        (nimro_home / "images").mkdir(parents=True)
        upload = nimro_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("NIMRO_HOME", str(nimro_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.nimro/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        nimro_home = tmp_path / ".nimro"
        img_dir = nimro_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.nimro/cache/images/generated.png"
        )
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(nimro_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        nimro_home = tmp_path / ".nimro"
        doc_dir = nimro_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — NIMRO_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping NIMRO_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"NIMRO_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.nimro/google_token.json" in paths
        assert "/root/.nimro/.env" not in paths
