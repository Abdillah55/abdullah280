...
Covers:
- :class:`NimroTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.NimroMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _NIMRO_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# NimroTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = NimroTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("NIMRO_HOME", str(tmp_path))
        storage = NimroTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (NimroMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: NimroTokenStorage, **context_attrs):
    ...
    provider = _NIMRO_PROVIDER_CLS.__new__(_NIMRO_PROVIDER_CLS)
    provider._nimro_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIMRO_HOME", str(tmp_path))
        storage = NimroTokenStorage("mgr-srv")
        ...
        with patch.object(
            _NIMRO_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIMRO_HOME", str(tmp_path))
        storage = NimroTokenStorage("flow-srv")
        ...
        with patch.object(
            _NIMRO_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
