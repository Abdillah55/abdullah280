...
def fake_nimro(tmp_path, monkeypatch):
    """Build a two-profile Nimro layout and point NIMRO_HOME at
    the nimro-security profile (matching the original-incident shape).
    """
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_nimro):
        from tools.file_tools import write_file_tool
        target = fake_nimro["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_nimro):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_nimro["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "nimro-security" in result["error"]
        # File untouched.
        assert target.read_text() == original


    def test_non_nimro_path_unaffected(self, fake_nimro, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_nimro):
    ...
    def test_cross_profile_patch_bypass(self, fake_nimro):
        from tools.file_tools import patch_tool
        target = fake_nimro["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_nimro):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_nimro, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_nimro["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up NIMRO_HOME (set in
        ...
        assert "not found in active profile 'nimro-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.nimro/profiles/<name>/."""
        # Don't set NIMRO_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_nimro_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_nimro_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_nimro):
        """When active profile is 'nimro-security', the prompt warns
        ...
        assert "Active Nimro profile" in src
        assert "cross_profile=True" in src
        assert "~/.nimro/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Nimro profile: default" in src
        assert "Active Nimro profile: {active_profile}" in src
