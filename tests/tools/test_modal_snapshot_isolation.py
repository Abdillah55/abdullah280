...
def _restore_tool_modules():
    original_nimro_home = os.environ.get("NIMRO_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_nimro_home is None:
            os.environ.pop("NIMRO_HOME", None)
        else:
            os.environ["NIMRO_HOME"] = original_nimro_home
        _reset_modules(("tools", "nimro_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    nimro_cli = types.ModuleType("nimro_cli")
    nimro_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["nimro_cli"] = nimro_cli
    nimro_home = tmp_path / "nimro-home"
    os.environ["NIMRO_HOME"] = str(nimro_home)
    sys.modules["nimro_cli.config"] = types.SimpleNamespace(
        get_nimro_home=lambda: nimro_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="nimro-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": nimro_home / "modal_snapshots.json",
