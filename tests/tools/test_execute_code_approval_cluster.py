...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad NIMRO_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: NIMRO_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_nimro_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "NIMRO_HOME": "/h", "NIMRO_PROFILE": "p",
        "NIMRO_CONFIG": "/c.yaml", "NIMRO_ENV": "/e",
        "NIMRO_DELEGATED_CHILD_CONTEXT": "1",
        # other NIMRO_* → dropped (broad prefix removed)
        "NIMRO_BASE_URL": "https://x", "NIMRO_INTERACTIVE": "1",
        "NIMRO_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "NIMRO_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "NIMRO_BASE_URL", "NIMRO_INTERACTIVE", "NIMRO_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped NIMRO_* vars."""
    import logging
