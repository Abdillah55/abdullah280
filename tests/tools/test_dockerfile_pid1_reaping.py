...
- Signal forwarding runs through the init so ``docker stop`` triggers
  nimro's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, nimro accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, nimro accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    PID 1 and nimro would run without zombie reaping.
    """
    ...
    assert "/opt/nimro/docker/entrypoint-dispatch.sh" in entrypoint_line, (
    ...
    assert "exec /init /opt/nimro/docker/main-wrapper.sh" in dispatcher, (
