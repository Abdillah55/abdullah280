...
def server():
    ...
    # e.g. nimro_cli.active_sessions would bind the mocked get_nimro_home
    ...
    with patch.dict("sys.modules", {
        ...
        "nimro_cli.env_loader": MagicMock(),
        "nimro_cli.banner": MagicMock(),
        "nimro_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `NIMRO_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("NIMRO_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("NIMRO_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/nimro-agent-dev": {"name": "nimro-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "nimro-agent-dev", "session_id": sid},
        })
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `nimro config set` does)
    ...
    import nimro_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_nimro_home", lambda: tmp_path)
    ...
    # Activate midnight, as `nimro config set display.skin midnight` would.
