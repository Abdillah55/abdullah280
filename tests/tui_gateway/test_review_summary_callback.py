...
def server():
    ...
    with patch.dict(
        "sys.modules",
        {
            "nimro_constants": MagicMock(
                get_nimro_home=MagicMock(return_value="/tmp/nimro_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'nimro-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'nimro-release' patched"
    }
