...
``platform="tui"`` / ``chat_id=NIMRO_SESSION_KEY``, but no component ever
...
class TestCollectKanbanNotifications:
    ...
    def test_profile_scoped_session_reads_the_shared_board(self, tmp_path):
        ...
        nimro_cli/kanban_db.py module docstring): ``kanban_home()`` anchors on
        ``get_default_nimro_root()``, which resolves the process env and
        ...
        token = set_nimro_home_override(str(other_profile_home))
        try:
