...
def nimro_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves NIMRO_HOME.
...
@pytest.fixture()
def server(nimro_home, monkeypatch):
    ...
    # Pin config resolution to the isolated NIMRO_HOME. Sibling test
    ...
    # module-level ``_nimro_home = get_nimro_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_nimro_home", nimro_home)
...
def test_moa_bare_returns_usage(server, session, nimro_home):
    _write_moa_config(nimro_home, """
moa:
