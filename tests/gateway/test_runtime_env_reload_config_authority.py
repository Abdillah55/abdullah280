...
NIMRO_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env NIMRO_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``nimro config get terminal.backend`` still says local.
    """
    nimro_home = tmp_path / ".nimro"
    nimro_home.mkdir()
    (nimro_home / "config.yaml").write_text(
    ...
    (nimro_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
