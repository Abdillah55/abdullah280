...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.nimro."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /nimro, /btw, /stop,
        ...
        for expected in ("/nimro", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``nimro gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "nimro_feedback" in created_apps[0].registered_actions
        assert "nimro_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("nimro_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("nimro_clarify_choice")
            for pattern in clarify_choice_patterns
        )
...
class TestSlashCommands:
    ...
    # instead of as /nimro subcommands. This is the Discord/Telegram parity
    ...
    @pytest.mark.asyncio
    async def test_legacy_nimro_prefix_still_works(self, adapter):
        """Backward compat: /nimro btw foo must still route to /btw foo.

        Old workspace manifests only declared /nimro as the single slash.
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "nimro gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestTrackingStructureBounds:
    ...
    async def test_slash_command_contexts_bounded(self, adapter):
        ...
        adapter.handle_nimro_command = AsyncMock(return_value=None)
        for i in range(10):
            command = {
                "command": "/nimro",
                "text": "/status",
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/nimro-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        assert msg_event.media_urls == ["/tmp/nimro-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_NIMRO_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_nimro_slack_user_agent_prefix_format(self):
        """Module constant matches the NimroAgent/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._NIMRO_SLACK_USER_AGENT_PREFIX.startswith("NimroAgent/")
