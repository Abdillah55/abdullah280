...
(/opt/nimro/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable NIMRO_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_nimro_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable NIMRO_HOME."""
    ...
    monkeypatch.setattr(
        "nimro_constants.get_nimro_home", lambda: nimro_home
    )
    ...
    expected = nimro_home / "scripts" / "whatsapp-bridge"
