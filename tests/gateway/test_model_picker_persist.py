...
closure the PR changed, against a real temp ``NIMRO_HOME``.
"""
...
def _fake_switch_result():
    ...
    from nimro_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "nimro_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "nimro_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_nimro_home", nimro_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_nimro_home() / config.yaml`` — point it here.
    monkeypatch.setattr("nimro_constants.get_nimro_home", lambda: nimro_home)
    monkeypatch.setattr("nimro_cli.config.get_nimro_home", lambda: nimro_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_nimro_home", default_home)
    _stub_picker_dependencies(monkeypatch)
