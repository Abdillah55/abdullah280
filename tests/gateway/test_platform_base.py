...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    nimro_home.mkdir()
    monkeypatch.setattr(base, "_NIMRO_HOME", nimro_home)
    monkeypatch.setattr(base, "_NIMRO_ROOT", nimro_home)
    path = nimro_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.nimro/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("NIMRO_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("NIMRO_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("NIMRO_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("NIMRO_KANBAN_HOME", str(tmp_path / "nimro"))
        monkeypatch.setenv("NIMRO_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "nimro" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("NIMRO_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("NIMRO_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("NIMRO_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.nimro/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        nimro_dir = fake_home / ".nimro"
        (nimro_dir / "mcp-tokens").mkdir(parents=True)
        secret = nimro_dir / rel
        secret.write_text('{"access_token": "live-bearer-abc123"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._NIMRO_HOME",
            nimro_dir,
        )
        monkeypatch.setattr(
            "gateway.platforms.base._NIMRO_ROOT",
            nimro_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the NIMRO_HOME root (google_token.json)
        ...
        fake_home = tmp_path / "home"
        nimro_dir = fake_home / ".nimro"
        nimro_dir.mkdir(parents=True)
        token = nimro_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._NIMRO_HOME", nimro_dir)
        monkeypatch.setattr("gateway.platforms.base._NIMRO_ROOT", nimro_dir)

        assert BasePlatformAdapter.validate_media_delivery_path(str(token)) is None


    def test_denylist_blocks_non_cache_file_under_nimro_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.nimro
        ...
        nimro_dir = fake_home / ".nimro"
        nimro_dir.mkdir(parents=True)
        artifact = nimro_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting NIMRO_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose NIMRO_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.nimro) emits
        ...
        nimro_root = denied_root / ".nimro"
        prof_cache = nimro_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._NIMRO_ROOT", nimro_root
        )
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "NIMRO_HUMAN_DELAY_MODE": "natural",
            "NIMRO_HUMAN_DELAY_MIN_MS": "oops",
            "NIMRO_HUMAN_DELAY_MAX_MS": "still-bad",
        }
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"NIMRO_MEDIA_DELIVERY_STRICT": "1",
                                     "NIMRO_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIMRO_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Nimro
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../nimro/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.nimro/cache/media/sensitive_host_path_abc123.bin"
