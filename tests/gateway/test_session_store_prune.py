...
def test_session_store_default_db_uses_runtime_nimro_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime NIMRO_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    nimro_state before a fixture redirected NIMRO_HOME used to pin every
    default SessionDB() at the developer's real ~/.nimro/state.db.
    """
    config = GatewayConfig(default_reset_policy=SessionResetPolicy(mode="none"))
    fake_home = tmp_path / "alt_nimro_home"
    fake_home.mkdir()
    monkeypatch.setenv("NIMRO_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "nimro sessions list" in raw["_README"]
