...
* api_server subscriptions wake the creator's REAL session via the
  ...
  that never matches the raw X-Nimro-Session-Id session real turns use).
"""
...
def test_apiserver_sub_wakes_real_session_via_self_post(tmp_path, monkeypatch):
    ...
    match the raw X-Nimro-Session-Id session)."""
    monkeypatch.setenv("NIMRO_KANBAN_DB", str(tmp_path / "apiserver.db"))
    kb.init_db()
