...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Nimro"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^nimro"]}
        )
        assert adapter._message_matches_mention_patterns("nimro please help") is True
        assert adapter._message_matches_mention_patterns("please nimro help") is False
