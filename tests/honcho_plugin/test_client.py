...
from nimro_cli.profiles import _get_default_nimro_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "nimro"
        assert config.workspace_id == "nimro"
...
class TestFromGlobalConfig:
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"nimro": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="nimro-agent"
        ):
            result = config.resolve_session_name("/home/user/nimro-agent/subdir")
        assert result == "nimro-agent"


class TestResolveConfigPath:
    def test_prefers_nimro_home_when_exists(self, tmp_path):
        nimro_home = tmp_path / "nimro"
        nimro_home.mkdir()
        local_cfg = nimro_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: NIMRO_HOME points at ~/.nimro/profiles/<name>, so
        # _get_default_nimro_home() must resolve back to ~/.nimro — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".nimro"
        ...
        assert _get_default_nimro_home() == default_home
        assert result == default_cfg


class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "nimro_coder"
        assert profile_host_key("default") == "nimro"


    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"NIMRO_HONCHO_HOST": "nimro.coder"}):
            assert resolve_active_host() == "nimro.coder"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False), patch(
            ...
            os.environ.pop("NIMRO_HONCHO_HOST", None)
            # Temporarily remove nimro_cli.profiles to simulate import failure
            saved = sys.modules.get("nimro_cli.profiles")
            sys.modules["nimro_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "nimro"
            finally:
                if saved is not None:
                    sys.modules["nimro_cli.profiles"] = saved
                else:
                    sys.modules.pop("nimro_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="nimro_coder")
        assert config.host == "nimro_coder"
        assert config.workspace_id == "nimro"  # shared workspace
        assert config.ai_peer == "nimro_coder"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"nimro": {"enabled": True, "aiPeer": "nimro"}},
        }))
...
class TestGetHonchoClient:
    ...
    def test_timeout_change_triggers_client_rebuild(self):
        ...
        cfg_yaml = get_nimro_home() / "config.yaml"
    ...
    def test_managed_config_timeout_does_not_thrash_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIMRO_MANAGED_DIR", str(managed_dir))
...
class TestGetHonchoClientBaseUrlDoublePrefixFix:
    ...
    def test_local_base_url_with_v3_suffix_stripped(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("nimro_cli.config.load_config", return_value={}):
    ...
    def test_lan_default_host_empty_key_uses_local_placeholder(self, tmp_path):
        ...
        with patch.dict(os.environ, {}, clear=True), \
             patch("nimro_cli.profiles.get_active_profile_name", return_value="default"), \
