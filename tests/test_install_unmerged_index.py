...
The ``nimro update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m nimro_cli.main gateway run`` — image name
    ``pythonw``, not ``nimro.exe`` — so the ``taskkill /IM nimro.exe`` guard
    ...
    # The nimro.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM nimro.exe' in text
