...
from nimro_cli import lifecycle, plugins as plugin_api
...
""",
    ...
    monkeypatch.setenv("NIMRO_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
    return plugins_toml
...
def test_shared_metrics_and_rich_plugin_share_one_core_session(
    ...
    monkeypatch.setenv("NIMRO_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv(
        "NIMRO_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif")
    )
    monkeypatch.setattr(
    ...
    register_metrics = next(
        ...
        and item[1].startswith("nimro.nemo_relay.shared_metrics.")
    )
    register_atif = next(
    ...
    packages = list(
        (nimro_home / "telemetry" / "shared_metrics" / "outbox").glob("*.json")
    )
    assert len(packages) == 1
    package = json.loads(packages[0].read_text(encoding="utf-8"))
    assert package["metrics"][0]["name"] == "nimro.model_call.count"
    assert package["metrics"][0]["value"] == 1
    assert (tmp_path / "atif" / "nimro-atif-s1.json").exists()
...
def test_relay_tool_request_rewrite_precedes_nimro_authorization_boundary(
    tmp_path,
    monkeypatch,
):
