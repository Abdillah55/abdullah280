...
Run from the PR worktree:
    ...
    cd ~/.nimro/nimro-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.nimro/nimro-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.nimro/nimro-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a nimro-agent checkout"
)
...
# Isolated NIMRO_HOME for the config write.
