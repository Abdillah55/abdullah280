...
binary download are exercised manually by `nimro secrets bitwarden
...
def nimro_home(tmp_path, monkeypatch):
    """Point Nimro at an isolated home directory."""
    home = tmp_path / ".nimro"
    home.mkdir()
    monkeypatch.setenv("NIMRO_HOME", str(home))
    # Some modules cache get_nimro_home; clear if needed.
    import nimro_constants
    if hasattr(nimro_constants, "_NIMRO_HOME_CACHE"):
        nimro_constants._NIMRO_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(nimro_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from nimro_cli.env_loader import _apply_external_secret_sources
