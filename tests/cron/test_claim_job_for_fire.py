...
These exercise the real store against a temp NIMRO_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated NIMRO_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("NIMRO_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_nimro_home() reads the env live.
    yield tmp_path
