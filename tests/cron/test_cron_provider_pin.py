...
resolve_runtime_provider against a temp NIMRO_HOME) and the create_job
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("nimro_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
...
def _run_with_current_provider_and_model(
    ...
    *,
    ...
    fake_db = MagicMock()
    with patch("cron.scheduler._nimro_home", tmp_path), \
         patch("cron.scheduler._get_nimro_home", return_value=tmp_path), \
         patch("cron.scheduler._resolve_origin", return_value=None), \
         patch("nimro_cli.env_loader.load_nimro_dotenv"), \
         patch("nimro_cli.env_loader.reset_secret_source_cache"), \
         patch("nimro_state.SessionDB", return_value=fake_db), \
