...
class TestPerJobToolsetMcpMerge:
    ...
    def test_resolver_empty_per_job_falls_through_to_platform(self):
        ...
        with patch("nimro_cli.tools_config._get_platform_tools",
...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._nimro_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("nimro_cli.env_loader.load_nimro_dotenv"), \
             patch("nimro_cli.env_loader.reset_secret_source_cache"), \
             patch("nimro_state.SessionDB", return_value=fake_db), \
             patch(
    ...
    def _run_job_patches(self, tmp_path, extra=()):
        ...
        mock_agent.run_conversation.return_value = {"final_response": "ok"}
        base = [
            patch("cron.scheduler._nimro_home", tmp_path),
            patch("cron.scheduler._resolve_origin", return_value=None),
            patch("nimro_cli.env_loader.load_nimro_dotenv"),
            patch("nimro_cli.env_loader.reset_secret_source_cache"),
            patch("nimro_state.SessionDB", return_value=fake_db),
...
class TestRunJobConfigLogging:
    ...
    def test_bad_config_yaml_is_logged(self, caplog, tmp_path):
        ...
        with patch("cron.scheduler._nimro_home", tmp_path), \
             ...
             patch("nimro_cli.env_loader.load_nimro_dotenv"), \
             patch("nimro_cli.env_loader.reset_secret_source_cache"), \
             patch("nimro_cli.runtime_provider.resolve_runtime_provider",
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_NIMRO_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_NIMRO_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_NIMRO_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_NIMRO_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_NIMRO_TEST_CRON_UNSET_VAR}"
...
class TestRunJobModelResolution:
    ...
    ``model: null``. At fire time the scheduler must:
      1. fall back to ``NIMRO_MODEL`` env if set,
    ...
    def test_null_job_model_falls_back_to_env(self, tmp_path, monkeypatch):
        """``model: null`` on the job uses NIMRO_MODEL when set."""
        (tmp_path / "config.yaml").write_text("")
        monkeypatch.setenv("NIMRO_MODEL", "env-model")
    ...
    def test_config_model_alias_key_resolves(self, tmp_path, monkeypatch):
        ...
        ``nimro_cli/oneshot.py``, ``fallback_cmd.py`` and ``prompt_size.py``
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``nimro cron create --deliver=<platform>`` silently
