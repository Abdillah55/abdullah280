...
@pytest.fixture
def nimro_env(tmp_path, monkeypatch):
    """Isolate NIMRO_HOME for each test so jobs/scripts don't leak."""
    home = tmp_path / ".nimro"
    home.mkdir()
    (home / "scripts").mkdir()
    (home / "cron").mkdir()
    ...
    # Reload modules that cache get_nimro_home() at import time.
    import importlib
...
def test_create_job_no_agent_requires_script(nimro_env):
...
def test_update_job_roundtrips_no_agent_flag(nimro_env):
    from cron.jobs import create_job, update_job, get_job

    script_path = nimro_env / "scripts" / "w.sh"
...
def test_cronjob_tool_create_no_agent_without_script_errors(nimro_env):
...
def test_run_job_no_agent_success_returns_script_stdout(nimro_env):
    """Happy path: script exits 0 with output, delivered verbatim."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = nimro_env / "scripts" / "alert.sh"
...
def test_run_job_script_path_traversal_still_blocked(nimro_env):
