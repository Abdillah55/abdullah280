...
- Unlimited timeout (NIMRO_CRON_TIMEOUT=0)
- Backward compat: NIMRO_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """NIMRO_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """NIMRO_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("NIMRO_CRON_TIMEOUT", "1200")
...
class TestSysPathOrdering:
    ...
    def test_nimro_time_importable(self):
        """nimro_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _nimro_now
        assert callable(_nimro_now)
