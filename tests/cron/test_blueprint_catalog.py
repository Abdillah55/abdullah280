...
the shared /blueprint command handler (nimro_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated NIMRO_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("nimro://blueprint/morning-brief?")
