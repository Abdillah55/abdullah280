...
- Path resolution (absolute, relative to NIMRO_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp NIMRO_HOME."""
    nimro_home = tmp_path / ".nimro"
    nimro_home.mkdir()
    (nimro_home / "cron").mkdir()
    (nimro_home / "cron" / "output").mkdir()
    (nimro_home / "scripts").mkdir()
    monkeypatch.setenv("NIMRO_HOME", str(nimro_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "NIMRO_DIR", nimro_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", nimro_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", nimro_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", nimro_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Nimro provider env (SECURITY.md §2.3)."""
        from tools.environments.local import _NIMRO_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_NIMRO_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestCronjobToolScript:
    ...
    def test_clear_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("NIMRO_INTERACTIVE", "1")
        from tools.cronjob_tools import cronjob
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.nimro/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("NIMRO_SESSION_PLATFORM") is None
        assert os.environ.get("NIMRO_SESSION_CHAT_ID") is None
        assert os.environ.get("NIMRO_SESSION_CHAT_NAME") is None
