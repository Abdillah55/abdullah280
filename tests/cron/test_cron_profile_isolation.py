...
profile's NIMRO_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.nimro/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active NIMRO_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever NIMRO_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    NIMRO_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import nimro_constants

    monkeypatch.setattr(
        nimro_constants, "_get_platform_default_nimro_home", lambda: root
    )
    monkeypatch.setenv("NIMRO_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile NIMRO_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "nimro_home"
    ...
    assert nimro_constants.get_nimro_home().resolve() == profile_home.resolve()
    assert nimro_constants.get_default_nimro_root().resolve() == root.resolve()

    # cron/jobs.py computes NIMRO_DIR from get_nimro_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.NIMRO_DIR.resolve() == profile_home.resolve()
        assert (
