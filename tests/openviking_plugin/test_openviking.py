...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/nimro/.overview.md") == "viking://user/nimro"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/nimro/memories/profile.md") == "viking://user/nimro/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_nimro_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIMRO_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingConfigSchema:
    ...
    def test_recall_config_reads_from_config_yaml(self, monkeypatch, tmp_path):
        ...
        # Populate config.yaml in the temp NIMRO_HOME
        nimro_home = tmp_path / "nimro_test"
        nimro_home.mkdir(exist_ok=True)
        config_yaml = nimro_home / "config.yaml"
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/nimro"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/nimro/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/nimro/.overview.md"
        assert result["resolved_uri"] == "viking://user/nimro"
    ...
    def test_read_accepts_uri_batch_and_caps_batch_full_content(self):
        provider = OpenVikingMemoryProvider()
        uris = [
            "viking://user/nimro/memories/a.md",
            "viking://user/nimro/memories/b.md",
            "viking://user/nimro/memories/c.md",
            "viking://user/nimro/memories/d.md",
        ]
        provider._client = FakeVikingClient(
            {
                (
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/nimro/.overview.md", "level": "overview"})
            assert False, "Expected summary endpoint error to be raised"
        except RuntimeError:
            pass

        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/nimro"}),
        ]
...
class TestOpenVikingAutoRecallPrefetch:
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/peers/nimro/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        monkeypatch.setenv("OPENVIKING_AGENT", "nimro")
        ...
        assert records["reads"] == ["viking://user/peers/nimro/memories/e2e-full.md"]
        ...
        assert all(headers.get("x-openviking-actor-peer") == "nimro" for headers in normalized_headers)
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/nimro/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/nimro/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/nimro"}))

        assert result["path"] == "viking://user/nimro"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/nimro/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/nimro/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/nimro"},
...
class TestEnsureClientReloadsEnv:
    ...
    def test_ensure_client_rebuilds_when_api_key_changes(self, monkeypatch):
        ...
        class _StubClient:
            def __init__(self, endpoint, api_key, account="", user="", agent="nimro"):
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert instances[1].posts[0][1]["uri"].startswith(
            "viking://user/peers/nimro/memories/"
        )
...
class TestUnavailableWarningsPromiseRetry:
    ...
    reads as "it never recovers" and sends people restarting nimro for
    nothing (#5721).
    """

    @staticmethod
    def _assert_promises_retry(message: str) -> None:
        assert "for this Nimro run" not in message, message
