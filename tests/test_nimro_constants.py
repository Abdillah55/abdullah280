"""Tests for nimro_constants module."""
...
from nimro_constants import (
    ...
    find_nimro_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_nimro_root,
    get_nimro_dir,
    get_nimro_home,
    get_process_nimro_home,
    heal_nimro_managed_node,
    nimro_managed_node_tree_present,
    iter_nimro_node_dirs,
    is_container,
    node_tool_runnable,
    ...
    with_nimro_node_path,
)


class TestGetDefaultNimroRoot:
    """Tests for get_default_nimro_root() — Docker/custom deployment awareness."""

    def test_no_nimro_home_returns_native(self, tmp_path, monkeypatch):
        """When NIMRO_HOME is not set, returns ~/.nimro."""
        monkeypatch.delenv("NIMRO_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_nimro_root() == tmp_path / ".nimro"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (NIMRO_HOME=<root>/profiles/<name>),
        returns the Docker root, not the profile dir."""
        docker_root = tmp_path / "opt" / "data"
        profile = docker_root / "profiles" / "coder"
        profile.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIMRO_HOME", str(profile))
        assert get_default_nimro_root() == docker_root

    def test_no_nimro_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\nimro, not ~/.nimro."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("NIMRO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(nimro_constants.sys, "platform", "win32")

        assert get_default_nimro_root() == local_appdata / "nimro"



class TestGetNimroHome:
    """Tests for get_nimro_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When NIMRO_HOME is unset on Windows, use %LOCALAPPDATA%\\nimro."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("NIMRO_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(nimro_constants.sys, "platform", "win32")
        monkeypatch.setattr(nimro_constants, "_profile_fallback_warned", False)

        assert get_nimro_home() == local_appdata / "nimro"


class TestGetProcessNimroHome:
    """Tests for get_process_nimro_home() — process launch scope.
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_nimro_home() == home




class TestNimroManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_nimro_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nimro_constants.sys, "platform", "win32")
        monkeypatch.setenv("NIMRO_HOME", str(home))
        monkeypatch.setattr(nimro_constants, "node_tool_runnable", lambda path: True)

        assert find_nimro_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nimro_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(nimro_constants, "heal_nimro_managed_node", lambda: False)
        monkeypatch.setattr(
            nimro_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert nimro_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Nimro-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIMRO_HOME", str(profile_home))
        ...
        monkeypatch.setattr(nimro_constants, "heal_nimro_managed_node", _heal)
    ...
    def test_outdated_managed_node_heals_to_target_major(self, tmp_path, monkeypatch):
        ...
        target = nimro_constants._NIMRO_NODE_TARGET_MAJOR
        profile_home = tmp_path / "profiles" / "assistant"
        managed_bin = profile_home / "node" / "bin"
        managed_bin.mkdir(parents=True)
        old_node = self._stub(
        ...
        resolved = nimro_constants.find_nimro_node_executable("node")
        assert heal_called["value"] is True
    ...
    def test_outdated_managed_node_survives_failed_heal(self, tmp_path, monkeypatch):
        ...
        assert nimro_constants.find_nimro_node_executable("node") == str(old_node)
    ...
    def test_target_major_managed_node_does_not_heal(self, tmp_path, monkeypatch):
        ...
        assert nimro_constants.find_nimro_node_executable("node") == str(node)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(nimro_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(nimro_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from nimro_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".nimro"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import nimro_cli._subprocess_compat as subprocess_compat
...
class TestGetNimroDir:
    """Tests for ``get_nimro_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_nimro_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_nimro_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert nimro_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert nimro_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert nimro_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert nimro_constants.windows_path_to_wsl("/home/alex") is None
        assert nimro_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(nimro_constants, "is_wsl", lambda: True)
        assert nimro_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert nimro_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert nimro_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
