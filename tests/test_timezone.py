"""
Tests for timezone support (nimro_time module + integration points).
...
def _reset_nimro_time_cache():
    """Reset the nimro_time module cache (replacement for removed reset_cache)."""
    nimro_time._cached_tz = None
    nimro_time._cached_tz_name = None
    nimro_time._cache_resolved = False


# =========================================================================
# nimro_time.now() — core helper
# =========================================================================

class TestNimroTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_nimro_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["NIMRO_TIMEZONE"] = "Asia/Kolkata"
        result = nimro_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["NIMRO_TIMEZONE"] = "Europe/London"
        tz = nimro_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When NIMRO_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) NIMRO_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("NIMRO_TIMEZONE=" + os.environ.get("NIMRO_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "NIMRO_TIMEZONE=NOT_SET" in result["output"], (
            "NIMRO_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When NIMRO_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("NIMRO_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=nimro_tz) which shifted
        absolute time when system-local tz != Nimro tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Nimro tz
        ...
        # Use a Nimro timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _nimro_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["NIMRO_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_nimro_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Nimro timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["NIMRO_TIMEZONE"] = "US/Eastern"
        _reset_nimro_time_cache()
