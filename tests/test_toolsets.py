...
class TestToolsetConsistency:
    ...
    def test_nimro_platforms_share_core_tools(self):
        """All nimro-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on nimro-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["nimro-cli", "nimro-telegram", "nimro-discord", "nimro-whatsapp", "nimro-slack", "nimro-signal", "nimro-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_nimro_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("nimro-whatsapp")
