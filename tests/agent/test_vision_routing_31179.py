...
def isolated_home(monkeypatch):
    """Temp NIMRO_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="nimro_test_31179_")
    nimro_home = os.path.join(test_home, ".nimro")
    os.makedirs(nimro_home)
...
def _fresh_modules():
    """Drop cached nimro modules so each test reloads against current env."""
    _drop_reload_targets()
