"""Tests for the core Relay-managed Nimro tool adapter."""
...
def relay_turn(tmp_path, monkeypatch):
    monkeypatch.setenv("NIMRO_HOME", str(tmp_path / "profile"))
...
def test_request_rewrite_reaches_authorized_callback_once(relay_turn):
    ...
    relay.intercepts.register_tool_request(
        "nimro-test-tool-request", 1, False, rewrite_request
    )
    relay.intercepts.register_tool_execution(
        "nimro-test-tool-execution", 1, wrap_execution
    )
    try:
        ...
        relay.intercepts.deregister_tool_execution("nimro-test-tool-execution")
        relay.intercepts.deregister_tool_request("nimro-test-tool-request")
