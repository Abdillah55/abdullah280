...
from nimro_cli.nous_subscription import NousFeatureState, NousSubscriptionFeatures
...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("nimro_cli.config.load_config", default_load_config)
        monkeypatch.setattr("nimro_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("nimro_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("nimro_cli.config.load_config_readonly", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("nimro_cli.config.load_config", lambda: {})
        monkeypatch.setattr("nimro_cli.config.load_config_readonly", lambda: {})
...
class TestBuildNousSubscriptionPrompt:
    def test_includes_active_subscription_features(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nimro_cli.nous_subscription.get_nous_subscription_features",
...
class TestBuildContextFilesPrompt:
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("NIMRO_HOME", str(tmp_path / "nimro_home"))
        nimro_home = tmp_path / "nimro_home"
        nimro_home.mkdir()
        (nimro_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
...
# =========================================================================
# .nimro.md helper functions
# =========================================================================


class TestFindNimroMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".nimro.md").write_text("rules")
        assert _find_nimro_md(tmp_path) == tmp_path / ".nimro.md"



    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".nimro.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_nimro_md(sub) == tmp_path / ".nimro.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .nimro.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """NIMRO_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("NIMRO_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
