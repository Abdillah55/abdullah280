...
``nimro_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``NIMRO_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "nimro_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "nimro_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / nimro_subprocess_env /
