...
@pytest.fixture
def nimro_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.nimro/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".nimro"
    home.mkdir()
...
def test_cache_invalidates_on_mtime_change(nimro_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = nimro_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different NIMRO_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".nimro"
    ...
    monkeypatch.setenv("NIMRO_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("NIMRO_HOME", str(home_b))
