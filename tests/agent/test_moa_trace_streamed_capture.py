...
path via ``nimro chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp NIMRO_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point NIMRO_HOME at a temp dir and turn moa.save_traces on."""
    nimro_home = tmp_path / ".nimro"
    nimro_home.mkdir()
    monkeypatch.setenv("NIMRO_HOME", str(nimro_home))

    # save_moa_turn reads config via nimro_cli.config.load_config; stub it to
