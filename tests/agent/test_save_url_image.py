...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate NIMRO_HOME under tmp_path."""
    monkeypatch.setenv("NIMRO_HOME", str(tmp_path / ".nimro"))
    (tmp_path / ".nimro").mkdir()

    # Force the constants/image cache helpers to re-read NIMRO_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("nimro_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_nimro_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under NIMRO_HOME — gateway cleanup
