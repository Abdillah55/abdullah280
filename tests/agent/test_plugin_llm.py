...
""",
        ...
        from nimro_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        nimro_home = tmp_path / ".nimro"
        nimro_home.mkdir()
        (nimro_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("NIMRO_HOME", str(nimro_home))
        from nimro_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from nimro_cli.plugins import PluginContext, PluginManifest, PluginManager
