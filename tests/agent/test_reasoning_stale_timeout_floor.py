...
thinking phase.  Nimro's default cloud-stream stale detector
(``NIMRO_STREAM_STALE_TIMEOUT`` = 180s) and non-stream detector
(``NIMRO_API_CALL_STALE_TIMEOUT`` = 90s) both fire before the
...
def test_non_reasoning_model_keeps_default(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("NIMRO_API_CALL_STALE_TIMEOUT", raising=False)
