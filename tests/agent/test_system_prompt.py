...
def test_coding_prompt_preserves_legacy_workspace_order(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "NIMRO_AGENT_HELP_GUIDANCE", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_nimro_home", lambda: Path("/nimro"))

    expected_profile = (
        "Active Nimro profile: default. Other profiles (if any) live "
        "under /nimro/profiles/<name>/. Each profile has its own skills/, "
    ...
    with (
        ...
        patch("nimro_time.now", return_value=datetime(2026, 1, 2)),
    ):
...
class TestTelegramRichMessagesHint:
    ...
    def test_base_hint_without_rich_messages(self, monkeypatch):
        ...
        with patch("nimro_cli.config.load_config_readonly") as mock_cfg:
