...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated NIMRO_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="nimro_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".nimro"))
    prev_home = os.environ.get("NIMRO_HOME")
    os.environ["NIMRO_HOME"] = os.path.join(test_home, ".nimro")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("nimro_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["NIMRO_HOME"] = prev_home
