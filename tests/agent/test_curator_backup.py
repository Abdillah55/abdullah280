...
def backup_env(monkeypatch, tmp_path):
    """Isolate NIMRO_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".nimro"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_nimro_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd nimro_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under NIMRO_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level NIMRO_DIR picks up the tmp HOME."""
    import nimro_constants
    importlib.reload(nimro_constants)
