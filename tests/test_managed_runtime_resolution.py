"""Guard: Nimro-owned subprocesses must not resolve managed runtimes by bare PATH.

Nimro installs runtimes for itself — ``uv`` at ``$NIMRO_HOME/bin/uv``, Node at
``$NIMRO_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Nimro's own
...
* when a system copy also exists, the one Nimro does not own wins — which is
...
``iter_nimro_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Nimro provisions into NIMRO_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Nimro-owned subprocess code: plugins ship their own
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Nimro-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$NIMRO_HOME/bin (uv) and $NIMRO_HOME/node are not on an "
        ...
        "  node/npm -> nimro_constants.find_node_executable()\n"
        "  PATH env -> nimro_constants.iter_nimro_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(nimro_constants, helper))


def test_managed_uv_helpers_exist():
    from nimro_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
