"""Tests for the direct Nimro-to-Relay shared-metrics runtime."""
...
from nimro_cli import lifecycle, plugins
from nimro_cli.observability import relay_runtime, relay_shared_metrics
from nimro_cli.plugins import PluginManager
...
def direct_runtime(tmp_path, monkeypatch):
    fake = _Relay()
    monkeypatch.setenv("NIMRO_HOME", str(tmp_path / "nimro-home"))
    monkeypatch.setattr(relay_runtime, "_load_nemo_relay", lambda: fake)
    monkeypatch.setattr(
        "nimro_cli.config.read_raw_config_readonly",
...
def test_direct_runtime_records_without_enabling_a_plugin(direct_runtime, tmp_path):
    ...
    assert scope_starts[1][1] == "nimro.task_run"
    ...
    root = tmp_path / "nimro-home" / "telemetry" / "shared_metrics"
    ...
    assert set(metrics) == {
        "nimro.model_call.count",
        "nimro.task_run.finished",
        "nimro.task_run.started",
    }
    assert metrics["nimro.model_call.count"]["dimensions"]["model_family"] == "claude"
    assert metrics["nimro.model_call.count"]["value"] == 1
    assert metrics["nimro.task_run.started"] == {
        "name": "nimro.task_run.started",
        "type": "counter",
        "dimensions": {
            "entrypoint": "interactive",
            "execution_surface": "cli",
    ...
    terminal = metrics["nimro.task_run.finished"]["dimensions"]
...
def test_real_binding_drives_lifecycle_aggregation_export_and_snapshot(
    ...
    from nimro_cli.observability.shared_metrics import SharedMetricsStore

    root = tmp_path / "nimro-home" / "telemetry" / "shared_metrics"
    store = SharedMetricsStore(root / "metrics.sqlite3", root / "outbox")
    tomorrow = datetime.now(timezone.utc) + timedelta(days=1)
    monkeypatch.setattr(
        "nimro_cli.observability.shared_metrics._utc_now",
        lambda: tomorrow,
    )
    ...
    assert len(by_metric["nimro.task_run.started"]) == 1
    assert by_metric["nimro.task_run.started"][0]["value"] == 3
    assert {
        counter["dimensions"]["outcome"]
        for counter in by_metric["nimro.model_call.count"]
    } == {"success", "failed", "cancelled"}
    terminal_by_outcome = {
        counter["dimensions"]["outcome"]: counter
        for counter in by_metric["nimro.task_run.finished"]
    }
    ...
    for package in package_payloads:
        assert package["schema_version"] == "nimro.shared_metrics.v1"
...
def test_core_runtime_is_fail_open_without_a_published_binding(monkeypatch, caplog):
    ...
    assert not relay_runtime.emit_mark("nimro.probe", session_id="s1")
    assert "Nimro Relay runtime initialization failed" in caplog.text
    relay_runtime._reset_for_tests()
...
def test_managed_config_cannot_override_shared_metrics_consent(
    ...
    from nimro_cli import config, managed_scope
    ...
    monkeypatch.setenv("NIMRO_MANAGED_DIR", str(managed))
    config._LOAD_CONFIG_CACHE.clear()
    config._RAW_CONFIG_CACHE.clear()
    managed_scope.invalidate_managed_cache()

    token = set_nimro_home_override(profile)
    try:
        assert (
...
def test_disabling_shared_metrics_stops_collection_and_shutdown_export(
    ...
    runtime.subscriber(SimpleNamespace(
        ...
        scope_category="start",
        metadata={
            "nimro.metrics.schema_version": "nimro.metrics.event.v1",
...
def test_failed_flush_keeps_daily_export_open_for_later_task(
    ...
    metrics = {metric["name"]: metric for metric in package["metrics"]}
    assert metrics["nimro.task_run.started"]["value"] == 2
    assert metrics["nimro.task_run.finished"]["value"] == 2
    assert flush_attempts == 2
    assert "Nimro shared-metrics task flush failed" in caplog.text
