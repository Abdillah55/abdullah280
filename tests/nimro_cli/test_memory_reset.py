"""Tests for the `nimro memory reset` CLI command.
...
- Profile-scoped reset (uses NIMRO_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake NIMRO_HOME with memory files."""
    nimro_home = tmp_path / ".nimro"
    memories = nimro_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("NIMRO_HOME", str(nimro_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nNimro repo is at ~/.nimro/nimro-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `nimro memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        nimro_home = tmp_path / ".nimro"
        (nimro_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("NIMRO_HOME", str(nimro_home))
