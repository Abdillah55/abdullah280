...
def test_find_install_script_from_checkout(tmp_path):
    ...
    from nimro_cli.dep_ensure import _find_install_script
    scripts_dir = tmp_path / "scripts"
    scripts_dir.mkdir()
    (scripts_dir / "install.sh").write_text("#!/bin/bash", encoding="utf-8")
    with patch("nimro_cli.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(package_dir=tmp_path / "nimro_cli", repo_root=tmp_path)
...
def test_ensure_dependency_uses_powershell_on_windows(tmp_path):
    ...
    with patch("nimro_cli.dep_ensure._IS_WINDOWS", True), \
         patch("nimro_cli.dep_ensure._DEP_CHECKS", {"node": lambda: False}), \
         patch("nimro_cli.dep_ensure._find_install_script", return_value=(scripts_dir / "install.ps1", "powershell")), \
         patch("nimro_cli.dep_ensure.shutil") as mock_shutil, \
         patch("nimro_constants.get_nimro_home", return_value=tmp_path / "fakehome"), \
