...
- Bare `nimro setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `nimro setup --quick` runs the narrower "fill in missing items" flow.
- `nimro setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("nimro_cli.setup.ensure_nimro_home", {}),
        ("nimro_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("nimro_cli.config.is_managed", {"return_value": False}),
        ("nimro_cli.setup.load_config", {"return_value": {}}),
        ("nimro_cli.setup.save_config", {}),
        ("nimro_cli.setup.get_env_value", {"return_value": None}),
        ("nimro_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("nimro_cli.setup._print_setup_summary", {}),
        ("nimro_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("nimro_cli.setup.load_config", {"return_value": {}}),
        ("nimro_cli.setup.save_config", {}),
        ("nimro_cli.auth.get_active_provider", {"return_value": None}),
        ("nimro_cli.setup.get_env_value", {"return_value": None}),
        ("nimro_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `nimro setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="nimro_cli.setup.prompt_choice",
                quick="nimro_cli.setup._run_quick_setup",
                model="nimro_cli.setup.setup_model_provider",
                terminal="nimro_cli.setup.setup_terminal_backend",
                agent="nimro_cli.setup.setup_agent_settings",
                gateway="nimro_cli.setup.setup_gateway",
                tools="nimro_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("nimro_cli.setup.prompt_choice", {"return_value": 0}),
                first="nimro_cli.setup._run_first_time_quick_setup",
            )
            from nimro_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "nimro_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["nimro", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
