...
The terminal `nimro model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import nimro_cli.providers as providers_mod
import pytest
from nimro_cli.model_switch import list_authenticated_providers, switch_model
from nimro_cli.providers import resolve_provider_full
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "NIMRO_OVERLAYS", {})
    monkeypatch.setattr("nimro_cli.models.fetch_api_models", lambda *a, **k: [])
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("nimro_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("nimro_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("nimro_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_picker_selection_resolves_named_custom_provider_model_id(monkeypatch):
    ...
    monkeypatch.setattr("nimro_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr(
        "nimro_cli.model_switch.get_model_capabilities",
        lambda *a, **k: None,
    )
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("nimro_cli.models.fetch_lmstudio_models", _fake_fetch)
...
def test_resolve_custom_provider_passes_key_env():
    ...
    """
    from nimro_cli.providers import resolve_custom_provider
...
def test_discovered_models_auto_saved_to_cache(monkeypatch):
    ...
    monkeypatch.setattr("nimro_cli.models.fetch_api_models", fake_fetch_api_models)
    monkeypatch.setattr(
        "nimro_cli.model_switch._save_discovered_models_to_config",
...
def test_save_discovered_models_preserves_dict_form(monkeypatch):
    ...
    from nimro_cli.model_switch import _save_discovered_models_to_config
    ...
    monkeypatch.setattr("nimro_cli.config.save_config", fake_save)
    monkeypatch.setattr(
