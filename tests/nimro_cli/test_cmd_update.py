...
from nimro_cli.main import cmd_update, PROJECT_ROOT
...
def _patch_managed_uv(request):
    ...
    with patch("nimro_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         patch("nimro_cli.managed_uv.ensure_uv", side_effect=_fake_ensure_uv), \
         patch("nimro_cli.managed_uv.update_managed_uv", side_effect=_fake_update_managed_uv):
        yield


class TestCmdUpdateNpmLockfileCache:
    @staticmethod
    def _cache_file(nimro_root, project_root):
        cache_key = hashlib.sha256(str(project_root).encode()).hexdigest()[:12]
        return nimro_root / f".npm_lock_hash_{cache_key}"



    def test_record_npm_lockfile_hash(self, tmp_path, monkeypatch):
        from nimro_cli import main as hm
    ...
    def test_package_json_only_edit_defeats_skip(self, tmp_path, monkeypatch):
        ...
        npm — lockfile unchanged. `nimro update` must still install (the
    ...
    def test_update_uses_one_shared_npm_cache_across_profiles(
        ...
        shared_root = tmp_path / ".nimro"
        named_profile = shared_root / "profiles" / "work"
        named_profile.mkdir(parents=True)

        monkeypatch.setattr(hm, "PROJECT_ROOT", checkout)
        monkeypatch.setattr(nimro_constants.Path, "home", lambda: tmp_path)
        monkeypatch.setattr(
            nimro_constants, "find_node_executable", lambda _name: "/usr/bin/npm"
        )
        ...
        with patch.object(
            ...
            monkeypatch.setenv("NIMRO_HOME", str(shared_root))
            hm._update_node_dependencies()

            monkeypatch.setenv("NIMRO_HOME", str(named_profile))
            hm._update_node_dependencies()
...
class TestCmdUpdateTermuxUvBootstrap:
    ...
    def test_termux_reuses_existing_path_uv_without_pip(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm, "_is_termux_env", lambda env=None: True)
        # Production resolve_uv only checks $NIMRO_HOME/bin/uv; model an empty
        # managed dir so the PATH probe is what surfaces the packaged uv.
        monkeypatch.setattr("nimro_cli.managed_uv.resolve_uv", lambda: None)
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind NousResearch/nimro-agent silently misses updates.
        """
        from nimro_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/nimro-agent.git",
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "nimro_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "nimro_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("nimro_cli.config.check_config_version", return_value=(1, 2)), patch(
            "nimro_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("nimro_cli.main.sys") as mock_sys:
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "nimro_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "nimro_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "nimro_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "nimro_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "nimro_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "nimro_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "nimro_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "nimro_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("nimro_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.nimro"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.nimro/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.nimro/profiles/work"))
        ...
        with (
            patch("nimro_cli.profiles.list_profiles", return_value=all_profiles),
            patch("nimro_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("nimro_cli.profiles.list_profiles", return_value=[default_p]),
            patch("nimro_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
...
class TestCmdUpdateBranchFlag:
    """``nimro update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``nimro update --check --branch <name>`` honors the branch override.
    ...
    @patch("nimro_cli.config.detect_install_method", return_value="git")
    @patch("subprocess.run")
...
class TestCmdUpdateZipBranchRefusal:
    """``nimro update --branch=<non-main>`` must refuse on the ZIP fallback path.
    ...
    def test_zip_fallback_refuses_non_main_branch(self, capsys):
        from nimro_cli.main import _update_via_zip
