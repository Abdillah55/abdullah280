...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from nimro_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # nimro model, not via upgrade prompts.
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_prefers_live_api(self, monkeypatch):
        monkeypatch.setattr(
            "nimro_cli.auth.resolve_api_key_provider_credentials",
            lambda provider_id: {
        ...
        monkeypatch.setattr(
            "nimro_cli.models.fetch_api_models",
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in NIMRO_OVERLAYS
        overlay = NIMRO_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    def test_provider_env_hints_include_gmi(self):
        from nimro_cli.doctor import _PROVIDER_ENV_HINTS
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a NimroAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("NimroAgent/")

    def test_gmi_profile_declares_nimro_user_agent(self):
        """The GMI plugin sets a NimroAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("NimroAgent/"), (
            f"expected GMI profile User-Agent to start with 'NimroAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("nimro_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "nimro_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["nimro", "chat", "--provider", "gmi"])

        from nimro_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("nimro_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("nimro_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("nimro_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)
    ...
    def test_model_flow_api_key_provider_persists_gmi_selection(self, monkeypatch):
        ...
        with patch(
            ...
            "nimro_cli.auth.deactivate_provider",
        ), patch(
            "builtins.input",
            return_value="",
        ):
            from nimro_cli.main import _model_flow_api_key_provider
        ...
        config = yaml.safe_load((get_nimro_home() / "config.yaml").read_text()) or {}
