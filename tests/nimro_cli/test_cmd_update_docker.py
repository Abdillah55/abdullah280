"""Tests for ``nimro update`` / ``--check`` inside the Docker container.
...
fix, ``nimro update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Nimro, not an update to the running container, so the message
...
from nimro_cli.main import _cmd_update_check, cmd_update
...
@patch("nimro_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``nimro update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull nousresearch/nimro-agent:latest" in out
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull nousresearch/nimro-agent:latest" in msg
    ...
    assert "NIMRO_HOME" in msg or "/opt/data" in msg, (
