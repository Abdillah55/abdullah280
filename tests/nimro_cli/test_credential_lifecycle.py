...
NIMRO_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def nimro_home(monkeypatch, tmp_path):
    """Fresh NIMRO_HOME with .env + auth.json + config.yaml fixtures."""
    home = tmp_path / "cred_home"
    home.mkdir()
    monkeypatch.setenv("NIMRO_HOME", str(home))
    from nimro_cli.config import invalidate_env_cache
...
def test_delete_clears_provider_models_cache(nimro_home):
    _write_env(nimro_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(nimro_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = nimro_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(nimro_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(nimro_home, OPENAI_API_KEY=old)
    _write_config(
        nimro_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = nimro_home.joinpath("config.yaml").read_text(encoding="utf-8")
