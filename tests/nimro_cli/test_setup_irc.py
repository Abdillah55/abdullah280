"""Tests for IRC gateway configuration via `nimro setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Nimro install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Nimro install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#nimro")
            monkeypatch.setenv("IRC_NICKNAME", "nimro-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `nimro setup gateway` experience with IRC present."""
