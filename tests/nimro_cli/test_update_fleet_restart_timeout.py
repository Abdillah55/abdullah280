...
On hosts with many profile-backed ``nimro-gateway*.service`` units,
``nimro update`` used to wrap the entire per-scope unit loop in a single
...
class TestFleetRestartTimeoutIsolation:
    def test_timeout_on_middle_unit_continues_remaining_units(self):
        units = [
            "nimro-gateway-xiaomo1",
            "nimro-gateway-xiaomo2",
            "nimro-gateway-xiaomo3",
            "nimro-gateway-xiaomo4",
            "nimro-gateway-xiaomo5",
            "nimro-gateway-xiaomo6",
            "nimro-gateway-xiaomo7",
            "nimro-gateway",
        ]
        restarted: list[str] = []
        failed: list[str] = []
        ...
        def process_unit(svc_name: str) -> None:
            if svc_name == "nimro-gateway-xiaomo5":
        ...
        assert failed == ["nimro-gateway-xiaomo5"]
        assert restarted == [
            "nimro-gateway-xiaomo1",
            "nimro-gateway-xiaomo2",
            "nimro-gateway-xiaomo3",
        ...
        assert timeout_cmds == [
            ["systemctl", "--user", "--no-ask-password", "restart", "nimro-gateway-xiaomo5"]
        ]
    ...
    def test_non_gateway_units_in_list_output_are_ignored(self):
        ...
        _for_each_systemd_gateway_unit(
            "\n".join(
                [
                    ...
                    "nimro-gateway-coder.service loaded active running",
        ...
        assert seen == ["nimro-gateway-coder"]
    ...
    def test_process_errors_other_than_timeout_still_propagate(self):
        ...
        with pytest.raises(RuntimeError, match="not a timeout"):
            _for_each_systemd_gateway_unit(
                _list_units_stdout(["nimro-gateway"]),
                process_unit=process_unit,
...
class TestIncompleteFleetRestartWarning:
    def test_warns_with_exact_unrestarted_units(self, capsys):
        _warn_incomplete_gateway_fleet_restart(
            ["nimro-gateway-xiaomo5", "nimro-gateway-xiaomo6", "nimro-gateway-xiaomo5"]
        )
        out = capsys.readouterr().out
        assert "Update incomplete" in out
        assert out.count("nimro-gateway-xiaomo5") == 1
        assert "nimro-gateway-xiaomo6" in out
