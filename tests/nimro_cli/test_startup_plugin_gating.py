...
``nimro_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``nimro --help``, ``nimro version``,
``nimro logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``nimro --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from nimro_cli import main as _main
...
def test_deferred_platform_cli_resolution_targets_matching_platform():
    ...
    Photon registers ``nimro photon`` only when its adapter module is
...
def test_deferred_platform_loader_registers_cli_command_before_parser_table():
    ...
    helper calls it. This regression exercises the full chain nimro-sweeper
    asked for:
    ...
    from nimro_cli.plugins import PluginContext, PluginManager, PluginManifest
    ...
    def _fake_loader():
        ...
        # top-level nimro <name> CLI command via PluginContext.
