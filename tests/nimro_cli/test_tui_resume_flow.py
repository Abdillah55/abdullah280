...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_nimro_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import nimro_cli.oneshot as oneshot
        from nimro_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """nimro -z bypasses NimroCLI, but recall still needs SessionDB."""
    from nimro_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "nimro_state", mod("nimro_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "nimro_cli.config",
        mod("nimro_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "nimro_cli.models",
        mod("nimro_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "nimro_cli.tools_config",
        mod("nimro_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["NIMRO_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/nimro-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["NIMRO_MODEL"] == "nous/nimro-test"
    assert env["NIMRO_INFERENCE_MODEL"] == "nous/nimro-test"
    assert env["NIMRO_TUI_PROVIDER"] == "nous"
    assert env["NIMRO_INFERENCE_PROVIDER"] == "nous"
    assert env["NIMRO_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["NIMRO_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("nimro-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_nimro_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("NIMRO_TUI_DIR", raising=False)
