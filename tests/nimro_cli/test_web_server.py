"""Tests for nimro_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_nimro_home):
    """Drop the example-dashboard fixture into the per-test NIMRO_HOME
    ...
    ``NIMRO_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, nimro-achievements, model providers
    ...
    from nimro_cli import web_server

    user_plugins_dir = get_nimro_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `nimro plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down NIMRO_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Nimro vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints NIMRO_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import nimro_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("NIMRO_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("NIMRO_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nimro_home):
        """Create a TestClient and isolate the state DB under the test NIMRO_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import nimro_state
        from nimro_constants import get_nimro_home
        from nimro_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(nimro_state, "DEFAULT_DB_PATH", get_nimro_home() / "state.db")
    ...
    def test_get_status_loads_gateway_config_off_event_loop(self, monkeypatch):
        ...
        import nimro_cli.web_server as web_server
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        NIMRO_HOME contextvar override (#56986), so /api/messaging/platforms
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from nimro_constants import get_nimro_home
        from nimro_cli.config import load_env
        ...
        config_path = get_nimro_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from nimro_constants import get_nimro_home
        from nimro_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide NIMRO_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from nimro_constants import get_nimro_home

        path = get_nimro_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_nimro_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["nimro"]["workspace"] == "myws"
        assert cfg["hosts"]["nimro"]["peerName"] == "eri"
        assert cfg["hosts"]["nimro"]["environment"] == "local"
        assert cfg["hosts"]["nimro"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["nimro"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from nimro_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_nimro_returns_docker_guidance_without_spawning(self, monkeypatch):
        import nimro_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn nimro update")
        ...
        monkeypatch.setattr(web_server, "_spawn_nimro_action", fail_spawn)
        web_server._ACTION_PROCS.pop("nimro-update", None)
        web_server._ACTION_RESULTS.pop("nimro-update", None)

        resp = self.client.post("/api/nimro/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "nimro-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/nimro-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/nimro-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/nimro-agent:latest" in line for line in status_data["lines"])
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/nimro_cli/test_setup_hidden_env.py.
        """
        from nimro_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Nimro provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from nimro_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from nimro_cli.config import save_env_value
        from nimro_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "nimro_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/NimroSetupBot/nimro_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/NimroSetupBot/nimro_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "nimro_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_nimro_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from nimro_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from nimro_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``nimro model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from nimro_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from nimro_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from nimro_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from nimro_cli.web_server import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from nimro_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from nimro_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_nimro_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from nimro_constants import get_nimro_home
        import nimro_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        import nimro_cli.tools_config as tools_config
        from nimro_cli.nous_subscription import NousSubscriptionFeatures
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        """
        import nimro_cli.tools_config as tools_config
        from nimro_cli.nous_account import NousPortalAccountInfo

        # Logged out of Nous Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "nimro_cli.nous_subscription.get_nous_portal_account_info",
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "nimro"
        save_config(config)
        ...
        assert "nimro@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from nimro_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from nimro_cli.web_server import _denormalize_config_from_web
        from nimro_cli.config import save_config
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from nimro_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.nimro/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_nimro_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(nimro_home, name="ocean"):
        themes_dir = nimro_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="nimro-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nimro_home):
        ...
        monkeypatch.setattr(
            nimro_state, "DEFAULT_DB_PATH", get_nimro_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``nimro_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in nimro_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nimro_home):
        ...
        # Pin the SessionDB to the isolated NIMRO_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_nimro_home, _install_example_plugin):
        ...
        installs it into the per-test ``NIMRO_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into NIMRO_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, nimro-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (nimro-achievements is loaded by default).
        resp = self.client.get("/api/plugins/nimro-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        NIMRO_HOME override scopes a request to another profile."""
        from nimro_constants import (
            reset_nimro_home_override,
            set_nimro_home_override,
        )
        ...
        monkeypatch.setenv("NIMRO_HOME", str(launch_home))
        from nimro_cli import web_server
        token = set_nimro_home_override(str(other))
        try:
...
# or ``sh -c 'printf …'``) instead of the real ``nimro --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"nimro-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "NIMRO_CWD": str(tmp_path),
            "NIMRO_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["NIMRO_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from nimro_cli.pty_bridge import PtyUnavailableError
        ...
        import nimro_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from nimro_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("NIMRO_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_nimro_home, _install_example_plugin):
        ...
        the per-test ``NIMRO_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_nimro_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("NIMRO_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under NIMRO_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("NIMRO_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/nimro"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/nimro/ds-assets/bg.png)" in prefixed.text
        assert "url(/nimro/fonts-terminal/x.woff2)" in prefixed.text
