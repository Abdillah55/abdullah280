"""Tests for nimro_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from nimro_cli import relaunch as relaunch_mod


class TestResolveNimroBin:
    def test_prefers_absolute_argv0_when_executable(self, monkeypatch):
        fake = "/nix/store/abc/bin/nimro"
        ...
        assert relaunch_mod.resolve_nimro_bin() == fake
    ...
    def test_resolves_relative_argv0(self, monkeypatch, tmp_path):
        ...
        # Ensure we don't accidentally match a real 'nimro' on PATH
        monkeypatch.setattr(relaunch_mod.shutil, "which", lambda _name: None)
        assert relaunch_mod.resolve_nimro_bin() == str(fake)

    def test_falls_back_to_path_which(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["-c"])  # not a real path
        monkeypatch.setattr(
            relaunch_mod.shutil, "which", lambda name: "/usr/bin/nimro" if name == "nimro" else None
        )
        assert relaunch_mod.resolve_nimro_bin() == "/usr/bin/nimro"
...
class TestBuildRelaunchArgv:
    def test_uses_bin_when_available(self, monkeypatch):
        monkeypatch.setattr(relaunch_mod, "resolve_nimro_bin", lambda: "/usr/bin/nimro")
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv[0] == "/usr/bin/nimro"
    ...
    def test_can_disable_preserve(self, monkeypatch):
        ...
        assert argv == ["/usr/bin/nimro", "--resume", "abc"]
...
class TestRelaunch:
    def test_calls_execvp(self, monkeypatch):
        ...
        assert calls == [("/usr/bin/nimro", ["/usr/bin/nimro", "--resume", "abc"])]
    ...
    def test_windows_uses_subprocess_not_execvp(self, monkeypatch):
        ...
        nimro).  relaunch() must detect win32 and use subprocess.run +
        sys.exit instead."""
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_nimro_bin", lambda: r"C:\Users\test\nimro.exe")
    ...
    def test_windows_propagates_child_exit_code(self, monkeypatch):
        ...
        monkeypatch.setattr(relaunch_mod.sys, "platform", "win32")
        monkeypatch.setattr(relaunch_mod, "resolve_nimro_bin", lambda: r"C:\nimro.exe")
...
class TestResolveNimroBinWindowsPyGuard:
    """On Windows, resolve_nimro_bin MUST NOT return a .py path.
    ...
    def test_windows_rejects_py_argv0_falls_through_to_path(self, monkeypatch, tmp_path):
        ...
        # Force PATH lookup to return a nimro.exe so the test doesn't
        # exercise the None-fallback path (that's a separate test).
        monkeypatch.setattr(
            relaunch_mod.shutil, "which",
            lambda name: r"C:\venv\Scripts\nimro.exe" if name == "nimro" else None,
        )

        bin_path = relaunch_mod.resolve_nimro_bin()
        # Must NOT be the .py — must be the nimro.exe PATH entry.
    ...
    def test_posix_still_accepts_py_argv0(self, monkeypatch, tmp_path):
        ...
        script = tmp_path / "nimro"
        script.write_text("#!/usr/bin/env python3\n")
        script.chmod(0o755)
        monkeypatch.setattr(relaunch_mod.sys, "argv", [str(script), "chat"])
        assert relaunch_mod.resolve_nimro_bin() == str(script)

    def test_windows_py_argv0_with_no_nimro_on_path_returns_none(self, monkeypatch, tmp_path):
        """Bulletproof fallback: if argv0 is .py on Windows AND nimro.exe
        isn't on PATH, return None so the caller falls back to
        python -m nimro_cli.main."""
        script = tmp_path / "main.py"
        script.write_text("# stub")
        ...
        assert relaunch_mod.resolve_nimro_bin() is None
