"""Tests for the Nimro plugin system (nimro_cli.plugins)."""
...
from nimro_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<nimro_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<nimro_home>/plugins/``; we derive
    ``<nimro_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <NIMRO_HOME>/config.yaml.
        # Config is always read from NIMRO_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        nimro_home_str = os.environ.get("NIMRO_HOME")
        if nimro_home_str:
            nimro_home = Path(nimro_home_str)
        else:
            nimro_home = base.parent
        nimro_home.mkdir(parents=True, exist_ok=True)
        cfg_path = nimro_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        plugins_dir = tmp_path / "nimro_test" / "plugins"
        _make_plugin_dir(
            plugins_dir,
            "mw_plugin",
            register_body=(
        ...
        monkeypatch.setenv("NIMRO_HOME", str(tmp_path / "nimro_test"))
    ...
    def test_middleware_helpers_skip_no_listener_work(self, monkeypatch):
        ...
        monkeypatch.setattr("nimro_cli.plugins.get_plugin_manager", lambda: manager)
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under nimro_plugins.<name>."""
        plugins_dir = tmp_path / "nimro_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("NIMRO_HOME", str(tmp_path / "nimro_test"))

        # Clean up any prior namespace module
        sys.modules.pop("nimro_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "nimro_plugins.ns_plugin" in sys.modules
    ...
    def test_user_memory_plugin_auto_coerced_to_exclusive(self, tmp_path, monkeypatch):
        ...
        nimro_home = tmp_path / "nimro_test"
...
class TestPreToolCallDirective:
    ...
    def test_approve_without_message_is_valid(self, monkeypatch):
        ...
        from nimro_cli.plugins import get_pre_tool_call_directive
        monkeypatch.setattr(
            "nimro_cli.plugins.invoke_hook",
...
class TestResolvePreToolBlock:
    ...
    def test_approve_passes_plugin_rule_key_to_gate(self, monkeypatch):
        from nimro_cli.plugins import resolve_pre_tool_block
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("nimro_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("nimro_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from nimro_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from nimro_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("nimro_plugins.delayed_override_plugin")
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        """
        import nimro_cli.plugins as plugins_mod
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `nimro plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_register_command_empty_name_rejected(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="nimro_cli.plugins"):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        nimro_home = tmp_path / "nimro_test"
        plugins_dir = nimro_home / "plugins"
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("nimro_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("nimro_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("nimro_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("nimro_cli.plugins.PluginContext.dispatch_tool.__module__", "nimro_cli.plugins"):
...
class TestPluginDebugLogging:
    """NIMRO_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("NIMRO_PLUGINS_DEBUG", raising=False)
        from nimro_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from NIMRO_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """NIMRO_HOME at the root resolves to 'default'."""
        home = tmp_path / ".nimro"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """NIMRO_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".nimro" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIMRO_HOME", str(prof))
