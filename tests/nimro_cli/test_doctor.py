...
import nimro_cli.doctor as doctor
import nimro_cli.gateway as gateway_cli
from nimro_cli import doctor as doctor_mod
from nimro_cli.doctor import _has_provider_env_config
...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull nousresearch/nimro-agent:latest" in hint
        assert "recreate all Nimro containers" in hint
        assert "nimro update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor._sqlite_upgrade_hint("git")

        assert "run `nimro update`" in hint
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `nimro doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "NIMRO_HOME", nimro_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_marks_kanban_available_only_when_missing_worker_env_gate(self, monkeypatch):
        ...
        monkeypatch.delenv("NIMRO_KANBAN_TASK", raising=False)
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("NIMRO_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_nimro_home(self, tmp_path, provider=""):
        """Create a minimal NIMRO_HOME with config.yaml."""
        home = tmp_path / ".nimro"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_nimro_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "NIMRO_HOME", home)
        ...
        try:
            from nimro_cli import auth as _auth_mod
...
def test_run_doctor_accepts_nimro_provider_ids_that_catalog_aliases(
...
def _run_doctor_with_managed_agent_browser(monkeypatch, tmp_path, runnable):
    ...
    Nimro-managed node bin (~/.nimro/node/bin), not on PATH or in
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp NIMRO_HOME.

        ``run_doctor`` reads the module-level ``NIMRO_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("NIMRO_HOME")`` alone
        leaves doctor probing the REAL ~/.nimro. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from nimro_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("nimro_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "nimro_cli.doctor", raising=False)
        from nimro_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale NIMRO_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        nimro_home = tmp_path / ".nimro"
        nimro_home.mkdir(parents=True)
        (nimro_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"NIMRO_MAX_ITERATIONS={ghost}\n")
        (nimro_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "NIMRO_HOME", nimro_home)
        monkeypatch.setattr(doctor_mod, "get_nimro_home", lambda: nimro_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("NIMRO_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("NIMRO_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale NIMRO_MAX_ITERATIONS" in out
        env_after = (nimro_home / ".env").read_text(encoding="utf-8")
        assert "NIMRO_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_nimro_tool_progress_warning_says_unsupported_since_floor(self):
        """NIMRO_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"NIMRO_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["NIMRO_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"NIMRO_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["NIMRO_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        nimro_home = tmp_path / ".nimro"
        nimro_home.mkdir(parents=True)
        (nimro_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (nimro_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"NIMRO_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: NIMRO_TOOL_PROGRESS_MODE" in out
