...
def nimro_home(tmp_path, monkeypatch):
    ...
    # /etc/nimro on the dev/CI box can't influence the test.
    monkeypatch.setenv("NIMRO_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(nimro_home, monkeypatch):
    from nimro_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(nimro_home, monkeypatch):
    from nimro_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_nimro_dotenv(nimro_home=str(home))
