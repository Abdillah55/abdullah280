"""Focused tests for the Nimro shared-metrics durable store."""
...
import pytest
from nimro_cli.observability import shared_metrics as shared_metrics_module
from nimro_cli.observability.shared_metrics import SharedMetricsStore
from nimro_cli.observability.shared_metrics_contract import (
    COUNT_BUCKETS,
    DURATION_BUCKETS,
    EXECUTION_SURFACES,
    MODEL_FAMILIES,
...
def test_model_call_counter_survives_restart_and_exports_only_new_deltas(tmp_path):
    ...
    assert first_package["schema_version"] == "nimro.shared_metrics.v1"
    assert first_package["resource"] == {"nimro_version": "test-version"}
...
def test_due_export_runs_once_per_utc_day_and_catches_up_pending_deltas(
    ...
    assert store.counter_snapshot()[0] == {
        ...
        "metric_name": "nimro.model_call.count",
        "nimro_version": "test-version",
        "dimensions": _dimensions(),
        "value": 2,
        "packaged_value": 1,
    }
...
def test_retention_prunes_only_expired_exported_history(tmp_path):
    ...
    with sqlite3.connect(database_path) as connection:
        connection.execute(
            ...
            WHERE nimro_version = 'expired-version'
            """
        )
        connection.execute(
            """
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        aggregate_versions = {
            ...
            for row in connection.execute(
                "SELECT nimro_version FROM counter_aggregates"
            ).fetchall()
        }
