...
2. ``nimro_cli.main._refresh_windows_gateway_launchers`` — ``nimro
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "nimro_cli.main", "gateway", "run"]
    ...
    ), mock.patch("nimro_cli.config.get_nimro_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: nimro update regenerates launchers
# ---------------------------------------------------------------------------
