...
from nimro_cli.console_engine import NimroConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/nimro-console-test.zip",
    "import /tmp/nimro-console-test.zip",
...
def test_sessions_list_and_stats_use_isolated_session_store(_isolate_nimro_home):
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_nimro_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_nimro_home):
    ...
    assert code == 0
    assert "Nimro Console" in stdout.getvalue()
    assert "nimro>" not in stdout.getvalue()
