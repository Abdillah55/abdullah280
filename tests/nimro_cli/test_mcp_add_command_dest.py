"""Regression test: ``nimro mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``nimro_cli/_parser.py``).  The dispatcher in ``nimro_cli/main.py``
...
result, ``nimro mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the nimro parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`nimro mcp add foo --url ...` must keep args.command == "mcp".
