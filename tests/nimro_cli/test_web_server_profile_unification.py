...
profile switcher can target any profile's NIMRO_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via NIMRO_HOME.
"""
import pytest
import yaml
...
@pytest.fixture
def isolated_profiles(tmp_path, monkeypatch, _isolate_nimro_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_nimro_home", lambda: default_home)
...
class TestProfileScopedMcp:
    ...
    def test_mcp_test_oauth_server_without_token_is_not_ok(
        ...
        import nimro_cli.mcp_config as mcp_config
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same NIMRO_HOME the rest
        ...
        monkeypatch.setattr(
            "nimro_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid(*args, **kwargs):
            ...
            seen_homes.append(str(get_nimro_home()))
            return None
        ...
        assert resp.json()["nimro_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_nimro_home(self, isolated_profiles, monkeypatch):
        import nimro_cli.web_server as web_server

        monkeypatch.setattr(
            "nimro_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["NIMRO_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "NIMRO_TUI_GATEWAY_URL" not in env
