"""ensure_nimro_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".nimro"
    second = tmp_path / "b" / ".nimro"

    monkeypatch.setenv("NIMRO_HOME", str(first))
    cfg.ensure_nimro_home()

    # Profile switch: NIMRO_HOME moves → the new path is ensured too.
    monkeypatch.setenv("NIMRO_HOME", str(second))
    cfg.ensure_nimro_home()
