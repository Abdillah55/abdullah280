...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_nimro_home", lambda: nimro_home)
    monkeypatch.setattr(uninstall, "_is_default_nimro_home", lambda home: False)
    ...
    assert str(nimro_home) in output
    assert project_root.exists()
    assert nimro_home.exists()
