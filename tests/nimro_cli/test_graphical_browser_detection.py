"""Tests for `_can_open_graphical_browser()` in nimro_cli.auth.

Guards the fix for the May 2026 report where `nimro auth add xai-oauth`
...
from nimro_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("nimro_cli.auth.sys.platform", "linux")
