"""Tests for the unified provider catalog (nimro_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``nimro model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from nimro_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
