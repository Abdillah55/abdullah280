...
the *mutation* side: `nimro plugins enable/disable` must resolve a bare name
...
class TestResolvePluginKey:
    @patch("nimro_cli.plugins.get_bundled_plugins_dir")
    ...
    def test_full_key_resolves_to_itself(self, mock_user, mock_bundled, nested_plugin_env):
        from nimro_cli.plugins_cmd import _resolve_plugin_key
...
class TestEnableDisableNested:
    @patch("nimro_cli.plugins.get_bundled_plugins_dir")
    @patch("nimro_cli.plugins_cmd._plugins_dir")
    @patch("nimro_cli.plugins_cmd._save_disabled_set")
    @patch("nimro_cli.plugins_cmd._save_enabled_set")
    @patch("nimro_cli.plugins_cmd._get_disabled_set", return_value=set())
    @patch("nimro_cli.plugins_cmd._get_enabled_set", return_value=set())
    def test_enable_bare_name_writes_key(
        ...
        from nimro_cli.plugins_cmd import cmd_enable
        mock_user.return_value = nested_plugin_env
        mock_bundled.return_value = nested_plugin_env / "nonexistent"
...
class TestEnableToolOverrideConsent:
    ...
    @patch("nimro_cli.plugins.get_bundled_plugins_dir")
    @patch("nimro_cli.plugins_cmd._plugins_dir")
    @patch("nimro_cli.plugins_cmd._set_plugin_entry_flag")
    @patch("nimro_cli.plugins_cmd._save_disabled_set")
    @patch("nimro_cli.plugins_cmd._save_enabled_set")
    @patch("nimro_cli.plugins_cmd._get_disabled_set", return_value=set())
    @patch("nimro_cli.plugins_cmd._get_enabled_set", return_value=set())
...
class TestCompositeMenuWritesCanonicalKey:
    """#40190 follow-up: the interactive `nimro plugins` menu must persist
    ...
    def test_fallback_unchecked_plugin_disables_by_key_not_name(
        ...
        from nimro_cli.plugins_cmd import _run_composite_fallback
