...
* Be supported-platform-only — no-op silently elsewhere so ``nimro update``
...
class TestInstallCuaDriverUpgrade:
    def test_upgrade_on_unsupported_platform_is_silent_noop(self):
        from nimro_cli import tools_config
...
class TestRequireConfirmedUpdate:
    """`nimro update` passes require_confirmed_update=True: the full
    ...
    Explicit `nimro computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `nimro computer-use install --upgrade` (default flag): the old
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `nimro update`.
    """
