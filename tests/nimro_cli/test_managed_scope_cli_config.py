...
nimro_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `nimro config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_nimro_home = get_nimro_home()`` at import time (module
    singleton), so monkeypatching NIMRO_HOME after import doesn't move it.
    ...
    this only matters for tests that swap NIMRO_HOME.)
    """
    import cli

    cli._nimro_home = home
