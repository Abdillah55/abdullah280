...
from nimro_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import nimro_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "nimro_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `nimro tools`
# ---------------------------------------------------------------------------
...
def test_has_agent_browser_resolves_via_nimro_managed_node_path(monkeypatch, tmp_path):
    """The managed-Node rung: a runnable agent-browser under the Nimro Node
    ...
    monkeypatch.setattr(
        "nimro_constants.with_nimro_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "nimro_constants.agent_browser_runnable",
