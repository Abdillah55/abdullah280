"""Tests for the Kanban DB layer (nimro_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated NIMRO_HOME with an empty kanban DB."""
    home = tmp_path / ".nimro"
    home.mkdir()
...
def test_stale_claim_reclaim_event_records_diagnostic_payload(
    ...
    timing context)."""
    import json
    import nimro_cli.kanban_db as _kb
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("NIMRO_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("NIMRO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Nimro' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Nimro' DB/metadata/logs on task completion.
    """
...
# spawned with `nimro -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's NIMRO_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's NIMRO_HOME."""

    def _set_home(self, monkeypatch, tmp_path, nimro_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NIMRO_HOME", str(nimro_home))
        monkeypatch.delenv("NIMRO_KANBAN_HOME", raising=False)


    def test_profile_worker_resolves_to_shared_root(
        self, tmp_path, monkeypatch
    ):
        # Reproduces the bug: dispatcher uses ~/.nimro/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.nimro/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.nimro/kanban.db.
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # NIMRO_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile NIMRO_HOME and reads.
    ...
    def test_dispatcher_spawn_injects_kanban_paths_without_stale_session(
        ...
        # NIMRO_SESSION_* identity inherited from the long-lived gateway.
        # The one exception is NIMRO_SESSION_SOURCE, which the dispatcher
        ...
        env = captured["env"]
        assert env["NIMRO_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["NIMRO_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["NIMRO_KANBAN_TASK"] == "t_dispatch_env"
        assert env["NIMRO_KANBAN_BRANCH"] == "wt/t_dispatch_env"
        for key in sc._VAR_MAP:
            if key == "NIMRO_SESSION_SOURCE":
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see nimro_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import nimro_state as _nimro_state
    monkeypatch.setattr(
        _nimro_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _nimro_state._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    nimro_state._wal_fallback_warned_paths.clear()
    ...
    with _patch("nimro_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("ERROR", logger="nimro_state"):
            conn = kb.connect()
...
def test_connect_works_when_wal_is_silently_refused(tmp_path, monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        nimro_state, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    ...
    with _patch(
        "nimro_cli.kanban_db.sqlite3.connect",
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'nimro kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_nimro_argv()
#
# Workers spawned by the dispatcher must use a `nimro` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["nimro", ...]`
...
def test_resolve_nimro_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m nimro_cli.main`.

    Pins the correct module name (NOT `nimro` — there is no top-level
    `nimro` package). Regression for #23198: the original PR shipped
    `python -m nimro` which fails with `No module named nimro` on every
    invocation.
    """
    import shutil
    import sys
    import nimro_cli.kanban_db as kb

    monkeypatch.delenv("NIMRO_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_nimro_argv()
    assert argv == [sys.executable, "-m", "nimro_cli.main"]


def test_resolve_nimro_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `nimro_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m nimro_cli.main --version`
    ...
    assert "Nimro Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="nimro_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from nimro_cli.kanban_db import connect
