"""Tests for nimro_cli._early_recovery — the dependency-light bootstrap
repair that runs BEFORE nimro_cli.main's third-party imports (#57828 / #58004).

Covers:
- entry-point lifecycle: a broken core import (dotenv) crashes the import of
  nimro_cli.main WITHOUT early recovery, and imports fine when recovery runs
...
from nimro_cli import _early_recovery as er
...
def _run_lifecycle_subprocess(tmp_path: Path, *, repair: bool) -> subprocess.CompletedProcess:
    ...
    script.write_text(
        textwrap.dedent(
            ...
            import nimro_cli._early_recovery as er
            ...
            import nimro_cli.main  # noqa: F401
...
def test_broken_dotenv_crashes_main_import_without_repair(tmp_path):
    """Negative control: the shadow really breaks importing nimro_cli.main,
...
def test_early_recovery_module_is_stdlib_only(tmp_path):
    ...
    script.write_text(
        textwrap.dedent(
            ...
            STDLIB = set(sys.stdlib_module_names) | {"nimro_cli"}
            ...
            import nimro_cli._early_recovery  # noqa: F401
