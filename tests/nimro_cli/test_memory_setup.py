...
import nimro_cli.memory_setup as memory_setup
from nimro_cli.memory_setup import _CANCELLED, _curses_select
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_nimro_home", lambda: tmp_path)
    monkeypatch.setattr("nimro_cli.config.load_config", lambda: {"memory": {}})
...
def test_install_dependencies_force_reinstalls_versioned_specs(tmp_path, monkeypatch):
    ...
    so a downgraded/stripped bridge package is restored on nimro update."""
