...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (nimro_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from nimro_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "nimro" / "auth.json").read_text())
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nimro_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `nimro auth add nous` must also populate providers.nous so the
    ...
    # raises "Nimro is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`nimro auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``nimro auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``nimro auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("nimro_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from nimro_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from nimro_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """nimro auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``nimro auth add xai-oauth`` runs must produce independent pool entries.
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`nimro logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (nimro_home / "config.yaml").read_text()
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from nimro_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Nimro reads from has a
...
def test_seed_from_singletons_respects_nimro_pkce_suppression(tmp_path, monkeypatch):
    """anthropic nimro_pkce must not re-seed from ~/.nimro/.anthropic_oauth.json when suppressed."""
    nimro_home = tmp_path / "nimro"
    nimro_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("NIMRO_HOME", str(nimro_home))

    import yaml
    (nimro_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (nimro_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["nimro_pkce"]},
    }))

    # Stub the readers so only nimro_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_nimro_oauth_credentials", lambda: {
    ...
    # nimro_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "nimro_pkce" not in active
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from nimro_cli.auth import is_source_suppressed
    from nimro_cli.auth_commands import auth_remove_command

    with patch(
        "nimro_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "nimro_cli.copilot_auth.get_copilot_api_token",
