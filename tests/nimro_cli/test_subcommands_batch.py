...
def test_config_get_unset_subcommands_parse():
    """`nimro config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="nimro")
...
# ── deprecated `nimro login` fails gracefully, not with argparse error ────
#
# `nimro login` is a removed command; its handler (`login_command` in
# `nimro_cli/auth.py`) prints a deprecation notice pointing at `nimro auth` /
# `nimro model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `nimro --help`.
