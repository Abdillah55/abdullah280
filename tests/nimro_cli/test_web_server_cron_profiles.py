...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    monkeypatch.setattr(scheduler, "_nimro_home", None)
    captured = {}

    class RecordingProvider:
        def fire_due(self, job_id, *, adapters=None, loop=None):
            captured["job_id"] = job_id
            captured["runtime_home"] = scheduler._get_nimro_home()
    ...
    outer_token = set_nimro_home_override(default_home)
    try:
        ...
        assert scheduler._get_nimro_home() == default_home
    finally:
        reset_nimro_home_override(outer_token)
