...
When ``NIMRO_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.nimro/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.nimro            (has its own auth.json fixture)
    * Profile     -> tmp_path/.nimro/profiles/coder   (active, NIMRO_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Nimro is not
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_nimro_home_override(profile_b)
        try:
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (NIMRO_HOME != ~/.nimro, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    nimro_home = tmp_path / "classic"
    nimro_home.mkdir()
