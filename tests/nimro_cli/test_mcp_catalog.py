"""Tests for nimro_cli.mcp_catalog and nimro_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("NIMRO_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_nimro_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp NIMRO_HOME."""
    hh = tmp_path / "nimro-home"
    hh.mkdir()
    monkeypatch.setenv("NIMRO_HOME", str(hh))
    monkeypatch.setattr(
        "nimro_cli.config.get_nimro_home", lambda: hh
    )
    monkeypatch.setattr(
        "nimro_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "nimro_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_nimro_home() lazily through nimro_constants
    monkeypatch.setattr(
        "nimro_constants.get_nimro_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from nimro_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from nimro_cli.mcp_catalog import list_catalog
    ...
    def test_http_api_key_builds_bearer_headers_template(self, catalog_dir):
        ...
        _write_manifest(catalog_dir, "demo", body)
        from nimro_cli.mcp_catalog import _build_server_config
    ...
    def test_http_api_key_requires_matching_env_declaration(self, catalog_dir):
        ...
        from nimro_cli.mcp_catalog import CatalogError, _parse_manifest
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from nimro_cli.mcp_catalog import install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from nimro_cli import mcp_catalog

        monkeypatch.setattr(mcp_catalog, "_prompt_input", lambda *a, **kw: "secret-val")

        from nimro_cli.mcp_catalog import install_entry
        from nimro_cli.config import get_env_value, load_config
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from nimro_cli.mcp_catalog import install_entry, uninstall_entry
        from nimro_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from nimro_cli.mcp_catalog import uninstall_entry
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from nimro_cli.mcp_catalog import list_catalog, catalog_diagnostics
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from nimro_cli import mcp_catalog
        from nimro_cli.mcp_catalog import _do_git_install
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "nimro_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no NIMRO_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("NIMRO_OPTIONAL_MCPS", raising=False)
        from nimro_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything Nimro fetches/launches is pinned
        to an exact version.
