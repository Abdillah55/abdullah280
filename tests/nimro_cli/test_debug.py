"""Tests for ``nimro debug`` CLI command and debug utilities."""
...
@pytest.fixture
def nimro_home(tmp_path, monkeypatch):
    """Set up an isolated NIMRO_HOME with minimal logs."""
    home = tmp_path / ".nimro"
    home.mkdir()
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO nimro_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from nimro_cli.debug import _upload_paste_rs
        ...
        with patch("nimro_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "nimro_cli.debug.urllib.request.urlopen",
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from nimro_cli.debug import upload_to_pastebin

        with patch("nimro_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("nimro_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_race_truncate_after_resolve_reports_empty(self, nimro_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = nimro_home / "logs" / "agent.log"
        from nimro_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, nimro_home):
        """When truncation lands on a line boundary, keep the first full line."""
        from nimro_cli.debug import _capture_log_snapshot
        ...
        (nimro_home / "logs" / "agent.log").write_text(line * num_lines)
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of NIMRO_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def nimro_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated NIMRO_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".nimro"
        home.mkdir()
        monkeypatch.setenv("NIMRO_HOME", str(home))
        ...
        monkeypatch.delenv("NIMRO_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, nimro_home_with_secret):
        from nimro_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, nimro_home_with_secret):
        from nimro_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, nimro_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when NIMRO_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("NIMRO_REDACT_SECRETS", "false")

        from nimro_cli.debug import _capture_log_snapshot

        assert os.environ.get("NIMRO_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, nimro_home_with_secret
    ):
        from nimro_cli.debug import _capture_log_snapshot

        log_path = nimro_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, nimro_home_with_secret):
        from nimro_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from nimro_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, nimro_home):
        from nimro_cli.debug import collect_debug_report

        with patch("nimro_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- nimro dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- nimro dump ---" in report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, nimro_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from nimro_cli.debug import run_debug_share
        ...
        with patch("nimro_cli.dump.run_dump"), \
             patch("nimro_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("nimro_cli.debug.upload_to_pastebin",
    ...
    def test_share_uploads_five_pastes(self, nimro_home, capsys):
        ...
        with patch("nimro_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- nimro dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, nimro_home_with_secret, capsys
    ):
        ...
        with patch("nimro_cli.dump.run_dump"), \
             patch("nimro_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("nimro_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "nimro debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, nimro_home):
        from nimro_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from nimro_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from nimro_cli.debug import delete_paste
        ...
        with patch("nimro_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.nimro/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``nimro debug``
    invocation.
    """


    def test_records_pending_to_json(self, nimro_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from nimro_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, nimro_home):
        """Same URL recorded twice → one entry with the later expire_at."""
        from nimro_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, nimro_home):
        from nimro_cli.debug import (
        ...
        with patch("nimro_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, nimro_home):
        from nimro_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("nimro_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, nimro_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, nimro_home):
        from nimro_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("nimro_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    ...
    def test_handles_delete_failure(self, capsys):
        from nimro_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("nimro_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_shows_privacy_notice(self, nimro_home, capsys):
        from nimro_cli.debug import run_debug_share
        ...
        with patch("nimro_cli.dump.run_dump"), \
             patch("nimro_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("nimro_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
...
class TestBuildDebugShare:
    ...
    Backs both ``nimro debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, nimro_home):
        from nimro_cli.debug import build_debug_share

        secret = "sk-proj-SUPERSECRETtoken1234567890"
        (nimro_home / "logs" / "agent.log").write_text(
        ...
        with patch("nimro_cli.dump.run_dump"), patch(
            "nimro_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("nimro_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, nimro_home):
        from nimro_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    ...
    def test_no_redact_omits_banner(self, nimro_home):
        from nimro_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, nimro_home):
        from nimro_cli.debug import collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, nimro_home):
        import gzip
        import json as _json

        from nimro_cli.debug import build_nous_bundle
        ...
        assert envelope["format"] == "nimro-debug-share/1"
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, nimro_home, capsys):
        from nimro_cli.debug import run_debug_share
        ...
        with patch("nimro_cli.dump.run_dump"), patch(
            "nimro_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, nimro_home, capsys):
        from nimro_cli.debug import run_debug_share

        with patch("nimro_cli.dump.run_dump"), patch(
            "nimro_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, nimro_home):
        from nimro_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("nimro_cli.dump.run_dump"), patch(
            "nimro_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("nimro_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    ``NimroCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("nimro_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    """`nimro debug share` requires explicit consent before uploading.
    ...
    def test_non_interactive_requires_yes(self, nimro_home, capsys, monkeypatch):
        ...
        with patch("nimro_cli.dump.run_dump"), \
             patch("nimro_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
    ...
    def test_local_never_prompts(self, nimro_home, capsys, monkeypatch):
