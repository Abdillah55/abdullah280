"""Surfacing tests — managed scope shown in `config show` and `nimro doctor`."""
import pytest
...
def test_config_show_no_managed_scope_silent(tmp_path, monkeypatch, capsys):
    ...
    home.mkdir()
    monkeypatch.setenv("NIMRO_HOME", str(home))
    monkeypatch.setenv("NIMRO_MANAGED_DIR", str(tmp_path / "nope"))
    (home / "config.yaml").write_text("model:\n  default: user/model\n", encoding="utf-8")
    import nimro_cli.config as cfg
    from nimro_cli import managed_scope
...
def test_doctor_silent_with_no_managed_scope(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("NIMRO_MANAGED_DIR", str(tmp_path / "nope"))
    from nimro_cli import managed_scope, doctor
