"""Tests for the ``nimro send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``nimro_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from nimro_cli import send_cmd
...
def test_load_nimro_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``nimro send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``nimro config set``) lives
    ...
    nimro_home.mkdir()
    (nimro_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_nimro_home() to re-resolve under the patched env.
    from importlib import reload

    import nimro_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_nimro_env()
