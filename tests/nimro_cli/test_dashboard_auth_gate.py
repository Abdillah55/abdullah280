...
@pytest.mark.parametrize("host,allow_public,expected", [
    ...
    # bind (June 2026 nimro-0day hardening). Non-loopback always requires auth.
    ("0.0.0.0",   True,  True),
    ("0.0.0.0",   False, True),
    ...
    ("nimro-agent-prod-abc.fly.dev", False, True),
])
...
def test_start_server_insecure_public_no_longer_bypasses_gate(monkeypatch):
    ...
    """
    from nimro_cli.dashboard_auth import clear_providers
    clear_providers()
    _stub_uvicorn_run(monkeypatch)
    web_server.app.state.auth_required = None
