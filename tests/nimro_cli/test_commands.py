...
class TestSlackNativeSlashes:
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /nimro <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_nimro_norm = {_norm(n) for n in _SLACK_VIA_NIMRO_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_nimro_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `nimro slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_skips_already_listed(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "nimro_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from nimro_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch nimro_cli.plugins.get_plugin_commands() to a fixed dict."""
        from nimro_cli import plugins as _plugins_mod
