"""Tests for nimro_cli.gateway_windows."""

from pathlib import Path

import pytest

import nimro_cli.gateway as gateway
import nimro_cli.gateway_windows as gateway_windows
import nimro_cli.setup as setup
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda nimro_home: "")
    monkeypatch.setattr("nimro_cli.config.get_nimro_home", lambda: str(nimro_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(venv_python), "-m", "nimro_cli.main"]
    assert cwd == str(nimro_home.resolve())
    assert env_overlay["VIRTUAL_ENV"] == str(project / "venv")
    assert str(project) in env_overlay["PYTHONPATH"].split(gateway_windows.os.pathsep)


class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_nimro_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".nimro"
        home.mkdir()
        monkeypatch.setattr("nimro_cli.config.get_nimro_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".nimro"
        project = tmp_path / "checkout"
        monkeypatch.setattr("nimro_cli.config.get_nimro_home", lambda: missing)
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Nimro_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Nimro_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Nimro_Gateway_alice")
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Nimro\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Nimro\venv\Scripts\python.exe"
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Nimro_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Nimro_Gateway_alice")
    ...
    assert "Nimro_Gateway_alice.vbs" in xml_seen["text"]
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "nimro_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("NIMRO_HOME", "PYTHONIOENCODING", "NIMRO_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
# resume_pending=True) never fires when `nimro gateway stop` kills the
