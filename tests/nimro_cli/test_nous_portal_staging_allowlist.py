...
``NIMRO_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` in its
...
class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("NIMRO_PORTAL_BASE_URL", raising=False)
    ...
    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        ...
        monkeypatch.setenv(
            "NIMRO_PORTAL_BASE_URL", "https://portal.staging-nousresearch.com"
        )
...
class TestResolveAccessTokenEnvOverrideWins:
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        caplog_records = []
        logger = logging.getLogger("nimro_cli.auth")
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior NIMRO_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        monkeypatch.setenv("NIMRO_PORTAL_BASE_URL", staging_portal)
