...
nimro_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # nimro_time checks an env override first; ensure it's unset so config wins.
    monkeypatch.delenv("NIMRO_TIMEZONE", raising=False)
    monkeypatch.delenv("TZ", raising=False)
    _seed(home, managed, user="timezone: America/New_York\n", mgd="timezone: Asia/Tokyo\n")
    import nimro_time

    assert nimro_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (NIMRO_TIMEZONE, NIMRO_REDACT_SECRETS, NIMRO_MAX_ITERATIONS,
    ...
    # writing NIMRO_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
