...
the SAME provider universe as ``nimro model`` (the CLI/TUI picker).
...
def test_every_nimro_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `nimro model` universe."""
    ...
    assert not missing, (
        "providers shown in `nimro model` but not configurable in the desktop "
