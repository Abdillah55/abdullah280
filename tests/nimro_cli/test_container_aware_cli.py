...
def container_env(tmp_path, monkeypatch):
    """Set up a fake NIMRO_HOME with .container-mode file."""
    nimro_home = tmp_path / ".nimro"
    nimro_home.mkdir()
    monkeypatch.setenv("NIMRO_HOME", str(nimro_home))
    monkeypatch.delenv("NIMRO_DEV", raising=False)

    container_mode = nimro_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=nimro-agent\n"
        "exec_user=nimro\n"
        "nimro_bin=/data/current-package/bin/nimro\n"
    )
    return nimro_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("nimro_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "nimro-agent"
    assert info["exec_user"] == "nimro"
    assert info["nimro_bin"] == "/data/current-package/bin/nimro"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from nimro_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "nimro"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "nimro-agent" in cmd
    assert "/data/current-package/bin/nimro" in cmd
    assert "chat" in cmd
