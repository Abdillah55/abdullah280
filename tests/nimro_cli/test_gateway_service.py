...
import nimro_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`nimro gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `nimro gateway restart` in the background; when a
        ...
        assert "nimro gateway status" in output
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``NIMRO_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../nimro_test`` NIMRO_HOME — silently breaking
        ...
        carry those markers.
        """
        unit_path = tmp_path / "nimro-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")

        monkeypatch.setattr(
            gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path
        )
        # Realistic generated unit referencing a pytest tmpdir NIMRO_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="NIMRO_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/nimro_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_tmp_home_in_systemd_unit(self):
        unit = '[Service]\nEnvironment="NIMRO_HOME=/tmp/nimro-e2e-41264"\n'
        assert (
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="NIMRO_HOME={tmp_path}/nimro-home"\n'
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "nimro gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".nimro" / "profiles" / "jarvis" / "node" / "bin"
...
class TestLaunchdServiceRecovery:
    ...
    def test_refresh_defers_reload_when_running_inside_gateway_tree(self, tmp_path, monkeypatch):
        ...
        plist_path = tmp_path / "ai.nimro.gateway.plist"
        ...
        monkeypatch.setattr(
            ...
            lambda: (
                "<plist>--replace\n<key>NIMRO_HOME</key>"
                "<string>/Users/alice/.nimro</string></plist>"
            ),
        )
    ...
    def test_launchd_status_reports_fallback_when_unsupported_and_pid_running(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "get_nimro_home", lambda: tmp_path)
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.nimro.gateway"])
            ),
        )
...
class TestSystemUnitNimroHome:
    """NIMRO_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'NIMRO_HOME=/home/alice/.nimro' in unit
        assert '/root/.nimro' not in unit


    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's NIMRO_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        nimro_home = str(gateway_cli.get_nimro_home().resolve())
        assert f'NIMRO_HOME={nimro_home}' in unit


class TestSystemUnitRefreshSyncsNimroHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.nimro."""

    def test_refresh_adopts_unit_nimro_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_nimro = root_home / ".nimro"
        alice_nimro = alice_home / ".nimro"
        root_nimro.mkdir(parents=True)
        alice_nimro.mkdir(parents=True)
        (root_nimro / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_nimro / "config.yaml").write_text(
        ...
        monkeypatch.delenv("NIMRO_RESTART_DRAIN_TIMEOUT", raising=False)

        # Correct installed unit (operator's NIMRO_HOME + drain timeout).
        monkeypatch.setenv("NIMRO_HOME", str(alice_nimro))
        ...
        # Simulate sudo without inherited NIMRO_HOME (falls back to root).
        monkeypatch.setenv("NIMRO_HOME", str(root_nimro))
        ...
        assert os.environ["NIMRO_HOME"] == str(alice_nimro)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned NIMRO_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_nimro_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_nimro_home_from_systemd_unit",
...
class TestNimroHomeForTargetUser:
    """Unit tests for _nimro_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("NIMRO_HOME", raising=False)

        result = gateway_cli._nimro_home_for_target_user("/home/alice")
        assert result == "/home/alice/.nimro"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "nimro gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_profile = root_home / ".nimro" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("NIMRO_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_nimro_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
        ...
        assert f'NIMRO_HOME={target_home / ".nimro" / "profiles" / "mybot"}' in unit



    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".nimro" / "profiles" / "orcha"
        ...
        monkeypatch.setenv("NIMRO_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_nimro_home", lambda: profile_dir)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.nimro.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".nimro" / "nimro-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".nimro" / "nimro-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".nimro" / "nimro-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("NIMRO_HOME", str(root_home / ".nimro"))
        monkeypatch.setattr(gateway_cli, "get_nimro_home", lambda: root_home / ".nimro")
        ...
        # WorkingDirectory is anchored at the target user's NIMRO_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.nimro" in unit
        assert "WorkingDirectory=/home/alice/.nimro/nimro-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "nimro-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "nimro-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'nimro gateway install' inside Docker exits 0 with container guidance."""
        import pytest
...
class TestLegacyNimroUnitDetection:
    """Tests for _find_legacy_nimro_units / has_legacy_nimro_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``nimro.service`` unit behind when the service was
    renamed to ``nimro-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``nimro.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``nimro-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Nimro Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m nimro_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        (user_dir / "nimro.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "nimro.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_nimro_units()
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m nimro_cli.main gateway run
          - python path/to/nimro_cli/main.py gateway run
          - nimro gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m nimro_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/nimro/nimro_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/nimro gateway run --replace",
            "ExecStart=/venv/bin/python /opt/nimro/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "nimro.service" if i == 0 else "nimro.service"  # same name
            # Test each variant fresh
            (user_dir / "nimro.service").write_text(
                f"[Unit]\nDescription=Old Nimro\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_nimro_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "nimro.service" in out
        assert "nimro gateway migrate-legacy" in out



class TestRemoveLegacyNimroUnits:
    """Tests for remove_legacy_nimro_units (the migration action)."""
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        legacy = user_dir / "nimro.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        removed, remaining = gateway_cli.remove_legacy_nimro_units(interactive=False)
        ...
        assert any("--user stop nimro.service" in c for c in cmds_joined)
        assert any("--user disable nimro.service" in c for c in cmds_joined)
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (nimro-gateway-coder.service)
        ...
        profile_unit = user_dir / "nimro-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "nimro-gateway.service"
...
class TestMigrateLegacyCommand:
    """Tests for the `nimro gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import nimro_cli.main as cli_main
        ...
        # public, just confirm that `nimro gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import nimro_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "nimro_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_nimro_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "nimro_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_nimro_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_nimro_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_nimro_units", fake_remove)
    ...
    def test_install_skips_legacy_check_when_none_present(
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_nimro_units", lambda: False)
        monkeypatch.setattr(gateway_cli, "remove_legacy_nimro_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "nimro-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "nimro-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "nimro-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``nimro gateway start --system``).
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "nimro-gateway")
        ...
        assert "sudo systemctl start nimro-gateway" in out
        assert "sudo nimro gateway uninstall --system" in out
        assert "nimro gateway install" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``nimro gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (NIMRO_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """



    def test_user_unit_workingdirectory_is_nimro_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".nimro"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_nimro_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_nimro_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.nimro.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.nimro.gateway"
