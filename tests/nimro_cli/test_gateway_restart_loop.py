...
Covers:
- Defense 1: gateway stop/restart refuse when _NIMRO_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "nimro gateway stop",
        "nimro  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "NIMRO GATEWAY RESTART",           # uppercase
    ])
    def test_nimro_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.nimro.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.nimro/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l nimro-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.nimro.gateway.restart-once.plist",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.nimro.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.nimro/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated nimro note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        "nimro cron list",
        "nimro update",
        "nimro config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `nimro gateway start` is benign — starting a gateway from inside a
        ...
        "nimro gateway start",
        "nimro gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway nimro
        # services must not be falsely blocked (the old `.*nimro` matched any
        # nimro token).
        "launchctl unload ai.nimro.update-checker.plist",
        "launchctl restart ai.nimro.daemon",
        ...
        "systemctl restart nimro-meta.service",
        "systemctl restart nimro-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_nimro_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade nimro then run nimro gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_nimro_gateway_once.sh). The script must live under
        # NIMRO_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("NIMRO_HOME", str(tmp_path / ".nimro"))
        scripts_dir = tmp_path / ".nimro" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify nimro gateway stop/restart refuse when _NIMRO_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_NIMRO_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_NIMRO_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _NIMRO_GATEWAY=1.

    Issue #37453: systemctl --user restart nimro-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart nimro-gateway",
        "systemctl --user restart nimro-gateway",
        "systemctl stop nimro-gateway.service",
        "nimro gateway restart",
        "launchctl kickstart gui/501/ai.nimro.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.nimro.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.nimro/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.nimro.gateway.restart-once.plist",
        "pkill -f nimro.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart nimro-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.nimro.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-nimro label: label-independent detection is the point
        # (#62891 second reproduction used `ai.nimro.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.nimro.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.nimro.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.nimro.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-nimro systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run nimro gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("nimro gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfenimro gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under NIMRO_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.nimro.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("nimro gateway restart")\n')
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    nimro_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run nimro gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NIMRO_HOME", str(tmp_path / ".nimro"))
        (tmp_path / ".nimro").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run nimro gateway restart nightly",
        ))
