"""Tests for nimro_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $NIMRO_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Nimro-managed node dir.
"""
...
def _make_nimro_node(nimro_home: Path) -> Path:
    """Create a fake $NIMRO_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = nimro_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    nimro_home = fake_home / ".nimro"
    _make_nimro_node(nimro_home)
    ...
    removed = uninstall.remove_node_symlinks(nimro_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    nimro_home = fake_home / ".nimro"
    node_bin = _make_nimro_node(nimro_home)
