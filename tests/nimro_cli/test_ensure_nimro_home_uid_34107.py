"""Regression tests for #34107 — Docker UID/GID handling in ensure_nimro_home.

When Nimro runs in Docker with ``NIMRO_UID=1000`` / ``NIMRO_GID=911``,
the entrypoint chowns the top-level ``NIMRO_HOME`` once at startup. But
...
The fix is a ``_chown_to_nimro_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_nimro_uid_gid
# ---------------------------------------------------------------------------


class TestResolveNimroUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("NIMRO_UID", "1000")
        monkeypatch.setenv("NIMRO_GID", "911")
        from nimro_cli.config import _resolve_nimro_uid_gid
...
class TestChownToNimroUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_nimro_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("NIMRO_UID", raising=False)
        monkeypatch.delenv("NIMRO_GID", raising=False)
        from nimro_cli import config as cfg
