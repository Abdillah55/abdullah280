"""Tests for nimro_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestGetDisabledSkills:
    def test_empty_config(self):
        from nimro_cli.skills_config import get_disabled_skills
...
class TestSaveDisabledSkills:
    @patch("nimro_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from nimro_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    ...
    @patch("nimro_cli.config.load_config")
    ...
    @patch("nimro_cli.config.load_config")
    @patch.dict("os.environ", {"NIMRO_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """NIMRO_SESSION_PLATFORM should be used when NIMRO_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_nimro_platform_takes_precedence(self, tmp_path, monkeypatch):
        """NIMRO_PLATFORM should win over NIMRO_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from nimro_cli.skills_config import _get_categories
        skills = [
