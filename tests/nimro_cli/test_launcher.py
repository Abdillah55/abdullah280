"""Tests for the top-level `./nimro` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./nimro` should use `nimro_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "nimro"
    called = []

    fake_main_module = types.ModuleType("nimro_cli.main")

    def fake_main():
        called.append("nimro_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "nimro_cli.main", fake_main_module)
