...
from nimro_cli import config as nimro_config
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(nimro_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(nimro_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(nimro_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(nimro_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(nimro_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(nimro_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(nimro_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
    monkeypatch.setattr(nimro_main, "_upgrade_pip_before_lazy_refresh", lambda *a, **kw: None)
    monkeypatch.setattr(nimro_main, "_refresh_active_lazy_features", lambda *a, **kw: True)
...
def test_refresh_active_memory_provider_dependencies_reinstalls_active_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "nimro_cli.memory_setup._install_dependencies",
        lambda provider_name, force=False: recorded.append((provider_name, force)),
    )

    nimro_main._refresh_active_memory_provider_dependencies()
...
def test_reload_updated_runtime_modules_restores_new_nimro_constants_symbol(monkeypatch):
    """A pre-pull module object missing a new helper is repaired by reload."""
    import nimro_constants

    monkeypatch.delattr(nimro_constants, "apply_subprocess_home_env", raising=False)
    assert not hasattr(nimro_constants, "apply_subprocess_home_env")

    nimro_main._reload_updated_runtime_modules()

    assert callable(nimro_constants.apply_subprocess_home_env)
...
def test_cmd_update_skips_stash_restore_when_reset_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        nimro_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        nimro_main, "_restore_stashed_changes",
        lambda *a, **kw: restore_calls.append(1) or True,
    )

    side_effect, _ = _make_update_side_effect(ff_only_fails=True, reset_fails=True)
    monkeypatch.setattr(nimro_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        nimro_main.cmd_update(SimpleNamespace())
...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    monkeypatch.setattr(
        nimro_main, "_discard_stashed_changes",
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``nimro update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    repo_gitignore = Path(nimro_main.__file__).resolve().parents[1] / ".gitignore"
    ...
    marker = tmp_path / ".nimro-bootstrap-complete"
    marker.write_text("")

    # Exact flags used by nimro update (nimro_cli/main.py).
    git("stash", "push", "--include-untracked", "-m", "nimro-update-autostash")

    assert marker.exists(), (
        ".nimro-bootstrap-complete was swept into the update autostash — it must "
    ...
    assert ".nimro-bootstrap-complete" not in status
