"""Tests for nimro_cli configuration management."""
...
class TestGetNimroHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("NIMRO_HOME", None)
            home = get_nimro_home()
            assert home == Path.home() / ".nimro"


class TestEnsureNimroHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from nimro_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``nimro logs`` surfaces it)
    ...
    def test_corrupt_config_is_backed_up(self, tmp_path, capsys):
        ...
        """
        from nimro_cli import config as cfg_mod
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from nimro_cli.config import _deep_merge
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NIMRO_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NIMRO_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NIMRO_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NIMRO_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("nimro_cli.config.sanitize_env_file", return_value=2),
            patch(
                "nimro_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("nimro_cli.config.read_raw_config", return_value={}),
            patch("nimro_cli.config.get_missing_env_vars", return_value=[]),
            patch("nimro_cli.config.get_missing_config_fields", return_value=[]),
            patch("nimro_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from nimro_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """NIMRO_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `nimro setup`) lets a
        ...
        via config.yaml; NIMRO_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from nimro_cli.config import OPTIONAL_ENV_VARS
        assert "NIMRO_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `nimro setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `nimro setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_nimro_home(self):
        from nimro_cli.config_migrations import support_floor_message
        from nimro_constants import display_nimro_home

        msg = support_floor_message()
        assert f"{display_nimro_home()}/config.yaml" in msg
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from nimro_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``NIMRO_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Nimro
    process on next startup via the dotenv → ``os.environ`` chain in
    ``nimro_cli/env_loader.py``.
    ...
    def _nimro_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "NIMRO_SPOTIFY_CLIENT_ID",
            "NIMRO_QWEN_BASE_URL",
            "NIMRO_MAX_ITERATIONS",
        ],
    )
    def test_nimro_integration_keys_still_writable(self, allowed_key):
        """``NIMRO_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``NIMRO_*`` convention must keep
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from nimro_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"NIMRO_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: nimro\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "nimro"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import nimro_cli.config as cfg_mod
