"""Dashboard Nimro Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_nimro_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from nimro_cli.console_engine import ConsoleResult, NimroConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(NimroConsoleEngine, "execute", slow_execute)
