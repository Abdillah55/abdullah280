...
A credential added via `nimro auth add openrouter` lives in the credential
...
def _seed_openrouter_pool(token: str = "sk-or-FAKEKEY123") -> None:
    """Mimic `nimro auth add openrouter <token>` — a manual pool entry."""
...
def test_auto_detects_openrouter_from_pool(tmp_path, monkeypatch):
    ...
    (tmp_path / "nimro").mkdir(parents=True, exist_ok=True)
