...
and the ``nimro kanban repair`` CLI verb."""
...
def test_dispatch_tick_runs_wal_checkpoint_at_interval(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NIMRO_KANBAN_DB", str(db_path))
...
# ---------------------------------------------------------------------------
# repair_db() API + `nimro kanban repair` CLI verb
# ---------------------------------------------------------------------------

def _run_kanban_cli(argv: list[str]) -> int:
    """Drive the real argparse surface exactly like `nimro kanban …`."""
    import argparse

    from nimro_cli import kanban as kc
...
def cli_home(tmp_path, monkeypatch):
    """Isolated NIMRO_HOME so kanban_db_path() resolves inside tmp_path."""
    home = tmp_path / ".nimro"
    home.mkdir()
