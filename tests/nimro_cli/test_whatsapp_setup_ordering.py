...
Before the fix, ``nimro whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``nimro gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    nimro = home / ".nimro"
    nimro.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("NIMRO_HOME", str(nimro))
...
def _env_value(nimro_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from nimro_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("nimro_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``nimro whatsapp`` with an existing paired session and
