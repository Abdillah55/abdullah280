"""Tests for the Command Installation check in nimro doctor."""
...
import nimro_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal NIMRO_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".nimro"
    ...
    nimro_bin = venv_bin_dir / "nimro"
    nimro_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    nimro_bin.chmod(0o755)
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, nimro_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "nimro"
        wrong_target = tmp_path / "wrong_nimro"
        ...
        assert cmd_link.resolve() == nimro_bin.resolve()
