"""Tests for ``nimro gui`` desktop launcher wiring."""
...
from nimro_cli import main as cli_main
...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    desktop_dir = root / "apps" / "desktop"
    if platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Nimro.app" / "Contents" / "MacOS" / "Nimro"
    elif platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Nimro.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "nimro"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("nimro_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("nimro_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("nimro_cli.main._desktop_build_needed", return_value=True), \
         patch("nimro_cli.main._write_desktop_build_stamp"), \
         patch("nimro_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("nimro_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_install_env_prepends_managed_node_on_bare_path(tmp_path, monkeypatch):
    ...
    the parent PATH is stripped, so the install env MUST carry the Nimro-managed
    ...
    # A managed Node tree on disk so with_nimro_node_path() actually prepends it.
    home = tmp_path / "nimro-home"
    ...
    install_ok = subprocess.CompletedProcess(["npm", "ci"], 0)
    launch_ok = subprocess.CompletedProcess(["nimro"], 0)

    with patch("nimro_cli.main._resolve_node_runtime_npm", return_value="/usr/bin/npm"), \
         patch("nimro_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("nimro_cli.main._desktop_build_needed", return_value=True), \
         patch("nimro_cli.main._write_desktop_build_stamp"), \
         patch("nimro_cli.main.subprocess.run", side_effect=[subprocess.CompletedProcess([], 0), launch_ok]), \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(skip_build=False))

    managed_dirs = [str(p) for p in iter_nimro_node_dirs() if p.is_dir()]
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    Nimro.app/Contents/MacOS/Nimro in place. Re-downloading Electron can't
    ...
    with patch("nimro_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("nimro_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("nimro_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("nimro_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("nimro_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("nimro_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def _make_signable_app(desktop_dir: Path) -> Path:
    """Build a fake packaged Nimro.app with the pieces the signer must find."""
    ...
    app = desktop_dir / "release" / "mac-arm64" / "Nimro.app"
    _write_info_plist(app, "com.nousresearch.nimro")
    (app / "Contents" / "MacOS").mkdir(parents=True)
    (app / "Contents" / "MacOS" / "Nimro").write_text("", encoding="utf-8")

    helper = app / "Contents" / "Frameworks" / "Nimro Helper.app"
    _write_info_plist(helper, "com.nousresearch.nimro.helper")
...
def test_desktop_macos_local_codesign_signs_native_binaries(tmp_path, monkeypatch):
    ...
    Nimro.app component, silently skipping every .node/.dylib/crashpad
