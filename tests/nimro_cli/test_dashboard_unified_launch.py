...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "nimro_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # NIMRO_HOME.  For a standard install (NIMRO_HOME unset) that root is
        # the platform-native default (~/.nimro), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from nimro_constants import get_default_nimro_root
        assert env.get("NIMRO_HOME") == str(get_default_nimro_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (NIMRO_DESKTOP=1) must NOT
