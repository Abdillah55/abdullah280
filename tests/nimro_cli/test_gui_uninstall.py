"""Tests for nimro_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import nimro_cli.gui_uninstall as gu


def _make_agent(nimro_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = nimro_home / "nimro-agent"
    (agent_root / "nimro_cli").mkdir(parents=True)
    (agent_root / "nimro_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(nimro_home: Path) -> None:
    """Create the source-built GUI artifacts a `nimro desktop` run produces."""
    desktop = nimro_home / "nimro-agent" / "apps" / "desktop"
    ...
    (nimro_home / "nimro-agent" / "node_modules").mkdir(parents=True)
    (nimro_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(nimro_home: Path) -> None:
    (nimro_home / "config.yaml").write_text("x: 1\n")
    (nimro_home / ".env").write_text("KEY=secret\n")
    (nimro_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(nimro_home)
    _make_gui_build(nimro_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(nimro_home)
    ...
    assert summary["nimro_home"] == str(nimro_home)
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import nimro_cli.uninstall as uninstall
