"""Tests for the per-profile Projects store (nimro_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Nimro Agent", folders=["/tmp/nimro"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "nimro-agent"
    assert proj.name == "Nimro Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/nimro"
    assert [f.path for f in proj.folders] == ["/tmp/nimro"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "nimro-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' NIMRO_HOME.
