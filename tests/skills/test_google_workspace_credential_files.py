...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        nimro_home.mkdir()
        (nimro_home / "google_token.json").write_text("{}")
        (nimro_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.nimro/google_token.json" in container_paths
            assert "/root/.nimro/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
