...
Uses a throwaway NIMRO_HOME so it never touches ~/.nimro. No real names.
"""
...
def _fresh_home():
    """Point NIMRO_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_nimro_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["NIMRO_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER NIMRO_HOME is set.
...
# a different get_nimro_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"NIMRO_HOME (throwaway): {HOME}")
