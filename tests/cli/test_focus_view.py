...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = NimroCLI.__new__(NimroCLI)
        host.model = "opus"
        ...
        with patch.object(NimroCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(NimroCLI, "_is_session_yolo_active", return_value=False):
            text = NimroCLI._build_status_bar_text(host, width=width)
