...
def _isolate(tmp_path, monkeypatch):
    """Isolate NIMRO_HOME so tests don't touch real config."""
    home = tmp_path / ".nimro"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal NimroCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = NimroCLI.__new__(NimroCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["nimro-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Nimro' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Nimro' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Nimro' minimum context length."""
