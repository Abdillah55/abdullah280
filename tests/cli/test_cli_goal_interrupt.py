...
minimal ``NimroCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def nimro_home(tmp_path, monkeypatch):
    """Isolated NIMRO_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".nimro"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("NIMRO_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves NIMRO_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal NimroCLI stub with an active goal wired in."""
    from cli import NimroCLI
    from nimro_cli.goals import GoalManager

    cli = NimroCLI.__new__(NimroCLI)
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, nimro_home):
        ...
        with patch("nimro_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, nimro_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, nimro_home):
        ...
        src = inspect.getsource(NimroCLI.chat)
