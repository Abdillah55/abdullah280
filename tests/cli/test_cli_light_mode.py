...
class TestLightModeDetection:
    def test_nimro_light_env_true_forces_light(self, cli_mod, monkeypatch):
        monkeypatch.setenv("NIMRO_LIGHT", "1")
        assert cli_mod._detect_light_mode() is True

    def test_nimro_light_env_false_forces_dark(self, cli_mod, monkeypatch):
        monkeypatch.setenv("NIMRO_LIGHT", "0")
        # Also blank out other signals so nothing else flips it light.
        monkeypatch.delenv("NIMRO_TUI_LIGHT", raising=False)
        monkeypatch.delenv("NIMRO_TUI_THEME", raising=False)
        monkeypatch.delenv("NIMRO_TUI_BACKGROUND", raising=False)
        monkeypatch.delenv("COLORFGBG", raising=False)
        assert cli_mod._detect_light_mode() is False


    def test_background_hex_hint_light(self, cli_mod, monkeypatch):
        monkeypatch.delenv("NIMRO_LIGHT", raising=False)
        monkeypatch.delenv("NIMRO_TUI_LIGHT", raising=False)
        monkeypatch.delenv("NIMRO_TUI_THEME", raising=False)
        monkeypatch.setenv("NIMRO_TUI_BACKGROUND", "#FFFFFF")
        assert cli_mod._detect_light_mode() is True
...
class TestSkinConfigHook:
    ...
    def test_hook_installed(self, cli_mod):
        from nimro_cli.skin_engine import SkinConfig

        assert getattr(SkinConfig, "_nimro_light_mode_hook_installed", False) is True
