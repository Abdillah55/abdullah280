...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_nimro_home(), so
        # point NIMRO_HOME at the temp dir (the _nimro_home import-time
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "nimro_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in NIMRO_HOME/config.yaml
    ...
    install whose NIMRO_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read NIMRO_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh NIMRO_HOME with NO config.yaml (managed/desktop first launch).
        nimro_home = tmp_path / ".nimro"
        nimro_home.mkdir()
        monkeypatch.setenv("NIMRO_HOME", str(nimro_home))
    ...
    def test_does_not_write_repo_cli_config(self, tmp_path, monkeypatch):
        ...
        result = yaml.safe_load((nimro_home / "config.yaml").read_text())
        assert result["wake_word"]["enabled"] is True
