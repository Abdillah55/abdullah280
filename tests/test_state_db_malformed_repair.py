...
def test_auto_heal_attempted_once_per_process(tmp_path, monkeypatch):
    ...
    _build_healthy_db(db_path)
    _corrupt_duplicate_fts(db_path)
    monkeypatch.setattr(nimro_state, "_repair_attempted_paths", set())

    calls = {"n": 0}
    real_repair = nimro_state.repair_state_db_schema
    ...
    monkeypatch.setattr(nimro_state, "repair_state_db_schema", fake_repair)
    ...
    monkeypatch.setattr(nimro_state, "repair_state_db_schema", real_repair)
...
def test_fts_read_corruption_repaired_in_place(tmp_path):
    ...
    from nimro_state import _db_opens_cleanly
...
def test_repair_rebuilds_stale_btree_indexes(tmp_path):
    ...
    reason = nimro_state._db_opens_cleanly(db_path)
    assert reason is not None
    ...
    assert nimro_state._db_opens_cleanly(db_path) is None
    raw = sqlite3.connect(str(db_path))
