...
from nimro_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(nimro_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <nimro_home>/plugins/<name> and opt it in."""
    plugin_dir = nimro_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NIMRO_HOME", str(tmp_path / "nimro_empty"))
