...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    nimro_state = ModuleType("nimro_state")
    nimro_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "nimro_state", nimro_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Nimro"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Nimro"
