...
def test_credential_rotation_replaces_route_scoped_tls_settings():
    ...
    with patch("nimro_cli.config.load_config_readonly", return_value=config):
...
def test_credential_rotation_does_not_carry_global_headers_across_routes():
    ...
    with (
        patch("nimro_cli.config.load_config_readonly", return_value=config),
        patch(
            "nimro_cli.config.get_compatible_custom_providers",
