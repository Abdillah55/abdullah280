...
def _make_agent(*tool_names: str, max_iterations: int = 10, config: dict | None = None) -> AIAgent:
    with (
        ...
        patch("nimro_cli.config.load_config", return_value=config or {}),
        patch("nimro_cli.config.load_config_readonly", return_value=config or {}),
...
def test_plugin_pre_tool_block_wins_without_counting_as_toolguard_block():
    ...
    with (
        patch("nimro_cli.plugins.resolve_pre_tool_block", return_value="plugin policy"),
