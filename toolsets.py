...
_NIMRO_CORE_TOOLS = [
    ...
    # pane, and react to a message with an emoji (all gated on NIMRO_DESKTOP
    ...
    # spawned as a kanban worker (NIMRO_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `nimro tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (NIMRO_KANBAN_TASK env "
    ...
    # Coding posture (base Nimro — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in nimro_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Nimro toolsets (CLI + messaging platforms)
    #
    ...
    # the `nimro send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "nimro-cli": {
        ...
        "tools": _NIMRO_CORE_TOOLS,
        "includes": []
    },

    "nimro-cron": {
        # Mirrors nimro-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `nimro tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as nimro-cli; gated by `nimro tools`",
        "tools": _NIMRO_CORE_TOOLS,
        "includes": []
    },
    ...
    "nimro-email": {
        "description": "Email bot toolset - interact with Nimro via email (IMAP/SMTP)",
        "tools": _NIMRO_CORE_TOOLS,
        "includes": []
    },
    ...
    "nimro-sms": {
        "description": "SMS bot toolset - interact with Nimro via SMS (Twilio)",
        "tools": _NIMRO_CORE_TOOLS,
        "includes": []
    },
    ...
    "nimro-webhook": {
        ...
        "tools": _NIMRO_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "nimro-gateway": {
        ...
        "tools": [],
        "includes": ["nimro-telegram", "nimro-discord", "nimro-whatsapp", "nimro-slack", "nimro-signal", "nimro-bluebubbles", "nimro-homeassistant", "nimro-email", "nimro-sms", "nimro-mattermost", "nimro-matrix", "nimro-dingtalk", "nimro-feishu", "nimro-wecom", "nimro-wecom-callback", "nimro-weixin", "nimro-qqbot", "nimro-webhook", "nimro-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``nimro-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_NIMRO_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``nimro-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (nimro-<name>).
        # Gives them _NIMRO_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("nimro-"):
            platform_name = name[len("nimro-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_NIMRO_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
