...
For occasional contributors and PR authors. Full developer docs: https://nimro-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.nimro/config.yaml` (settings), `~/.nimro/.env` (API keys) — both under `$NIMRO_HOME` when it is set.
...
`_NIMRO_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_nimro_home()` for paths,
never hardcode `~/.nimro`. For custom/local-only tools, write a plugin in
`~/.nimro/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `nimro_cli/commands.py`
...
- Tests auto-redirect `NIMRO_HOME` to temp dirs — never touch real `~/.nimro/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/nimro_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_nimro_home()` from `nimro_constants` for all paths (profile-safe)
