...
Author widget apps for the Nimro TUI (`nimro --tui`): glanceable ambient
...
- The TUI must be in use (`nimro --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.nimro/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.nimro/logs/tui_gateway_crash.log` if a widget never appears.
