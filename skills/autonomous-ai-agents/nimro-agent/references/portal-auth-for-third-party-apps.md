...
## Layer 1 — Is this thing a Nimro plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Nimro process**. Its LLM calls go through Nimro's already-configured provider. | Already uses Portal if user's Nimro is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Nimro. That LLM call is already authenticated
via Nimro's provider config. The plugin's own server auth (e.g.
...
What Nimro Agent has that *feels* like OAuth — `nimro login --provider nous`
opening a browser, user signs in, token lands in `~/.nimro/auth.json` — is a
**Nimro-specific browser flow**. Under the hood it produces a credential
Nimro uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Nimro's existing Portal credential out of `~/.nimro/auth.json`.
...
rides on the credential Nimro already holds.

Where this could live in Nimro:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Nimro itself.
...
1. First decide: Nimro plugin (runs inside Nimro) or separate app? If plugin,
   it already uses Portal via Nimro's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Nimro agent itself") is fully supported. Nimro treats agent
...
pitch in the Tool Gateway announcement is specifically about Nimro Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Nimro is configured to use Portal as its provider. It is
