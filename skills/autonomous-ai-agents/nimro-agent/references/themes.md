# Themes / Skins — Author a Nimro Color Theme

Author a Nimro **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`nimro_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.nimro/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Nimro itself.
...
- Write access to the Nimro home dir — `~/.nimro` by default, or `$NIMRO_HOME`
  / the active profile's dir. Skins live in `<nimro-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `nimro config set`).
...
3. `write_file` it to `<nimro-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `nimro config set display.skin default` (or they can `/skin default`).
...
```
nimro skin set <key> <hex>      # e.g. nimro skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `nimro skin set background "#08201f"` changes
only the background; `nimro skin use <name>` / `nimro skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.nimro`** when a profile is active — resolve the real home
  from `$NIMRO_HOME` first, falling back to `~/.nimro`.
...
- **Never hand-edit `config.yaml` to activate.** Use `nimro config set
...
- **You apply it, not the user.** `nimro config set display.skin <name>` is
...
- `read_file` the written `<nimro-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `nimro config get display.skin` and confirm it reports `<name>`.
