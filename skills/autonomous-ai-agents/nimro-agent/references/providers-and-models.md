...
Set via `nimro model` (picker) or `nimro setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://nimro-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `nimro auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `nimro auth add openai-codex` |
| qwen-oauth | OAuth | `nimro auth add qwen-oauth` |
| minimax-oauth | OAuth | `nimro auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`nimro auth`).
Fallback chain when the primary fails: `nimro fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`nimro_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   nimro config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
