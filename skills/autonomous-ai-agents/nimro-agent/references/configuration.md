...
Edit with `nimro config edit` or `nimro config set section.key value`.
Full reference: https://nimro-agent.nousresearch.com/docs/user-guide/configuration
...
`nimro config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `nimro tools` (interactive) or `nimro tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_NIMRO_CORE_TOOLS` is the default bundle most platforms inherit).
