...
Write plugins for the Nimro desktop app: statusbar items, layout panes,
...
system (`~/.nimro/plugins/`) is otherwise documented separately.
...
- The Nimro desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$NIMRO_HOME/desktop-plugins/` (usually
  `~/.nimro/desktop-plugins/`).

## How to Run

1. Create `$NIMRO_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.nimro/...` by default, or `~/.nimro/profiles/<profile>/...` under a
...
The ONLY import surface is `@nimro/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.nimro/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@nimro/plugin-sdk`, `react`, and
