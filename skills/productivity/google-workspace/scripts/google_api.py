#!/usr/bin/env python3
"""Google Workspace API CLI for Nimro Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Nimro-facing JSON contract and falls back to the Python client
...
from _nimro_home import get_nimro_home
...
def _gws_binary() -> str | None:
    override = os.getenv("NIMRO_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Nimro Agent")
