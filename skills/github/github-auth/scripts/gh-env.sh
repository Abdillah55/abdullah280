#!/usr/bin/env bash
# GitHub environment detection helper for Nimro Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _nimro_env="${NIMRO_HOME:-$HOME/.nimro}/.env"; [ -f "$_nimro_env" ] && grep -q "^GITHUB_TOKEN=" "$_nimro_env" 2>/dev/null; then
