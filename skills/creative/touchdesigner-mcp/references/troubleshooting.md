...
## 5. Nimro Configuration

### Config location

`$NIMRO_HOME/config.yaml` (defaults to `~/.nimro/config.yaml` when `NIMRO_HOME` is unset)
...
Restart the Nimro session for changes to take effect. The MCP connection is
