...
NIMRO_HOME_DIR="${NIMRO_HOME:-$HOME/.nimro}"
NIMRO_CFG="${NIMRO_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Nimro config has twozero_td MCP entry ──
if [[ ! -f "$NIMRO_CFG" ]]; then
    echo -e " ${FAIL} Nimro config not found at ${NIMRO_CFG}"
    manual_steps+=("Create ${NIMRO_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$NIMRO_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Nimro config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Nimro config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${NIMRO_CFG} manually"); }
    manual_steps+=("Restart Nimro session to pick up config change")
fi
