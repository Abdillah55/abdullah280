...
``nimro_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import nimro_state (cycle) — shared
module-level constants live in nimro_state_common.
"""
...
from nimro_state_common import (
    DEFERRED_INDEX_SQL,
    FTS_CJK_STALE_KEY,
    FTS_SQL,
    FTS_STORAGE_VERSION,
...
# Moved methods logged under the "nimro_state" logger before the split;
# keep that logger identity so log filtering/capture behavior is unchanged.
logger = logging.getLogger("nimro_state")
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._nimro_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._nimro_fts5_probe")
            return True
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `nimro sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `nimro db optimize`) must keep its EXISTING
