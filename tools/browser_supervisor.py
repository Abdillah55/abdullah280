...
One ``CDPSupervisor`` runs per Nimro ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Nimro logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "nimro-dialog-bridge.invalid"
...
(() => {
  if (window.__nimroDialogBridgeInstalled) return;
  window.__nimroDialogBridgeInstalled = true;
  const ENDPOINT = "http://nimro-dialog-bridge.invalid/";
