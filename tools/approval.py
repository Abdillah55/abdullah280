...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("NIMRO_YOLO_MODE", ""))
...
# os.environ["NIMRO_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export NIMRO_INTERACTIVE.
_nimro_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "nimro_interactive",
    default=None,
)


def set_nimro_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["NIMRO_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``NIMRO_INTERACTIVE`` env var for legacy callers.
    """
    return _nimro_interactive_ctx.set("1" if interactive else "")


def reset_nimro_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_nimro_interactive_context`."""
    _nimro_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``NIMRO_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _nimro_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("NIMRO_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("NIMRO_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("NIMRO_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("NIMRO_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("NIMRO_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("NIMRO_CRON_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set NIMRO_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind NIMRO_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds NIMRO_SESSION_PLATFORM via
    ...
    if _is_cron_approval_context():
        return False
    if env_var_enabled("NIMRO_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $NIMRO_HOME, or by the resolved absolute
# active profile home path such as /home/nimro/.nimro/config.yaml. The
# resolved-absolute form is folded into the ~/.nimro/ patterns at detection
...
# go stale when NIMRO_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_NIMRO_ENV_PATH = (
    r'(?:~\/\.nimro/|'
    r'(?:\$home|\$\{home\})/\.nimro/|'
    r'(?:\$nimro_home|\$\{nimro_home\})/)'
    r'\.env\b'
)
# ~/.nimro/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _NIMRO_ENV_PATH; matches the NIMRO_HOME override form as
# well as ~/.nimro/.
_NIMRO_CONFIG_PATH = (
    r'(?:~\/\.nimro/|'
    r'(?:\$home|\$\{home\})/\.nimro/|'
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_nimro_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Nimro: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # `nimro` and `gateway` (e.g. `nimro -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bnimro\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart nimro gateway (kills running agents)"),
    (r'\bnimro\s+update\b', "nimro update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `nimro gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `nimro ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart nimro-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart nimro-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(nimro|gateway|cli\.py)\b', "kill nimro/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill nimro` but not
    # `kill -9 $(pgrep -f nimro)` because the substitution is opaque
    ...
    # the `nimro gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.nimro.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(nimro|ai\.nimro)\b', "stop/restart nimro launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Nimro file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.nimro/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Nimro-managed security file (~/.nimro/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_NIMRO_CONFIG_PATH}|{_NIMRO_ENV_PATH})', "in-place edit of Nimro config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_NIMRO_CONFIG_PATH}|{_NIMRO_ENV_PATH})', "in-place edit of Nimro config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_NIMRO_CONFIG_PATH}|{_NIMRO_ENV_PATH})', "in-place edit of Nimro config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.nimro/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / NIMRO_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Nimro home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\nimro), so folding the user home
    # first would eat the prefix the Nimro-home fold needs.
    command = _rewrite_resolved_nimro_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / NIMRO_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.nimro``); the matched
...
def _rewrite_resolved_nimro_home(command: str) -> str:
    """Rewrite the resolved absolute Nimro home prefix to ``~/.nimro/``.

    Resolves the active ``NIMRO_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.nimro/`` so the static ``_NIMRO_CONFIG_PATH`` / ``_NIMRO_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/nimro/.nimro/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$NIMRO_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from nimro_constants import get_nimro_home
        home = get_nimro_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.nimro")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``nimro send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Nimro ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"nimro-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Nimro YOLO.
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["NIMRO_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "NIMRO_SPINNER_PAUSE" in os.environ:
            del os.environ["NIMRO_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Nimro approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``NIMRO_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            NIMRO_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set NIMRO_INTERACTIVE or "
                "NIMRO_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set NIMRO_INTERACTIVE or "
            "NIMRO_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("NIMRO_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``NIMRO_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("NIMRO_EXEC_ASK")
