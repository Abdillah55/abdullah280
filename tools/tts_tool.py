...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.nimro/config.yaml``. Nimro
...
Configuration is loaded from ~/.nimro/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``nimro_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from nimro_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``nimro auth add <provider_id>``).
...
from tools.xai_http import nimro_xai_user_agent
...
# Base URL now resolved via nimro_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_nimro_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.nimro/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.nimro/config.yaml.
    ...
    try:
        ...
        logger.debug("nimro_cli.config not available, using default TTS config")
        return {}
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``nimro tools``); otherwise the historical Edge backend remains active.
    """
...
# custom voice-cloning scripts, etc.) into Nimro without any Python code
...
#           output_format: wav
#
# Nimro writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__NIMRO_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Nimro secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Nimro secrets (salvage of #56332) while still
    ...
    scrubbed = nimro_subprocess_env(inherit_credentials=False)
...
# file, so retired models disappear from nimro the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``nimro_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `nimro setup` to configure, "
    ...
    from nimro_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `nimro model` or set XAI_API_KEY.")
    ...
    # output_format when Nimro actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": nimro_xai_user_agent(),
        },
        json=payload,
        timeout=60,
        stream=True,
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import nimro_cli as _nimro_cli

            _nimro_version = str(_nimro_cli.__version__)
        except Exception:
            _nimro_version = "0.0.0"
        # Include Nimro client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Nimro caches Piper voice models.

    Resolves to ``~/.nimro/cache/piper-voices/`` under the active
    NIMRO_HOME so voice downloads follow profile boundaries.
    """
    from nimro_constants import get_nimro_dir
    root = Path(get_nimro_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.nimro/config.yaml (tts: section).
    ...
    platform = get_session_env("NIMRO_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `nimro setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run nimro setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'nimro setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'nimro tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_nimro_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
