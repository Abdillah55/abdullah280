#!/usr/bin/env python3
"""Open a URL, dev server, or file in the Nimro desktop GUI's preview pane.

Gated on ``NIMRO_DESKTOP`` (like ``read_terminal`` / ``close_terminal``) so it
...
def open_preview_tool(url: str, label: str = "") -> str:
    ...
    if not ok:
        return tool_error("The preview pane is only available in the Nimro desktop app.")

    return json.dumps({"success": True, "url": target, "label": label}, ensure_ascii=False)


def check_open_preview_requirements() -> bool:
    """Desktop GUI only — NIMRO_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("NIMRO_DESKTOP")


OPEN_PREVIEW_SCHEMA = {
    "name": "open_preview",
    "description": (
        "Open something in the preview pane beside the chat in the Nimro desktop "
