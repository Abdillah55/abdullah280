...
etc.) surface as specific blocked checks via `nimro computer-use doctor`
...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and NousResearch/nimro-agent#67052.
    """
...
# fetches the latest release, so a `NIMRO_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `NIMRO_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "NIMRO_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when Nimro should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Nimro should disable cua-driver telemetry for this user.
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Nimro runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Nimro process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Nimro session's YOLO choice
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of NousResearch/nimro-agent#47072: instead of hardcoding
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Nimro an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``NIMRO_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    Nimro Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `nimro computer-use doctor` succeeds from a login shell.
    """
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``NIMRO_CUA_DRIVER_CMD``) is never silently
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `nimro computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `nimro tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of NousResearch/nimro-agent#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of NousResearch/nimro-agent#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Nimro-managed secrets out of the child env.
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `nimro computer-use doctor` and check "
                f"{display_nimro_home()}/logs/agent.log for the phase timings."
            )
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    NousResearch/nimro-agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of NousResearch/nimro-agent#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Nimro run, with
        ...
        #     concurrent Nimro runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Nimro run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"nimro-{uuid.uuid4().hex[:12]}"
    ...
    def start(self) -> None:
        ...
        # `computer-use` / `mcp` extras), not part of Nimro' minimal core.
    ...
    def capture(
        ...
        Maps nimro `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of NousResearch/nimro-agent#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of NousResearch/nimro-agent#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See NousResearch/nimro-agent#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # NousResearch/nimro-agent#47072). cua-driver-rs gained an explicit
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Nimro adapter, and uses cua-driver's
