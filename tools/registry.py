"""Central registry for all nimro-agent tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (nimro_constants itself is stdlib-only, so no cycle).
        from nimro_constants import get_nimro_home

        return Path(get_nimro_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``nimro tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Nimro-home override for every
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``nimro tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("nimro_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``nimro_plugins.{name}``), not the exact module string.
                # A handler defined in ``nimro_plugins.pkg.handlers`` is
                # still owned by the ``nimro_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("nimro_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``nimro tools enable foo``)
