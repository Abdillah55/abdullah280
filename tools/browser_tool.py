...
from nimro_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Nimro secret
...
def _build_browser_env() -> dict:
    ...
    Strips Nimro-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``nimro_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = nimro_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    nimro_node_bin = str(nimro_home / "node" / "bin")
    nimro_node_root = str(nimro_home / "node")
    nimro_nm_bin = str(nimro_home / "node_modules" / ".bin")
    return [nimro_node_bin, nimro_node_root, nimro_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``nimro doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.nimro/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.nimro/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Nimro silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Nimro CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-nimro_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed nimro processes — this way every clean nimro exit sweeps
    ...
    # Sweep orphans from other crashed nimro processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live nimro processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current nimro PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live nimro process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning nimro process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced nimro PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent nimro instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-nimro_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live nimro process.
            continue
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Nimro-managed
    ...
    # node_modules binary, which disappears on the next ``nimro update`` and
    ...
    # Build an extended search PATH including Nimro-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_nimro_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_nimro_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_nimro_home() / "node")),
            ]
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this nimro PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_nimro_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = nimro_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Nimro
    ...
    from nimro_constants import get_nimro_dir
    screenshots_dir = get_nimro_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/nimro-agent:latest")
                else:
