#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Nimro desktop GUI.
...
``read_terminal`` it is gated on ``NIMRO_DESKTOP`` so it never appears outside
the GUI.
"""
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Nimro desktop GUI (the tabs mirroring terminal(background=true) runs). "
