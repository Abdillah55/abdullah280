#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Nimro Skills Hub.
...
Used by nimro_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _nimro_home() -> Path:
    return get_nimro_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _nimro_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            nimro_meta = metadata.get("nimro", {})
            if isinstance(nimro_meta, dict):
                tags = nimro_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.nimro/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "NousResearch/nimro-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                nimro_meta = meta_block.get("nimro", {})
                if isinstance(nimro_meta, dict):
                    tags = nimro_meta.get("tags", [])
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``nimro_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Nimro centralized index source
# ---------------------------------------------------------------------------

NIMRO_INDEX_URL = "https://nimro-agent.nousresearch.com/docs/api/skills-index.json"
NIMRO_INDEX_TTL = 6 * 3600  # 6 hours


def _nimro_index_cache_file() -> Path:
    return _index_cache_dir() / "nimro-index.json"


def _load_nimro_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for NIMRO_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    nimro_index_cache_file = _nimro_index_cache_file()
    if nimro_index_cache_file.exists():
        try:
            age = time.time() - nimro_index_cache_file.stat().st_mtime
            if age < NIMRO_INDEX_TTL:
                return json.loads(nimro_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Nimro index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Nimro index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Nimro index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        nimro_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        nimro_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class NimroIndexSource(SkillSource):
    """Skill source backed by the centralized Nimro Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "nimro-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "nimro-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        NimroIndexSource(auth=auth), # Centralized index (search + resolved install paths)
