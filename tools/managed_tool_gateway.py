...
def auth_json_path():
    """Return the Nimro auth store path, respecting NIMRO_HOME overrides."""
    return get_nimro_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`nimro tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Nimro release, and
