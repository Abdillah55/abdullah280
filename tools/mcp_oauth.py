...
This module provides the glue:
    - ``NimroTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Nimro Agent"
"""
...
from nimro_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__nimro_user_skipped__"
...
def _get_token_dir(nimro_home: str | Path | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses NIMRO_HOME so each profile gets its own OAuth tokens.
    Layout: ``NIMRO_HOME/mcp-tokens/``
    """
    from nimro_constants import get_nimro_home

    base = Path(nimro_home) if nimro_home is not None else Path(get_nimro_home())
...
def _cached_redirect_port(storage: "NimroTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Nimro restarts and chooses a
...
def _cached_redirect_uri(storage: "NimroTokenStorage | None") -> str | None:
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``nimro mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `nimro mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# NimroTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class NimroTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        NIMRO_HOME/mcp-tokens/<server_name>.json         -- tokens
        NIMRO_HOME/mcp-tokens/<server_name>.client.json   -- client info
        NIMRO_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """

    def __init__(self, server_name: str, *, nimro_home: str | Path | None = None):
        self._server_name = _safe_filename(server_name)
        self._nimro_home = Path(nimro_home) if nimro_home is not None else None

    def _tokens_path(self) -> Path:
        return _get_token_dir(self._nimro_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._nimro_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._nimro_home) / f"{self._server_name}.meta.json"
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if OAuthToken is None and not _ensure_sdk_loaded():
            return None
        # Nimro records an absolute wall-clock ``expires_at`` alongside the
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._nimro_home)
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Nimro.</p></body></html>"
            ) if code else (
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://nimro-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `nimro mcp login <server>` later to "
...
def remove_oauth_tokens(
    ...
    *,
    ...
    storage = NimroTokenStorage(server_name, nimro_home=nimro_home)
    storage.remove()
...
def _configure_callback_port(
    cfg: dict,
    storage: "NimroTokenStorage | None" = None,
) -> int:
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Nimro Agent" / "Nimro" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    if OAuthClientMetadata is None:
        _ensure_sdk_loaded()
    client_name = cfg.get("client_name", "Nimro Agent")
    scope = cfg.get("scope")
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Nimro auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Nimro is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    storage = NimroTokenStorage(server_name)
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`nimro mcp login {server_name}` interactively first to complete "
