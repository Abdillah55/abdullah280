...
_FORCE_SYNC_ENV = "NIMRO_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.nimro") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.nimro to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``NIMRO_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, nimro_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.nimro/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(nimro_home=nimro_home)

    def _sync_back_transaction(self, nimro_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="nimro-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.nimro/skills/a.md`` →
        ``~/.nimro/skills/a.md``, a new remote file at
        ``/root/.nimro/skills/b.md`` maps to ``~/.nimro/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
