...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Nimro was already launched from (for example ``nimro-agent``
    while the process cwd is ``~/.nimro/nimro-agent``), passing it through
    unchanged makes the wrapper run ``cd nimro-agent`` *inside* the project
    ...
    # Common recovery for config values like ``nimro-agent`` when Nimro was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./nimro-agent``; use the current directory instead.
...
# Nimro-internal env vars that should NOT leak into terminal subprocesses.
_NIMRO_PROVIDER_ENV_FORCE_PREFIX = "_NIMRO_FORCE_"

# Nimro-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Nimro inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from nimro_cli.config import OPTIONAL_ENV_VARS
    ...
    # Nimro-managed inference credential — Claude subscription auth is not a
    # working Nimro provider path.  Stripping it broke agent-spawned
...
_NIMRO_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Nimro venv path instead of the project's own
# ``.venv`` — silently clobbering the Nimro environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Nimro venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _is_nimro_internal_secret(key: str) -> bool:
    """Return True for Nimro-internal secrets injected under *dynamic* names.

    ``_NIMRO_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Nimro-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`nimro_subprocess_env` helper all call it, so the
...
def _inject_context_nimro_home(env: dict) -> None:
    """Bridge the context-local Nimro home override into subprocess env."""
    try:
        from nimro_constants import get_nimro_home_override

        value = get_nimro_home_override()
        if value:
            env["NIMRO_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (NIMRO_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Nimro-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_NIMRO_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_nimro_internal_secret(key):
            continue
        passthrough = _is_passthrough(key)
        if key in _NIMRO_PROVIDER_ENV_BLOCKLIST and not passthrough:
            continue
    ...
    for key, value in (extra_env or {}).items():
        if key.startswith(_NIMRO_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_NIMRO_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_nimro_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_nimro_internal_secret(key):
            continue
        else:
            passthrough = _is_passthrough(key)
            if key in _NIMRO_PROVIDER_ENV_BLOCKLIST and not passthrough:
    ...
    _inject_context_nimro_home(sanitized)

    from nimro_constants import apply_subprocess_home_env
...
# legitimate child Nimro spawns needs them, and they are the highest-value
...
# narrow subset of _NIMRO_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in nimro_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_nimro_internal_secret, but _ID has no secret suffix, so it must be
...
def nimro_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_NIMRO_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Nimro spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_NIMRO_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Nimro-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_nimro_internal_secret`.
    for key in list(env):
        if key.startswith(_NIMRO_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_nimro_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _NIMRO_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_nimro_home(env)
    from nimro_constants import apply_subprocess_home_env
    ...
    # copies os.environ, whose NIMRO_SESSION_* mirror is a last-writer-wins
    ...
    # still see the parent's NIMRO_HOME but lose the DB mutation guard.
...
def build_subprocess_env(
    ...
    *,
    ...
    function (or :func:`nimro_subprocess_env` for the model-driving-CLI
    surface) instead of copying ``os.environ`` directly, so profile-home
    propagation (``NIMRO_HOME`` / subprocess ``HOME`` contract) and the
    Nimro secret-scrub policy have a single owner.  History: ~11 separate
    ...
    * ``scrub_secrets=True`` (default) — delegate to
      ...
      list (provider blocklist + ``_is_nimro_internal_secret`` dynamic
      patterns + kanban/venv-marker/session-context guards) **and** of
      ``NIMRO_HOME`` / subprocess-HOME propagation.  On this path profile
    ...
    * ``inherit_profile_home`` — on the non-scrub path, when True, bridge the
      context-local Nimro home override into ``NIMRO_HOME`` and apply the
      subprocess HOME contract (``nimro_constants.apply_subprocess_home_env``).
    ...
    * ``extra`` — applied **last** on the non-scrub path so explicit caller
      overrides (e.g. a session-scoped ``NIMRO_HOME``) always win.  On the
      scrub path it is forwarded as ``_sanitize_subprocess_env``'s
      ``extra_env`` (same force-prefix / blocklist handling as today).
    """
    if scrub_secrets:
        # _sanitize_subprocess_env already performs NIMRO_HOME override
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("NIMRO_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale NIMRO_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\nimro\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\nimro\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _nimro_portable_git = os.path.join(_local_appdata, "nimro", "git") if _local_appdata else ""
    if _nimro_portable_git:
        for candidate in (
            os.path.join(_nimro_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_nimro_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # NIMRO_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    ...
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "NIMRO_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. Nimro Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set NIMRO_GIT_BASH_PATH to your bash.exe location."
    )
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Nimro. If the override is blocked or later re-applied, "
...
# Cached directory containing the ``nimro`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_NIMRO_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_nimro_bin_dir() -> str | None:
    """Return the directory holding the ``nimro`` console-script, or None.
    ...
    nimro install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``nimro``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``nimro`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``nimro`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("nimro")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``nimro`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _NIMRO_BIN_DIR
    if _NIMRO_BIN_DIR is not _SENTINEL:
        return _NIMRO_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("nimro")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "nimro" or base.startswith("nimro."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "nimro.exe" if _IS_WINDOWS else "nimro"
    ...
    _NIMRO_BIN_DIR = candidate
    return candidate


def _prepend_nimro_bin_dir(existing_path: str) -> str:
    """Prepend the nimro install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_nimro_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _managed_runtime_path_entries() -> list[str]:
    """Return existing Nimro-managed runtime dirs for the terminal subshell PATH.

    The terminal tool spawns a subshell whose PATH is the agent process's PATH
    plus ``_SANE_PATH``. Neither carries the runtimes Nimro installs for
    itself, so on a machine where Nimro provisioned its own toolchain a
    command the agent runs resolves a system copy instead — or nothing at all:

    - ``$NIMRO_HOME/node`` (+ ``/bin``) — installed to satisfy the desktop and
      browser toolchain. ``tools/browser_tool.py`` already does this for its own
      subprocesses; the agent's shell deserves the same.
    - ``$NIMRO_HOME/bin`` — the managed ``uv``. ``install.sh`` writes it there
    ...
    ``get_nimro_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_nimro_managed_node``, a first browser install).
    """
    try:
        from nimro_constants import get_nimro_home, iter_nimro_node_dirs

        candidates = [*iter_nimro_node_dirs(), get_nimro_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Nimro-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Nimro
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_NIMRO_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_NIMRO_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_nimro_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_nimro_internal_secret(k):
            continue
        else:
            passthrough = _is_passthrough(k)
            if k in _NIMRO_PROVIDER_ENV_BLOCKLIST and not passthrough:
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the nimro install dir is reachable so plugins can shell out
        # to bare ``nimro`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_nimro_bin_dir(new_path)

    _inject_context_nimro_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Nimro trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``NIMRO_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under NIMRO_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_nimro_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "nimro_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._nimro_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_nimro_pgid", None)
                    if pgid is None:
                        raise
