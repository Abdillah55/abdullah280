...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Nimro'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``NIMRO_HOME`` and restores
...
def _ensure_vercel_sdk() -> None:
    ...
    # (vercel-internal-telemetry posts to telemetry.vercel.com). Nimro
...
def _snapshot_store_path() -> Path:
    return get_nimro_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            container_base = "/.nimro"
        else:
            container_base = f"{self._remote_home.rstrip('/')}/.nimro"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_nimro = (
            "/.nimro"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.nimro"
        )
        archive_member = remote_nimro.lstrip("/")
        remote_tar = f"/tmp/.nimro_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
