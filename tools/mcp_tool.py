...
transport, discovers their tools, and registers them into the nimro-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.nimro/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.nimro/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Nimro' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Nimro explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Nimro and
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            nimro_home = os.path.expanduser(
                os.getenv(
                    "NIMRO_HOME", os.path.join(os.path.expanduser("~"), ".nimro")
                )
            )
            candidates = [
                os.path.join(nimro_home, "node", "bin", resolved_command),
                ...
                # image (which the Nimro Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Nimro MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Nimro gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Nimro document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`nimro_cli.mcp_config._unwrap_exception_group` with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Nimro' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (NIMRO_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit NIMRO_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Nimro' logging (agent.log via nimro_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `nimro setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Nimro process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Nimro' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Nimro is fully
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `nimro mcp login {server_name}` (or delete the tokens "
        f"file under ~/.nimro/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Nimro processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_nimro_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local NIMRO_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_nimro_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_nimro_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local NIMRO_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_nimro_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Nimro config file.
    ...
    ``os.environ`` (which includes ``~/.nimro/.env`` loaded at startup).
    """
    try:
        from nimro_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("NIMRO_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Nimro' MEDIA: tag convention and out to messaging
            ...
            # Nimro' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Nimro's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Nimro uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Nimro registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``nimro tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Nimro registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple nimro
