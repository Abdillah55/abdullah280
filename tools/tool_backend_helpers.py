...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `nimro model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``nimro auth add
    ...
    2. The environment / ``~/.nimro/.env``. Under a multiplexed gateway turn
       ...
       ``nimro_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``nimro auth
    ...
    when omitted, ``nimro_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `nimro auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``nimro auth add openai-api``) via
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.nimro/.env`` (via
    ``nimro_cli.config.get_env_value`` when available) so tool-side
