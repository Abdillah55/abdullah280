...
`nimro skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.nimro.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.nimro.blueprint`` is a mapping containing
    ...
    nimro = meta.get("nimro") if isinstance(meta, dict) else None
    blueprint = nimro.get("blueprint") if isinstance(nimro, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.nimro.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.nimro.blueprint`` block) they can hand
    to ``nimro skills publish`` to share. ``body`` is the plain-language
