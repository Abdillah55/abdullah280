...
provider** — turn on a backend by enabling a plugin (``nimro plugins
enable video_gen/<name>``) and selecting it in ``nimro tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/nimro-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `nimro tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `nimro plugins list` to see "
            f"installed video gen backends, or `nimro tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `nimro tools` → "
...
# `nimro tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`nimro tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `nimro tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
