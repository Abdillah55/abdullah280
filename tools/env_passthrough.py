...
def _is_nimro_provider_credential(name: str) -> bool:
    """True if ``name`` is a Nimro-managed provider credential (API key,
    token, or similar) per ``_NIMRO_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Nimro API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Nimro credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Nimro-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_nimro_internal_secret(name):
        return True
    return name in _NIMRO_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Nimro-managed provider credentials (from
    ``_NIMRO_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Nimro-managed
    ...
    Non-Nimro third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_nimro_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Nimro provider "
                "credential %r (blocked by _NIMRO_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Nimro-managed provider credentials must not be passed
                ...
                if _is_nimro_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Nimro "
                        "provider credential %r from config.yaml (blocked "
                        "by _NIMRO_PROVIDER_ENV_BLOCKLIST). Operator "
