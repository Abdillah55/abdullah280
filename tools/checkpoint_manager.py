...
-----------------------------------------------------------------------------
    ...
    ~/.nimro/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/nimro/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_nimro_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/nimro"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Nimro convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("NIMRO_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Nimro infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``nimro checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `nimro checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "nimro@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Nimro Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``NIMRO_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at NIMRO_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "NIMRO_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Nimro changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Nimro changed", not an exact per-session ledger.
...
# ---------------------------------------------------------------------------
# Public helpers for `nimro checkpoints` CLI
# ---------------------------------------------------------------------------
