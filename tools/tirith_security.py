...
it is automatically downloaded from GitHub releases to $NIMRO_HOME/bin/tirith.
...
def _get_nimro_home() -> str:
    """Return the Nimro home directory, respecting NIMRO_HOME env var."""
    return str(get_nimro_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_nimro_home(), ".tirith-install-failed")
...
def _nimro_bin_dir() -> str:
    """Return $NIMRO_HOME/bin, creating it if needed."""
    d = os.path.join(_get_nimro_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $NIMRO_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_nimro_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $NIMRO_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $NIMRO_HOME/bin/tirith
    ...
    nimro_bin = os.path.join(_nimro_bin_dir(), "tirith")
    if os.path.isfile(nimro_bin) and os.access(nimro_bin, os.X_OK):
        _resolved_path = nimro_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return nimro_bin
