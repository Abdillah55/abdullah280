...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``nimro tools`` repaint, tool-registration scans,
    ...
    2. ``~/.nimro/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``nimro auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_nimro_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.nimro/.env`` first, then ``os.environ``.

    Wraps :func:`nimro_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from nimro_cli.config import get_env_value as _nimro_get_env_value
    except ImportError:
        return os.environ.get(name, default)

    value = _nimro_get_env_value(name)
    return value if value is not None else default


def nimro_xai_user_agent() -> str:
    """Return a stable Nimro-specific User-Agent for xAI HTTP calls."""
    try:
        from nimro_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Nimro-Agent/{__version__}"


def nimro_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Nimro Agent,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": nimro_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Nimro config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Nimro home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_nimro_home() / "state"
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Nimro-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``nimro_cli.config.get_env_value`` so keys
    stored in ``~/.nimro/.env`` (the standard Nimro location) are honored —
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("NIMRO_XAI_BASE_URL")
