...
Used by ``nimro profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``nimro_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Nimro Agent kanban board.
...
Rules:
  ...
  - Never write "Nimro Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_nimro_home())
        else:
