...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or nimro -c.",
    ...
    "Ctrl+Z suspends Nimro to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "nimro -c resumes your most recent CLI session. nimro -c \"project name\" resumes by title.",
    "nimro -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "nimro -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "nimro chat -t web,terminal enables only specific toolsets for a focused session.",
    "nimro chat -s github-pr-workflow preloads a skill at launch.",
    "nimro chat -q \"query\" runs a single non-interactive query and exits.",
    "nimro chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "nimro chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "nimro --yolo bypasses all dangerous command approval prompts for the entire session.",
    "nimro chat --source telegram tags the session for filtering in nimro sessions list.",
    "nimro -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "nimro doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "nimro dump outputs a compact setup summary — great for bug reports.",
    "nimro config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "nimro config edit opens config.yaml in your default editor.",
    "nimro config check scans for missing or stale configuration options.",
    "nimro sessions browse opens an interactive session picker with search.",
    "nimro sessions stats shows session counts by platform and database size.",
    "nimro sessions prune --older-than 30 cleans up old sessions.",
    "nimro skills search react --source skills-sh searches the skills.sh public directory.",
    "nimro skills check scans installed hub skills for upstream updates.",
    "nimro skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "nimro skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "nimro mcp add github --command npx adds MCP servers from the command line.",
    "nimro mcp serve runs Nimro itself as an MCP server for other agents.",
    "nimro auth add lets you add multiple API keys for credential pool rotation.",
    "nimro completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "nimro logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "nimro backup creates a zip backup of your entire Nimro home directory.",
    "nimro profile create coder creates an isolated profile that becomes its own command.",
    "nimro profile create work --clone copies your current config and keys to a new profile.",
    "nimro update syncs new bundled skills to ALL profiles automatically.",
    "nimro gateway install sets up Nimro as a system service (systemd/launchd).",
    "nimro memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "nimro webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: nimro tools disables unused tools, nimro skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "nimro models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as nimro -w).",
    ...
    "NIMRO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Nimro tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'nimro profile create coder' creates the 'coder' command.",
    "nimro profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Nimro prints a resume command with session ID and stats.",
    "nimro sessions export backup.jsonl exports all sessions for backup or analysis.",
    "nimro -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "nimro skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .nimro/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: nimro cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Nimro runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "nimro gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Nimro via a local macOS server.",
    ...
    "Context files (.nimro.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Nimro loads project context from .nimro.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "nimro mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "nimro mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "nimro mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — nimro tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.nimro/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "nimro chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "nimro plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "nimro acp runs Nimro as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "NIMRO_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "nimro auth add nous or nimro auth add openai-codex sets up OAuth-based providers.",
    ...
    "nimro status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.nimro/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Nimro your own.",
    ...
    "Before a gateway session reset, Nimro auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.nimro/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Nimro state via NIMRO_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "NIMRO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in NIMRO_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.nimro/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'nimro profile create ls' would be rejected.",
    "nimro profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "nimro claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "NIMRO_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via nimro tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.nimro/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.nimro/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Nimro auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Nimro config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Nimro is working.',
    ...
    '/reload reloads ~/.nimro/.env into the running session — pick up new API keys without restarting.',
    ...
    "NIMRO_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.nimro/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.nimro/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'nimro curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "nimro curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'nimro curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'nimro auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'NIMRO_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "NIMRO_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'nimro dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'nimro dashboard embeds the full Nimro TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.nimro/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.nimro/dashboard-plugins/ — no npm build required.',
    ...
    'NIMRO_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'NIMRO_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'NIMRO_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'NIMRO_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.nimro/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.nimro/skills/ so drop-in skills appear without restarting the session.',
    ...
    'nimro -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'nimro chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'nimro chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'nimro chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "nimro chat --source tool tags programmatic chats so they don't clutter nimro sessions list.",
    'nimro dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'nimro sessions rename <ID> "new title" renames any past session; nimro sessions delete <ID> removes one.',
    'nimro import restores a full Nimro backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'nimro fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'nimro pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'nimro setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'nimro status --deep runs the full health sweep across every component; plain nimro status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'NIMRO_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'NIMRO_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.nimro/plugins/ — trust-gated by design.',
    "NIMRO_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'NIMRO_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'NIMRO_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'NIMRO_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'NIMRO_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'NIMRO_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'NIMRO_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'NIMRO_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'NIMRO_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'NIMRO_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'NIMRO_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Nimro block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from nimro sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.nimro/dashboard-plugins/.',
]
