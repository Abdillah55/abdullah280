"""`nimro checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.nimro/checkpoints/``.  Actions:

    nimro checkpoints               # same as `status`
    nimro checkpoints status        # total size, project count, breakdown
    nimro checkpoints list          # per-project checkpoint counts + workdir
    nimro checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    nimro checkpoints clear [-f]    # nuke the entire base (asks first)
    nimro checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    nimro checkpoints
    nimro checkpoints prune --retention-days 3 --max-size-mb 200
    nimro checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: nimro checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``nimro checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `nimro checkpoints` → status
