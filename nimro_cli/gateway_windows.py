...
_TASK_NAME_DEFAULT = "Nimro_Gateway"
_TASK_DESCRIPTION = "Nimro Agent Gateway - Messaging Platform Integration"
...
def _preserve_nimro_home_path(path: str | Path) -> str:
    """Render Nimro-owned paths under the configured NIMRO_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\nimro`` as a symlink/junction to
    ...
    path lives under NIMRO_HOME.
    """
    candidate = Path(path)
    try:
        from nimro_cli.config import get_nimro_home

        home = Path(get_nimro_home())
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Nimro profile."""
    from nimro_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "nimro_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("NIMRO_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("NIMRO_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("NIMRO_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["NIMRO_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["NIMRO_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["NIMRO_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("NIMRO_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("NIMRO_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("NIMRO_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Nimro_Gateway``
    Named profile X: ``Nimro_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during nimro_cli boot.
    from nimro_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\nimro\\gateway-service\\<task_name>.cmd``
    (or ``<NIMRO_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Nimro installs stay self-contained).
    """
    _assert_windows()
    from nimro_cli.config import get_nimro_home

    script_dir = Path(get_nimro_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``NIMRO_HOME`` whenever it
    ...
    source checkout only if ``NIMRO_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports NIMRO_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m nimro_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``nimro gateway run``.
    ...
    lines.append(f'set "NIMRO_HOME={nimro_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "NIMRO_GATEWAY_DETACHED=1"')
    python_exe_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports nimro_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_nimro_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_nimro_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_nimro_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [python_exe_path, "-m", "nimro_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_nimro_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_nimro_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('NIMRO_HOME')}) = {_quote_vbs_string(nimro_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('NIMRO_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_nimro_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_nimro_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    nimro_home = str(Path(get_nimro_home()))
    profile_arg = _profile_arg(nimro_home)

    content = _build_gateway_cmd_script(python_path, working_dir, nimro_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, nimro_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Nimro builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_nimro_home_path(get_python_path())
    )
    project_root = _preserve_nimro_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    nimro_home = str(Path(get_nimro_home()))
    profile_arg = _profile_arg(nimro_home)

    argv = [python_exe, "-m", "nimro_cli.main"]
    if profile_arg:
    ...
    env_overlay = {
        ...
        "NIMRO_GATEWAY_DETACHED": "1",
        "VIRTUAL_ENV": _preserve_nimro_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_nimro_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (NIMRO_HOME, VIRTUAL_ENV,
    PYTHONPATH) so the respawn doesn't depend on the watcher's transient
    working directory.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m nimro_cli.main
    ...
    from nimro_cli.config import get_nimro_home
    from nimro_cli.gateway import PROJECT_ROOT
    ...
    try:
        nimro_home = str(Path(get_nimro_home()).resolve())
    except Exception:
        nimro_home = ""
    ...
    if nimro_home:
        env_overlay["NIMRO_HOME"] = nimro_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m nimro_cli.main gateway run`` directly — NOT
    ...
    log_dir = Path(get_nimro_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("NIMRO_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("NIMRO_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from nimro_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `nimro -p <profile> gateway install` must be safe.
    ...
    from nimro_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"nimro {profile_arg} gateway start" if profile_arg else "nimro gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: nimro gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Nimro gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: nimro gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: nimro gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from nimro_cli.config import get_nimro_home
        print(f"    type {Path(get_nimro_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_nimro_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from nimro_cli.config import get_nimro_home

    nimro_home = Path(get_nimro_home())
    print()
    print("Next steps:")
    print("  nimro gateway status                      # Check status")
    print(f"  type {nimro_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Nimro gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: nimro gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: nimro gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: nimro gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `nimro gateway status`."
        )
