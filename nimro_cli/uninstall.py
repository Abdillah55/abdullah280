"""
Nimro Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.nimro/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Nimro PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing nimro-agent or nimro PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Nimro Agent" comment and following line
                if '# Nimro Agent' in line or '# nimro-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('nimro' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing nimro
                if 'nimro' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the nimro wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "nimro",
        Path.home() / ".local" / "bin" / "nimro-acp",
        Path.home() / ".local" / "bin" / "nimro-agent",
        Path("/usr/local/bin/nimro"),
        Path("/usr/local/bin/nimro-acp"),
        Path("/usr/local/bin/nimro-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains nimro_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'nimro_cli' in content or 'nimro-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(nimro_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``nimro`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Nimro home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (nimro_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``nimro gateway run`` processes
    ...
    try:
        from nimro_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from nimro_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``NIMRO_HOME`` and ``NIMRO_GIT_BASH_PATH``
...
def _nimro_path_markers(nimro_home: Path) -> list[str]:
    """Path-entry substrings that identify Nimro-owned User-PATH entries."""
    root = str(nimro_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare nimro-agent install dir.
    markers = [root + "\\nimro-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if NIMRO_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "nimro".  We don't want to catch
    # unrelated entries like "cnimro-foo" or "ephermeral", so we look for
    # backslash-nimro as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(nimro_home: Path) -> list[str]:
    """Strip Nimro-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _nimro_path_markers(nimro_home)
            kept: list[str] = []
...
def remove_nimro_env_vars_windows() -> list[str]:
    """Delete NIMRO_HOME and NIMRO_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("NIMRO_HOME", "NIMRO_GIT_BASH_PATH"):
                try:
...
def _is_default_nimro_home(nimro_home: Path) -> bool:
    """Return True when ``nimro_home`` points at the default (non-profile) root."""
    try:
        from nimro_constants import get_default_nimro_root
        return nimro_home.resolve() == get_default_nimro_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its NIMRO_HOME directory.

    We shell out to ``nimro -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current NIMRO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m nimro_cli.main` so we don't depend on a `nimro`
    #    wrapper that may be half-removed mid-uninstall.
    nimro_invocation = [_sys.executable, "-m", "nimro_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                nimro_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's NIMRO_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``nimro uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$NIMRO_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from nimro_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(nimro_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Nimro Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Nimro Chat GUI installation was found.")
        print(f"  Checked: {nimro_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Nimro agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(nimro_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Nimro agent at {nimro_home / 'nimro-agent'}")
        print(f"  • Your config, sessions, and secrets under {nimro_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Nimro agent is still installed. Run 'nimro' to use the CLI,")
    print("or 'nimro uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.nimro/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.nimro/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie NIMRO_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_nimro_home(nimro_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.nimro data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Nimro Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {nimro_home / 'config.yaml'}")
    print(f"  Secrets: {nimro_home / '.env'}")
    print(f"  Data:    {nimro_home / 'cron/'}, {nimro_home / 'sessions/'}, {nimro_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their NIMRO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Nimro data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Nimro code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, nimro_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Nimro PATH entries from shell configs / Windows User PATH")
    print("  • Nimro wrapper scripts and Nimro-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Nimro config/data: {nimro_home}")
        if _is_default_nimro_home(nimro_home):
        ...
        print(f"  • Keep Nimro config/data: {nimro_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``nimro`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$NIMRO_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in nimro_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\nimro\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(nimro_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Nimro-owned PATH entries in User environment")

        log_info("Removing NIMRO_HOME / NIMRO_GIT_BASH_PATH User env vars...")
        removed_env = remove_nimro_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Nimro-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing nimro command...")
    ...
    #     (only when they still point into this Nimro home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Nimro-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(nimro_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Nimro-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(nimro_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE NIMRO_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from nimro_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(nimro_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.nimro/, just remove the nimro-agent subdir
            if nimro_home in project_root.parents or project_root.parent == nimro_home:
    ...
    #     under NIMRO_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(nimro_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(nimro_home)
    ...
    # 5. Optionally remove ~/.nimro/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile NIMRO_HOME dirs live under
        ...
        try:
            if nimro_home.exists():
                shutil.rmtree(nimro_home)
                log_success(f"Removed {nimro_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {nimro_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {nimro_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://nimro-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://nimro-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Nimro Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m nimro_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import nimro_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``nimro_constants`` + ``nimro_cli.colors``
    (and lazily ``nimro_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m nimro_cli.uninstall")
    parser.add_argument(
        "--mode",
