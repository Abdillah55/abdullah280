...
# xAI's public API is OpenAI-compatible for the endpoints Nimro commonly
# uses. The Responses endpoint is included because Nimro' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Nimro-managed OAuth credentials."""

    auth_hint = "nimro auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`nimro auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`nimro auth reset xai-oauth` or re-authenticate with "
                    "`nimro auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `nimro auth add xai-oauth --type oauth`."
            )
