...
Used by ``nimro kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``nimro_cli/goals.py`` — same aux
...
NIMRO_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("NIMRO_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Nimro Agent board.
...
def _profile_author() -> str:
    """Mirror of ``nimro_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("NIMRO_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=NIMRO_KANBAN_SPECIFY_MAX_TOKENS,
