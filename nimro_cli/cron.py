...
# commands at execution time when ``_NIMRO_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: nimro gateway install", Colors.DIM))
    print(color("                    sudo nimro gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  nimro cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'nimro cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: nimro gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec nimro "
                        "nimro cron ...`). Fix ownership to match the "
        ...
        print("    nimro gateway install    # Install as a user service")
        print("    sudo nimro gateway install --system  # Linux servers: boot-time system service")
        print("    nimro gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: nimro cron [list|create|edit|pause|resume|run|remove|status|runs|tick]")
    sys.exit(1)
