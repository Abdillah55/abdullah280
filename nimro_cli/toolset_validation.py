...
the tool registry or launching Nimro (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``nimro-cli`` to the non-existent ``nimro``. ``resolve_toolset('nimro')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``nimro-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``nimro`` instead of ``nimro-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `nimro tools` to reconfigure."
        )
    return warnings
