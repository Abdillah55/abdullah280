"""
Interactive setup wizard for Nimro Agent.
...
from nimro_constants import get_optional_skills_dir

logger = logging.getLogger(__name__)

PROJECT_ROOT = Path(__file__).parent.parent.resolve()

_DOCS_BASE = "https://nimro-agent.nousresearch.com/docs"
...
# display_nimro_home imported lazily at call sites (stale-module safety during nimro update)
...
from nimro_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from nimro_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Nimro Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Nimro using environment variables or config commands:")
    print_info("  nimro config set model.provider custom")
    print_info("  nimro config set model.base_url http://localhost:8080/v1")
    print_info("  nimro config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'nimro setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``NIMRO_NONINTERACTIVE=1`` (see ``nimro_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("NIMRO_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``NIMRO_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from nimro_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'nimro setup')")


def _print_setup_summary(config: dict, nimro_home):
    ...
    try:
        from nimro_cli.auth import resolve_provider
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Nimro cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    nimro model            (pick any provider/model)")
        print_info("    nimro setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'nimro setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from nimro_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `nimro tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from nimro_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'nimro setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'nimro setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'nimro tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'nimro setup terminal'"))
    ...
    # Spotify (OAuth via nimro auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'nimro setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {nimro_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('nimro setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('nimro setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('nimro setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('nimro setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('nimro setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('nimro config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('nimro config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('nimro config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('nimro', Colors.GREEN)}              Start chatting")
    print(f"   {color('nimro gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('nimro doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `nimro tools` and `nimro setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``nimro model``)
    ...
    provider added to ``nimro model`` is automatically available here.
    ...
    # Delegate to the shared nimro model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from nimro_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: nimro model")
    ...
    # on demand via `nimro auth add`, `nimro setup` vision, and
    # `nimro setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from nimro_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``nimro model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from nimro_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.nimro/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Nimro auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via nimro setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'nimro setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Nimro runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `nimro setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `nimro setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `nimro egress setup` then `nimro egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `nimro egress setup`."
            )
        ...
        print_info("Requires the optional SDK: pip install 'nimro-agent[vercel]'")
        ...
        try:
            ...
            # Managed uv first: $NIMRO_HOME/bin is never on PATH, so a bare
            # which() misses the uv Nimro installed. Bootstrapping one is
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'nimro-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into NIMRO_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("NIMRO_MAX_ITERATIONS")
    ...
    print_info("  Run `nimro setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # NIMRO_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.nimro/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from nimro_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_nimro_home(Path(get_nimro_home()))
    except Exception:
        pass
...
def _profile_name_from_nimro_home(nimro_home) -> str | None:
    """Return the active profile name when NIMRO_HOME is a profile dir."""
    if nimro_home.parent.name == "profiles":
        return nimro_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Nimro delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Nimro to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from nimro_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://nimro-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://nimro-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  nimro config edit")
    print_info("   Open config in your editor:  nimro config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from nimro_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Nimro from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'nimro setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     nimro config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_nimro_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: nimro gateway install --system")
                print_info("  Or run in foreground:  nimro gateway")
        else:
            from nimro_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   nimro gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   nimro gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `nimro setup tools` and `nimro tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Nimro profile and are not uploaded.")
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``nimro_cli.auth`` — lists every supported
    ...
    """
    try:
        from nimro_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from nimro_cli.config
    ...
    if section_key == "model":
        ...
        from nimro_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_nimro.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_nimro migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_nimro", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Nimro.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Nimro to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Nimro at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Nimro at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Nimro at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Nimro at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Nimro equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Nimro config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Nimro.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Nimro's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(nimro_home: Path) -> bool:
    ...
    print_info("Nimro can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: nimro claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=nimro_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: nimro claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Nimro configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Nimro config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Nimro (use nimro claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``nimro setup --portal`` and ``nimro portal``. This is the
    ...
    Nimro session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``nimro setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``nimro model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``nimro portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Nimro Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `nimro portal` == quick setup's Nous step.
    try:
        from nimro_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `nimro portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `nimro portal`: %s", exc)
        print()
    ...
    print_info("  Run `nimro portal info` to inspect routing.")
    print_info("  Run `nimro` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      nimro setup           — full or quick (auto-detected)
      nimro setup model     — just model/provider
      nimro setup tts       — just text-to-speech
      nimro setup terminal  — just terminal backend
      nimro setup gateway   — just messaging platforms
      nimro setup tools     — just tool configuration
      nimro setup telemetry — just local shared metrics
      nimro setup agent     — just agent settings
    """
    from nimro_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Nimro Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Nimro Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Nimro Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, nimro_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Nimro configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'nimro setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(nimro_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Nimro?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, nimro_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, nimro_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {nimro_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'nimro config edit'")
    ...
    # Tune later with `nimro setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, nimro_home)


def _run_first_time_quick_setup(config: dict, nimro_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``nimro setup <section>``
    or switch providers with ``nimro model``.
    """
    from nimro_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'nimro setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    nimro setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  nimro setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``nimro tools`` (which
    ...
    try:
        ...
        from nimro_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``nimro setup agent`` /
    ``nimro config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, nimro_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `nimro update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        nimro tools")
        print_info("  Seed skills:         nimro skills opt-in --sync")
        print_info("  Add MCP servers:     nimro mcp add")
        print_info("  Enable plugins:      nimro plugins")
        print_info("  Tune agent settings: nimro setup agent")
        print()
        _print_setup_summary(config, nimro_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, nimro_home)


def _blank_slate_walkthrough(config: dict, nimro_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`nimro update` runs from re-injecting them. Opt back in any")
            print_info("time with `nimro skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `nimro tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `nimro plugins list` / `nimro plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `nimro plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `nimro mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `nimro mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   nimro tools")
    print_info("  Seed skills:         nimro skills opt-in --sync")
    print_info("  Add MCP servers:     nimro mcp add")
    print_info("  Tune agent settings: nimro setup agent")
    print()
...
def _run_quick_setup(config: dict, nimro_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'nimro setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Nimro to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'nimro setup gateway'.")
