"""
Top-level argparse construction for the nimro CLI.
...
# argparse runs (it sets ``NIMRO_HOME`` and strips itself from ``sys.argv``),
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``nimro_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    nimro                        Start interactive chat
    nimro chat -q "Hello"        Single query mode
    nimro --tui                  Launch the modern TUI (or set display.interface: tui)
    nimro --cli                  Force the classic REPL (overrides display.interface: tui)
    nimro -c                     Resume the most recent session
    nimro -c "my project"        Resume a session by name (latest in lineage)
    nimro --resume <session_id>  Resume a specific session by ID
    nimro setup                  Run setup wizard
    nimro logout                 Clear stored authentication
    nimro auth add <provider>    Add a pooled credential
    nimro auth list              List pooled credentials
    nimro auth remove <p> <t>    Remove pooled credential by index, id, or label
    nimro auth reset <provider>  Clear exhaustion status for a provider
    nimro model                  Select default model
    nimro fallback [list]        Show fallback provider chain
    nimro fallback add           Add a fallback provider (same picker as `nimro model`)
    nimro fallback remove        Remove a fallback provider from the chain
    nimro config                 View configuration
    nimro config edit            Edit config in $EDITOR
    nimro config set model gpt-4 Set a config value
    nimro gateway                Run messaging gateway
    nimro -s nimro-agent-dev,github-auth
    nimro -w                     Start in isolated git worktree
    nimro gateway install        Install gateway background service
    nimro sessions list          List past sessions
    nimro sessions browse        Interactive session picker
    nimro sessions rename ID T   Rename/title a session
    nimro logs                   View agent.log (last 50 lines)
    nimro logs -f                Follow agent.log in real time
    nimro logs errors            View errors.log
    nimro logs --since 1h        Lines from the last hour
    nimro debug share             Upload debug report for support
    nimro console                Open the safe Nimro command console
    nimro update                 Update to latest version
    nimro dashboard              Start web UI dashboard (port 9119)
    nimro dashboard --stop       Stop running dashboard processes
    nimro dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="nimro",
        description="Nimro Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `nimro chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via NIMRO_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `nimro setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to NIMRO_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.nimro/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Nimro Agent",
    )
    ...
    # top-level parser: when the user writes `nimro -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--yolo`, etc. (see tests/nimro_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also NIMRO_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.nimro/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Nimro itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
