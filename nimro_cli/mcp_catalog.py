...
entries via ``nimro mcp catalog`` or the interactive ``nimro mcp picker``,
and install them with ``nimro mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into nimro-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``nimro mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.nimro/.env`` (the
...
from nimro_constants import get_nimro_home, get_optional_mcps_dir
...
from nimro_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.nimro/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Nimro install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Nimro understands version {_MANIFEST_VERSION})"
        )
    ...
    if t_type == "http" and a_type == "api_key":
        ...
        # at connect time.
        from nimro_cli.mcp_config import _env_key_for_server
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Nimro is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Nimro
        understands. Update Nimro to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_nimro_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.nimro/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.nimro/.env via save_env_value()."""
...
def _build_server_config(
    ...
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from nimro_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from nimro_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``nimro mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `nimro mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `nimro mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `nimro mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `nimro mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `nimro auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`nimro auth {entry.auth.provider}` if you have not "
    ...
    from nimro_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new Nimro session to load its tools.",
        Colors.GREEN,
    ))
