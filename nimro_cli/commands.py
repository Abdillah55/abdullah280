"""Slash command definitions and autocomplete for the Nimro CLI.
...
from nimro_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``nimro_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``nimro_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Nimro config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Nimro works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Nimro' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Nimro is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.nimro/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Nimro Agent to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Nimro Agent version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`nimro_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/nimro`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Nimro ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Nimro
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``nimro skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``nimro skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``nimro skills list`` and
...
# unrelated command landed. These claim their slots right after /nimro,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /nimro <command>).
...
# native slot, the alias spelling stays reachable via /nimro reset).
...
# ``/nimro <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/nimro" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /nimro topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /nimro moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /nimro debug on Slack.
#   - egress: Docker-only proxy status; reachable as /nimro egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /nimro init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /nimro version on
...
#   - diff: git working-tree diff; reached via /nimro diff on Slack so it
...
#     /nimro update on Slack. Demoted to free the native slot /approvals now
...
_SLACK_VIA_NIMRO_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/nimro <verb>`` subcommand.
    ...
    can still reach them via ``/nimro <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/nimro`` is always reserved as the first entry so the
    legacy ``/nimro <subcommand>`` form keeps working for anything that
    ...
    # Reserve /nimro as the catch-all top-level command.
    entries.append(("nimro", "Talk to Nimro or run a subcommand", "[subcommand] [args]"))
    seen.add("nimro")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_NIMRO_ONLY:
            # Intentionally Slack-via-/nimro only (see _SLACK_VIA_NIMRO_ONLY).
            return
    ...
    # everything except /nimro, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://nimro-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /nimro handler.

    Maps both canonical names and aliases so /nimro bg do stuff works
    the same as /nimro background do stuff.

    Plugin-registered slash commands are included so ``/nimro <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from nimro_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
