"""Cross-process mutual exclusion for in-flight Nimro updates.

Three different surfaces can start an update of the same install tree:

* ``nimro update`` from a terminal,
* the dashboard's Update button (``POST /api/nimro/update`` →
  ``_spawn_nimro_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``nimro-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``nimro update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <NIMRO_HOME>/.nimro-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``nimro update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Nimro is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``nimro-setup`` binary under ``~/.nimro`` is only refreshed by a full
  installer run (``copy_self_to_nimro_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Nimro is still running") on every
  GUI update forever, with no Nimro process actually running.
"""
...
MARKER_NAME = ".nimro-update-in-progress"

# Set by an orchestrating updater (the Tauri `nimro-setup --update` flow) to
# its own pid before spawning `nimro update` as a child stage. The parent
...
HANDOFF_PID_ENV = "NIMRO_UPDATE_HANDOFF_PID"
...
# to show "Nimro is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Nimro home (never the context-local profile override):
    the Rust updater resolves ``$NIMRO_HOME`` or the platform default, and the
    ...
    """
    from nimro_constants import get_process_nimro_home

    return get_process_nimro_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``nimro update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``nimro-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Nimro update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `nimro update` as a stage): we run under ITS claim
