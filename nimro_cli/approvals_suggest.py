"""``nimro approvals suggest`` — mine approval history into allowlist proposals.

Nimro has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.nimro/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, nimro self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/nimro home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_nimro_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_nimro_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  nimro approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.nimro/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``nimro approvals suggest``."""
