"""Dependency-light venv recovery that runs BEFORE nimro_cli.main's imports.

The ``nimro`` console entry point is ``nimro_cli.main:main``.  Importing
``nimro_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``nimro_cli.env_loader``, ``yaml`` via ``nimro_cli.config``, ...).  In
...
a corrupted venv.  ``nimro_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``nimro_cli.main`` to become
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``nimro acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``nimro_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``nimro_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``nimro update`` exists AND an import probe
