...
Pure display functions with no NimroCLI state dependency.
"""
import json
import logging
import os
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `nimro ... > file`) raises
...
from nimro_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built nimro — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/nimro-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/nimro-agent"
...
def check_for_updates() -> Optional[int]:
    """Check whether a Nimro update is available.

    Two paths: if ``NIMRO_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = nimro_home / ".update_check"
    embedded_rev = os.environ.get("NIMRO_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # NIMRO_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/nimro/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from nimro_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $NIMRO_HOME/nimro-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = nimro_home / "nimro-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Nimro git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$NIMRO_HOME/nimro-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``nimro_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from nimro_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/nimro-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Nimro checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/nimro-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Nimro Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else NIMRO_CADUCEUS
    except Exception:
        _bskin = None
        _hero = NIMRO_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or nimro setup[/]"
            )
        else:
    ...
    try:
        from nimro_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from nimro_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built nimro; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else NIMRO_AGENT_LOGO
        console.print(_logo)
        console.print()
