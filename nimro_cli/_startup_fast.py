...
This module is imported by ``nimro_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no nimro_cli.config,
...
``nimro_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(nimro_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect NIMRO_HOME?"""
    active_profile = os.path.join(nimro_root, "active_profile")
    try:
...
def _resolved_home() -> str:
    nimro_home = os.environ.get("NIMRO_HOME", "").strip()
    if nimro_home:
        return nimro_home
    return os.path.join(os.path.expanduser("~"), ".nimro")
...
def container_mode_may_be_active() -> bool:
    ...
    nimro_home = os.environ.get("NIMRO_HOME", "").strip()
    if nimro_home:
        if os.path.exists(os.path.join(nimro_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(nimro_home)))
        return (
            parent_name != "profiles"
            and active_profile_may_override_home(nimro_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".nimro")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect NIMRO_HOME.
    """
...
def print_fast_version_info() -> None:
    from nimro_cli import __release_date__, __version__

    print(f"Nimro Agent v{__version__} ({__release_date__})")
    ...
    print("Run 'nimro version' for update status.")


def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``nimro --version`` before the heavy import wall.

    Termux keeps its historical contract (also accepts the ``version``
    subcommand + the NIMRO_TERMUX_DISABLE_FAST_CLI escape hatch). Everywhere
    ...
    if is_termux and os.environ.get("NIMRO_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
