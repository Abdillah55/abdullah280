...
def active_session_limit_message(
    ...
    return (
        f"Nimro is at the active session limit ({active_count}/{max_sessions})."
        f"{detail} Try again when another session finishes."
    )


def _state_dir() -> Path:
    return Path(get_nimro_home()) / "runtime"
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``nimro dashboard`` / ``serve``) never trips that
