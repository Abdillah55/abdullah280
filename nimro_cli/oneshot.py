...
configured for "cli" in `nimro tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (NIMRO_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `nimro chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"nimro -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "nimro -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"nimro -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "nimro -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "nimro -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to NIMRO_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("NIMRO_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "nimro -z: --provider requires --model (or NIMRO_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"nimro -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("nimro -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``nimro -z`` / oneshot mode.

    Oneshot bypasses ``NimroCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when nimro is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from nimro_cli.config import load_config
    from nimro_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("NIMRO_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from nimro_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("NIMRO_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    # NimroCLI._init_agent's wait — it builds AIAgent directly here, so the
    ...
    from nimro_cli.mcp_startup import ensure_mcp_discovery_before_agent_build
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                NIMRO_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via NIMRO_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via NIMRO_YOLO_MODE=1
