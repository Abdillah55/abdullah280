"""Profile distributions — shareable, packaged Nimro profiles via git.

A distribution is a Nimro profile published as a git repository (or
...
* ``nimro profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``nimro skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``nimro profile``, not a parallel tree):

    nimro profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "nimro-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            nimro_requires=str(data.get("nimro_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.nimro_requires:
            out["nimro_requires"] = self.nimro_requires
        if self.author:
...
def check_nimro_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Nimro {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``nimro profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Nimro profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Nimro distribution."
        )

    # Version check up-front so we fail fast
    check_nimro_requires(manifest.nimro_requires, nimro_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.nimro).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="nimro_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `nimro profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`nimro profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="nimro_dist_update_") as tmp:
        plan = plan_install(
