...
Pure logic: walks a Nimro config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `nimro doctor` and a future `nimro migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Nimro config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Nimro config and return retirement issues.
...
def apply_migration(
    ...
    from nimro_cli.config import require_readable_config_before_write
