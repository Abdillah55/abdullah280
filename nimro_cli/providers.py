"""
Single source of truth for provider identity in Nimro Agent.
...
2. **Nimro overlays** — transport type, auth patterns, aggregator flags,
...
# -- Nimro overlay ----------------------------------------------------------
# Nimro-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class NimroOverlay:
    """Nimro-specific provider metadata layered on top of models.dev."""
...
NIMRO_OVERLAYS: Dict[str, NimroOverlay] = {
    "moa": NimroOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": NimroOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": NimroOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": NimroOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": NimroOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": NimroOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="NIMRO_QWEN_BASE_URL",
    ),
    "lmstudio": NimroOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": NimroOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": NimroOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": NimroOverlay(
    ...
    "kimi-for-coding": NimroOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": NimroOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": NimroOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": NimroOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": NimroOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": NimroOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": NimroOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": NimroOverlay(
        transport="openai_chat",
    ...
    "opencode": NimroOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": NimroOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": NimroOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": NimroOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "nimro", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Nimro overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Nimro overlay
    ...
    overlay = NIMRO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + nimro extra
    ...
    if overlay is not None:
        # Nimro-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Nimro overlay still advertises openai_chat
    ...
    # Direct provider checks for providers not in NIMRO_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Nimro provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from nimro_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
