...
- ``nimro_cli.providers``        -- canonical provider identity + overlays
- ``nimro_cli.model_normalize``  -- per-provider name formatting
...
_NIMRO_MODEL_WARNING = (
    "Nous Research Nimro 3 & 4 models are NOT agentic and are not designed "
    "for use with Nimro Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Nimro 3 / Nimro 4 chat families.
# The previous substring check (`"nimro" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``nimro-brain:qwen3-14b-ctx16k`` that just
# happen to carry "nimro" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Nimro-3-Llama-3.1-70B, nimro-4-405b, openrouter/nimro3:70b
# Negative examples it must NOT match:
#   nimro-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_NIMRO_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])nimro[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_nimro_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Nimro 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_NIMRO_NON_AGENTIC_RE.search(model_name))


def _check_nimro_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Nimro 3/4 chat model."""
    if is_nous_nimro_non_agentic(model_name):
        return _NIMRO_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``nimro config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from nimro_cli.model_switch import
...
async def resolve_display_context_length_async(
    ...
    ``nimro_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'nimro model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'nimro doctor' — config issues detected:"
        ...
        from nimro_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from nimro_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from nimro_cli.config import load_config
            from nimro_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from nimro_cli.providers import resolve_user_provider as _ruser
    ...
    if target_provider in {"nous", "nous-portal", "nousresearch"}:
        from nimro_cli.providers import nous_api_mode
    ...
    # nimro_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from nimro_cli.models import normalize_opencode_base_url
    ...
    nimro_warn = _check_nimro_model_warning(new_model)
    if nimro_warn:
        warnings.append(nimro_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from nimro_cli.config import normalize_extra_headers
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from nimro_cli/models.py (OPENROUTER_MODELS,
    ...
    # Compared against nimro_id / mdev_id (section 1), pid / nimro_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from nimro_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by nimro provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://nimro-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Nimro release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from nimro_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from nimro_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Nimro-mapped providers ---
    from nimro_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from nimro_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for nimro_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(nimro_id)
        if (
            _alias_target
            and _alias_target != nimro_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        # Resolve the canonical provider profile name.  Skip nimro_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose nimro_id matches the canonical profile name so
        ...
        # same nimro_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if nimro_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        pconfig = PROVIDER_REGISTRY.get(nimro_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (NIMRO_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Nimro may not route yet.
        ...
        from nimro_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(nimro_id):
            continue
        ...
        if not has_creds:
            try:
                from nimro_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(nimro_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        nimro_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `nimro model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(nimro_id)
        if not model_ids:
            model_ids = curated.get(nimro_id, [])
            if nimro_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(nimro_id, model_ids)
    ...
    # --- 2. Check Nimro-only providers (nous, openai-codex, copilot, opencode-go) ---
    from nimro_cli.providers import NIMRO_OVERLAYS
    from nimro_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Nimro provider ID.
    # NIMRO_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Nimro IDs ("copilot").
    _mdev_to_nimro = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in NIMRO_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Nimro slug — e.g. "github-copilot" → "copilot"
        nimro_slug = _mdev_to_nimro.get(pid, pid)
        if nimro_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or nimro_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(nimro_slug)
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or nimro_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(nimro_slug):
                    has_creds = True
                elif for_picker:
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(nimro_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", nimro_slug, exc)
        ...
        # configured.
        if not has_creds and nimro_slug == "anthropic":
            try:
                ...
                if (nimro_creds and nimro_creds.get("accessToken")) or \
        ...
        if nimro_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(nimro_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(nimro_slug)
                model_ids = _ids if _ids else (curated.get(nimro_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(nimro_slug, []) or curated.get(pid, [])
        elif nimro_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `nimro model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from nimro_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(nimro_slug, []) or curated.get(pid, [])
                if nimro_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(nimro_slug, model_ids)
        total = len(model_ids)
        if nimro_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": nimro_slug,
            "name": get_label(nimro_slug),
            "is_current": nimro_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "nimro",
        ...
        seen_slugs.add(nimro_slug.lower())
        _record_builtin_endpoint(nimro_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or NIMRO_OVERLAYS (keeps /model in sync
    # with `nimro model`).
    try:
        from nimro_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Nimro writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Nimro's own writer uses (mirrors section-4 grouping).
        ...
        for grp in ep_groups.values():
            ...
            if should_probe:
                try:
                    from nimro_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Nimro's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, nimro_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from nimro_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`nimro_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from nimro_cli.models import fetch_openrouter_models
