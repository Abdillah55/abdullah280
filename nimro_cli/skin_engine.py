"""Nimro skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and Nimro itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.nimro/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.nimro/skins/ therefore
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Nimro Agent"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye! ⚕"              # Shown on exit
      response_label: " ⚕ Nimro "       # Response box header label
...
.. code-block:: python
    ...
    from nimro_cli.skin_engine import get_active_skin, list_skins, set_active_skin

    skin = get_active_skin()
    print(skin.colors["banner_title"])    # "#FFD700"
    print(skin.get_branding("agent_name"))  # "Nimro Agent"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.nimro/skins/
...
Drop a YAML file in ``~/.nimro/skins/<name>.yaml`` following the schema above.
...
class SkinConfig:
    ...
    banner_logo: str = ""    # Rich-markup ASCII art logo (replaces NIMRO_AGENT_LOGO)
    banner_hero: str = ""    # Rich-markup hero art (replaces NIMRO_CADUCEUS)
...
_BUILTIN_SKINS: Dict[str, Dict[str, Any]] = {
    "default": {
        "name": "default",
        "description": "Classic Nimro — gold and kawaii",
        # Dark-authored. Values match the TUI's DARK_THEME so the classic CLI
        # and the TUI render the same Nimro gold.
        "colors": {
        ...
        "branding": {
            ...
            "welcome": "Welcome to Nimro Agent! Type your message or /help for commands.",
            "goodbye": "Goodbye! ⚕",
            "response_label": " ⚕ Nimro ",
            "prompt_symbol": "❯",
