"""
Doctor command for nimro CLI.

Diagnoses issues with Nimro Agent setup.
"""
...
from nimro_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
NIMRO_HOME = get_nimro_home()
_DHH = display_nimro_home()  # user-facing display path (e.g. ~/.nimro or ~/.nimro/profiles/coder)

# Load environment variables from ~/.nimro/.env so API key checks work
_env_path = get_env_path()
load_nimro_dotenv(nimro_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from nimro_cli.colors import Colors, color
from nimro_cli.models import _NIMRO_USER_AGENT
from nimro_cli.vercel_auth import describe_vercel_auth
from nimro_constants import OPENROUTER_MODELS_URL
...
from nimro_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Nimro containers"
    elif method in {"nix", "nixos"}:
        action = recommended_update_command_for_method(method)
    else:
        action = "run `nimro update`"
    return (
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.nimro/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("NIMRO_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from nimro_cli.auth import get_minimax_oauth_auth_status
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. NIMRO_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("NIMRO_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("NIMRO_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches nimro_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``nimro --version`` reporting a stale version while
    ...
    """
    try:
        from nimro_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != nimro_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'nimro update', or set "
            "nimro_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-nimro and dashboard static services are up
    ...
    for static in ("main-nimro", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `nimro profile create <name>`")
        return
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `nimro doctor --fix`, or "
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from nimro_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the NIMRO_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("NIMRO_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via NIMRO_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `nimro`.
    os.environ.setdefault("NIMRO_INTERACTIVE", "1")

    # Handle `nimro doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.nimro/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Nimro Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`nimro doctor --ack {hit.advisory.id}`."
                )
    ...
    try:
        import sqlite3
        from nimro_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Nimro already refuses to enable WAL on fresh DBs.
    ...
    # Detect drift between pyproject.toml and nimro_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.nimro/.env (primary location for user config)
    env_path = NIMRO_HOME / '.env'
    if env_path.exists():
        ...
        # matches nimro_cli.env_loader._load_dotenv_with_fallback.
        try:
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'nimro setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'nimro setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'nimro setup' to create one")
                issues.append("Run 'nimro setup' to create .env")
    
    # Check ~/.nimro/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = NIMRO_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from nimro_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if isinstance(user_providers, dict):
                from nimro_cli.config import is_provider_enabled
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'nimro config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from nimro_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.nimro/.env or run 'nimro setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'nimro setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'nimro config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from nimro_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from nimro_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'nimro setup' to migrate config")
                else:
                    issues.append("Run 'nimro doctor --fix' or 'nimro setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'nimro doctor --fix'")
        except Exception:
            pass

        # Detect stale NIMRO_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives NIMRO_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            from nimro_cli.config import load_env, read_user_config_raw, remove_env_value
            ...
            env_ghost = load_env().get("NIMRO_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"NIMRO_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `nimro setup` run)",
                )
                if should_fix:
                    if remove_env_value("NIMRO_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale NIMRO_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove NIMRO_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the NIMRO_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale NIMRO_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'nimro doctor --fix'"
                    )
        except Exception:
            pass
        ...
        try:
            from nimro_cli.config import load_env as _load_env_depr
            from nimro_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Nimro' own device-code flow — the Codex CLI is
    ...
    if nimro_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        nimro_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = nimro_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Nimro a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Nimro Agent Persona\n\n"
                "<!-- Edit this file to customize how Nimro communicates. -->\n\n"
                "You are Nimro, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = nimro_home / "memories"
    ...
    state_db_path = nimro_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from nimro_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'nimro doctor --fix' "
                        "(or 'nimro sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from nimro_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'nimro doctor --fix' "
                        "(or 'nimro sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = nimro_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'nimro doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "nimro"
        ...
        _cmd_link = _cmd_link_dir / "nimro"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(nimro not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/nimro → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/nimro points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/nimro → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/nimro — run 'nimro doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/nimro exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/nimro not found",
                    "(nimro command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/nimro → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/nimro symlink — run 'nimro doctor --fix'")
    ...
    try:
        from nimro_constants import is_container as _is_container
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'nimro-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # `nimro acp --setup-browser` installs agent-browser into the
        # Nimro-managed node prefix, which isn't necessarily on PATH. Mirror
        # dep_ensure._has_nimro_agent_browser() so doctor and dep_ensure agree
        ...
        _managed_ab = (
            _which_in(NIMRO_HOME / "node" / "bin")
            or _which_in(NIMRO_HOME / "node")
        )
        _legacy_ab = _which_in(NIMRO_HOME / "node_modules" / ".bin")
        if agent_browser_path.exists():
            ...
            # `nimro update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `nimro doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable NIMRO_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Nimro one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'nimro config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``nimro doctor``.
    ...
    # never the legitimate source for `nimro doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'nimro setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = NIMRO_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: nimro skills list)")
    ...
    try:
        from nimro_cli.config import read_user_config_raw as _read_raw_mem
        _mem_cfg_path = NIMRO_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: nimro memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: nimro memory setup",
                    "No Honcho API key — run 'nimro memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run nimro memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: nimro memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: nimro memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from nimro_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "nimro -p" in content:
                            _m = _re.search(r"nimro -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'nimro doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
