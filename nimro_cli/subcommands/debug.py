...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Nimro Agent. Use 'nimro debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    nimro debug share              Upload debug report (asks for confirmation)
    nimro debug share --yes        Skip confirmation (for scripts/CI)
    nimro debug share --lines 500  Include more log lines
    nimro debug share --expire 30  Keep paste for 30 days
    nimro debug share --local      Print report locally (no upload)
    nimro debug share --no-redact  Disable upload-time secret redaction
    nimro debug share --nous       Upload to Nous-internal storage (private)
    nimro debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'nimro debug share'",
    )
