...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``nimro login`` was removed in favor of ``nimro auth`` / ``nimro model``
    (the runtime handler in ``nimro_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``nimro login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``nimro --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``nimro login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``nimro model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `nimro auth` to manage credentials, "
            "`nimro model` to select a provider, or `nimro setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `nimro model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: nimro-cli)"
    )
