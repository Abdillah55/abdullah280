"""``nimro sync`` subcommand parser — Skill Sync.

Cloned from ``nimro_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    nimro sync status                 show gate/opt-in/head state
    nimro sync pull                   pull and materialize opted-in skills
    nimro sync push                   push opted-in skills
    nimro sync now                    reconcile: pull then push
    nimro sync enable <skill>         opt a skill into sync
    nimro sync disable <skill>        opt a skill out of sync
    nimro sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    nimro sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  nimro sync status            what is synced, and from where\n"
            "  nimro sync enable my-skill   include a skill in your sync\n"
            "  nimro sync now               pull, then push\n"
            "  nimro sync propose my-skill  share a skill with your team\n"
        ),
