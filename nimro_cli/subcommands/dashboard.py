"""``nimro dashboard`` / ``nimro serve`` subcommand parsers.
...
``nimro_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `nimro dashboard` / `nimro serve` cmdlines and SIGTERM them directly —
    # the same path `nimro update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Nimro web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Nimro web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Nimro Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Nimro desktop app shells (<= 0.15.x) spawn the
    # backend as `nimro dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Nimro couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Nimro Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Nimro backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Nimro backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports NIMRO_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `nimro dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.nimro/.env.
    # Nested subparser so bare `nimro dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "NIMRO_DASHBOARD_OAUTH_CLIENT_ID into ~/.nimro/.env, and prints "
            "how to engage the login gate. Requires being logged in (nimro setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://nimro.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via NIMRO_DASHBOARD_PORTAL_URL. Mainly for "
