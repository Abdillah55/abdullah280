"""``nimro backup`` subcommand parser.
...
def build_backup_parser(subparsers, *, cmd_backup: Callable) -> None:
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Nimro home directory to a zip file",
        description="Create a zip archive of your entire Nimro configuration, "
        "skills, sessions, and data (excludes the nimro-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/nimro-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
