"""On-demand supply-chain audit for Nimro Agent installs.

Scans three surfaces a Nimro user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Nimro venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.nimro/plugins``
...
def _discover_plugins(nimro_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.nimro/plugins``.
...
def _discover_components(
    *,
    ...
    home = nimro_home or Path(get_nimro_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `nimro security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, nimro_home=home
    )
