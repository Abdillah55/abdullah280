...
late-binding seam in :mod:`nimro_cli.web_deps` (``late`` for callables,
...
from nimro_cli.web_deps import late, LateState
from nimro_cli.web_models import (
...
_log = logging.getLogger("nimro_cli.web_server")
...
_spawn_nimro_action = late("_spawn_nimro_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from nimro_cli.platforms import platform_label
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``nimro tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `nimro tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``nimro tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.nimro/.env`` via ``save_env_value`` —
    the same store ``nimro tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``nimro tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``nimro -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same NIMRO_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``nimro computer-use permissions grant`` as a background action.
