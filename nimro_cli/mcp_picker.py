"""MCP picker — interactive `nimro mcp picker` (also the default `nimro mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``nimro mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `nimro plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from nimro_cli.cli_output import prompt_yes_no
from nimro_cli.curses_ui import curses_single_select
from nimro_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Nimro session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from nimro_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `nimro mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: nimro mcp install <name>    Picker: nimro mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Nimro is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Nimro — run `nimro update` "
...
def show_catalog() -> None:
    """`nimro mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`nimro mcp picker` (and default `nimro mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`nimro mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `nimro mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
