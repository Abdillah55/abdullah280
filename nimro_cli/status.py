"""
Status command for nimro CLI.

Shows the status of all Nimro Agent components.
"""
...
from nimro_cli.auth import AuthError, resolve_provider
from nimro_cli.colors import Colors, color
from nimro_cli.config import get_env_path, get_env_value, get_nimro_home, load_config
from nimro_cli.models import provider_label
from nimro_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from nimro_cli.nous_subscription import get_nous_subscription_features
from nimro_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``nimro config``'s
...
def show_status(args):
    """Show status of all Nimro Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Nimro Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `nimro status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: nimro portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: nimro model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: nimro auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: nimro auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: nimro model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from nimro_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'nimro-agent[vercel]')"
    ...
    try:
        from nimro_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   nimro gateway")
    ...
    jobs_file = get_nimro_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_nimro_home() / "sessions" / "sessions.json"
    ...
    try:
        from nimro_cli.active_sessions import (
    ...
    print(color("  Run 'nimro doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'nimro setup' to configure", Colors.DIM))
    print()
