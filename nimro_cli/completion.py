"""Shell completion script generation for nimro CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_nimro_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Nimro Agent bash completion
# Add to ~/.bashrc:
#   eval "$(nimro completion bash)"

_nimro_profiles() {{
    local profiles_dir="$HOME/.nimro/profiles"
...
_nimro_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_nimro_profiles)" -- "$cur"))
        return
    fi
...
complete -F _nimro_completion nimro
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef nimro
# Nimro Agent zsh completion
# Add to ~/.zshrc:
...
_nimro_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.nimro/profiles" ]]; then
        profiles+=($HOME/.nimro/profiles/*(N/:t))
    fi
...
_nimro() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_nimro_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'nimro command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Nimro Agent fish completion",
        "# Add to your config:",
        "#   nimro completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __nimro_profiles",
        "    echo default",
        "    if test -d $HOME/.nimro/profiles",
        "        for d in $HOME/.nimro/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c nimro -f -s p -l profile"
        " -d 'Profile name' -xa '(__nimro_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__nimro_profiles)' -d 'Profile name'"
                )
