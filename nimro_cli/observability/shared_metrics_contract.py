"""Bounded product contract for the first Nimro shared-metrics slice."""
...
SCHEMA_KEY = "nimro.metrics.schema_version"
SCHEMA_VERSION = "nimro.metrics.event.v1"
MODEL_CALL_SCOPE = "nimro.model_call"
TASK_SCOPE = "nimro.task_run"
SUBSCRIBER_NAME = "nimro.nemo_relay.shared_metrics"
PRIMARY_MODEL_CALL_ROLE = "primary"
MODEL_CALL_METRIC = "nimro.model_call.count"
TASK_STARTED_METRIC = "nimro.task_run.started"
TASK_FINISHED_METRIC = "nimro.task_run.finished"
...
# in Nimro's execution metadata because that flag has narrower routing/catalog
# semantics there.
...
# Nimro intentionally resolves these local runtimes through the generic custom
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from nimro_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Nimro terminal state to bounded task outcome dimensions."""
...
def provider_family(kwargs: dict[str, Any]) -> str:
    """Map a Nimro provider to a bounded product category."""
...
def _provider_metadata(provider: str) -> tuple[str, bool, bool]:
    ...
    try:
        ...
        from nimro_cli.providers import NIMRO_OVERLAYS, normalize_provider
...
def _known_provider_ids() -> frozenset[str]:
    """Cache Nimro's static provider catalog for the process lifetime."""
    try:
        from nimro_cli.provider_catalog import provider_catalog_by_slug
