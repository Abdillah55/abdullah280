"""Durable aggregation and local export for Nimro shared metrics."""
...
from nimro_cli.sqlite_util import write_txn
...
_PACKAGE_SCHEMA_VERSION = "nimro.shared_metrics.v1"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_nimro_home() / "telemetry" / "shared_metrics"
    ...
    def record_model_call(
        ...
        self.record_counter(MODEL_CALL_METRIC, dimensions, nimro_version)
    ...
    def counter_snapshot(self) -> list[dict[str, Any]]:
        ...
        with self._connection() as connection:
            rows = connection.execute(
                ...
                ORDER BY period_start, nimro_version, metric_name, dimensions_json
                """
            ).fetchall()
        return [
            {
                ...
                "nimro_version": row["nimro_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across Nimro processes.
    ...
    def _pending_period_count(self) -> int:
        with self._connection() as connection:
            row = connection.execute(
                ...
                FROM (
                    SELECT period_start, nimro_version
                    FROM counter_aggregates
                    WHERE value > packaged_value
                    GROUP BY period_start, nimro_version
                )
                """
            ).fetchone()
    ...
    def _create_package_in_transaction(
        ...
        period_row = connection.execute(
            """
                ...
                FROM counter_aggregates
                WHERE value > packaged_value
                ORDER BY period_start, nimro_version
                LIMIT 1
                """
        ).fetchone()
        ...
        rows = connection.execute(
            """
                ...
                """,
            (period_value, period_row["nimro_version"]),
        ).fetchall()
        ...
        payload = {
            ...
            "resource": {"nimro_version": period_row["nimro_version"]},
