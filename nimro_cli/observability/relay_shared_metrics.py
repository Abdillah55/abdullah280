"""Direct NeMo Relay integration for Nimro shared client metrics."""
...
class _Runtime:
    """Own shared-metrics state layered on the Nimro core Relay host."""
    ...
    def start_task(self, event: dict[str, Any]) -> _TaskRun | None:
        """Open one Relay function scope for a Nimro task run."""
        ...
        with session.lock:
            ...
            if task is not None:
                ...
                if retry_ordinal is not None and retry_ordinal > 0:
                    # A real Nimro retry can advance api_request_id while
    ...
    def finish_task(self, event: dict[str, Any]) -> None:
        ...
        if finished:
            try:
                ...
                logger.warning(
                    "Nimro shared-metrics task flush failed",
                    exc_info=True,
                )
            else:
                self._export()
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Nimro shared-metrics session %s closed with errors: %s",
                session.session_id,
                "; ".join(failures),
            )
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Nimro shared-metrics shutdown flush failed",
                exc_info=True,
            )
        else:
            self._export()
    ...
    def _finish_model_call(
        ...
        try:
            ...
            logger.warning(
                "Nimro shared-metrics model call close failed", exc_info=True
            )
    ...
    def _finish_task(
        ...
        try:
            ...
            logger.warning("Nimro shared-metrics task close failed", exc_info=True)
        finally:
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Nimro shared metrics operation failed", exc_info=True)
            return None


def enabled() -> bool:
    """Return the shared-metrics policy for the active Nimro profile."""
    profile_key = relay_runtime.current_profile_key()
    try:
        from nimro_cli.config import read_raw_config_readonly
        ...
        logger.debug("Unable to read Nimro shared-metrics policy", exc_info=True)
        value = False
    else:
...
def observe_lifecycle(hook_name: str, **kwargs: Any) -> None:
    """Project one Nimro lifecycle event into the core Relay integration."""
    ...
    try:
        ...
        logger.warning(
            "Nimro shared metrics hook failed: %s", hook_name, exc_info=True
        )
...
def start_task_run(
    *,
    ...
    """Start task metrics at the outer Nimro execution boundary."""
    if not enabled():
        return
    ...
    *,
    ...
    with _RUNTIME_LOCK:
        ...
        try:
            ...
            logger.warning("Nimro shared metrics initialization failed", exc_info=True)
