...
from nimro_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import nimro_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $NIMRO_HOME/node is
...
def _has_nimro_agent_browser() -> bool:
    ...
    # install.sh installs globally into $NIMRO_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-NimroHome", str(get_nimro_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = nimro_subprocess_env(inherit_credentials=False)
