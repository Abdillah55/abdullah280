"""``nimro sessions`` command — extracted from ``nimro_cli/main.py``.
...
Helpers that stay in ``nimro_cli.main`` (``get_nimro_home``,
...
``nimro_cli.main.<name>`` keep reaching this code path, and so imports stay
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    nimro sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "recover":
        ...
        from nimro_cli.session_recovery import (
    ...
    if action == "list":
        from nimro_state import workspace_key as _ws_key
        ...
        from nimro_cli.session_filters import (
            build_prune_filters,
            describe_filters,
        )
        ...
        def _redact(data):
            if not args.redact or data is None:
                return data
            from nimro_cli.session_export_md import redact_session_data
        ...
        if args.format == "html":
            if not args.output or args.output == "-":
                ...
                return
            from nimro_cli.session_export_html import (
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_nimro_home() / "session-exports"
                    )
        ...
        # Markdown / QMD export
        from nimro_cli.session_export_md import (
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_nimro_home() / "session-exports"
        ...
        # `nimro sessions prune`: no time window and no filters at all
        ...
        # Launch nimro --resume <id> by replacing the current process
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`nimro sessions optimize` later to reclaim freed space.)")
