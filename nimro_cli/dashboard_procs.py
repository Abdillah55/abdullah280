"""Dashboard process-hygiene helpers — extracted from ``nimro_cli/main.py``.
...
``_detect_concurrent_nimro_instances``) are lifted verbatim. References to
helpers that STAY in ``nimro_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``nimro_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``nimro_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``nimro_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``nimro dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``nimro update`` replaces files on disk, the running
    ...
    ``nimro-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Nimro Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``nimro serve`` as
    ...
    environment variable ``NIMRO_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`nimro serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "nimro.*dashboard"`) keeps us consistent
            # with `nimro_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``nimro dashboard`` / ``nimro serve`` processes.

    Called at the end of ``nimro update`` (default ``reason``) and also
    from ``nimro dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``nimro update`` path), a detected
    ``nimro-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``nimro-serve.service``) has its owning unit
    ...
    # When the Nimro Desktop Electron app spawns this dashboard as a
    # backend child, it sets NIMRO_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("NIMRO_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `nimro update` path) — `--stop` must
    ...
    #    remote backend (nimro serve) under Restart=on-failure never comes
...
def _detect_concurrent_nimro_instances(
    ...
    The Nimro Desktop Electron app spawns ``nimro.EXE`` as a backend child,
    so during ``nimro update`` the user-invoked process and the desktop's
    ...
    shims (``nimro.exe`` / ``nimro-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``nimro update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``nimro.exe``) is a separate process from the Python
    ...
    for shim in _m()._nimro_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated nimro.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``nimro update``
    ...
    #      nimro.exe sitting *under* a non-Nimro parent (e.g. a Nimro
