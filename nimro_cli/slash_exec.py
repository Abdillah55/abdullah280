...
``CommandDef.execute`` (nimro_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/nimro_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``nimro_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_version(ctx: CommandContext) -> CommandReply:
    ...
    from nimro_cli.banner import format_banner_version_label
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_nimro_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: nimro bundles create <name> --skill <s1> --skill <s2>\n"
...
def _exec_help(ctx: CommandContext) -> CommandReply:
    ...
    from nimro_cli.commands import gateway_help_lines
