"""
Dump command for nimro CLI.

Outputs a compact, plain-text summary of the user's Nimro setup
...
from nimro_cli.config import get_nimro_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.nimro/.env.
    ...
    exports. ``nimro debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.nimro_build_sha`` by the Dockerfile's
    ``NIMRO_GIT_SHA`` build-arg (see ``nimro_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``nimro dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from nimro_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(nimro_home: Path) -> int:
...
def _cron_summary(nimro_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = nimro_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"nimro_home:      {display_nimro_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.nimro/.env: a managed
        ...
        # A credential added via `nimro auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `nimro auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["nimro-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(nimro_home)}")
    lines.append(f"  skills:             {_count_skills(nimro_home)}")
