"""
Configuration management for Nimro Agent.

Config files are stored in ~/.nimro/ for easy access:
- ~/.nimro/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.nimro/.env         - API keys and secrets

This module provides:
- nimro config          - Show current configuration
- nimro config edit     - Open config in editor
- nimro config get      - Print a resolved configuration value
- nimro config set      - Set a specific value
- nimro config unset    - Remove a user configuration value
- nimro config wizard   - Re-run setup wizard
"""
...
from nimro_cli.route_identity import normalize_route_base_url
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``nimro config set`` (which rewrites
    ...
    ``config.yaml`` in place: nimro never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.nimro/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``nimro logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  nimro config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Nimro makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Nimro itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``nimro update``, the TUI build.
...
#   on every plugin install / ``nimro update``.
...
# * ``NIMRO_HOME`` / ``NIMRO_PROFILE`` / ``NIMRO_CONFIG`` /
#   ``NIMRO_ENV`` — Nimro runtime location flags. Writing these into
...
#
# IMPORTANT: ``NIMRO_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (NIMRO_LANGFUSE_PUBLIC_KEY,
# NIMRO_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Nimro runtime location — never via dashboard env writer.
    # NOT a NIMRO_* blanket: integration credentials (NIMRO_GEMINI_*,
    # NIMRO_LANGFUSE_*, NIMRO_SPOTIFY_*, ...) ARE allowed.
    "NIMRO_HOME", "NIMRO_PROFILE", "NIMRO_CONFIG", "NIMRO_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Nimro runtime location "
            "(NIMRO_HOME, NIMRO_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # NIMRO_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `nimro plugins enable
    # observability/langfuse` or `nimro tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "NIMRO_LANGFUSE_ENV",
    "NIMRO_LANGFUSE_RELEASE",
    "NIMRO_LANGFUSE_SAMPLE_RATE",
    "NIMRO_LANGFUSE_MAX_CHARS",
    "NIMRO_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so different
    # profiles can use different ACP backends without cross-leak.
    "NIMRO_ACP_AUTH_METHOD",
    "NIMRO_ACP_AUTO_APPROVE",
    "NIMRO_COPILOT_ACP_COMMAND",
    "NIMRO_COPILOT_ACP_ARGS",
    "COPILOT_CLI_PATH",
...
from nimro_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set NIMRO_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("NIMRO_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_nimro_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Nimro is running in package-manager-managed mode.

    Two signals: the NIMRO_MANAGED env var (set by the systemd service),
    or a .managed marker file in NIMRO_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Nimro through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``nimro_cli/`` — i.e. the git checkout for source
    installs, ``/opt/nimro`` inside the published image. It is a property of
    the running interpreter, NOT of ``$NIMRO_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Nimro was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$NIMRO_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. NIMRO_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.nimro``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$NIMRO_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.nimro:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$NIMRO_HOME``, a
    ...
    ``docker`` and ``nimro update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/nimro-agent`` image bakes a ``docker``
        stamp into ``/opt/nimro`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $NIMRO_HOME.
    try:
    ...
    #    host install whose shared $NIMRO_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``nimro update``.
    try:
        method = (
            (get_nimro_home() / ".install_method")
    ...
    # detect Nix installs that don't set NIMRO_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``nimro_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$NIMRO_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/nimro`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull nousresearch/nimro-agent:latest"
    return "nimro update"
...
# Long-form text for ``nimro update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# nimro_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Nimro, it doesn't update
...
✗ ``nimro update`` doesn't apply inside the Docker container.

Nimro Agent runs as a published image (nousresearch/nimro-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/nimro-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate nimro-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/nimro-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/nimro-agent/tags
  • Your config and session history live under ``$NIMRO_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``nimro update`` inside Docker.
...
def format_managed_message(action: str = "modify this Nimro installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("NIMRO_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Nimro installation is managed by NixOS "
            f"(NIMRO_MANAGED={env_hint}).\n"
            "Edit services.nimro-agent.settings in your configuration.nix and run:\n"
            "  sudo nixos-rebuild switch"
        )

    return (
        f"Cannot {action}: this Nimro installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Nimro."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NIMRO_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, nimro_bin
    or None if container mode is not active, we're already inside the
    container, or NIMRO_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("NIMRO_DEV") == "1":
        return None
    ...
    container_mode_file = get_nimro_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "nimro-agent")
    exec_user = info.get("exec_user", "nimro")
    nimro_bin = info.get("nimro_bin", "/data/current-package/bin/nimro")
...
# Re-export from nimro_constants — canonical definition lives there.
from nimro_constants import get_nimro_home, get_process_nimro_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_nimro_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_nimro_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_nimro_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the NIMRO_UID / NIMRO_GID env vars set by Docker deployments.

    Docker containers running Nimro commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level NIMRO_HOME once, but
    subdirectories created at runtime by ``ensure_nimro_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("NIMRO_UID", "").strip()
    gid_str = os.environ.get("NIMRO_GID", "").strip()
    try:
...
def _chown_to_nimro_uid(path) -> None:
    """Chown ``path`` to ``NIMRO_UID:NIMRO_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_nimro_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_nimro_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the nimro group can
    share state with the gateway service.

    The mode can be overridden via the NIMRO_HOME_MODE environment variable
    (e.g. NIMRO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NIMRO_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``NIMRO_UID``/``NIMRO_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NIMRO_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_nimro_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Nimro runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("NIMRO_CONTAINER") or os.environ.get("NIMRO_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set NIMRO_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NIMRO_HOME, upgrading legacy empty templates.
...
# ensure_nimro_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_NIMRO_HOME_ENSURED: set = set()


def ensure_nimro_home():
    """Ensure ~/.nimro directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``NIMRO_HOME`` we only re-run
    ...
    ``get_nimro_home()`` and therefore re-run for the new path.
    """
    home = get_nimro_home()
    key = str(home)

    if key in _NIMRO_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``nimro profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old NIMRO_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_nimro_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _NIMRO_HOME_ENSURED.add(key)


def _ensure_nimro_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"NIMRO_HOME {home} does not exist. "
...
from nimro_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `nimro config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.nimro.config`` entries, then checks
    ...
    try:
        ...
        # should never break `nimro update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Nimro' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Nimro versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``nimro_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (nimro_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (nimro_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (nimro_cli/setup.py)
    "known_plugin_toolsets", # written/read by nimro_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'nimro setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Nimro won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, nimro send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'nimro doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "nimro update / nimro -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from nimro_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ nimro config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in nimro_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from nimro_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: nimro config set <key> <value>")
    ...
    # `nimro update`, but only the version bump is persisted.
    ...
    # metadata.nimro.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.nimro/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_nimro_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``nimro config set``
    ...
    backends) — while display paths (``nimro status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _nimro_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.nimro/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_nimro_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.nimro/config.yaml.
    ...
    (which change ``NIMRO_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/nimro/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_nimro_dotenv()
...
#   openai-codex (OAuth — nimro auth) — OpenAI Codex
#   nous         (OAuth — nimro auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.nimro/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Nimro writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.nimro/.env.
    ...
    interactive menu render (`nimro tools`, `nimro setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `nimro tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.nimro/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.nimro/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``NIMRO_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"NIMRO_CUSTOM_{slug}_API_KEY" if slug else "NIMRO_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.nimro/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.nimro/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Nimro — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Nimro vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` or ``~/.nimro/.env``, scope-aware.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.nimro/.env`` over ``os.environ``.

    Used for Nimro-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Nimro Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from nimro_cli.auth import get_anthropic_key
    ...
    # Warn on stale NIMRO_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("NIMRO_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale NIMRO_MAX_ITERATIONS={_env_ghost} "
                f"(run 'nimro doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  nimro config edit     # Edit config file", Colors.DIM))
    print(color("  nimro config set <key> <value>", Colors.DIM))
    print(color("  nimro setup           # Run setup wizard", Colors.DIM))
    print()
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `nimro cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``nimro config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``nimro config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            CLI exposes this via ``nimro config set --force``.
    """
    if is_managed():
    ...
    if config_path.exists():
        try:
            ...
            print(
                ...
                f"  (nimro config edit will open it in your editor.)",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    # ``nimro config set model.provider openai`` doesn't silently
    ...
    # (e.g. ``nimro config set model gpt-5.6-sol`` when model already
    ...
    if "." not in key:
        ...
        if isinstance(_existing, dict):
            if key == "model":
                ...
                print(
                    f"    nimro config set {key}.<sub-key> <value>",
                    file=sys.stderr,
                )
                print(
                ...
                print(
                    f"    nimro config set --force {key} {value!r}",
                    file=sys.stderr,
                )
                sys.exit(1)
    _set_nested(user_config, key, value)
    # Normalize the api_base → base_url alias at set-time too (issue #8919),
    # so a fresh `nimro config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_nimro_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `nimro config set model.api_key cfut_...` routes to config.yaml
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `nimro config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: nimro config get <key> [--json]")
            print()
            print("Examples:")
            print("  nimro config get model")
            print("  nimro config get terminal.backend")
            print("  nimro config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: nimro config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  nimro config set model anthropic/claude-sonnet-4")
            print("  nimro config set terminal.backend docker")
            print("  nimro config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: nimro config unset <key>")
            print()
            print("Examples:")
            print("  nimro config unset model")
            print("  nimro config unset terminal.backend")
            print("  nimro config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'nimro config migrate' to add them")
        ...
        print("  nimro config           Show current configuration")
        print("  nimro config edit      Open config in editor")
        print("  nimro config get <key>          Print a resolved config value")
        print("  nimro config set <key> <value>   Set a config value")
        print("  nimro config unset <key>        Remove a config value")
        print("  nimro config check     Check for missing/outdated config")
        print("  nimro config migrate   Update config with new options")
        print("  nimro config path      Show config file path")
        print("  nimro config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``nimro config`` UI so users
