"""
Unified tool configuration for Nimro Agent.

`nimro tools` and `nimro setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.nimro/config.yaml under
...
from nimro_cli.nous_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    NousSubscriptionFeatures,
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from nimro_cli.nous_account import format_nous_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``nimro tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`nimro_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from nimro_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _NIMRO_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `nimro tools` → Video Generation, which walks
...
# `nimro tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `nimro tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from nimro_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `nimro tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``nimro tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from nimro_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``nimro tools`` checklist actually offers
    for ``platform``.
    ...
    so ``nimro tools`` never claims to add or remove a toolset the user was
...
from nimro_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Nimro routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set NIMRO_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "NIMRO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "NIMRO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "NIMRO_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "NIMRO_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "NIMRO_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `nimro model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("NIMRO_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Nimro telemetry policy applied.
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $NIMRO_HOME/bin is never on PATH, so a bare which()
    # misses the uv Nimro installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``nimro update`` and
      by ``nimro computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``nimro update`` sets
    ...
    a further ~600s wait on Windows). ``nimro computer-use install
    ...
    ``nimro update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `nimro update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `nimro computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `nimro update` (require_confirmed_update=True) keeps the
    ...
    # `nimro computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: nimro computer-use install --upgrade"
            )
            return True
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `nimro update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `nimro update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.nimro/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Nimro process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Managed Node first — $NIMRO_HOME/node is not on PATH, so a bare
        # which() reports "no npm" on installs whose only Node is the one
        # Nimro installed for exactly this toolchain.
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_nimro_home()}/nimro-agent && npm install --workspaces=false")
                if result.stderr:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in nimro-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/nimro-agent:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.nimro/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.nimro/config.yaml")
        ...
        # Run the full `nimro auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.nimro/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from nimro_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: nimro auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `nimro auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: nimro auth spotify")
        except Exception as exc:
        ...
        try:
            from nimro_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: nimro plugins enable observability/langfuse")
        _print_info("    Restart Nimro for tracing to take effect.")
        _print_info("    Verify: nimro plugins list")
        ...
        try:
            ...
            _print_info("    Run later: nimro auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `nimro auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: nimro auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: nimro auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``nimro tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``nimro tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: nimro tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``nimro-discord``, which contains those
...
#:
#: Saving ``nimro tools`` (or one toggle in the desktop Toolsets UI) replaces
...
#: has touched the picker, while everyone still on ``[nimro-cli]`` inherits it
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``nimro tools``
    ...
    default_ts = plat_info["default_toolset"] if plat_info else f"nimro-{platform}"
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (nimro-acp, nimro-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``nimro-discord``) is an opt-in to the platform's native default-off
    ...
    # "nimro-cli" (which include all _NIMRO_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[nimro-cli, spotify]`` after enabling Spotify via ``nimro
        ...
        # (e.g. "nimro-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``nimro tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `nimro tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"nimro-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("nimro-")}
    ...
    # shipped as a bundled plugin but user must opt in via `nimro tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `nimro tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by nimro tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `nimro` instead of
    # `nimro-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `nimro update`/`nimro doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `nimro tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (nimro-cli, nimro-telegram, etc.)
    ...
    # Opening `nimro tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `nimro tools` gets a silent no-op
...
def _agent_browser_installed() -> bool:
    ...
    from nimro_cli.nous_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``nimro tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from nimro_cli.nous_subscription import _has_agent_browser
...
# Kept in sync with the dashboard selects (nimro_cli/web_server.py
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `nimro model`).
    if managed_feature:
        from nimro_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``nimro model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``nimro model`` aux-task
    ...
    """
    try:
        from nimro_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `nimro model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `nimro model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `nimro tools` and `nimro setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Nimro Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://nimro-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from nimro_constants import display_nimro_home
    print(color(f"  Tool configuration saved to {display_nimro_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'nimro' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, nimro mcp
