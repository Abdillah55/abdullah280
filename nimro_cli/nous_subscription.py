...
# coverage category (nimro_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `nimro tools` picker scope its entitlement gate to the selected backend, so a
...
def _has_agent_browser() -> bool:
    ...
    from nimro_constants import agent_browser_runnable, with_nimro_node_path
    ...
    # Nimro-managed Node dirs (Windows installer / POSIX $NIMRO_HOME/node)
    ...
    managed_path = with_nimro_node_path().get("PATH", "")
    if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("NIMRO_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `nimro model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("NIMRO_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("NIMRO_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from nimro_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`nimro tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``nimro tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``nimro model``'s Nous
    login, this:
