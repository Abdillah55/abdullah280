...
``<root>`` is the **shared Nimro root** (the parent of any active
...
``nimro -p <profile>`` joins the same board as the dispatcher that
...
the profile's Nimro home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``NIMRO_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``NIMRO_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``nimro kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.nimro``. In Docker / custom
deployments where ``NIMRO_HOME`` points outside ``~/.nimro`` (e.g.
``/opt/nimro``), ``<root>`` is ``NIMRO_HOME``. Legacy env-var
overrides still work:

* ``NIMRO_KANBAN_DB`` — pin the database file path directly.
* ``NIMRO_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``NIMRO_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``NIMRO_KANBAN_DB``,
``NIMRO_KANBAN_WORKSPACES_ROOT``, and ``NIMRO_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/nimro-kanban-v1-spec.pdf`` for the full
...
from nimro_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def normalize_reasoning_effort(effort: Optional[str]) -> Optional[str]:
    ...
    Accepts any level in ``nimro_constants.VALID_REASONING_EFFORTS`` plus
    ...
    """
    from nimro_constants import VALID_REASONING_EFFORTS
...
def _assert_not_delegated_child_mutation() -> None:
    ...
    try:
        ...
        delegated = bool(os.environ.get("NIMRO_DELEGATED_CHILD_CONTEXT"))
    if delegated:
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active NIMRO_HOME so dispatcher- and
...
# ``NIMRO_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``NIMRO_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("NIMRO_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``NIMRO_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("NIMRO_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``NIMRO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "NIMRO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "nimro_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``nimro-agent``
...
def kanban_home() -> Path:
    """Return the shared Nimro root that anchors the kanban board.

    Resolution order:

    1. ``NIMRO_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_nimro_root()``, which already returns ``<root>``
       when ``NIMRO_HOME`` is ``<root>/profiles/<name>``, and returns
       ``NIMRO_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``NIMRO_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("NIMRO_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from nimro_constants import get_default_nimro_root
    return get_default_nimro_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``nimro kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``NIMRO_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``nimro kanban boards
    ...
    env = os.environ.get("NIMRO_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``nimro kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``NIMRO_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("NIMRO_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``NIMRO_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("NIMRO_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``NIMRO_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("NIMRO_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``nimro kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # ``nimro_constants.VALID_REASONING_EFFORTS``, or ``"none"`` for thinking
    ...
    # within an agent loop that propagated ``NIMRO_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (nimro_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``NIMRO_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("NIMRO_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _sqlite_connect(path: Path) -> sqlite3.Connection:
    ...
    advisory locks on the database (see ``nimro_cli.sqlite_safe_read``).
    The registration is released automatically when the connection closes.
    """
    from nimro_cli.sqlite_safe_read import connect_tracked
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``nimro gateway run --replace`` /
...
def _validate_sqlite_header(path: Path) -> None:
    ...
    from nimro_cli.sqlite_safe_read import read_header_bytes_preopen
...
def _backup_corrupt_db(path: Path) -> Optional[Path]:
    ...
    # see nimro_cli.sqlite_safe_read), so it must only run once the board has
    ...
    # rescue.
    from nimro_cli.sqlite_safe_read import has_live_connection
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Nimro treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``nimro kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``NIMRO_KANBAN_DB`` env → ``NIMRO_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `nimro kanban list` mid-integrity-probe) block
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from nimro_state import apply_wal_with_fallback
    ...
    with _cross_process_init_lock(path):
        ...
        from nimro_state import preflight_db_writability
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See nimro_state._WAL_INCOMPAT_MARKERS for detection logic.
                from nimro_state import apply_wal_with_fallback
                apply_wal_with_fallback(conn, db_label=f"kanban.db ({path.name})")
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``nimro kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``NIMRO_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _check_file_length_invariant(conn: sqlite3.Connection) -> None:
    ...
    """
    from nimro_cli.sqlite_safe_read import file_length_matches_header
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from nimro_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``nimro --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        from nimro_cli import projects_db as _pdb
    ...
    # `nimro --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``nimro kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `nimro kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``nimro kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``nimro kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``nimro kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``nimro kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``nimro kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``NIMRO_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Nimro' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Nimro-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``nimro kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Nimro checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Nimro treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Nimro creates/reuses that linked worktree. With
      no ``workspace_path``, Nimro anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Nimro uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``NIMRO_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Nimro
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``nimro kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``nimro kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Nimro profile.
    ...
    if not rows:
        return False
    try:
        from nimro_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from nimro_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Nimro profile.
        # `_default_spawn` invokes ``nimro -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Nimro
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `nimro kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from nimro_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_nimro_argv() -> list[str]:
    """Return the interpreter-bound Nimro CLI invocation."""
    # ``nimro_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``nimro`` package — there is no
    # ``nimro`` package to import.
    return [sys.executable, "-m", "nimro_cli.main"]


def _absolute_nimro_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Nimro shim."""
...
def _nimro_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Nimro executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_nimro_argv()
    return [_absolute_nimro_path(path)]


def _resolve_nimro_argv() -> list[str]:
    """Resolve the ``nimro`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$NIMRO_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("nimro")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\nimro.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m nimro_cli.main`` — fallback for setups where
       Nimro is launched from a venv and the ``nimro`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_nimro_bin`` for the same reason. Kept
    local (not imported from gateway) because ``nimro_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("NIMRO_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _nimro_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _nimro_path_argv(resolved_env_bin)
        return _module_nimro_argv()

    nimro_bin = _safe_which_no_cwd("nimro") if _IS_WINDOWS else shutil.which("nimro")
    if nimro_bin:
        return _nimro_path_argv(nimro_bin)
    return _module_nimro_argv()
...
def _resolve_worker_cli_toolsets(nimro_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``NIMRO_KANBAN_TASK`` is set.
    """
    if not nimro_home:
        return None
    try:
        ...
        token = set_nimro_home_override(nimro_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for NIMRO_HOME=%r (%s)",
            nimro_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``nimro -p <profile> chat -q ...`` subprocess.
    ...
    ``NIMRO_KANBAN_DB`` / ``NIMRO_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject NIMRO_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `nimro -p <name>` the
    # _apply_profile_override() runs *before* nimro_constants is imported.
    # If NIMRO_HOME is absent from the child's env, get_nimro_home() falls
    # back to Path.home() / ".nimro" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from nimro_cli.profiles import resolve_profile_env
    try:
        env["NIMRO_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via NIMRO_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # NIMRO_HOME never had profiles created.
        pass
    if task.tenant:
        env["NIMRO_TENANT"] = task.tenant
    env["NIMRO_KANBAN_TASK"] = task.id
    env["NIMRO_KANBAN_WORKSPACE"] = workspace
    # Tag the worker's session so it lands in state.db as `kanban`, not as an
    # untitled `cli` row. A worker is a dispatcher-owned run whose transcript is
    # read on the board and in `nimro kanban log` — it is not a conversation
    ...
    # ("work kanban task t_…").
    env["NIMRO_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["NIMRO_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["NIMRO_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["NIMRO_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["NIMRO_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["NIMRO_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`nimro -p <name>`
    # rewrites NIMRO_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_nimro_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["NIMRO_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["NIMRO_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["NIMRO_KANBAN_WORKSPACES_ROOT"])
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["NIMRO_KANBAN_BOARD"] = resolved_board
    # NIMRO_PROFILE is the author the kanban_comment tool defaults to.
    # `nimro -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["NIMRO_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited NIMRO_TUI=1
    ...
    # older nimro builds on PATH that predate the flag's precedence.
    env.pop("NIMRO_TUI", None)

    cmd = [
        *_resolve_nimro_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped NIMRO_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("NIMRO_HOME"))
    if worker_toolsets:
    ...
    # `nimro kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`nimro` executable not found on PATH. "
            "Install Nimro Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``nimro kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled NIMRO_PROFILE values like "nimro-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Nimro root exists

    Reads profile paths directly so this module has no import dependency on
    ``nimro_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from nimro_constants import get_default_nimro_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``nimro kanban assignees`` for the terminal.
