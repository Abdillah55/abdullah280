"""Nimro update pipeline — extracted from ``nimro_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``nimro_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``nimro_cli.main`` reference — so existing call sites and test monkeypatches
that target ``nimro_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``nimro_cli.main``.
...
Imports are one-way: ``nimro_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``nimro_cli.main`` reference.

    Lets callers keep patching ``nimro_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``nimro_cli.main`` -> ``nimro_cli.update_cmd`` stays one-way
    at import time.
    """
    from nimro_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``nimro update`` keeps running in the pre-pull Python process. After a
...
# Critical files that Nimro must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "nimro_cli/main.py",
    "nimro_cli/config.py",
    "nimro_cli/__init__.py",
    "nimro_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``nimro`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="nimro-syntax-check-") as tmpdir:
...
def _validate_critical_modules_import(root) -> tuple[bool, str | None, str | None]:
    ...
    ``_venv_core_imports_healthy``): ``nimro update`` can be driven by a
    ...
    """
    from nimro_constants import FIRST_PARTY_MODULE_ROOTS
    ...
    probe = (
        ...
        # The root set is injected from nimro_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('nimro_'):\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``nimro update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `nimro update`.
    ...
    print("  Preview now:  nimro curator run --dry-run")
    print("  Pause it:     nimro curator pause")
    print(
        "  Docs:         https://nimro-agent.nousresearch.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `nimro update`.
    ...
    try:
        ...
        return
    db_path = get_nimro_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `nimro sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    nimro sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  nimro sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``nimro update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``nimro update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: nimro curator status)"
    )
...
def _finish_dashboard_update_cleanup(node_failures: list[str]) -> None:
    ...
    print("    nimro dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``nimro_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.nimro-update-staging"
    backup = f"{dst}.nimro-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `nimro update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``nimro_constants.py``
...
def _update_via_zip(args):
    """Update Nimro Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `nimro update "
            f"--branch {branch}`, or update against main with `nimro update`."
        )
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/nimro-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="nimro-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"nimro-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for nimro-agent itself
        ...
        # GitHub ZIPs extract to nimro-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"nimro-agent-{branch}")
        ...
        print(
            "  Re-run `nimro update` to retry; if the agent won't start, "
            "reinstall from https://nimro-agent.nousresearch.com"
        )
        _m().sys.exit(1)
    finally:
    ...
    from nimro_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if not import_ok:
        ...
        print("  Re-run `nimro update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: nimro skills list-modified  "
    ...
    try:
        from nimro_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from nimro_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_nimro_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_nimro_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "nimro-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Nimro behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes nimro completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Nimro couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Nimro couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Nimro behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Nimro couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Nimro couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/nimro-agent.git",
    "git@github.com:NousResearch/nimro-agent.git",
    "https://github.com/NousResearch/nimro-agent",
    "git@github.com:NousResearch/nimro-agent",
}

OFFICIAL_REPO_URL = "https://github.com/NousResearch/nimro-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_nimro_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_nimro_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Nimro repository.")
        print("  This means you may miss updates from NousResearch/nimro-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/nimro-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/nimro-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``nimro update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from nimro_constants import get_default_nimro_root

    default_home = get_default_nimro_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "nimro.exe"
    lines = ["✗ Another nimro.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Nimro Desktop, exit any open `nimro` REPLs, and")
    lines.append("  stop the gateway (`nimro gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `nimro update --force` if you've already")
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``nimro update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `nimro update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `nimro update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Nimro' editable-install extras or
    ...
    try:
        from nimro_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from nimro_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$NIMRO_HOME/bin/uv``, but on Termux the official
    ...
    """
    from nimro_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `nimro update` is exactly the step
...
def _npm_lockfile_changed(nimro_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `nimro
    ...
    try:
        ...
        cache_file = nimro_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(nimro_root: Path) -> None:
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from nimro_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `nimro update`.")
            failed = ["repo root"]
            if any(
    ...
    # Nimro profile using this checkout. Keep one per-checkout cache under the
    # shared Nimro root rather than rerunning npm once per named profile.
    shared_nimro_root = get_default_nimro_root()
    if not _m()._npm_lockfile_changed(shared_nimro_root):
    ...
    # need desktop during `nimro update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `nimro update`.")
        return list(labels)
    ...
    nixos_env = with_nimro_node_path(_m()._nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `nimro update` comes from
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_nimro_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.nimro/logs/update.log`` only, never the terminal.

    During ``nimro update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``nimro update --check``: fetch and report without installing.
    ...
    """
    from nimro_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``nimro update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from nimro_cli.config import format_docker_update_message
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``nimro update`` without requiring a reinstall.  The
    ...
    ``nimro`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/nimro.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v nimro'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/nimro, code at /usr/local/lib/nimro-agent).
    fhs_link = Path("/usr/local/bin/nimro")
    ...
    path_comment = (
        "# Nimro Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``nimro-acp`` launcher next to the ``nimro`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``nimro update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``nimro-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Nimro as not installed even when it is.

    The shim simply delegates to the sibling ``nimro`` launcher with the
    ...
    ``nimro-acp.exe`` already resolves) and wherever a ``nimro-acp`` is
    already present next to the ``nimro`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        nimro_cmd = bin_dir / "nimro"
        acp_cmd = bin_dir / "nimro-acp"
        try:
            if not (nimro_cmd.is_file() or nimro_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of NIMRO_HOME under
      ``backups/`` (restorable via ``nimro import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from nimro_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from nimro_constants import
        # get_nimro_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from nimro_cli.config import get_nimro_home as _get_home
    ...
    try:
        from nimro_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_nimro_home so the user sees ~/.nimro/...
    try:
        from nimro_constants import get_nimro_home, display_nimro_home

        home = get_nimro_home()
        try:
            display_path = f"{display_nimro_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  nimro import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``nimro update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``nimro update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run nimro from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.nimro-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".nimro-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The nimro.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m nimro_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``nimro
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Nimro processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Nimro Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Nimro desktop app / other Nimro terminals, then re-run:"
    )
    lines.append("    nimro update")
    lines.append("  (or use `nimro update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``nimro update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from nimro_cli._scan_venv_blockers import _is_pausable_gateway
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    nimro.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Nimro...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m nimro_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: nimro gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``nimro gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from nimro_cli import gateway_windows
        from nimro_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``nimro-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("nimro-gateway"):
            continue
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``nimro update`` never touched them, so installs created before the
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``nimro update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``nimro update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Nimro Agent...")
    print()

    # On Windows, abort early if another nimro.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_nimro_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `nimro serve` backend) will keep .pyd
    ...
    # the nimro.exe shim guard above, but its lock probe only checks the shim
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://nimro-agent.nousresearch.com/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Nimro windows/gateways and re-run: nimro update")
            else:
                print("✓ Already up to date!")
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Nimro gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in nimro_cli/config.py bricked
        # every user who ran ``nimro update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``nimro
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``nimro update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_nimro_home added to nimro_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``nimro`` launch finishes the install
        ...
        # symbols in nimro_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `nimro` again "
        ...
        if not import_ok:
            ...
            print("    Run `nimro update` again — if it persists, reinstall:")
            print("    https://nimro-agent.nousresearch.com")

        node_failures = _update_node_dependencies()
        _m()._build_web_ui(_m().PROJECT_ROOT / "web")

        # Rebuild the desktop app if the source tree changed since the last
        # build.  ``nimro desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``nimro desktop`` shouldn't be forced into a full
        # Electron build by ``nimro update``.
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `nimro desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                _desktop_build_cmd = [sys.executable, "-m", "nimro_cli.main", "desktop", "--build-only"]
                ...
                # debuggable.
                #
                # Start the build subprocess with the Nimro-managed Node on PATH:
                # when `nimro update` runs inside the desktop updater chain
                # (Desktop → nimro-setup → nimro update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from nimro_constants import with_nimro_node_path

                _build_env = with_nimro_node_path()
                build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    print("  ⚠ Desktop build failed (non-fatal; run `nimro desktop` to retry)")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_nimro_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.nimro/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit NIMRO_HOME so
        # it is not affected by sync_skills()'s module-level NIMRO_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's NIMRO_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from nimro_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'nimro config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: nimro config migrate")
            else:
                print()
                print("Skipped. Run 'nimro config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from nimro_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `nimro curator status`. Self-stamps after printing so it
        ...
        # Self-heal the nimro-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Nimro on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("nimro-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``nimro update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `nimro update`
                # must stay fast; `nimro computer-use install --upgrade`
        ...
        # When running as ``nimro update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_nimro_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... nimro-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... nimro-gateway*)
            ...
            try:
                from nimro_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all nimro-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: nimro gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `nimro update` restart it automatically, allow\n"
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: nimro gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: nimro -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Nimro gateway unit files are still installed.
        # When both nimro.service (from a pre-rename install) and the
        # current nimro-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `nimro update` surfaces the issue until the user migrates.
        try:
            from nimro_cli.gateway import (
                has_legacy_nimro_units,
                _find_legacy_nimro_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_nimro_units():
                print()
                print("⚠ Legacy Nimro gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_nimro_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (nimro.service) fight the current")
                print("  nimro-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    nimro gateway migrate-legacy")
                print()
        ...
        print("  nimro model              # Select provider and model")
