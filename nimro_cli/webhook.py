"""nimro webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.nimro/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _nimro_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: nimro gateway run
"""
...
def webhook_command(args):
    """Entry point for 'nimro webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: nimro webhook {subscribe|list|remove|test}")
        print("Run 'nimro webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (nimro gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: nimro webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from nimro webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (nimro gateway run)")
