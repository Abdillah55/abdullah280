...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with nimro-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.nimro/plugins/<name>/``
3. **Project plugins** – ``./.nimro/plugins/<name>/`` (opt-in via
   ``NIMRO_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``nimro_agent.plugins``
   entry-point group.
...
from nimro_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``NIMRO_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("NIMRO_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``NIMRO_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.nimro/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("NIMRO_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When NIMRO_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Nimro loggers.
    """
    ...
    logger.debug(
        "NIMRO_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Nimro' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by nimro_cli.kanban_db when a task
    ...
    # separate `nimro -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `nimro kanban dispatch`),
...
ENTRY_POINTS_GROUP = "nimro_agent.plugins"

_NS_PARENT = "nimro_plugins"
...
class PluginManifest:
    ...
    #              in ~/.nimro/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``nimro plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Nimro profile name (e.g. ``"default"``).

        Derived from ``NIMRO_HOME`` via
        :func:`nimro_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.nimro/profiles/<name>``, or ``"custom"`` when
        ``NIMRO_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Nimro core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``nimro honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``nimro <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`nimro_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_nimro_dotenv()`` startup — after
        ``~/.nimro/.env`` loads, before Nimro reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Nimro' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``nimro model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: nimro_cli.main imports plugins indirectly
        from nimro_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.nimro/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("NIMRO_SAFE_MODE"):
            logger.info("NIMRO_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to nimro_cli/. Two layouts are
        ...
        # 2. User plugins (~/.nimro/plugins/)
        user_dir = get_nimro_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.nimro/plugins/)
        if _env_enabled("NIMRO_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".nimro" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set NIMRO_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with nimro and must
            ...
            # all ~20 of them added several seconds to every `nimro`
            # invocation — including plain `nimro chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Nimro ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `nimro plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``nimro plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`nimro plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `nimro plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``nimro_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``nimro_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="nimro-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``nimro tools`` TUI so plugin-provided toolsets appear
