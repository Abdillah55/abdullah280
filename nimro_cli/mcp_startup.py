...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local NIMRO_HOME override (profile
        ...
        def _discover() -> None:
            token = None
            try:
                from nimro_constants import set_nimro_home_override
            ...
            try:
                ...
                if token is not None:
                    try:
                        from nimro_constants import reset_nimro_home_override
...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``nimro -z "..."`` / ``-q``), the larger
    ...
    try:
        from nimro_cli.config import load_config, DEFAULT_CONFIG
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``nimro dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``nimro -z``) can construct
