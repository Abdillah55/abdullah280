"""Default configuration data for Nimro Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from nimro_cli/config.py. Must not import from nimro_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["nimro-cli"],
    # SQLite journal mode used by every Nimro database opener. WAL is the
    ...
    "agent": {
        ...
        # the Nimro-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Nimro
        ...
        # identity slot (SOUL.md). Empty by default. The NIMRO_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Nimro adds a coding operating brief
        ...
        # ``NIMRO_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             NIMRO_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force NIMRO_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Nimro auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Nimro sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Nimro
        ...
        # bundled Nimro image, which drops to the `nimro` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Nimro sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Nimro should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``nimro chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.nimro/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: nimro sweeps the checkpoint base at startup
        ...
        # ``nimro checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .nimro.md, or .cursorrules before Nimro applies
    ...
    # Single-query (``nimro -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Nimro truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Nimro' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); nimro =
                                      # Nimro' compression threshold triggers
    ...
    "auxiliary": {
        ...
        # Invoked by ``nimro kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``nimro kanban decompose`` and the kanban
        ...
        # ``nimro profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `nimro model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `nimro` (and `nimro chat`) launches by default:
        ...
        # REPL and `--tui` (or NIMRO_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `nimro --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `nimro -c` muscle memory.  Default off so existing
        # users aren't surprised.  NIMRO_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `nimro --tui` drops a one-time hint
        ...
        # model — see nimro_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to NIMRO_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/nimro`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `nimro pets`. Disabled until
        ...
        "pet": {
            ...
            # get_nimro_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``NIMRO_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``NIMRO_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``NIMRO_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``NIMRO_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``NIMRO_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/nimro``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Nimro appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.nimro/cache/piper-voices/
    ...
    # "Hey Nimro" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey nimro",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_nimro" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.nimro/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.nimro/cache/delegation/
    ...
    # response. If not, Nimro feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Nimro auto-pauses the goal and
    ...
    "moa": {
        ...
        # <nimro_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.nimro/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${NIMRO_SKILL_DIR} and ${NIMRO_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `nimro curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `nimro curator run --consolidate` overrides
        ...
        # normally restored on every `nimro update`, so pruning them only
        ...
        # skipped), snapshot ~/.nimro/skills/ into
        # ~/.nimro/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `nimro curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for nimro-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Nimro Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (NIMRO_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Nimro' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.nimro/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or NIMRO_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `nimro doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``nimro_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Nimro to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $NIMRO_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via NIMRO_CRON_MAX_PARALLEL env var.
        ...
        # NIMRO_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `nimro -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `nimro` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `nimro kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     nimro-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Nimro tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.nimro/logs/.
    ...
    # update model picker lists without shipping a nimro-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://nimro-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `nimro model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal NIMRO_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.nimro/sessions/sessions.json entirely.
        ...
        # the NIMRO_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.nimro.gateway` that defenses 1-2 don't
        ...
        # ``NIMRO_GATEWAY_MAX_STARTS`` / ``NIMRO_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.nimro/.env,
        ...
        # behavior: files must live under the Nimro cache, under
        ...
        # user. Bridged to NIMRO_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Nimro
        # cache (~/.nimro/cache/{documents,images,audio,video,screenshots})
        ...
        # to NIMRO_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # NIMRO_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to NIMRO_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.nimro/state.db.
    ...
    "sessions": {
        ...
        # the default of ``nimro sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.nimro/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `nimro sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `nimro update` prints a one-line notice with
        ...
        # ~/.nimro/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to NIMRO_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # NIMRO_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <NIMRO_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``nimro backup``-style zip
        #     of NIMRO_HOME into <NIMRO_HOME>/backups/, restorable with
        #     ``nimro import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `nimro update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<NIMRO_HOME>/lsp/bin/`` on first use.
    ...
    # `nimro tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.nimro/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.nimro/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.nimro/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Nimro may use this encrypted
            ...
            # ~/.nimro/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.nimro/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Nimro disables it for our users unless they
        # explicitly opt in here. When false (default), Nimro sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Nimro also calls
    ...
    #
    # Configure with `nimro egress setup`.  Disabled by default — the rest of
    # Nimro works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.nimro/bin/ on
    ...
    # Nimro Desktop (Electron app) launch options. These only affect
    # `nimro desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the NIMRO_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Nimro mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'nimro model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Nimro auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Nimro' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Nimro, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @nimro:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. NIMRO_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #nimro)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: nimro-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'nimro-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Nimro API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Nimro API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Nimro API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NIMRO_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to NIMRO_TOOL_PROGRESS_MODE for backward
    ...
    # NIMRO_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "NIMRO_PREFILL_MESSAGES_FILE": {
