"""
Backup and import commands for nimro CLI.

`nimro backup` creates a zip archive of the entire ~/.nimro/ directory
(excluding the nimro-agent repo and transient files).

`nimro import` restores from a backup zip, overlaying onto the current
NIMRO_HOME root.
"""
...
from nimro_constants import get_default_nimro_root, get_nimro_home, display_nimro_home
...
# ``nimro-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/nimro-agent/``
...
# NIMRO_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "nimro-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under NIMRO_HOME) —
...
# File names that ``nimro import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE NIMRO_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Nimro backup slot."""
...
@contextmanager
def _backup_operation_lock(nimro_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = nimro_home / ".backup.lock"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Nimro backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside NIMRO_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to nimro root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``nimro-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/nimro-agent/`` — must be preserved.
        if part == "nimro-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside NIMRO_HOME.
...
# to the user as a hung `nimro update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # nimro_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Nimro home directory."""
    nimro_root = get_default_nimro_root()

    if not nimro_root.is_dir():
        print(f"Error: Nimro home directory not found at {nimro_root}")
        sys.exit(1)

    try:
        with _backup_operation_lock(nimro_root):
            _run_backup_locked(args, nimro_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, nimro_root: Path) -> None:
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"nimro-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"nimro-backup-{stamp}.zip"
    ...
    print(f"Scanning {display_nimro_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(nimro_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(nimro_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``nimro-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "nimro-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(nimro_root)
    ...
    # outside NIMRO_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_nimro_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: nimro import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Nimro backup.
    ...
    # Look for telltale files that a nimro home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Nimro backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.nimro/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a nimro dir name
        if prefix in {".nimro", "nimro"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_nimro_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (nimro_root / "config.yaml").exists()
        has_env = (nimro_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Nimro configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under NIMRO_HOME.
            ...
            target = nimro_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(nimro_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_nimro_home()}."
            )
        ...
        profiles_dir = nimro_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # nimro_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: nimro profile list  (after installing nimro)")

        # Guidance
        print()
        if not (nimro_root / "nimro-agent").is_dir():
            print("Note: The nimro-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: nimro update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  nimro -p {pname} gateway install")

        print("Done. Your Nimro configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and nimro backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to NIMRO_HOME).
...
# — `nimro update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $NIMRO_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside NIMRO_HOME and the entry is silently
...
def _quick_snapshot_root(nimro_home: Optional[Path] = None) -> Path:
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: nimro snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(nimro_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside NIMRO_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``nimro update``.
    ...
    Args:
        ...
        nimro_home: Override for the Nimro home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(nimro_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for nimro backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_nimro_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, nimro_root: Path) -> Optional[Path]:
    """Single-flight wrapper for automatic full zip backups."""
    try:
        with _backup_operation_lock(nimro_root):
            return _write_full_zip_backup_locked(out_path, nimro_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, nimro_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``nimro_root`` to ``out_path``.
...
def _pre_update_backup_dir(nimro_home: Optional[Path] = None) -> Path:
    home = nimro_home or get_nimro_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of NIMRO_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<NIMRO_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``nimro update``) should continue even if the backup fails.
    """
    nimro_root = nimro_home or get_default_nimro_root()
    if not nimro_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(nimro_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, nimro_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `nimro claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of NIMRO_HOME under ``backups/`` before a
    ``nimro claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``nimro import <archive>``.  Writes to
    ``<NIMRO_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `nimro import` and the
