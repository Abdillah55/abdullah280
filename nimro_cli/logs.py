"""``nimro logs`` — view and filter Nimro log files.
...
Usage examples::
    ...
    nimro logs                    # last 50 lines of agent.log
    nimro logs -f                 # follow agent.log in real time
    nimro logs errors             # last 50 lines of errors.log
    nimro logs gateway -n 100    # last 100 lines of gateway.log
    nimro logs gui -f            # follow gui.log (dashboard/pty/ws)
    nimro logs desktop -f        # follow desktop.log (Electron app boot/backend)
    nimro logs --level WARNING    # only WARNING+ lines
    nimro logs --session abc123   # filter by session ID substring
    nimro logs --component tools  # only tool-related lines
    nimro logs --since 1h         # lines from the last hour
    nimro logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_nimro_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Nimro runs — try 'nimro chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_nimro_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_nimro_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_nimro_home()}/logs/")
        return

    print(f"Log files in {display_nimro_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'nimro chat' to generate logs)")
