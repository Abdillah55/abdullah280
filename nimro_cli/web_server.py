"""
Nimro Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m nimro_cli.main web          # Start on http://127.0.0.1:9119
    python -m nimro_cli.main web --port 8080
"""
...
from nimro_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
try:
    ...
    # running `nimro dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["NIMRO_WEB_DIST"]) if "NIMRO_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``nimro gateway run`` — but the
    desktop app spawns a ``nimro dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same NIMRO_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    ...
    ``nimro_cli.gateway``. But the first WS connection and its initial
    ...
    for mod in (
        ...
        "nimro_cli.auth",
        "nimro_cli.copilot_auth",
        "nimro_cli.runtime_provider",
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from nimro_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Import nimro_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # Desktop-spawned backends (NIMRO_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `nimro
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("NIMRO_DESKTOP") == "1":
...
app = FastAPI(title="Nimro Agent", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from nimro_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# NIMRO_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("NIMRO_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Nimro-Session-Token"
...
#
# This list is defined in ``nimro_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from nimro_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Nimro-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``nimro-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from nimro_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from nimro_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from nimro_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `nimro egress setup` "
            "and `nimro egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Nimro (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during nimro update. "
...
from nimro_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Nimro provider slug — fine.
    ...
    1. Vendor-name → Nimro-provider mapping: when the provider string is
       not a known Nimro provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from nimro_cli.config import get_compatible_custom_providers
    from nimro_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from nimro_cli.model_normalize import normalize_model_for_provider
    from nimro_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Nimro
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from nimro_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    while another Nimro process is writing to it.
    """
    from nimro_state import DEFAULT_DB_PATH, SessionDB
...
_MANAGED_FILES_ROOT_ENV = "NIMRO_DASHBOARD_FILES_ROOT"
...
# NIMRO_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to NIMRO_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Nimro credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no NIMRO_HOME resolution, so it blocks these trees wherever
...
def _default_nimro_root_is_opt_data() -> bool:
    raw = os.environ.get("NIMRO_HOME", "").strip()
    if not raw:
        return False
    try:
        from nimro_constants import get_default_nimro_root

        root = get_default_nimro_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``nimro update``.
    ...
    a container — e.g. the nimro-webui image sharing the Nimro source tree),
    the dashboard's ``nimro update`` button is the correct update path and
    ...
    """
    if _default_nimro_root_is_opt_data():
        return True
    try:
        from nimro_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Nimro root is actually /opt/data
    # (the container/hosted layout) or when NIMRO_DASHBOARD_FILES_ROOT is set.
    if _default_nimro_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Nimro inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``nimro:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.nimro-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``nimro_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from nimro_cli import web_git as _web_git  # noqa: E402
...
from nimro_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from nimro_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from nimro_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_nimro_home() at call time
    ...
    try:
        ...
        # NIMRO_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of nimro_cli.gateway blocks the
        ...
        # and which providers are registered so ``nimro status`` and the
        ...
        auth_flows: list[str] = []
        try:
            from nimro_cli.dashboard_auth import (
        ...
        try:
            from nimro_cli.auth import get_nous_session_validity
        ...
        status = {
            ...
            "can_update_nimro": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
        ...
        try:
            ...
            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_nimro_home())
            )
        ...
        try:
            from nimro_state import SessionDB as _SDB
            from nimro_constants import get_nimro_home as _ghh
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Nimro Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the nimro home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_nimro_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_nimro_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from nimro_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from nimro_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_nimro_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_nimro_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_nimro_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from nimro_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.nimro/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_nimro_home() / "logs"
...
def _spawn_nimro_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``nimro <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``nimro_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "nimro_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _NIMRO_GATEWAY=1. Inheriting it makes a spawned `nimro gateway restart`
    ...
    action_env = {**os.environ, "NIMRO_NONINTERACTIVE": "1"}
    action_env.pop("_NIMRO_GATEWAY", None)
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["nimro", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``nimro gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``nimro gateway restart`` children race each other on the
    ...
    return _spawn_nimro_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``nimro gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``NIMRO_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/nimro/update")
async def update_nimro():
    """Kick off ``nimro update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Nimro updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("nimro-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "nimro-update",
    ...
    try:
        proc = _spawn_nimro_action(["update"], "nimro-update")
    except Exception as exc:
        _log.exception("Failed to spawn nimro update")
...
@app.get("/api/nimro/update/check")
async def check_nimro_update(force: bool = False):
    """Report whether a Nimro update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/nimro/update`` actually runs ``nimro update``.

    Returns:
        install_method: 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed Nimro version string
    ...
    try:
        ...
        if force:
            try:
                (get_nimro_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.nimro/config.yaml`` under ``tts.``.
    """
...
from nimro_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from nimro_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from nimro_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from nimro_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Nimro supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_nimro_home() / provider.name / "config.json"
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/nimro is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (NIMRO_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    nimro_home = get_nimro_home()
    values: Dict[str, Any] = {}

    # Common native provider stores.
    for path in (
        nimro_home / f"{name}.json",
        nimro_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_nimro_home()))
            return
...
async def get_egress_status():
    ...
    from nimro_cli.proxy_cli import format_status_text
...
# in nimro_cli/config.py — listed here for deterministic ordering in the UI.
...
async def get_model_options(
    ...
    """
    try:
        from nimro_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `nimro model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from nimro_cli.auth import get_provider_auth_state
    ...
    try:
        from nimro_cli.inventory import build_models_payload, load_picker_context
        from nimro_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from nimro_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from nimro_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.nimro/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from nimro_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (nimro_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from nimro_cli.nous_subscription import apply_nous_managed_defaults
                from nimro_cli.tools_config import _get_platform_tools
        ...
        # endpoint, mirroring the ``nimro model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from nimro_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``nimro model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from nimro_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from nimro_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `nimro model` provider — would be
        ...
        # Vertex — a `nimro model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `nimro model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from nimro_cli.credential_lifecycle import save_provider_env_credential
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from nimro_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Nimro from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Nimro to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Nimro from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Nimro to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Nimro in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Nimro through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Nimro via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Nimro through an IMAP/SMTP mailbox.",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Nimro to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Nimro inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Nimro to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Nimro through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Nimro to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect Nimro to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Nimro.",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Nimro from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Nimro over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Nimro through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Nimro over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Nimro to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Nimro as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Nimro via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Nimro Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in nimro gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in nimro gateway setup",
...
# nimro_cli/setup_hidden_env.py. Shared with the `nimro setup gateway`
# wizard so the surfaces ask for the same things.
from nimro_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # NIMRO_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    from nimro_constants import get_nimro_dir

    return get_nimro_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.nimro-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"NimroDashboard/{__version__}"
@dataclass
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Nimro Agent").strip() or "Nimro Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Nimro appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the NIMRO_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized NIMRO_HOME)
...
# the canonical ``nimro auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.nimro/.anthropic_oauth.json`` — Nimro-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_nimro_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_nimro_oauth_credentials,
            _get_nimro_oauth_file,
        )
    except ImportError:
        read_nimro_oauth_credentials = None  # type: ignore
        _get_nimro_oauth_file = None  # type: ignore

    nimro_creds = None
    if read_nimro_oauth_credentials:
        try:
            nimro_creds = read_nimro_oauth_credentials()
        except Exception:
            nimro_creds = None
    if nimro_creds and nimro_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "nimro_pkce",
            "source_label": f"Nimro PKCE ({_get_nimro_oauth_file() if _get_nimro_oauth_file else None})",
            "token_preview": _truncate_token(nimro_creds.get("accessToken")),
            "expires_at": nimro_creds.get("expiresAt"),
            "has_refresh_token": bool(nimro_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from nimro_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Nimro even
    when they also have a separate Nimro-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Nimro never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "nimro auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "nimro auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "nimro auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "nimro auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "nimro auth add xai-oauth",
        "docs_url": "https://nimro-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "nimro auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from nimro_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Nimro, so the disconnect
    ...
    user sees exactly what executes, and Nimro then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Nimro — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``nimro model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `nimro model` universe (zero-edit for new plugins).
    try:
        from nimro_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"nimro auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("nimro_pkce", "claude_code", ...)
    ...
    sync with the `nimro model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Nimro-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_nimro_oauth_file
                oauth_file = _get_nimro_oauth_file()
            ...
            try:
                from nimro_cli.auth import clear_provider_auth
        ...
        try:
            from nimro_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.nimro/.anthropic_oauth.json AND credential pool
...
# Guarded so nimro web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Nimro file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``nimro auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_nimro_oauth_file
    oauth_file = _get_nimro_oauth_file()
    payload = {
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "nimro-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("NIMRO_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from nimro_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from nimro_cli.auth import persist_nous_credentials
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `nimro auth add xai-oauth`: first credential may become
            ...
            # prior ``nimro auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``nimro model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Nimro "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from nimro_cli.auth import _save_codex_tokens
...
def _open_session_db_for_profile(profile: Optional[str], *, read_only: bool):
    ...
    from nimro_state import SessionDB, _default_db_path, is_malformed_db_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``nimro serve`` — it runs neither the
    ...
    try:
        ...
        from nimro_cli.config import load_config as _load_full_config
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `nimro sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_nimro_home()
...
async def get_logs(
    ...
    from nimro_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_nimro_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from nimro_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from nimro_cli import profiles as profiles_mod
    try:
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (NIMRO_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.nimro``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    NIMRO_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, NIMRO_HOME)."""
    from nimro_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from nimro_constants import (
        reset_nimro_home_override,
        set_nimro_home_override,
    )

    token = set_nimro_home_override(str(home))
    try:
...
from nimro_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from nimro_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Nimro home so the job's store,
...
#
# Wraps the same config data layer the CLI uses (nimro_cli.mcp_config), so
# servers managed here show up under `nimro mcp list` and vice versa.  Secrets
...
def _normalize_mcp_server_create(
    ...
    """
    from nimro_cli.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from nimro_cli.mcp_security import validate_mcp_server_entry
...
from nimro_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from nimro_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from nimro_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.nimro_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from tools.mcp_oauth import NimroTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_nimro_home_override(flow.nimro_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.nimro_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = NimroTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        nimro_home=flow.nimro_home,
                    )
...
#
# Wraps the same JSON store the CLI uses (nimro_cli.webhook); the webhook
...
async def list_webhooks():
    import nimro_cli.webhook as wh
...
# spawn the real `nimro gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_nimro_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_nimro_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from nimro_cli.auth import read_credential_pool
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `nimro auth remove`) can seed again.
        # Mirrors the `nimro auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``nimro auth remove`` uses: each source cleans up its
    ...
    from nimro_cli.auth import suppress_credential_source
...
async def run_doctor():
    try:
        proc = _spawn_nimro_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_nimro_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_nimro_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"nimro-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_nimro_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_nimro_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from nimro_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from nimro_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <nimro_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_nimro_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_nimro_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``nimro`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_nimro_action`` tracks one process/log per name, so a shared
...
from nimro_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from nimro_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_nimro_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "nimro setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local NIMRO_HOME override so the write lands in the target
    ...
    """
    from nimro_constants import set_nimro_home_override, reset_nimro_home_override

    token = set_nimro_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local NIMRO_HOME override (same mechanism as
    ...
    """
    from nimro_constants import set_nimro_home_override, reset_nimro_home_override
    from nimro_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the NIMRO_HOME override. Returns the
    number of skills newly disabled.
    """
    from nimro_constants import set_nimro_home_override, reset_nimro_home_override
    from nimro_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_nimro_home()`` at call
       time — the context-local override from ``set_nimro_home_override``
    ...
    retargeted to the *current* ``get_nimro_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a NIMRO_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_nimro_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_nimro_home()`` at call time
...
from nimro_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from nimro_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _toolset_model_catalog(ts_key: str, plugin_name: str):
    ...
    from nimro_cli.tools_config import (
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Nimro,
...
#
# The endpoint spawns the same ``nimro --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from nimro_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from nimro_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from nimro_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from nimro_cli.dashboard_auth.audit import AuditEvent, audit_log
        from nimro_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``nimro --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``NIMRO_TUI_RESUME`` env var —
    matching what ``nimro_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``NIMRO_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``NIMRO_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``NIMRO_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``NIMRO_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_nimro_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``nimro -p <name>`` performs. The in-process
    ``NIMRO_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from nimro_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Nimro TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope below still overrides NIMRO_HOME afterwards.
    from tools.environments.local import build_subprocess_env
    ...
    env.setdefault("NIMRO_TUI_DISABLE_MOUSE", "1")
    env.setdefault("NIMRO_TUI_INLINE", "1")
    ...
    env["NIMRO_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["NIMRO_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["NIMRO_TUI_RESUME"] = resume

    if sidecar_url:
        env["NIMRO_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile NIMRO_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["NIMRO_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``NIMRO_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("NIMRO_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from nimro_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="nimro-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Nimro Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Nimro CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="nimro-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from nimro_cli.console_engine import NimroConsoleEngine

        engine = NimroConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Nimro Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Nimro inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("NIMRO_TUI_RESUME") or raw_resume
...
# NIMRO_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`nimro_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from nimro_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Nimro Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="nimro-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/nimro/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /nimro`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__NIMRO_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `nimro serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("NIMRO_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (nimro serve): web UI disabled — use "
            "`nimro dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/nimro``)
        ...
        ``__NIMRO_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__NIMRO_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__NIMRO_BASE_PATH__="{prefix}";'
                f"window.__NIMRO_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__NIMRO_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__NIMRO_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__NIMRO_BASE_PATH__="{prefix}";'
                f"window.__NIMRO_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``NIMRO_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Nimro Teal canvas, then
    ...
    # under ``/nimro`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Nimro backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Nimro Teal",         "description": "Classic dark teal — the canonical Nimro look"},
    {"name": "default-large", "label": "Nimro Teal (Large)", "description": "Nimro Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.nimro/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.nimro/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_nimro_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``NIMRO_HOME``.
    """
    themes_dir = get_process_nimro_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.nimro/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as nimro_cli.plugins):
    1. User plugins:    ~/.nimro/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.nimro/plugins/  (only if NIMRO_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from nimro_cli.plugins import get_bundled_plugins_dir
    ...
    # NIMRO_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
        (get_process_nimro_home() / "plugins", "user"),
    ...
    # ``nimro_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("NIMRO_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".nimro" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from nimro_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_nimro_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if cached_result is False:
                        auth_required = True
                        auth_command = f"nimro auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from nimro_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from nimro_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from nimro_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from nimro_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.nimro/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"nimro_dashboard_plugin_{plugin['name']}"
...
from nimro_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``NIMRO_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("NIMRO_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``NIMRO_SERVE_HEADLESS``), so
    ...
    try:
        from nimro_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the nimro-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing NIMRO_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Nimro Agent exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `nimro dashboard register` (Nous Portal) or "
            ...
            # (#54489).
            try:
                from nimro_cli.config import load_config as _load_cfg
                from nimro_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`nimro plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # NIMRO_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "NIMRO_BACKEND_READY" if headless else "NIMRO_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  Nimro backend listening on {host}:{actual_port}")
            else:
                print(f"  Nimro Web UI → http://{host}:{actual_port}")
