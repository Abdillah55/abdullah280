...
Nimro is developed on Linux / macOS and tested natively on Windows too.
...
# Electron exits, so the gateway never gets respawned after a `nimro
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Nimro' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Nimro shells out to git from many non-interactive contexts — MCP catalog
