"""Windows ConPTY bridge for the `nimro dashboard` chat tab.

Drop-in counterpart to ``nimro_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``nimro_cli.web_server`` consumes: ``spawn``, ``read``,
