...
``cli.py``'s ``NimroCLI`` class. ``NimroCLI`` inherits ``CLICommandsMixin`` so
...
from nimro_constants import display_nimro_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: nimro --checkpoints")
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Nimro changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Nimro hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Nimro config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_nimro_home()}/state-snapshots/):\n")
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `nimro journey`).
        ...
        from cli import _cprint
        from nimro_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from nimro_cli.clipboard import has_clipboard_image
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from nimro_cli.clipboard import (
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run nimro chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in nimro_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `nimro gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `nimro sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from nimro_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `nimro sessions list` to see available sessions.")
            return
        ...
        # same directory as a startup `nimro -c`/`--resume`. The startup resume
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from nimro_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from nimro_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from nimro_cli.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to nimro_cli.curator so the CLI and the `nimro curator`
        ...
        try:
            from nimro_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from nimro_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to nimro_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            from nimro_cli.write_approval_commands import handle_pending_subcommand
            ...
            if out is not None:
                print(out)
                return
        from nimro_cli.skills_hub import handle_skills_slash
    ...
    def _handle_init_command(self, cmd: str):
        ...
        """
        from nimro_cli.init_command import build_init_prompt_for_cwd
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``nimro bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: nimro bundles create "
        ...
        _cprint(
            ...
            f"Manage with `nimro bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Nimro-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        from nimro_cli.goals import parse_contract
        ...
        _cprint(
            ...
            f"Nimro keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from cli import _DIM, _RST, _cprint
        from nimro_cli.goals import draft_contract
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from nimro_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_nimro_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="nimro_prompt_")
        try:
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        """
        from nimro_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from nimro_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from nimro_cli.focus_view import format_hidden_line
    ...
    def _handle_approvals_command(self, cmd_original: str) -> None:
        ...
        from cli import _cprint
        from nimro_cli.approval_mode import run_approval_mode_command
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Nimro is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Nimro is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Nimro is busy; /stop still cancels it."
    ...
    def _handle_indicator_command(self, cmd: str):
        ...
        """
        from cli import _ACCENT, _DIM, _RST, _cprint, save_config_value
        from nimro_constants import DEFAULT_INDICATOR_STYLE, INDICATOR_STYLES
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from nimro_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Nimro Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``nimro update`` so the user sees update output directly and gets
        ...
        when cancelled.
        """
        from nimro_cli.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update Nimro Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Nimro Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Nimro Agent",
            detail="This will exit the current session and run `nimro update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Nimro' hotword listener.
