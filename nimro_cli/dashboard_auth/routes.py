...
from nimro_cli.dashboard_auth.base import (
    InvalidCodeError,
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      1. ``NIMRO_DASHBOARD_PUBLIC_URL`` env var or
      ...
      2. ``X-Forwarded-Prefix: /nimro`` (Mission Control deploys) — we
    ...
    from nimro_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``nimro_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from nimro_cli.dashboard_auth.prefix import prefix_from_request
...
async def auth_login(request: Request, provider: str, next: str = ""):
    ...
    pkce = ls.cookie_payload.get("nimro_session_pkce", "")
...
async def auth_native_authorize(
    ...
    from nimro_cli.dashboard_auth import native_flow
...
async def api_auth_ws_ticket(request: Request):
    ...
    from nimro_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
...
async def auth_native_refresh(request: Request, body: _NativeRefreshBody):
    ...
    """
    from nimro_cli.dashboard_auth import list_session_providers
    from nimro_cli.dashboard_auth.base import RefreshExpiredError
