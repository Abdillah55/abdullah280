...
Profile-aware location: ``$NIMRO_HOME/logs/dashboard-auth.log``.
...
from ``nimro_constants`` or other nimro_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$NIMRO_HOME/logs/dashboard-auth.log``.

    Uses ``nimro_constants.get_nimro_home()`` (a leaf module — no import
    ...
    return get_nimro_home() / "logs" / "dashboard-auth.log"
