...
from nimro_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /nimro``, the
    ``login_url`` is prefixed (``/nimro/login?next=...``) so the
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from nimro_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["nimro.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``nimro_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``nimro_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from nimro_cli.dashboard_auth.cookies import clear_session_cookies
        from nimro_cli.dashboard_auth.prefix import prefix_from_request
