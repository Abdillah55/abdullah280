"""
nimro fallback — manage the fallback provider chain.
...
Subcommands:
  nimro fallback [list]   Show the current fallback chain (default when no subcommand)
  nimro fallback add      Pick provider + model via the same picker as `nimro model`,
                           then append the selection to the chain
  nimro fallback remove   Pick an entry to delete from the chain
  nimro fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.nimro/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from nimro_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `nimro model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  nimro fallback add")
        print()
        return
    ...
    print("  Docs: https://nimro-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `nimro model`, then append the selection to the chain."""
    from nimro_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `nimro model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `nimro fallback list` to view, or `nimro fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``nimro fallback [subcommand]``."""
