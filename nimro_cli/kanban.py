"""CLI for the Nimro Kanban board — ``nimro kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/nimro-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from nimro_cli import kanban_swarm as ks
...
def _check_dispatcher_presence(
    nimro_home: Optional[Path] = None,
) -> tuple[bool, str]:
    """Return ``(running, message)``.

    - ``running=True``: a gateway is alive for this NIMRO_HOME and its
    ...
    Used by ``nimro kanban create`` (and callers) to warn when a task
    ...
    ``nimro_home`` scopes the probe to a named profile's directory. The
    dashboard plugin API passes it because the dashboard backend process can
    be running under a different NIMRO_HOME than the profile the request
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(
            profile_dir=nimro_home, use_cache=False
        )
    except Exception:
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`nimro kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Nimro profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://nimro-agent.nousresearch.com/docs/user-guide/features/kanban "
            "or docs/nimro-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # NIMRO_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `nimro kanban boards switch <slug>` or the "
            "NIMRO_KANBAN_BOARD env var). Use `nimro kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `nimro project list`.")
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $NIMRO_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $NIMRO_PROFILE or 'user')")
    ...
    p_attach.add_argument("--author", default=None,
                          help="uploaded_by label (default: $NIMRO_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `nimro gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.nimro/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $NIMRO_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $NIMRO_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``nimro kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: nimro kanban <action> [options]\n"
                "Run 'nimro kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0

    # Fast-fail for clearer CLI UX only. The durable trust boundary is lower in
    # nimro_cli.kanban_db, because children can import DB mutators directly.
    ...
    # env-var pin for the duration of this call. Using NIMRO_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `nimro kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # NIMRO_HOME. Previously only `init` and `daemon` triggered
    ...
    with board_scope:
        ...
        # every `nimro kanban repair` into "could not initialize database"
...
def _profile_author() -> str:
    ...
    for env in ("NIMRO_PROFILE_NAME", "NIMRO_PROFILE"):
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("NIMRO_DELEGATED_CHILD_CONTEXT"))


# ---------------------------------------------------------------------------
# Boards management (nimro kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``nimro kanban boards <action>``.
    ...
    DB, so a fresh NIMRO_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `nimro kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `nimro kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `nimro kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `nimro kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.nimro/profiles/.")
        print("Create one with `nimro -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  nimro gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `nimro -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from nimro_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``nimro kanban show <id>``
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ...
            # `nimro kanban complete <id>` from the terminal tool and
            ...
            if task and task.goal_mode:
                ...
                if judge_available:
                    ...
                    try:
                        ...
                        # nimro_cli/goals.py. Unpacking fewer raises
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "nimro kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    nimro gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`nimro kanban list --status ready` / "
                    f"`nimro kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Nimro profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from nimro_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from nimro_cli import kanban_decompose as decomp
