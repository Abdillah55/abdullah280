...
``nimro_cli.config.migrate_config``. Each step is a function
...
``nimro_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``nimro_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``nimro_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``nimro_cli.config`` (e.g.
``patch("nimro_cli.config.read_raw_config", ...)``) keep working, because
...
#: NIMRO_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_nimro_home()}/config.yaml and run `nimro setup` to "
...
def _cfg():
    """Return the live ``nimro_cli.config`` module (lazy, cycle-free)."""
    from nimro_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_nimro_home = _c.get_nimro_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$NIMRO_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `nimro plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `nimro curator status` has no
    ...
    #   3. Creates `~/.nimro/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_nimro_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_nimro_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `nimro config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `nimro config set`"
                )
...
# behavior set it to true explicitly via `nimro config set`.
