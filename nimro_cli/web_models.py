"""Pydantic request/response models for the Nimro dashboard web server.

Extracted verbatim from ``nimro_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from nimro_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``nimro model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `nimro import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`nimro -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the NIMRO_HOME
