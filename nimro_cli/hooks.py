"""nimro hooks — inspect and manage shell-script hooks.

Usage::

    nimro hooks list
    nimro hooks test <event> [--for-tool X] [--payload-file F]
    nimro hooks revoke <command>
    nimro hooks doctor

Consent records live under ``~/.nimro/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.nimro/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``nimro hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: nimro hooks {list|test|revoke|doctor}")
        print("Run 'nimro hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.nimro/config.yaml.")
        print("See `nimro hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.nimro/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `nimro hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `nimro hooks test` and `nimro hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Nimro wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `nimro hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `nimro hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `nimro hooks doctor`.")
