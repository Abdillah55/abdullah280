"""
Nimro Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``nimro desktop`` / ``nimro gui``)
     Built inside the agent checkout under ``$NIMRO_HOME/nimro-agent/``:
       ...
       - ``$NIMRO_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Nimro.app`` or ``~/Applications/Nimro.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Nimro`` (NSIS per-user)
...
the app name ("Nimro"), separate from ``$NIMRO_HOME``:
  - macOS:   ``~/Library/Application Support/Nimro``
  - Windows: ``%APPDATA%\\Nimro``
  - Linux:   ``$XDG_CONFIG_HOME/Nimro`` (default ``~/.config/Nimro``)
...
``nimro uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(nimro_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return nimro_home / "nimro-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Nimro"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Nimro"
...
def source_built_gui_artifacts(nimro_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``nimro desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``nimro-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(nimro_home)
    ...
    return [
        ...
        nimro_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Nimro" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Nimro.app"),
            home / "Applications" / "Nimro.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Nimro).
            local_base / "Programs" / "Nimro",
            # Older / alternate layout some builds used.
            local_base / "nimro-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Nimro")
    else:
        ...
        paths += [
            data_base / "applications" / "nimro.desktop",
            data_base / "applications" / "Nimro.desktop",
        ]
    return paths


def agent_is_installed(nimro_home: Path) -> bool:
    """Return True when a usable Python agent install exists under NIMRO_HOME.
    ...
    if (agent_root / "nimro_cli").is_dir():
        return True
...
def gui_is_installed(nimro_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(nimro_home):
        if p.exists():
            return True
...
def gui_install_summary(nimro_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = nimro_home if nimro_home is not None else get_nimro_home()
...
def uninstall_gui(nimro_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``nimro-agent/nimro_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$NIMRO_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove nimro' or "
            "'sudo dnf remove nimro'). AppImage builds are a single file you "
