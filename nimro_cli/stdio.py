...
on Japanese locales, etc.).  Nimro's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``nimro_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Nimro is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``NIMRO_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("NIMRO_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Nimro-managed Git install directories so
    ...
    # already-running shells don't see the change — which means nimro
    ...
    # install without Nimro-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Nimro's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Nimro's
    ``nimro config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Nimro (User env var in Windows Settings, or
    export in a PowerShell profile) and Nimro picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Nimro-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\nimro\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``nimro`` from right after install) retain their old PATH.

    Any subprocess Nimro spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Nimro-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "nimro", "git", "cmd"),
        os.path.join(local_appdata, "nimro", "git", "bin"),
        os.path.join(local_appdata, "nimro", "git", "usr", "bin"),
        # Nimro venv Scripts directory — host of the nimro.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes nimro, but harmless to include.
        os.path.join(local_appdata, "nimro", "nimro-agent", "venv", "Scripts"),
