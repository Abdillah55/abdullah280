#!/usr/bin/env python3
"""
Nimro CLI - Main entry point.

Usage:
    nimro                     # Interactive chat (default)
    nimro chat                # Interactive chat
    nimro gateway             # Run gateway in foreground
    nimro gateway start       # Start gateway as service
    nimro gateway stop        # Stop gateway service
    nimro gateway status      # Show gateway status
    nimro gateway install     # Install gateway service
    nimro gateway uninstall   # Uninstall gateway service
    nimro setup               # Interactive setup wizard
    nimro logout              # Clear stored authentication
    nimro status              # Show status of all components
    nimro cron                # Manage cron jobs
    nimro cron list           # List cron jobs
    nimro cron status         # Check if cron scheduler is running
    nimro doctor              # Check configuration and dependencies
    nimro honcho setup                    # Configure Honcho AI memory integration
    nimro honcho status                   # Show Honcho config and connection status
    nimro honcho sessions                 # List directory → session name mappings
    nimro honcho map <name>               # Map current directory to a session name
    nimro honcho peer                     # Show peer names and dialectic settings
    nimro honcho peer --user NAME         # Set user peer name
    nimro honcho peer --ai NAME           # Set AI peer name
    nimro honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    nimro honcho mode                     # Show current memory mode
    nimro honcho mode [hybrid|honcho|local]  # Set memory mode
    nimro honcho tokens                   # Show token budget settings
    nimro honcho tokens --context N       # Set session.context() token cap
    nimro honcho tokens --dialectic N     # Set dialectic result char cap
    nimro honcho identity                 # Show AI peer identity representation
    nimro honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    nimro honcho migrate                  # Step-by-step migration guide: OpenClaw native → Nimro + Honcho
    nimro version             Show version
    nimro update              Update to latest version
    nimro uninstall           Uninstall Nimro Agent
    nimro acp                 Run as an ACP server for editor integration
    nimro sessions browse     Interactive session picker with search

    nimro claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: nimro_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``nimro_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``nimro update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``nimro_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, nimro crashes on import and the user can't run
# ``nimro update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import nimro_bootstrap  # noqa: F401
...
from nimro_cli._subprocess_compat import suppress_platform_ver_console
...
# Two lines of inline path math so ``python nimro_cli/main.py`` (script
# mode — sys.path[0] is nimro_cli/, not the repo root) can import the
# canonical helpers; everything else lives in nimro_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from nimro_cli import _startup_fast  # noqa: E402

# Early venv self-heal — MUST run before any third-party import below.  When
# a prior ``nimro update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from nimro_cli.env_loader import ...`` / ``from nimro_cli.config import
...
# package directory, so if IT can't import, nothing else in nimro_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from nimro_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from nimro_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'nimro' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``nimro tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``nimro.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("nimro")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"nimro", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"nimro")
        # Windows: the .exe name is already ``nimro.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* nimro_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("NIMRO_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".nimro", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``NIMRO_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``nimro … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("NIMRO_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``NIMRO_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("NIMRO_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`nimro --tui … >log`, CI capture):
...
def _active_profile_may_override_home_fast(nimro_root: str) -> bool:
    return _startup_fast.active_profile_may_override_home(nimro_root)
...
def _try_ultrafast_version() -> bool:
    """Handle ``nimro --version`` before config/logging imports."""
...
from nimro_cli.sessions_cmd import cmd_sessions  # noqa: F401
from nimro_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from nimro_cli.subcommands.cron import build_cron_parser
from nimro_cli.subcommands.sync import build_sync_parser
from nimro_cli.subcommands.gateway import build_gateway_parser
from nimro_cli.subcommands.profile import build_profile_parser
from nimro_cli.subcommands.model import build_model_parser
from nimro_cli.subcommands.setup import build_setup_parser

from nimro_cli.subcommands.whatsapp import build_whatsapp_parser
from nimro_cli.subcommands.slack import build_slack_parser
from nimro_cli.subcommands.login import build_login_parser
from nimro_cli.subcommands.logout import build_logout_parser
from nimro_cli.subcommands.auth import build_auth_parser
from nimro_cli.subcommands.status import build_status_parser
from nimro_cli.subcommands.webhook import build_webhook_parser
from nimro_cli.subcommands.hooks import build_hooks_parser
from nimro_cli.subcommands.doctor import build_doctor_parser
from nimro_cli.subcommands.security import build_security_parser
from nimro_cli.subcommands.approvals import build_approvals_parser
from nimro_cli.subcommands.dump import build_dump_parser
from nimro_cli.subcommands.debug import build_debug_parser
from nimro_cli.subcommands.backup import build_backup_parser
from nimro_cli.subcommands.import_cmd import build_import_cmd_parser
from nimro_cli.subcommands.import_agent import build_import_agent_parser
from nimro_cli.subcommands.config import build_config_parser
from nimro_cli.subcommands.skin import build_skin_parser
from nimro_cli.subcommands.console import build_console_parser
from nimro_cli.subcommands.version import build_version_parser
from nimro_cli.subcommands.update import build_update_parser
from nimro_cli.subcommands.uninstall import build_uninstall_parser
from nimro_cli.subcommands.dashboard import build_dashboard_parser
from nimro_cli.subcommands.gui import build_gui_parser
from nimro_cli.subcommands.logs import build_logs_parser
from nimro_cli.subcommands.prompt_size import build_prompt_size_parser
from nimro_cli.subcommands.memory import build_memory_parser
from nimro_cli.subcommands.acp import build_acp_parser
from nimro_cli.subcommands.tools import build_tools_parser
from nimro_cli.subcommands.insights import build_insights_parser
from nimro_cli.subcommands.monitoring import build_monitoring_parser
from nimro_cli.subcommands.skills import build_skills_parser
from nimro_cli.subcommands.pairing import build_pairing_parser
from nimro_cli.subcommands.plugins import build_plugins_parser
from nimro_cli.subcommands.mcp import build_mcp_parser
from nimro_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (nimro tools, nimro setup, nimro model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'nimro {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any nimro module import.
#
# Many modules cache NIMRO_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("NIMRO_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.nimro/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set NIMRO_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `nimro mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Nimro' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo nimro -p <name>` against the invoking user's home.
        ...
        candidate = home / ".nimro" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`nimro chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors nimro_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If NIMRO_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.nimro/profiles/coder or
    # /opt/data/profiles/coder).  If NIMRO_HOME points to the nimro root
    # instead (e.g. systemd hardcodes NIMRO_HOME=/root/.nimro), we must
    # still read active_profile — the user may have switched profiles via
    # `nimro profile use` and the gateway should honour that choice.
    # See issue #22502.
    nimro_home_env = os.environ.get("NIMRO_HOME", "")
    if profile_name is None and nimro_home_env:
        if Path(nimro_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the nimro root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as NIMRO_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``nimro gateway run`` to mean
    # "the root NIMRO_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("NIMRO_S6_SUPERVISED_CHILD"):
        try:
            from nimro_constants import get_default_nimro_root

            active_path = get_default_nimro_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set NIMRO_HOME
    if profile_name is not None:
        try:
            from nimro_cli.profiles import resolve_profile_env

            nimro_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            nimro_home = _resolve_sudo_user_profile_env(profile_name)
            if not nimro_home:
            ...
            # A bug in profiles.py must NEVER prevent nimro from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["NIMRO_HOME"] = nimro_home
...
# Load .env from ~/.nimro/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from nimro_cli.config import get_nimro_home
from nimro_cli.env_loader import load_nimro_dotenv

load_nimro_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → NIMRO_REDACT_SECRETS env
# var BEFORE nimro_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves nimro_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from nimro_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_nimro_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "NIMRO_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["NIMRO_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `nimro` subcommands
...
try:
    from nimro_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from nimro_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if nimro_constants not importable yet
...
from nimro_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to nimro_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (nimro_cli.main._model_flow_*) keep resolving unchanged.
from nimro_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_nimro_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("NIMRO_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("NIMRO_TERMUX_PREFETCH_UPDATES") == "1"
...
def _relative_time(ts) -> str:
    ...
    in :mod:`nimro_cli.timefmt` so lightweight consumers don't have to
    import the whole CLI surface.
    """
    from nimro_cli.timefmt import relative_time
...
def _has_any_provider_configured() -> bool:
    ...
    from nimro_cli.config import get_env_path, get_nimro_home, load_config
    from nimro_cli.auth import get_auth_status

    # Determine whether Nimro itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_nimro_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from nimro_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_nimro_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Nimro has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Nimro to use their tokens.
    if _has_nimro_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``nimro -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, nimro_bin
        cli_args: the original CLI arguments (everything after 'nimro')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    nimro_bin = container_info["nimro_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo nimro {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo nimro {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  nimro --tui --resume {target}")
    if title:
        print(f'  nimro --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @nimro/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@nimro" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/nimro-ink/package.json",
    "packages/nimro-ink/index.js",
    "packages/nimro-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``NIMRO_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("NIMRO_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``NIMRO_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("NIMRO_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "NIMRO_HOME": nimro_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(nimro_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(nimro_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if nimro_cli_dir is None:
        nimro_cli_dir = Path(__file__).parent
    bundled = nimro_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``nimro update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("NIMRO_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Nimro checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `nimro update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Nimro checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `nimro --tui`\n"
        "If the checkout is still inconsistent, run `nimro update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (NIMRO_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("NIMRO_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        # find_node_executable() prefers the managed $NIMRO_HOME/node tree,
        # which is not on PATH — a bare which() would declare "node not found"
        # and exit on an install whose only Node is the one Nimro installed,
        # and would pick a system Node over the managed one when both exist.
        from nimro_constants import find_node_executable

        path = find_node_executable(bin)
        if not path and bin == "node":
            try:
                from nimro_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("NIMRO_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with NIMRO_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset NIMRO_TUI_DIR (e.g. `unset NIMRO_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # nimro_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            from nimro_constants import with_nimro_node_path
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env={**with_nimro_node_path(), "CI": "1"},
            )

        result = _run_tui_install()
        if result.returncode != 0:
            # An npm outside the root package.json's `engines.npm` range fails
            # here before doing any work; repair once (upgrade a Nimro-managed
            ...
            from nimro_cli.npm_engine import maybe_repair_npm_engine
    ...
    if tui_dev:
        # Keep the local @nimro/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @nimro/ink resolves through
        # packages/nimro-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "nimro-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from nimro_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("NIMRO_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["NIMRO_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("NIMRO_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["NIMRO_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("NIMRO_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["NIMRO_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["NIMRO_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a nimro process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    try:
        from nimro_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="nimro-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["NIMRO_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["NIMRO_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["NIMRO_MODEL"] = model
        env["NIMRO_INFERENCE_MODEL"] = model
    if provider:
        env["NIMRO_TUI_PROVIDER"] = provider
        env["NIMRO_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["NIMRO_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["NIMRO_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["NIMRO_TUI_SKILLS"] = value
    if query:
        env["NIMRO_TUI_QUERY"] = query
    if image:
        env["NIMRO_TUI_IMAGE"] = image
    if checkpoints:
        env["NIMRO_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["NIMRO_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["NIMRO_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["NIMRO_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["NIMRO_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["NIMRO_ACCEPT_HOOKS"] = "1"
    ...
    # NIMRO_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `nimro --tui` try to
    ...
    # still set NIMRO_TUI_RESUME themselves.
    env.pop("NIMRO_TUI_RESUME", None)
    if resume_session_id:
        env["NIMRO_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `nimro update` so
    ...
    if code == 42:
        from nimro_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``NIMRO_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``nimro kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``nimro kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("NIMRO_KANBAN_BOARD"):
        return
    try:
        from nimro_cli.kanban_db import get_current_board

        os.environ["NIMRO_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.nimro/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Nimro — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Nimro still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``nimro … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'nimro sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from nimro_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from nimro_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'nimro doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Nimro isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  nimro setup")
        print()

        from nimro_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'nimro setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from nimro_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["NIMRO_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.nimro/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["NIMRO_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["NIMRO_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["NIMRO_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from nimro_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from nimro_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from nimro_cli.config import get_env_value, save_env_value
    from nimro_constants import find_node_executable, with_nimro_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Nimro?")
        print()
    ...
    # has no creds.json.  Every subsequent `nimro gateway` then paid a 30s
    ...
    session_dir = get_nimro_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: nimro gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `nimro gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  nimro gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Nimro Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  nimro gateway")
        ...
        print()
        print("  Or install as a service: nimro gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'nimro whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``nimro whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``nimro_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from nimro_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from nimro_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from nimro_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``nimro model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from nimro_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from nimro_cli.providers import (
    ...
    effective_provider = (
        config_provider or os.getenv("NIMRO_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from nimro_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'nimro model' for "
                "available providers, or run 'nimro doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in nimro_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``nimro model``
    ...
    # matching against nimro_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    # a leftover OPENAI_BASE_URL in ~/.nimro/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.nimro/.env if the active provider is not 'custom'.
    ...
    """
    from nimro_cli.config import get_env_value, save_env_value, load_config
...
#
# Nimro uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `nimro model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `nimro model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`nimro_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from nimro_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from nimro_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Nimro only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``nimro model`` flow, then route aux tasks
    to them here.
    """
    from nimro_cli.config import load_config
    from nimro_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from nimro_cli.auth import _prompt_model_selection
    from nimro_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from nimro_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from nimro_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from nimro_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Nimro URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from nimro_cli.curses_ui import curses_radiolist
...
# downstream call sites read `nimro_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `nimro` invocation — including
# fast paths like `nimro --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from nimro_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``nimro setup`` / ``nimro model``.
    ...
    recover from a malformed paste without editing ``~/.nimro/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from nimro_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from nimro_cli.config import save_env_value
    from nimro_cli.secret_prompt import masked_secret_prompt
    ...
    from nimro_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `nimro setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from nimro_constants import display_nimro_home as _dhh_fn

            print(
                f"    Nimro will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Nimro CLI with a provider."""
    from nimro_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from nimro_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from nimro_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from nimro_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from nimro_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.nimro/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `nimro sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            ...
            print(
                "\nSync feature is off for this instance (set NIMRO_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "NIMRO_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="nimro sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="nimro sync now")
...
def cmd_webhook(args):
    ...
    from nimro_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``nimro slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: nimro slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `nimro slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from nimro_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from nimro_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from nimro_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from nimro_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from nimro_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `nimro security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from nimro_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `nimro approvals <subcmd>`."""
    from nimro_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from nimro_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from nimro_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from nimro_cli.config import config_command
...
def cmd_skin(args):
    ...
    from nimro_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Nimro home directory to a zip file."""
    if getattr(args, "quick", False):
        from nimro_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from nimro_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Nimro backup from a zip file."""
    from nimro_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from nimro_cli.config import detect_install_method
    from nimro_cli.slash_exec import CommandContext, execute_command
    ...
    try:
        from nimro_cli.banner import check_for_updates
        from nimro_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Nimro Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from nimro_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from nimro_cli.uninstall import run_gui_uninstall
    ...
    from nimro_cli.uninstall import run_uninstall
...
# Update pipeline extracted to nimro_cli/update_cmd.py (main.py decomposition,
# mechanical move). Every moved name is re-exported here so the argparse wiring
# and existing test monkeypatches (nimro_cli.main._cmd_update_impl,
# nimro_cli.main._run_pre_update_backup, ...) keep resolving unchanged.
from nimro_cli.update_cmd import (  # noqa: F401
...
# validated against. Lives next to the checkout (NOT in NIMRO_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``nimro update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``nimro update``
    ...
    ``nimro`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``nimro update``
    ...
    ``nimro_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "nimro_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $NIMRO_HOME."""
    from nimro_constants import get_nimro_home
    return get_nimro_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``nimro`` launcher present but ``nimro_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the nimro venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — nimro venv python3, no nix-shell overhead
    ...
    # a nimro venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``nimro update`` to stash the
    lockfile — repeatedly.
    ...
    # of a Nimro-managed install, or a freshly provisioned managed npm when
    ...
    run_env["PATH"] = with_nimro_node_path(run_env)["PATH"]
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "nimro_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``nimro web``).
    ...
    # build path usable on a stock `py -m nimro_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_nimro_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (nimro web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``nimro update`` can unconditionally call ``nimro desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``nimro desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $NIMRO_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $NIMRO_HOME."""
    from nimro_constants import get_nimro_home
    return get_nimro_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``nimro update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Nimro.app/Contents/MacOS/Nimro"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Nimro.exe",
            release_dir / "win-ia32-unpacked" / "Nimro.exe",
            release_dir / "win-arm64-unpacked" / "Nimro.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "nimro",
            release_dir / "linux-unpacked" / "Nimro",
            release_dir / "linux-arm64-unpacked" / "nimro",
            release_dir / "linux-arm64-unpacked" / "Nimro",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Nimro.exe
...
#
# The desktop self-update chain (Desktop → nimro-setup --update →
# `nimro update` → `nimro desktop --build-only` → relaunch) rebuilds
# Nimro.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Nimro.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Nimro.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 nimro-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Nimro.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Nimro.exe from backup.")
        print("    Your existing version was kept and still works. Run `nimro desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `nimro desktop --force-build` to rebuild, or re-run the Nimro")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Nimro'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_nimro_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Nimro.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Nimro.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Nimro.exe: Access is
    ...
    an unrelated "Nimro" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Nimro.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Nimro is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Nimro.app/Contents/MacOS/Nimro  ->  app bundle = .../Nimro.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``nimro desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Nimro Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Nimro Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the NIMRO_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from nimro_logging import setup_logging as _setup_logging_gui
    ...
    # with_nimro_node_path() copies os.environ when called with no arg.
    env = with_nimro_node_path()
    if getattr(args, "fake_boot", False):
        env["NIMRO_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["NIMRO_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "nimro_root", None):
        env["NIMRO_DESKTOP_NIMRO_ROOT"] = str(Path(args.nimro_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["NIMRO_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["NIMRO_DESKTOP_CWD"] = os.getcwd()
    ...
    # `NIMRO_DESKTOP_DISABLE_GPU=... nimro desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "NIMRO_DESKTOP_DISABLE_GPU" not in os.environ:
        env["NIMRO_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  nimro gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Nimro-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → nimro-setup →
            # nimro update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `nimro update` path).
            nixos_env = with_nimro_node_path(_nixos_build_env())
            ...
            if not source_mode:
                ...
                # holds Nimro.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Nimro.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Nimro.exe, close any")
                    print("  running Nimro desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> nimro desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Nimro is
                ...
                # success on a Nimro.exe Windows cannot load (truncated PE from
    ...
    if source_mode:
        print("→ Launching Nimro Desktop from source build...")
    ...
    print(f"→ Launching packaged Nimro Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers extracted to nimro_cli/dashboard_procs.py
# (main.py decomposition, mechanical move). Re-exported so callers and test
# monkeypatches on nimro_cli.main.<name> keep resolving unchanged.
from nimro_cli.dashboard_procs import (  # noqa: F401
    _detect_concurrent_nimro_instances,
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            "nimro dashboard",
            "nimro_cli.main dashboard",
            "nimro_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "nimro serve",
            "nimro_cli.main serve",
            "nimro_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
_DASHBOARD_SYSTEMD_UNIT = "nimro-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Nimro installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``nimro-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/nimro-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but nimro launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``nimro update``.
    ...
    log_path = get_nimro_home() / "logs" / "dashboard-restart.log"
    try:
...
# =========================================================================
# Fork detection and upstream management for `nimro update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $NIMRO_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``nimro update``.
    ...
    protocol stream (``nimro acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `nimro update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``nimro.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_nimro_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from nimro.exe; applying package-only first aid, "
    ...
    try:
        from nimro_cli.managed_uv import ensure_uv
        ...
        if self_locked:
            print(
                "  Nimro is still running from the launcher that needs "
                "replacing. Close other Nimro windows, restart from a "
...
def _windows_running_nimro_launcher_locked() -> bool:
    """True when a venv ``nimro*.exe`` shim is this process or an ancestor.
    ...
    if scripts_dir is None:
        return False
    shims = _nimro_exe_shims(scripts_dir)
    if not shims:
        return False
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``nimro update`` is still progressing even if pip/uv itself is silent.
    """
...
def _venv_scripts_dir() -> Path | None:
    ...
    if not venv_dir.is_dir():
        return None
    from nimro_constants import venv_bin_dir

    scripts = venv_bin_dir(venv_dir, windows=_is_windows())
    return scripts if scripts.is_dir() else None


def _nimro_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_nimro_exe(
    ...
    when ``nimro update`` runs, ``nimro.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``nimro.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next nimro invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Nimro Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Nimro Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _nimro_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Nimro Desktop, exit other `nimro` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `nimro update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_nimro_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``nimro.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``nimro.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``nimro`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_nimro_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``nimro.exe.old.*`` left by prior updates.

    Called early on every nimro invocation. The .old files are unlocked once
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``nimro.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``nimro.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `nimro update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``nimro.exe`` / ``nimro-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_nimro_exe`` for the rationale.
    """
    ...
    # hours later inside ``nimro update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``nimro.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``nimro.exe`` shim is locked during ``nimro update``, or when uv/distlib
    skips a launcher write. The symptom is ``nimro-agent.exe`` and
    ``nimro-acp.exe`` present but ``nimro.exe`` missing, so ``nimro`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `nimro update --force` after "
            "closing other nimro processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m nimro_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``nimro update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``nimro.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``nimro`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `nimro update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `nimro update --force` after closing other nimro processes."
        )
        return
...
def _resolve_install_target_python(
    ...
    if env and "VIRTUAL_ENV" in env:
        from nimro_constants import venv_python_path
...
class _UpdateOutputStream:
    """Stream wrapper used during ``nimro update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.nimro/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``nimro update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``nimro update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.nimro/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``nimro update --gateway``) the update is already
    ...
    try:
        ...
        # nimro_cli.config.get_nimro_home to simulate setup failure.
        from nimro_cli.config import get_nimro_home as _get_nimro_home

        logs_dir = _get_nimro_home() / "logs"
...
def cmd_update(args):
    """Update Nimro Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Nimro Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `nimro update --branch=<x>` would actually pull.
    ...
    from nimro_cli.update_lock import (
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``nimro -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from nimro_cli.profiles import (
        list_profiles,
        create_profile,
        delete_profile,
        seed_profile_skills,
    ...
    if action is None:
        # Bare `nimro profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_nimro_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → nimro -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.nimro)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  nimro profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     nimro -p {name} chat")
                else:
        ...
        from nimro_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from nimro_constants import get_nimro_home as _hh
        ...
        from nimro_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `nimro profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → nimro -p {name}  ({wrapper})")
        print()
        ...
        from nimro_cli.profiles import profile_exists, validate_alias_name
        ...
        from nimro_cli.profiles import rename_profile
        ...
        from nimro_cli.profiles import export_profile
        ...
        from nimro_cli.profiles import import_profile
        ...
        import tempfile
        from nimro_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="nimro_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  nimro -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      nimro -p {plan.manifest.name} chat")
        ...
        from nimro_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `nimro profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"nimro -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from nimro_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("nimro_requires"):
            print(f"Requires:     Nimro {data['nimro_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from nimro_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.nimro_requires:
        print(f"  Requires: Nimro {mf.nimro_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No nimro dashboard processes running.")
        return 0

    print(f"{len(live)} nimro dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``nimro dashboard register`` for OAuth.
    ...
    try:
        from nimro_cli.web_server import should_require_auth
    ...
    try:
        from nimro_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `nimro dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "the dashboard again:\n"
            "    nimro dashboard register\n"
            "  It provisions a Nous Portal OAuth client and writes "
            "NIMRO_DASHBOARD_OAUTH_CLIENT_ID into ~/.nimro/.env for you.\n"
            "  Docs: https://nimro-agent.nousresearch.com/docs/"
    ...
    try:
        from nimro_cli.config import load_config, save_config
        from nimro_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from nimro_cli.windows_ssh_runtime import read_token
    ...
    # The Desktop client writes the token under $HOME/.nimro/desktop-ssh: a
    # literal "~/.nimro/desktop-ssh" in apps/desktop/electron/remote-lifecycle.ts
    # expanded against the account's $HOME, independent of NIMRO_HOME and the
    # active profile. Anchor validation to that same OS-home path, NOT to
    # get_nimro_home(): a non-default sticky profile (or any NIMRO_HOME pointing
    # elsewhere, e.g. a Docker /opt/data root) re-homes get_nimro_home() and
    # would otherwise reject every token the client legitimately wrote (#69551).
    token_root = _Path.home() / ".nimro" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``NIMRO_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``nimro dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No nimro dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `nimro update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with nimro serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with NIMRO_DESKTOP=1 plus
    # NIMRO_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # NIMRO_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `nimro dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited NIMRO_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # NIMRO_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (NIMRO_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets NIMRO_SERVE_HEADLESS below.
    if os.environ.get("NIMRO_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("NIMRO_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("NIMRO_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("NIMRO_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → NIMRO_HOME points into
    ...
    try:
        from nimro_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        and os.environ.get("NIMRO_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "nimro_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: NIMRO_HOME is explicitly pinned to the
        ...
        # NIMRO_HOME.  We must resolve the root explicitly instead of just
        # dropping NIMRO_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV NIMRO_HOME=/opt/data`), so an unset
        # NIMRO_HOME falls back to $HOME/.nimro = /opt/data/.nimro — an
        ...
        # update-button guard also misfires).  get_default_nimro_root()
        # returns the root for both layouts: ~/.nimro for a standard install
        ...
        try:
            from nimro_constants import get_default_nimro_root
            env["NIMRO_HOME"] = str(get_default_nimro_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop NIMRO_HOME) rather than block the reroute.
            env.pop("NIMRO_HOME", None)
    ...
    # the same logs directory as every other Nimro surface.
    try:
        from nimro_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["NIMRO_SERVE_HEADLESS"] = "1"
    elif "NIMRO_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["NIMRO_WEB_DIST"])
            if "NIMRO_WEB_DIST" in os.environ
            else PROJECT_ROOT / "nimro_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom NIMRO_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "NIMRO_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # NIMRO_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["NIMRO_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ NIMRO_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset NIMRO_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads NIMRO_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["NIMRO_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from NIMRO_WEB_DIST: {_dist_root}")
    ...
    try:
        from nimro_cli.mcp_startup import start_background_mcp_discovery
    ...
    from nimro_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from nimro_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from nimro_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from nimro_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from nimro_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Nimro log files."""
    from nimro_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Nimro command console."""
    from nimro_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from nimro_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``nimro -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``nimro_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``nimro -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``nimro`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``nimro <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``nimro photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("NIMRO_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that NIMRO_YOLO_MODE is set before ANY
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if not _is_termux_startup_environment():
        return False
    if os.environ.get("NIMRO_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    ...
    from nimro_cli._parser import build_top_level_parser
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["NIMRO_DEFER_AGENT_STARTUP"] = "1"
            os.environ["NIMRO_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["NIMRO_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `nimro --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from nimro_constants import get_nimro_home, display_nimro_home

        mem_dir = get_nimro_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_nimro_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_nimro_home()}/memories/\n")
    else:
        from nimro_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Nimro Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from nimro_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from nimro_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from nimro_cli.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: nimro-agent[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from nimro_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from nimro_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from nimro_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from nimro_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from nimro_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from nimro_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for nimro CLI."""
    # Cosmetic: make the process show up as 'nimro' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from nimro_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``nimro.exe.old.*`` quarantine files left by previous
    # ``nimro update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_nimro_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # If the checkout changed since the last launch (nimro update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``nimro update``
    ...
    # ``nimro skills install update``) merely defers recovery one launch;
    # under-matching (missing ``nimro -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in nimro_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from nimro_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from nimro_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://nimro-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `nimro model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.nimro/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://nimro-agent.nousresearch.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from nimro_cli import secrets_cli as _secrets_cli
    from nimro_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `nimro proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://nimro-agent.nousresearch.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from nimro_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``nimro proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from nimro_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in nimro_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in nimro_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in nimro_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `nimro whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in nimro_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from nimro_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in nimro_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in nimro_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in nimro_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in nimro_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in nimro_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in nimro_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from nimro_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from nimro_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from nimro_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in nimro_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in nimro_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in nimro_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in nimro_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in nimro_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in nimro_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in nimro_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.nimro/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo nimro uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from nimro_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in nimro_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: nimro_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from nimro_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in nimro_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in nimro_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in nimro_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in nimro_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in nimro_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from nimro_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in nimro_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``nimro --help``, ``nimro version``, ``nimro logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from nimro_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from nimro_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Nimro shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from nimro_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from nimro_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in nimro_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in nimro_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `nimro computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `nimro tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `nimro computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Nimro app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from nimro_cli.tools_config import install_cua_driver
        ...
        if action == "status":
            ...
            if path:
                version = ""
                try:
                    from nimro_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: nimro computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: nimro computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: nimro computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: nimro computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: nimro computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: nimro computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in nimro_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <nimro home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    # cmd_sessions lives in nimro_cli/sessions_cmd.py (main.py decomposition).
    ...
    # =========================================================================
    # insights command  (parser built in nimro_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in nimro_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in nimro_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in nimro_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in nimro_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in nimro_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in nimro_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in nimro_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Nimro-Setup.exe success screen tells users to
    # run `nimro desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in nimro_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in nimro_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in nimro_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``nimro -c Pokemon Agent Dev`` → ``nimro -c 'Pokemon Agent Dev'``
    ...
    from nimro_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'nimro -c model' where 'model' is consumed as the
    ...
    # --yolo: set NIMRO_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (nimro hooks list, cron
    ...
    # ``nimro egress start`` refusing when credential_source=bitwarden
