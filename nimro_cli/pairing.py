...
Usage:
    nimro pairing list              # Show all pending + approved users
    nimro pairing approve <platform> <request-id|code>  # Approve a pairing request
    nimro pairing revoke <platform> <user_id> # Revoke user access
    nimro pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle nimro pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: nimro pairing {list|approve|revoke|clear-pending}")
        print("Run 'nimro pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: nimro pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.nimro/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'nimro pairing list' to see pending requests.\n")
