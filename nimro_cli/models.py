...
Add, remove, or reorder entries here — both `nimro setup` and
`nimro` provider-selection will pick up the change automatically.
"""
...
from nimro_cli import __version__ as _NIMRO_VERSION
from nimro_cli.urllib_security import open_credentialed_url

logger = logging.getLogger(__name__)

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_NIMRO_USER_AGENT = f"nimro-cli/{_NIMRO_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `nimro model`
    flow without maintaining a separate static list.
    """
    from nimro_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $NIMRO_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Nimro-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $NIMRO_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Nimro refreshes the cache from
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see nimro_cli/model_setup_flows.py).
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Nimro that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Nimro that doesn't ship
...
def check_nous_free_tier(*, force_fresh: bool = False) -> bool:
    ...
    try:
        from nimro_cli.nous_account import get_nous_portal_account_info
...
def _nous_recommended_disk_path() -> "Path":
    ...
    return get_nimro_home() / "cache" / "nous_recommended_cache.json"
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    (``$NIMRO_HOME/cache/nous_recommended_cache.json``) as last-known-good.
...
# this list:  nimro model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `nimro model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `nimro model` TUI
...
#
# Some vendors expose several Nimro provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `nimro model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``nimro model``, the
...
# In-repo fallback for the model Nimro silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from nimro_cli.model_catalog import get_default_model_from_cache
...
# ``nimro model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in nimro_cli/web_server.py and
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``nimro auth add openai-codex`` without
    ``nimro model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``nimro model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    nimro-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from nimro_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — nimro-agent
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Nimro lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from nimro_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from nimro_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return nimro-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; nimro's picker expects
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    env_base = None
    try:
        from nimro_cli.auth import _nous_inference_env_override
    ...
    creds_base = ""
    try:
        from nimro_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``nimro model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from nimro_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from nimro_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Nimro' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``nimro_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Nimro should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``nimro auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.nimro/.env``.
    ...
    """
    try:
        from nimro_cli.auth import resolve_api_key_provider_credentials
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from nimro_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Nimro release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Nimro release). Falls back to the hardcoded catalog if no token
        # or the endpoint is unreachable.
        access_token = None
        try:
            from nimro_cli.auth import resolve_codex_runtime_credentials
    ...
    if normalized == "nous":
        ...
        try:
            from nimro_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Nimro release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # ``nimro model`` shows.
            from nimro_cli.providers import is_official_openai_host
...
#
# Cache strategy:
#   - One JSON file at $NIMRO_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from nimro_constants import get_nimro_home
    return get_nimro_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$NIMRO_HOME/auth.json`` and external
    ...
    # env var, so it must change the fingerprint too or `nimro config set
...
def copilot_default_headers(*, is_agent_turn: bool = True) -> dict[str, str]:
    ...
    """
    try:
        from nimro_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=is_agent_turn)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "NimroAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _NIMRO_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Nimro' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Nimro' native Anthropic adapter. The live catalog may
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _NIMRO_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"nimro-agent/{_NIMRO_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=nimro"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from nimro_constants import get_nimro_home
    return get_nimro_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `nimro moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Nimro will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Nimro cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Nimro actually ships.  (#46850)
            #
