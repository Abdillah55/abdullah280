"""Nimro-managed uv and Python runtime repair.

Nimro owns its own uv binary at ``$NIMRO_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$NIMRO_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Nimro
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.nimro-runtime/python``. A
...
from nimro_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".nimro-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    """Return the path where Nimro keeps *its* uv binary.

    ``$NIMRO_HOME/bin/uv`` on POSIX, ``$NIMRO_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Nimro-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Nimro keeps databases "
            "out of WAL mode on this SQLite build. The next `nimro update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``nimro update`` runs
    the call site from the *old*, already-imported ``nimro_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``nimro update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under NIMRO_HOME
    caches the last successful self-update time.
    """
    try:
        from nimro_constants import get_nimro_home

        stamp = get_nimro_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``nimro update`` so the managed copy stays current.
...
def _reload_nimro_constants():
    """Re-execute ``nimro_constants`` from disk and return the fresh module.

    ``nimro update`` imports ``nimro_constants`` from the OLD checkout,
    ...
    return importlib.reload(nimro_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Nimro generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("nimro_cli.main")
    ...
    if holders:
        ...
        return True, f"other Nimro processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Nimro-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``nimro update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``nimro update`` repairs the runtime (issue class:
...
def _install_uv(target: Path) -> None:
    ...
    ``$NIMRO_HOME/bin/`` instead of ``~/.local/bin/``.
    """
