...
MOA_MARKER_PREFIX = "__NIMRO_MOA_TURN_V1__"
...
def _coerce_float_or_none(value: Any) -> float | None:
    ...
    provider default applies', matching how a single-model Nimro agent
...
def resolve_moa_preset(config: Any, name: str | None = None) -> dict[str, Any]:
    ...
    if preset is None:
        ...
        raise MoAPresetNotFoundError(
            ...
            f"{available}. Run `nimro moa list`."
        )
...
def exact_moa_preset_name(config: Any, text: str) -> str | None:
    ...
    ``nimro_cli/model_switch.py``) to recognize a bare ``/model <preset>``
