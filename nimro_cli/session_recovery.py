"""Offline, non-destructive recovery for a damaged Nimro session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``nimro_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``nimro_state._backup_db_file``.
    """
    from nimro_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Nimro process using this profile and retry."
            )
