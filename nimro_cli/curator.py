"""CLI subcommand: `nimro curator <subcommand>`.
...
def _print_unmanaged_summary() -> None:
    ...
    print(
        ...
        "`nimro curator adopt <name>` hands one over"
    )
...
def _cmd_run(args) -> int:
    ...
    if not synchronous:
        print("llm pass running in background — check `nimro curator status` later")
    if dry:
        if synchronous:
            print(
                "dry-run: no changes applied. Read the report with "
                "`nimro curator status` and run `nimro curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `nimro curator adopt <name>`, "
          "or all with `nimro curator adopt --all-unmanaged`")
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`nimro curator unpin {args.skill}`"
        )
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.nimro/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`nimro curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.nimro/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.nimro/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.nimro/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by nimro_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="nimro curator")
    register_cli(parser)
