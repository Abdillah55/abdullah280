...
def _default_registry_path() -> Path:
    return get_nimro_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_nimro_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_nimro_home = expected_nimro_home if expected_nimro_home is not None else str(get_nimro_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["NIMRO_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("nimro_home") or "")
        if got_home and got_home != self.expected_nimro_home:
            raise RuntimeError(f"compute host NIMRO_HOME mismatch: {got_home} != {self.expected_nimro_home}")
