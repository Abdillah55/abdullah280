...
load_nimro_dotenv(
    nimro_home=_nimro_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.nimro/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_nimro_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("NIMRO_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("NIMRO_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("NIMRO_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
class _SlashWorker:
    """Persistent NimroCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Nimro agent → needs provider credentials.
        ...
        # gateway's launch NIMRO_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["NIMRO_HOME"] assignment.
        from tools.environments.local import build_subprocess_env
        env = build_subprocess_env(
            nimro_subprocess_env(inherit_credentials=True),
            scrub_secrets=False,
            inherit_profile_home=False,  # base already carries the HOME contract
            extra={"NIMRO_HOME": str(profile_home)} if profile_home else None,
        )
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from nimro_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from nimro_cli.lifecycle import invoke_hook
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `nimro --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("NIMRO_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `nimro --tui`, so it must NOT count as dead here (doing
...
def _reap_idle_sessions() -> None:
    ...
    try:
        from nimro_cli.mem_trim import trim_memory
...
def _reclaim_orphaned_leases() -> None:
    ...
    try:
        from nimro_cli.active_sessions import release_orphaned_leases
...
def _max_live_sessions() -> int:
    try:
        from nimro_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its NIMRO_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_nimro_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s NIMRO_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_nimro_home``).
...
def _profile_configured_cwd(profile_home: Path | None) -> str | None:
    ...
    if profile_home is None:
        return None
    try:
        from nimro_cli.config import _expand_env_vars, read_user_config_raw
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``nimro dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("NIMRO_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `nimro` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # NIMRO_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_nimro_home_override(profile_home)
                try:
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    * A terminal session (``nimro`` / ``nimro --tui`` / CLI) is started from a
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from nimro_cli.runtime_provider import canonical_custom_identity
    ...
    try:
        ...
        from nimro_state import is_disk_full_error
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from nimro_constants import translate_cwd_for_wsl_backend
...
def _load_dashboard_process_isolation_config(cfg: dict | None = None) -> dict[str, Any]:
    ...
    ``nimro_cli.config.DEFAULT_CONFIG``. Keep
...
def _load_cfg_raw() -> dict:
    ...
    try:
        ...
        # launch profile's _nimro_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_nimro_home_override()
        home = override if isinstance(override, str) and override else _nimro_home
...
def _load_cfg() -> dict:
    ...
    ``nimro_cli.config.load_config_readonly`` — managed-scope overlay and
...
def _apply_managed(cfg: dict) -> dict:
    ...
    nimro_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _save_cfg(cfg: dict):
    ...
    from nimro_cli.config import atomic_config_write

    path = _nimro_home / "config.yaml"
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read NIMRO_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # NIMRO_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["NIMRO_GATEWAY_SESSION"] = "1"
    os.environ["NIMRO_EXEC_ASK"] = "1"
...
def resolve_skin() -> dict:
    try:
        from nimro_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``nimro config set display.skin``) OR edited the active skin's colors in
...
def _watcher_home() -> Path:
    ...
    override = get_nimro_home_override()
    return Path(override if isinstance(override, str) and override else _nimro_home)
...
def _ensure_skin_watcher() -> None:
    ...
    Nimro activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
    ...
    threading.Thread(target=_loop, name="nimro-change-watcher", daemon=True).start()


def _resolve_model() -> str:
    env = (
        os.environ.get("NIMRO_MODEL", "")
        or os.environ.get("NIMRO_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from nimro_cli.models import get_preferred_silent_default_model
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``NIMRO_DESKTOP=1`` and ``NIMRO_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``NIMRO_DESKTOP_TERMINAL=1`` → "tui"
        (``nimro --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``nimro --tui``.)
    """
    if is_truthy_value(os.environ.get("NIMRO_DESKTOP")) and not is_truthy_value(
        os.environ.get("NIMRO_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads NIMRO_MODEL /
    NIMRO_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`nimro --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads NIMRO_MODEL /
    # NIMRO_INFERENCE_MODEL, which `nimro --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("NIMRO_TUI_PROVIDER", "").strip()
    ...
    try:
        from nimro_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("NIMRO_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_approval_mode() -> str:
    ...
    canonical ``nimro_cli.config.load_config`` path applies managed-scope
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`nimro_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("NIMRO_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("NIMRO_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Nimro): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `nimro --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] NIMRO_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from nimro_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown NIMRO_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in NIMRO_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid NIMRO_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _NIMRO_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off nimro-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid NIMRO_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    from nimro_cli.runtime_provider import resolve_runtime_provider
    ...
    cfg = None
    try:
        from nimro_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (NIMRO_MODEL /
    # NIMRO_INFERENCE_MODEL / NIMRO_TUI_PROVIDER / NIMRO_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `nimro --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    # NIMRO_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("NIMRO_DEV_CREDITS")):
        try:
...
def _project_info_for_cwd(cwd: str) -> dict | None:
    ...
    if not str(cwd or "").strip():
        return None
    try:
        from nimro_cli import projects_db as pdb
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if agent is not None and not (session or {}).get("_compute_host_active"):
        try:
            ...
            pass
        try:
            from nimro_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from nimro_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from nimro_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from nimro_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("NIMRO_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("NIMRO_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``NimroCLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses nimro_cli.mcp_startup; TUI stdio keeps
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("NIMRO_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("NIMRO_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("NIMRO_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("NIMRO_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped NIMRO_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / NIMRO_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The NIMRO_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
    return Path(profile_home) if profile_home else Path(_nimro_home)
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Nimro has to support both the legacy 8-row petdex atlas and the current
...
def _pet_gen_root():
    ...
    root = get_nimro_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("NIMRO_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
#
# Layout:  $NIMRO_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_nimro_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=NIMRO_SESSION_KEY`` (see
    ...
    # DB when NIMRO_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``NIMRO_UI_SESSION_ID`` as
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn NIMRO_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_nimro_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from nimro_cli.config import load_config as _tui_load_config
                ...
                try:
                    ...
                    def _thinking_should_play() -> bool:
                        if is_audio_output_active():
                            return False
                        try:
                            from nimro_cli.voice import is_continuous_active
            ...
            if (
                ...
                try:
                    ...
                    logger.warning("voice TTS skipped: nimro_cli.voice unavailable")
                except Exception as e:
            ...
            # Run while any profile-specific NIMRO_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile NIMRO_HOME
    override, so ``get_nimro_home()`` here would return the gateway's launch
    ...
    """
    profile_home = session.get("profile_home")
    base = Path(profile_home) if profile_home else _nimro_home
    return base / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".nimro" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.nimro/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.nimro/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                from nimro_cli.model_switch import parse_model_switch_args
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from nimro_cli.models import resolve_fast_mode_overrides
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("NIMRO_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["NIMRO_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("NIMRO_YOLO_MODE", None)
                    nv = "0"
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under NIMRO_HOME (~/.nimro by default) — config/sessions/skills,
    ...
    nimro_home = os.path.realpath(str(get_nimro_home()))

    return real == home or real == nimro_home or real.startswith(nimro_home + os.sep)
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    NIMRO_HOME may be an intentional prose/data workspace. The pre-Projects
    ...
    nimro_home = os.path.normcase(os.path.realpath(str(get_nimro_home())))
    return real == home or real == nimro_home
...
def _discover_repos_payload(
    ...
    repos even with zero nimro sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (nimro home subtree + bare
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Nimro sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Nimro
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `nimro` is interactive — use `/nimro chat -q …` or run `nimro` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`nimro setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`nimro gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`nimro sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`nimro config edit` needs $EDITOR in a real terminal"
    return None
...
def _model_picker_context(agent):
    ...
    from nimro_cli.inventory import load_picker_context
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Nimro" if role == "assistant" else role.title()
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("NIMRO_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("NIMRO_VOICE_TTS", "").strip() == "1"


def _any_session_running() -> bool:
    """True while any session's agent turn is in flight.

    Registered as the voice busy-probe (``nimro_cli.voice.set_voice_busy_probe``)
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    os.environ["NIMRO_VOICE"] = "0"
                    os.environ["NIMRO_VOICE_TTS"] = "0"
                    try:
                        from nimro_cli.voice import stop_continuous
...
def _speak_text_with_barge(text: str) -> None:
    """Speak *text* via nimro_cli.voice.speak_text with spoken barge-in.
    ...
    """
    from nimro_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["NIMRO_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["NIMRO_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from nimro_cli.voice import start_continuous
            ...
            # are tolerated.
            try:
                from nimro_cli.voice import set_voice_busy_probe
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        import nimro_cli.voice  # noqa: F401
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from nimro_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from nimro_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
