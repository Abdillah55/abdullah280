...
def _(rid, params: dict) -> dict:
    ...
    try:
        from nimro_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        from nimro_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from nimro_cli.inventory import build_models_payload
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `nimro model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.nimro/.env via the unified credential lifecycle
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from nimro_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
