...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from nimro_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_nimro_home), "display": display_nimro_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _nimro_home / "config.yaml"
        try:
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Nimro provider is configured.",
                },
            )
