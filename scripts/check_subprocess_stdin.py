...
When Nimro runs in TUI mode, the gateway child process communicates with
...
# ``get_nimro_home() / "plugins"`` (user) and ``./.nimro/plugins/`` (project,
# gated behind ``NIMRO_ENABLE_PROJECT_PLUGINS``) — see
# ``nimro_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``nimro_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import nimro_constants (this script
    ...
    # plugins in get_nimro_home()/plugins/ and project plugins in
    # ./.nimro/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_nimro_home() / "plugins"]
    if os.environ.get("NIMRO_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".nimro" / "plugins")
