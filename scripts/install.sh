...
# Python-driven tool session (e.g. Nimro terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/nimro-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/nimro-agent.git"
NIMRO_HOME="${NIMRO_HOME:-$HOME/.nimro}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${NIMRO_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$NIMRO_INSTALL_DIR"
...
#   code at /usr/local/lib/nimro-agent, command at /usr/local/bin/nimro,
#   data still at /root/.nimro (NIMRO_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$NIMRO_HOME/.no-bundled-skills so future"
            echo "                   'nimro update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Nimro.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.nimro/nimro-agent"
            echo "                   default (root, Linux): /usr/local/lib/nimro-agent"
            echo "  --nimro-home PATH  Data directory (default: ~/.nimro, or \$NIMRO_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Nimro installs the code under"
            echo "  /usr/local/lib/nimro-agent and links the command into"
            echo "  /usr/local/bin/nimro (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$NIMRO_HOME"
            echo "  (default /root/.nimro).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$NIMRO_HOME/nimro-agent are preserved in-place."
...
# intentional, but they leave the checkout dirty — which forces `nimro update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Nimro-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Nimro Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install nimro command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `nimro` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $NIMRO_HOME/nimro-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $NIMRO_HOME/nimro-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/nimro-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $NIMRO_HOME/nimro-agent — then preserve it)
#
# Always no-op when the user set --dir or $NIMRO_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in NIMRO_HOME.
    if is_termux; then
        INSTALL_DIR="$NIMRO_HOME/nimro-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$NIMRO_HOME/nimro-agent/.git" ]; then
            INSTALL_DIR="$NIMRO_HOME/nimro-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/nimro-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/nimro-agent"
        ...
        # /usr/local/bin/nimro wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/nimro"
        log_info "  Data:    $NIMRO_HOME (unchanged)"
...
# Point a Nimro-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $NIMRO_HOME/node/bin — which is NOT on
...
# Nimro's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Nimro-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$NIMRO_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$NIMRO_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$NIMRO_HOME/node/etc/npmrc"
}

get_nimro_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/nimro" ]; then
        echo "$link_dir/nimro"
    else
        echo "nimro"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://nimro-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Nimro owns its own uv at $NIMRO_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (nimro_cli/managed_uv.py) looks in the same
    # place, so install.sh and `nimro update` stay in sync.
    local _managed_uv="$NIMRO_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $NIMRO_HOME/bin ..."
    mkdir -p "$NIMRO_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/nimro-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/nimro-uv-installer.$$.sh")"
    ...
    # directly into $NIMRO_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$NIMRO_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# with the Nimro-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Nimro-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && node_satisfies_build "$(node --version)"; then
        ...
        log_warn "min-release-age-exclude) — installing Nimro-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Nimro-managed Node from a previous run over a too-old system one.
    if [ -x "$NIMRO_HOME/node/bin/node" ] && node_satisfies_build "$("$NIMRO_HOME/node/bin/node" --version)"; then
        export PATH="$NIMRO_HOME/node/bin:$PATH"
        log_success "Node.js $("$NIMRO_HOME/node/bin/node" --version) found (Nimro-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old (Nimro requires Node >=26) — installing Nimro-managed Node $NODE_VERSION..."
...
install_node() {
    ...
    log_info "Extracting to ~/.nimro/node/..."
    ...
    # Place into ~/.nimro/node/ and symlink binaries into the same bin dir
    # the nimro command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$NIMRO_HOME/node"
    mkdir -p "$NIMRO_HOME"
    mv "$extracted_dir" "$NIMRO_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$NIMRO_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$NIMRO_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$NIMRO_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$NIMRO_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.nimro/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Nimro install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Nimro Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `nimro update` path
                # (#4735).
                ...
                local stash_name
                stash_name="nimro-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``nimro update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Nimro behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up nimro command..."

    if [ "$USE_VENV" = true ]; then
        NIMRO_BIN="$INSTALL_DIR/venv/bin/python"
        NIMRO_ENTRYPOINT="$INSTALL_DIR/nimro"
    else
        NIMRO_BIN="$(which nimro 2>/dev/null || echo "")"
        if [ -z "$NIMRO_BIN" ]; then
            log_warn "nimro not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$NIMRO_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$NIMRO_ENTRYPOINT" ]; }; then
        log_warn "Nimro launcher prerequisites not found"
    ...
    # Create a user-facing shim for the nimro command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $NIMRO_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$NIMRO_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/nimro"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/nimro" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NIMRO_BIN" "$NIMRO_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/nimro" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/nimro"
    log_success "Installed nimro launcher → $command_link_display_dir/nimro"

    # Also expose `nimro-agent`. The `nimro-agent` console script declared in
    ...
    rm -f "$command_link_dir/nimro-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/nimro-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NIMRO_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/nimro-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NIMRO_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/nimro-agent"
    log_success "Installed nimro-agent launcher → $command_link_display_dir/nimro-agent"

    # Also expose `nimro-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `nimro-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Nimro as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/nimro-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/nimro-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$NIMRO_BIN" "$NIMRO_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/nimro-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/nimro-acp"
    log_success "Installed nimro-acp launcher → $command_link_display_dir/nimro-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "nimro command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v nimro' \
                >/dev/null 2>&1; then
        ...
        log_info "nimro not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Nimro Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Nimro Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Nimro Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so nimro works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.nimro directory structure (config at top level, code in subdir)
    mkdir -p "$NIMRO_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.nimro/.env (top level, easy to find)
    if [ ! -f "$NIMRO_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$NIMRO_HOME/.env"
            log_success "Created ~/.nimro/.env from template"
        else
            touch "$NIMRO_HOME/.env"
            log_success "Created ~/.nimro/.env"
        fi
    else
        log_info "~/.nimro/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$NIMRO_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.nimro/config.yaml (top level, easy to find)
    if [ ! -f "$NIMRO_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$NIMRO_HOME/config.yaml"
            log_success "Created ~/.nimro/config.yaml from template"
        fi
    else
        log_info "~/.nimro/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in nimro_cli/default_soul.py — the
    ...
    if [ ! -f "$NIMRO_HOME/SOUL.md" ]; then
        cat > "$NIMRO_HOME/SOUL.md" << 'SOUL_EOF'
You are Nimro Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.nimro/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.nimro/"

    # Seed bundled skills into ~/.nimro/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `nimro update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'nimro update'." \
            > "$NIMRO_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $NIMRO_HOME/.no-bundled-skills"
        log_info "  Future 'nimro update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.nimro/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.nimro/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$NIMRO_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$NIMRO_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.nimro/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Nimro Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Nimro will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Nimro Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (nimro --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `nimro update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'nimro setup' after install."
        return 0
    fi
    ...
    # Run nimro setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m nimro_cli.main setup < /dev/tty
    else
        python -m nimro_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$NIMRO_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Nimro to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$NIMRO_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'nimro whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                NIMRO_CMD="$(get_nimro_command_path)"
                $NIMRO_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'nimro whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'nimro gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $NIMRO_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $NIMRO_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: nimro gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: nimro gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $NIMRO_CMD gateway > "$NIMRO_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.nimro/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: nimro gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: nimro gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.nimro-bootstrap-complete, which tells the Nimro
    ...
    local marker_path="$INSTALL_DIR/.nimro-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $NIMRO_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $NIMRO_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $NIMRO_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}nimro${NC}              Start chatting"
    echo -e "   ${GREEN}nimro setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}nimro config${NC}       View/edit configuration"
    echo -e "   ${GREEN}nimro config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}nimro gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}nimro update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'nimro' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'nimro' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'nimro' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$NIMRO_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$NIMRO_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$NIMRO_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$NIMRO_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$NIMRO_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
# extract a tree MISSING the electron binary, so the `electron`->`Nimro` rename
...
# _purge_electron_build_cache() used by `nimro desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (Node >=26) and prepends the Nimro-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `nimro desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Nimro" ]; then
            app="$desktop_dir/release/linux-unpacked/Nimro"
        elif [ -x "$desktop_dir/release/linux-unpacked/nimro" ]; then
            app="$desktop_dir/release/linux-unpacked/nimro"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Nimro.app" \
            "$desktop_dir/release/mac/Nimro.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `nimro desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading NIMRO_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if NIMRO_HOME="$NIMRO_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from nimro_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `nimro update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Nimro-managed Node
            # provisioned during prerequisites/node-deps (at $NIMRO_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $NIMRO_HOME.
    # $NIMRO_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'nimro update' on this host install.
