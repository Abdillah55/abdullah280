#!/usr/bin/env bash
# Canonical test runner for nimro-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.nimro/...)
#
# Usage:
...
# (NIMRO_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.nimro/nimro-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.nimro/nimro-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${NIMRO_PYTHON:-}" ] && [ -x "$NIMRO_PYTHON" ] \
    && "$NIMRO_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: NIMRO_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `nimro` binary rather
  # than the devShell hook.
  PYTHON="$NIMRO_PYTHON"
  echo "▶ no local venv — using Nix dev venv via NIMRO_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and NIMRO_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.nimro/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.nimro"
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${NIMRO_RUN_SLOW_PET_TESTS:+NIMRO_RUN_SLOW_PET_TESTS="$NIMRO_RUN_SLOW_PET_TESTS"} \
  ${NIMRO_E2E_BROWSER:+NIMRO_E2E_BROWSER="$NIMRO_E2E_BROWSER"} \
