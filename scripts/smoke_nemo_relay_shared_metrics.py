"""Run a real Nimro CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_nimro_executable(nimro_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "nimro",
        Path(".venv") / "Scripts" / "nimro.exe",
    ):
        candidate = nimro_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("nimro")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Nimro executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Nimro source checkout containing .venv/bin/nimro",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated NIMRO_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["nimro.model_call.count"] != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['nimro.model_call.count']}"
        )
    expected_start = {
    ...
    if by_name["nimro.task_run.started"] != expected_start:
        raise AssertionError(
            f"Unexpected task start: {by_name['nimro.task_run.started']}"
        )
    terminal = by_name["nimro.task_run.finished"]
...
def _validate_package(outbox: Path, schema_path: Path) -> tuple[Path, dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError(
            "The Nimro development environment requires jsonschema"
        ) from exc
    ...
    if metrics["nimro.model_call.count"]["dimensions"] != {
        "call_role": "primary",
        "locality": "local",
        "model_family": "gpt",
        "outcome": "success",
        ...
        raise AssertionError(
            f"Unexpected model metric: {metrics['nimro.model_call.count']}"
        )
    terminal = metrics["nimro.task_run.finished"]
...
def main() -> int:
    args = _arguments()
    nimro_repo = args.nimro_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    nimro = _resolve_nimro_executable(nimro_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="nimro-relay-shared-metrics-"))
    home = root / "nimro-home"
    ...
    (root / "nimro.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "nimro.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Nimro exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    if not _ModelHandler.requests:
        raise AssertionError("Nimro did not call the local model endpoint")
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Nimro model request did not contain the prompt canary")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Nimro did not print the mock model response")
    ...
    package_path, package = _validate_package(
        ...
        / "observability"
        / "schemas"
        / "nimro.shared_metrics.v1.schema.json",
    )

    print("Nimro -> NeMo Relay shared-metrics smoke test passed")
