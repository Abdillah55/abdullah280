#!/usr/bin/env python3
"""Live test harness for Nimro Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any nimro imports.
ORIGINAL_HOME = os.environ.get("NIMRO_HOME")
ORIGINAL_AUTH = Path.home() / ".nimro" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.nimro/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.nimro/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="nimro_ts_live_"))
    nimro_home = home_dir / ".nimro"
    nimro_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, nimro_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".nimro" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, nimro_home / ".env")
        ...
        load_nimro_dotenv(nimro_home=str(Path.home() / ".nimro"))
    ...
    (nimro_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return nimro_home
...
def reset_module_state():
    """Drop cached modules so the new NIMRO_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "nimro_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["NIMRO_HOME"] = str(home)
...
def main():
    ...
    # Restore original NIMRO_HOME
    if ORIGINAL_HOME is not None:
        os.environ["NIMRO_HOME"] = ORIGINAL_HOME
    else:
