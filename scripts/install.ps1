# ============================================================================
# Nimro Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://nimro-agent.nousresearch.com/install.ps1)
#
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$NimroHome = $(if ($env:NIMRO_HOME) { $env:NIMRO_HOME } else { "$env:LOCALAPPDATA\nimro" }),
    [string]$InstallDir = $(if ($env:NIMRO_HOME) { "$env:NIMRO_HOME\nimro-agent" } else { "$env:LOCALAPPDATA\nimro\nimro-agent" }),
    ...
    # builds apps/desktop into a launchable Nimro.exe.
    #
    # Why opt-in:
    #   * Nimro-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Nimro.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Nimro.exe
    ...
    #     `nimro desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:NousResearch/nimro-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/nimro-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Nimro Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $NimroHome)) {
        New-Item -ItemType Directory -Force -Path $NimroHome | Out-Null
    }
    $envFile = Join-Path $NimroHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $NimroHome "node"
...
function Install-Uv {
    # Nimro owns its own uv at $NimroHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (nimro_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `nimro update` stay in sync.
    $managedUv = Join-Path $NimroHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $NimroHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $NimroHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $NimroHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $NimroHome "bin"
...
# Put the Nimro-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone nimro-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Nimro-managed Node tree's bundled npm into $NpmRange.
#
...
function Update-ManagedNpm {
    ...
    $tmpCwd = Join-Path $env:TEMP ("nimro-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and NIMRO_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Nimro-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Nimro setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Nimro uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\nimro\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Nimro needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Nimro copy of Git ourselves is predictable and
    ...
    ``NIMRO_GIT_BASH_PATH`` (User scope) so Nimro can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Nimro-managed PortableGit install instead..."
    }

    # Download PortableGit into $NimroHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $NimroHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Nimro features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Nimro needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``NIMRO_GIT_BASH_PATH`` (User env scope) so Nimro can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $NimroHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $NimroHome\git\usr\bin\bash.exe
    $candidates += "$NimroHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$NimroHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("NIMRO_GIT_BASH_PATH", $candidate, "User")
            $env:NIMRO_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set NIMRO_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Nimro may not find Git Bash."
    Write-Info "If needed, set NIMRO_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if (Get-Command node -ErrorAction SilentlyContinue) {
        ...
        Write-Warn "Node.js $version is too old (Nimro requires Node >=26)"
    }

    # Prefer a Nimro-managed Node from a previous run over a too-old system one.
    $managedNode = "$NimroHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$NimroHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$NimroHome\node"
        Write-Success "Node.js $version found (Nimro-managed)"
        ...
        Update-ManagedNpm "$NimroHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Nimro-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $NimroHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $NimroHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                if (Test-Path "$NimroHome\node") { Remove-Item -Recurse -Force "$NimroHome\node" }
                Move-Item $extractedDir.FullName "$NimroHome\node"
                ...
                $version = & "$NimroHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $NimroHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\nimro-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `nimro
                    # update` path (#4735).
                    ...
                    $stashName = "nimro-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. nimro-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``nimro update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Nimro behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running nimro processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/nimro-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/nimro-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/nimro-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\nimro-agent-$zipLabel.zip"
                ...
                if ($extractedDir) {
                    ...
                    # `nimro update` (see the notes at the shared clone-path
    ...
    # next `nimro update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Nimro-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running nimro process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running nimro processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Nimro_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (nimro.exe) plus its child tree.
            & taskkill /F /T /IM nimro.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM nimro.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m nimro_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not nimro.exe.
            ...
            # user (see nimro_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `nimro update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `nimro.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install nimro-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and nimro.exe broken with
    ...
    # here, not 30 seconds later when the user runs `nimro`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register nimro.exe in dist-info/RECORD but fail to
        ...
        # `nimro` command while nimro-agent.exe / nimro-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m nimro_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `nimro dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\nimro_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `nimro dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `nimro dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: nimro_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up nimro command..."
    
    if ($NoVenv) {
        $nimroBin = "$InstallDir"
    } else {
        $nimroBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so nimro is globally available
    # On Windows, the nimro.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$nimroBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$nimroBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $nimroBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set NIMRO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\nimro instead
    # of the Unix default ~/.nimro
    $currentNimroHome = [Environment]::GetEnvironmentVariable("NIMRO_HOME", "User")
    if (-not $currentNimroHome -or $currentNimroHome -ne $NimroHome) {
        [Environment]::SetEnvironmentVariable("NIMRO_HOME", $NimroHome, "User")
        Write-Success "Set NIMRO_HOME=$NimroHome"
    }
    $env:NIMRO_HOME = $NimroHome
    
    # Update current session
    $env:Path = "$nimroBin;$env:Path"
    
    Write-Success "nimro command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.nimro-bootstrap-complete which tells the Nimro
    ...
    $markerPath = Join-Path $InstallDir ".nimro-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the NIMRO_HOME directory structure ($NimroHome, default %LOCALAPPDATA%\nimro)
    New-Item -ItemType Directory -Force -Path "$NimroHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NimroHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NimroHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NimroHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NimroHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NimroHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NimroHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NimroHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NimroHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$NimroHome\.env"
    ...
    # Create config.yaml
    $configPath = "$NimroHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Nimro's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$NimroHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in nimro_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $NimroHome"
    
    # Seed bundled skills into $NimroHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $NimroHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $NimroHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$NimroHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $NimroHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Nimro-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'nimro setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\nimro-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\nimro-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
...
# the final `electron` -> `Nimro` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Nimro.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_nimro_desktop command
    # resolves apps/desktop/release/win-unpacked/Nimro.exe directly,
    ...
    # enforces the build floor (Node >=26) and prepends the Nimro-managed
    ...
    # The --dir mode produces an unpacked Nimro.exe in
    ...
    # are NOT trying to work around). The Nimro icon + product name are
    # stamped onto Nimro.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\nimro-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Nimro.exe",
        "$desktopDir\release\win-arm64-unpacked\Nimro.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Nimro.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Nimro icon + identity are stamped onto Nimro.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, nimro-agent#38216).
    ...
    #    Nimro.exe. We deliberately do NOT point them at `nimro desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Nimro.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Nimro.lnk')
        )
        ...
        # with the Nimro icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.nimro/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'nimro setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m nimro_cli.main setup
    } else {
        python -m nimro_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $nimroCmd = "$InstallDir\venv\Scripts\nimro.exe"
    if (-not (Test-Path $nimroCmd)) {
        $nimroCmd = "nimro"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$NimroHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'nimro whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: nimro gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$NimroHome\logs\gateway.log"
            Start-Process -FilePath $nimroCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$NimroHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: nimro gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: nimro gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$NimroHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$NimroHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NimroHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NimroHome\nimro-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   nimro              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   nimro setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   nimro config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   nimro config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   nimro gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   nimro update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Nimro repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Nimro-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Nimro to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://nimro-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
