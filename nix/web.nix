# nix/web.nix — Nimro Web Dashboard (Vite/React) frontend build
{ nimroNpmLib, ... }:
nimroNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @nimro/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../nimro_cli/web_dist for the
