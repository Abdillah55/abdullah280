# nix/desktop.nix — Nimro Desktop (Electron) app build + wrapper
#
# `nimroAgent` is the fully-built `.#default` package — it ships the
# `nimro` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `NIMRO_DESKTOP_NIMRO` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Nimro CLI").
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "nimro-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "nimro-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = nimroNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "nimro-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe nimroNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/nimro-desktop $out/bin
    cp -r ${renderer}/* $out/share/nimro-desktop/
    ...
    substituteInPlace $out/share/nimro-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/nimro-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # NIMRO_DESKTOP_NIMRO to the absolute path of the nix-built `nimro`
    # binary so the desktop's resolver step 4 ("existing Nimro CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/nimro-desktop \
      --add-flags "$out/share/nimro-desktop" \
      --set NIMRO_DESKTOP_NIMRO "${lib.getExe nimroAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Nimro Agent";
    homepage = "https://github.com/NousResearch/nimro-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "nimro-desktop";
  };
}
