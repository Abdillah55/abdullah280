...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Nimro derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            nimro-agent = prev.nimro-agent.overrideAttrs (_old: {
              NIMRO_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $NIMRO_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$NIMRO_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        nimro-agent = prev.nimro-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "nimro-agent-env" {
    nimro-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "nimro-agent-editable-env" {
    nimro-agent = dependency-groups;
  };
}
