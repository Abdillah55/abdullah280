# nix/packages.nix — Nimro Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      minimal = pkgs.callPackage ./nimro-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.nimroTui;
        web = full.nimroWeb;
        desktop = full.nimroDesktop;

        update-npm-lockfile = full.nimroNpmLib.updateNpmLockfile;
      };
    };
}
