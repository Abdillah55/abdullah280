# nix/nixosModules.nix — NixOS module for nimro-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: nimro runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $NIMRO_HOME/.env
# and read by nimro at startup — no container recreation needed for env changes.
#
# Tool resolution: the nimro wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.nimro-agent;
    effectivePackage =
    ...
    nimro-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-nimro-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "nimro-config-attrs";
      description = "Nimro YAML config (attrset), merged deeply via lib.recursiveUpdate.";
      check = builtins.isAttrs;
      merge = _loc: defs: lib.foldl' lib.recursiveUpdate { } (map (d: d.value) defs);
    };

    # Generate config.yaml from Nix attrset (YAML is a superset of JSON).
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — nimro
    ...
    generatedConfigFile = pkgs.writeText "nimro-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # NIMRO_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "nimro-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'NIMRO_DOC_EOF'\n${value}\nNIMRO_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "nimro-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/nimro";
    ...
    # nimro user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "nimro-container-entrypoint" ''
      set -eu

      NIMRO_UID="''${NIMRO_UID:?NIMRO_UID must be set}"
      NIMRO_GID="''${NIMRO_GID:?NIMRO_GID must be set}"

      # ── Group: ensure a group with GID=$NIMRO_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$NIMRO_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="nimro"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$NIMRO_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$NIMRO_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$NIMRO_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$NIMRO_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="nimro"
        TARGET_HOME="/home/nimro"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$NIMRO_UID" -g "$NIMRO_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$NIMRO_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$NIMRO_UID:$NIMRO_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure NIMRO_HOME is owned by the target user.
      ...
      if [ -n "''${NIMRO_HOME:-}" ] && [ -d "$NIMRO_HOME" ]; then
        find "$NIMRO_HOME" \! -user "$NIMRO_UID" -exec chown "$NIMRO_UID:$NIMRO_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/nimro-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/nimro-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/nimro ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/nimro
        chmod 0440 /etc/sudoers.d/nimro
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$NIMRO_UID" --regid="$NIMRO_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $NIMRO_HOME/.env.
    ...
    # Default: /var/lib/nimro/workspace → /data/workspace.
  ...
  in {
    options.services.nimro-agent = with lib; {
      enable = mkEnableOption "Nimro Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = nimro-agent;
        description = "The nimro-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/nimro";
        description = "State directory. Contains .nimro/ subdir (NIMRO_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Nimro config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $NIMRO_HOME/.env at activation time.
          Nimro reads this file on every startup via load_nimro_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $NIMRO_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $NIMRO_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `nimro gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the nimro user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the nimro plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Nimro discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          nimro_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as nimro (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in nimro-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the nimro CLI to environment.systemPackages and export
          NIMRO_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.nimro symlink to the service
            stateDir. These users are automatically added to the nimro group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.nimro-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the nimro CLI to system PATH and export NIMRO_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.nimro/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.NIMRO_HOME = "${cfg.stateDir}/.nimro";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.nimro-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }

      # ── Warnings ──────────────────────────────────────────────────────
      # ── Per-user profile for extraPackages ───────────────────────────
      # Wire extraPackages into the nimro user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.nimro-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed nimro
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure nimro is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.nimro        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nimro/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nimro/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nimro/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nimro/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.nimro/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."nimro-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.nimro
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.nimro ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.nimro ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.nimro -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.nimro/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.nimro/$_subdir"
            chmod 2770 "${cfg.stateDir}/.nimro/$_subdir"
            find "${cfg.stateDir}/.nimro/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # nimro-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.nimro/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.nimro/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nimro/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.nimro/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.nimro/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nimro/.managed
          chmod 0644 ${cfg.stateDir}/.nimro/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.nimro/.container-mode <<'NIMRO_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    nimro_bin=${containerDataDir}/current-package/bin/nimro
    NIMRO_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nimro/.container-mode
            chmod 0644 ${cfg.stateDir}/.nimro/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.nimro/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nimro";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.nimro" ]; then
                  rm -f "${symlinkPath}"
                  echo "nimro-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.nimro -> stateDir/.nimro for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.nimro";
                target = "${cfg.stateDir}/.nimro";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "nimro-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nimro/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.nimro/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.nimro/auth.json
              fi
            ''}
          ''}
          ...
          # Nimro reads $NIMRO_HOME/.env at startup via load_nimro_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.nimro/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'NIMRO_NIX_ENV_EOF'
    ${envFileContent}
    NIMRO_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.nimro/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.nimro/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.nimro/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.nimro-agent = {
          description = "Nimro Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $NIMRO_HOME/.env by the activation script. load_nimro_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/nimro"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the nimro group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.nimro-agent = {
          description = "Nimro Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              NIMRO_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              NIMRO_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env NIMRO_UID="$NIMRO_UID" \
                --env NIMRO_GID="$NIMRO_GID" \
                --env NIMRO_HOME=${containerDataDir}/.nimro \
                --env NIMRO_MANAGED=true \
                --env HOME=${containerHomeDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/nimro gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
