...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      nimro-agent = self'.packages.default;
      nimroVenv = nimro-agent.nimroVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "nimro-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${nimroVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "nimro-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "nimro-build-package" { } ''
          echo "PASS: package built at ${nimro-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "nimro-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "nimro-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${nimro-agent}/bin/nimro || (echo "FAIL: nimro binary missing"; exit 1)
          test -x ${nimro-agent}/bin/nimro-agent || (echo "FAIL: nimro-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${nimro-agent}/bin/nimro version 2>&1 | grep -qi "nimro" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "nimro-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in nimro nimro-agent nimro-acp; do
            test -x ${nimro-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "nimro-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking nimro --help ==="
          ${nimro-agent}/bin/nimro --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${nimro-agent}/bin/nimro --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "nimro-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${nimro-agent}/share/nimro-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${nimro-agent}/share/nimro-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "NIMRO_BUNDLED_SKILLS" ${nimro-agent}/bin/nimro || \
            (echo "FAIL: NIMRO_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: NIMRO_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${nimro-agent}/share/nimro-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${nimro-agent}/share/nimro-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "NIMRO_OPTIONAL_SKILLS" ${nimro-agent}/bin/nimro || \
            (echo "FAIL: NIMRO_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, NIMRO_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "nimro-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${nimro-agent}/share/nimro-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${nimro-agent}/share/nimro-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "NIMRO_BUNDLED_PLUGINS" ${nimro-agent}/bin/nimro || \
            (echo "FAIL: NIMRO_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: NIMRO_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "nimro-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${nimro-agent}/share/nimro-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${nimro-agent}/share/nimro-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${nimro-agent}/share/nimro-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "NIMRO_BUNDLED_LOCALES" ${nimro-agent}/bin/nimro || \
            (echo "FAIL: NIMRO_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: NIMRO_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + NIMRO_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && NIMRO_BUNDLED_LOCALES=${nimro-agent}/share/nimro-agent/locales \
            ${nimroVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with NIMRO_BUNDLED_LOCALES set"; exit 1)
        ...
        # NIMRO_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "nimro-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${nimro-agent}/share/nimro-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${nimro-agent}/share/nimro-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "NIMRO_OPTIONAL_MCPS" ${nimro-agent}/bin/nimro || \
            (echo "FAIL: NIMRO_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: NIMRO_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${nimro-agent}/bin/nimro mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: nimro mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "nimro-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${nimro-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${nimro-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "NIMRO_TUI_DIR" ${nimro-agent}/bin/nimro || \
            (echo "FAIL: NIMRO_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: NIMRO_TUI_DIR set in wrapper"
        ...
        # Verify NIMRO_NODE is set in wrapper and points to Node 26+
        # (Nimro pins its toolchain to Node 26 everywhere)
        nimro-node = pkgs.runCommand "nimro-node-version" { } ''
          set -e
          echo "=== Checking NIMRO_NODE in wrapper ==="
          grep -q "NIMRO_NODE" ${nimro-agent}/bin/nimro || \
            (echo "FAIL: NIMRO_NODE not set in wrapper"; exit 1)
          echo "PASS: NIMRO_NODE present in wrapper"

          NIMRO_NODE=$(sed -n "s/^export NIMRO_NODE='\(.*\)'/\1/p" ${nimro-agent}/bin/nimro)
          test -x "$NIMRO_NODE" || (echo "FAIL: NIMRO_NODE=$NIMRO_NODE not executable"; exit 1)
          echo "PASS: NIMRO_NODE executable at $NIMRO_NODE"

          NODE_MAJOR=$("$NIMRO_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Nimro requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All NIMRO_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify NIMRO_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "nimro-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(NIMRO_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking NIMRO_MANAGED guards ==="
          check_blocked "config set" ${nimro-agent}/bin/nimro config set model foo
          check_blocked "config edit" ${nimro-agent}/bin/nimro config edit
        ...
        extra-python-packages = let
          ...
          nimroWithExtra = nimro-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "nimro-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${nimroWithExtra}/bin/nimro || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${nimroWithExtra}/bin/nimro || \
          ...
          if grep -q "PYTHONPATH" ${nimro-agent}/bin/nimro; then
        ...
        extra-dependency-groups = let
          nimroWithGroups = nimro-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "nimro-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${nimroWithGroups}"
          echo "venv: ${nimroWithGroups.nimroVenv}"
        ...
        messaging-variant = pkgs.runCommand "nimro-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.nimroVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "nimro-config-roundtrip" {
          ...
          merge_and_load() {
            local nimro_home="$1"
            export NIMRO_HOME="$nimro_home"
            ${configMergeScript} ${nixSettings} "$nimro_home/config.yaml"
            ${nimroVenv}/bin/python3 -c '
import json, sys
