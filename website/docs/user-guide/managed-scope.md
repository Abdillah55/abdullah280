...
`~/.nimro/config.yaml`, `~/.nimro/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/nimro`:

```text
/etc/nimro/
├── config.yaml     # managed config layer (wins over ~/.nimro/config.yaml)
└── .env            # managed env layer (wins over ~/.nimro/.env + shell)
```
...
The location can be relocated with the `NIMRO_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `NIMRO_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Nimro.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export NIMRO_MANAGED_DIR=/opt/org/nimro-policy
```

:::warning
A user who can set `NIMRO_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `nimro doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/nimro/config.yaml` (managed) | `/etc/nimro/.env` (managed) |
| 2 | `~/.nimro/config.yaml` (user) | `~/.nimro/.env` (user) |
...
```bash
nimro config        # shows a header naming the managed source + the pinned keys
nimro doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Nimro refuses and names the source:

```bash
$ nimro config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/nimro/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `nimro config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/nimro

# Pin some config values for every user on this machine
sudo tee /etc/nimro/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/nimro/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/nimro
sudo chmod 0644 /etc/nimro/config.yaml /etc/nimro/.env
```

Changes take effect on the next Nimro start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`nimro doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Nimro as `root`), managed scope is advisory.
