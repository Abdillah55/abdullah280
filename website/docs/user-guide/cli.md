...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Nimro Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Nimro Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `nimro setup --portal` — and you're ready to `nimro chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Nimro also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `nimro --tui` — see the [TUI](tui.md) guide.
:::
...
nimro chat --model "anthropic/claude-sonnet-4"

# With a specific provider
nimro chat --provider nous        # Use Nous Portal
nimro chat --provider openrouter  # Force OpenRouter
...
nimro chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
nimro --continue             # Resume the most recent CLI session (-c)
nimro --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
nimro chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
nimro -w                         # Interactive mode in worktree
nimro -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Nimro CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Nimro CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `NIMRO_YOLO_MODE` is on (either `nimro --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Nimro to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.nimro/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Nimro supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Nimro replies |
...
```bash
nimro -s nimro-agent-dev,github-auth
nimro chat -s github-pr-workflow -s github-auth
```

Nimro loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.nimro/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.nimro/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Nimro..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Nimro recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Nimro — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Nimro is working, Nimro prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Nimro to the background — just like any terminal process. The shell prints a confirmation:

```
Nimro Agent has been suspended. Run `fg` to bring Nimro Agent back.
```
...
Resume this session with:
  nimro --resume 20260225_143052_a1b2c3
...
```bash
nimro --continue                          # Resume the most recent CLI session
nimro -c                                  # Short form
nimro -c "my project"                     # Resume a named session (latest in lineage)
nimro --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
nimro --resume "refactoring auth"         # Resume by title
nimro -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `nimro sessions rename <id> <title>` from the command line. Use `nimro sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Nimro's SQLite state database under `~/.nimro/state.db`. The database keeps:
...
Nimro immediately confirms the task and gives you back the prompt:
