...
```
Nimro Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${NIMRO_HOME:-$HOME/.nimro}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Nimro config (if missing)
...
3. **Restart Nimro session** to pick up the new MCP server
