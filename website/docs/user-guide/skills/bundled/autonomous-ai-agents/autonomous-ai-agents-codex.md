...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`nimro-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-nimro-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Nimro terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Nimro itself, `model.provider: openai-codex` uses Nimro-managed Codex
OAuth from `~/.nimro/auth.json` after `nimro auth add openai-codex`. For the
...
## Nimro Gateway Caveat

When invoking the Codex CLI from a Nimro gateway/service context (for example,
