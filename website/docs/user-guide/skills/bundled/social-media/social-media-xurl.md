...
| Version | `1.1.3` |
| Author | xdevplatform + openclaw + Nimro Agent |
| License | MIT |
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Nimro tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Nimro Docker layout, `/opt/data` is `NIMRO_HOME`, but Nimro tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Nimro-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Nimro tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Nimro subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Nimro adaptation: reformatted for Nimro skill conventions; safety guardrails preserved verbatim.
