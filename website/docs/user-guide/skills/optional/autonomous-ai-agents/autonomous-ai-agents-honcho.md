---
title: "Honcho — Configure and troubleshoot Honcho memory for Nimro"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Nimro"
---
...
Configure and troubleshoot Honcho memory for Nimro.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nimro skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Nimro

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Nimro profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/nimro#running-honcho-locally-with-nimro

### Verify

```bash
nimro honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Nimro profile's AI peer
...
Honcho models conversations as interactions between **peers**. Nimro creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Nimro instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Nimro run |
| `global` | Single session across all directories |

Manual override: `nimro honcho map my-project-name`
...
Each Nimro profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block nimro.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `nimro.coder` host block in `honcho.json`
...
```bash
nimro honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Nimro when Honcho memory is active.
...
Config file: `$NIMRO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `nimro honcho setup`. Ensure `memory.provider: honcho` is in `~/.nimro/config.yaml`.

### Memory not persisting across sessions
Check `nimro honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `nimro profile create <name> --clone`. For existing profiles: `nimro honcho sync`.
...
| Command | Description |
|---------|-------------|
| `nimro honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `nimro honcho status` | Show resolved config, connection test, peer info for active profile |
| `nimro honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `nimro honcho disable` | Disable Honcho for the active profile |
| `nimro honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `nimro honcho peers` | Show peer identities across all profiles |
| `nimro honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `nimro honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `nimro honcho sessions` | List known directory-to-session-name mappings |
| `nimro honcho map <name>` | Map current working directory to a Honcho session name |
| `nimro honcho identity` | Seed AI peer identity or show both peer representations |
| `nimro honcho sync` | Create host blocks for all Nimro profiles that don't have one yet |
| `nimro honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Nimro + Honcho |
| `nimro memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `nimro memory status` | Show active memory provider and config |
| `nimro memory off` | Disable external memory provider |
