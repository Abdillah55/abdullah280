...
| | |
|---|---|
| Source | Optional — install with `nimro skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Nimro Agent |
| License | MIT |
...
import upstream classes into Nimro itself.
...
```bash
mkdir -p ~/.nimro/cache/darwinian-evolver && cd ~/.nimro/cache/darwinian-evolver
...
```bash
cd ~/.nimro/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.nimro/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.nimro/skills/research/darwinian-evolver
DE_DIR=~/.nimro/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Nimro core.
   Custom driver scripts under `~/.nimro/skills/...` are user-side and fine.
