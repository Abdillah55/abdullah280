...
| | |
|---|---|
| Source | Optional — install with `nimro skills install official/mcp/fastmcp` |
...
| Related skills | [`nimro-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-nimro-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Nimro or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Nimro. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.nimro/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.nimro/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Nimro integration, either:

- configure the server in `~/.nimro/config.yaml` using the `native-mcp` skill, or
...
### Nimro cannot see the deployed server

The server-building part may be correct while the Nimro config is not. Load the `native-mcp` skill and configure the server in `~/.nimro/config.yaml`, then restart Nimro.
