...
| | |
|---|---|
| Source | Optional — install with `nimro skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Nimro Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Nimro terminal calls (desktop app flow only)
...
### Service Account (recommended for Nimro)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${NIMRO_HOME:-~/.nimro}/.env` (the skill will prompt for this on first load).
...
## Nimro Execution Pattern (desktop app flow)

Nimro terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/nimro-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/nimro-op.sock"
