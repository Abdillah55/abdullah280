---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Nimro"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Nimro"
---
...
Import an OpenClaw setup (memories, skills) into Nimro.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `nimro skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Nimro Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Nimro`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Nimro Migration

Use this skill when a user wants to move their OpenClaw setup into Nimro Agent with minimal manual cleanup.
...
```bash
nimro claw migrate              # Full interactive migration
...
nimro claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `nimro setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_nimro.py` to:

- import `SOUL.md` into the Nimro home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Nimro memory entries
- merge OpenClaw command approval patterns into Nimro `command_allowlist`
- migrate Nimro-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Nimro working-directory configuration
- copy OpenClaw skills into `~/.nimro/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Nimro workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.nimro/tts/`
- archive non-secret docs that do not have a direct Nimro destination
...
- `scripts/openclaw_to_nimro.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.nimro/skills/migration/openclaw-migration/scripts/openclaw_to_nimro.py`

Do not guess a shorter path like `~/.nimro/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.nimro/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Nimro-compatible secrets, currently:
...
Nimro CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Nimro. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Nimro workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Nimro skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.nimro/skills/migration/openclaw-migration/scripts/openclaw_to_nimro.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.nimro/skills/migration/openclaw-migration/scripts/openclaw_to_nimro.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.nimro/skills/migration/openclaw-migration/scripts/openclaw_to_nimro.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.nimro/skills/migration/openclaw-migration/scripts/openclaw_to_nimro.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.nimro/skills/migration/openclaw-migration/scripts/openclaw_to_nimro.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.nimro/skills/migration/openclaw-migration/scripts/openclaw_to_nimro.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Nimro unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Nimro targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Nimro destination. Unsupported auth blobs must still be reported as skipped.
...
- Nimro persona state imported
- Nimro memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.nimro/skills/openclaw-imports/`
