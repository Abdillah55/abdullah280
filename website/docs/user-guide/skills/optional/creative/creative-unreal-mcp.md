...
| | |
|---|---|
| Source | Optional — install with `nimro skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Nimro MCP catalog. The
...
Two halves, in this order: the editor side must be up before Nimro connects.
...
### One-time, Nimro side
    ...
    nimro mcp install unreal-engine
...
`~/.nimro/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Nimro — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Nimro connects
...
2. Start the Nimro session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Nimro
they appear as:
...
then happens at `nimro mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Nimro runs batched calls
...
- **Start order matters.** Editor + server up first, then the Nimro
...
- **Timeouts on long operations.** Nimro' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.nimro/config.yaml` for
