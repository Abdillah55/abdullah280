...
| | |
|---|---|
| Source | Optional — install with `nimro skills install official/email/agentmail` |
...
Add to `~/.nimro/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `nimro-agent`)
   - The agent gets address: `nimro-agent@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "nimro-outreach")
...
```
nimro --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
