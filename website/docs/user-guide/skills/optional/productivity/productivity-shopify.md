...
| | |
|---|---|
| Source | Optional — install with `nimro skills install official/productivity/shopify` |
...
4. Save to `${NIMRO_HOME:-~/.nimro}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Nimro","productType":"Apparel","tags":["test"]}}'
```
