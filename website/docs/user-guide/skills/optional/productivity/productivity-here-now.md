...
| | |
|---|---|
| Source | Optional — install with `nimro skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${NIMRO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${NIMRO_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${NIMRO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client nimro
```
...
```bash
PUBLISH="${NIMRO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client nimro
```
...
```bash
DRIVE="${NIMRO_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `nimro`)    |
