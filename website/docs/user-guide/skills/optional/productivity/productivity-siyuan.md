...
| | |
|---|---|
| Source | Optional — install with `nimro skills install official/productivity/siyuan` |
...
3. Store it in `${NIMRO_HOME:-~/.nimro}/.env`:
   ```
...
```yaml
# In ~/.nimro/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
