...
| | |
|---|---|
| Source | Optional — install with `nimro skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Nimro Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${NIMRO_HOME:-~/.nimro}/.env`:
...
```bash
CANVAS="python $NIMRO_HOME/skills/productivity/canvas/scripts/canvas_api.py"
