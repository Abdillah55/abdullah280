...
Nimro Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Nimro checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.nimro/config.yaml`:
...
| `timeout` | `300` | Seconds Nimro waits for an approval reply before timing out. |
...
1. **CLI flag**: Start a session with `nimro --yolo` or `nimro chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `NIMRO_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `NIMRO_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Nimro shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Nimro refuses to run them **regardless** of:
...
Configure the timeout in `~/.nimro/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.nimro/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.nimro/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` nimro/gateway | Self-termination prevention |
...
The `NIMRO_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.nimro/config.yaml`:
...
:::tip
Use `nimro config edit` to review or remove patterns from your permanent allowlist.
:::

### Mining Approval History (`nimro approvals suggest`)
...
```bash
nimro approvals suggest            # dry run — prints a numbered proposal
nimro approvals suggest --apply 1,3  # merge picks into command_allowlist
nimro approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.nimro/state.db`) for
...
Before `write_file` or `patch` touches disk, Nimro checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](./configuration.md#file-mutation-verifier) over the assistant's closing summary.

### Protected paths (always blocked)

These categories are always denied, even when `NIMRO_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/`, `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| Nimro credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under NIMRO_HOME (active profile and global root) |
| Project secret files | `.env`, `.env.local`, `.env.production`, `.envrc` anywhere on disk |

Sensitive paths inside the safe root are still blocked — pointing `NIMRO_WRITE_SAFE_ROOT` at `$HOME` does not allow writing `~/.ssh/id_rsa`.

Safe-root violations return `Write denied: '…' is outside NIMRO_WRITE_SAFE_ROOT (…)`. Credential-path blocks use `Write denied: '…' is a protected system/credential file.`

### NIMRO_WRITE_SAFE_ROOT (optional sandbox)
...
- Set automatically in the [official Docker image](https://github.com/NousResearch/nimro-agent) (`NIMRO_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.nimro/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.nimro/cron/jobs.json`, profile skills, or other Nimro state outside that prefix

To allow both a workspace and Nimro home:

```bash
export NIMRO_WRITE_SAFE_ROOT=/path/to/project:/home/you/.nimro
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [NIMRO_WRITE_SAFE_ROOT](../reference/environment-variables.md#nimro_write_safe_root).

### Cron and other Nimro state

Do not ask the agent to `patch` `~/.nimro/cron/jobs.json` directly. Use the `cronjob` tool, [`nimro cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Nimro control files when write safety blocks direct edits.
...
When running the messaging gateway, Nimro controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.nimro/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.nimro/.env to allow open access,
...
For more flexible authorization, Nimro includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `nimro pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.nimro/config.yaml`:
...
nimro pairing approve telegram ABC12DEF

# Revoke a user's access
nimro pairing revoke telegram 123456789

# Clear all pending codes
nimro pairing clear-pending
```

:::tip Docker users: run pairing commands as the `nimro` user
The official Docker image runs the gateway as the unprivileged `nimro` user
...
```bash
docker exec -u nimro nimro-agent nimro pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/NousResearch/nimro-agent/issues/10270
:::

**Storage:** Pairing data is stored in `~/.nimro/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Nimro applies strict security hardening to every container.
...
Container resources are configurable in `~/.nimro/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.nimro/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `NIMRO_HOME`. Skills declare these in frontmatter:
...
When loaded, Nimro checks if these files exist in the active profile's `NIMRO_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Nimro infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Nimro infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Nimro integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Nimro under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.nimro/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.nimro/logs/` for unauthorized access attempts
10. **Keep updated** — run `nimro update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.nimro/.env`:
...
TERMINAL_SSH_USER=nimro
TERMINAL_SSH_KEY=~/.ssh/nimro_agent_key
```
...
Nimro ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `nimro_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `nimro doctor` for the full remediation.
- **`nimro doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Nimro installs these **lazily** on first use rather than eagerly under `nimro-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `nimro tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `nimro tools`.
