...
Connect Nimro to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Nimro — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Nimro runs it in a
...
nimro photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Nimro Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.nimro/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.nimro/.env`
...
(device token, dashboard project id) lives in `~/.nimro/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Nimro
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Nimro replies with a pairing code. Approve it with:

```bash
nimro pairing approve photon <CODE>
```

Use `nimro pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.nimro/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.nimro/.env`):
...
By default Nimro responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Nimro` and
`@Nimro agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Nimro will reply.
...
assigned iMessage line Nimro uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run nimro photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `nimro photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `nimro photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `nimro photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Nimro sends images, voice
  ...
  media.
- **Native polls are supported.** Nimro sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Nimro sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `nimro send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<nimro-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Nimro is set
...
| `PHOTON_MENTION_PATTERNS` | Nimro wake words  | JSON list / comma / newline regex patterns for group mentions |
