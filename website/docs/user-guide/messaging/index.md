...
description: "Chat with Nimro from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Nimro from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Nimro](/guides/use-voice-mode-with-nimro).
...
```mermaid
flowchart TB
    subgraph Gateway["Nimro Gateway"]
...
For group chats, hooks, and automation flows, Nimro supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Nimro keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Nimro does not hide failures just because the text resembles a silence token.
...
```bash
nimro gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
nimro gateway              # Run in foreground
nimro gateway setup        # Configure messaging platforms interactively
nimro gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo nimro gateway install --system   # Linux only: install a boot-time system service
nimro gateway start        # Start the default service
nimro gateway stop         # Stop the default service
nimro gateway status       # Check default service status
nimro gateway status --system         # Linux only: inspect the system service explicitly
```
...
```bash
nimro gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Nimro sends heartbeats
...
| `/update` | Update Nimro Agent to the latest version |
...
with the `session_reset` section in `~/.nimro/config.yaml`:
...
Configure per-platform overrides in `~/.nimro/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.nimro/gateway-config.yaml`:
...
nimro pairing approve telegram XKGH5N7P

# Other pairing commands:
nimro pairing list          # View pending + approved users
nimro pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Nimro appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.nimro/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.nimro/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `NIMRO_HOME`:

- `~/.nimro/status_phrases.yaml` or any `*.yaml` in `~/.nimro/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to NIMRO_HOME
...
Off by default. When enabled, Nimro prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.nimro/config.yaml`:
...
```bash
NIMRO_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
nimro gateway install               # Install as user service
nimro gateway start                 # Start the service
nimro gateway stop                  # Stop the service
nimro gateway status                # Check status
journalctl --user -u nimro-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo nimro gateway install --system
sudo nimro gateway start --system
sudo nimro gateway status --system
journalctl -u nimro-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Nimro installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit nimro-gateway` (or `sudo systemctl edit nimro-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `nimro update`. When `nimro update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart nimro-gateway` command (it never blocks on an interactive password prompt).
...
```bash
nimro gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `nimro update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo nimro update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/nimro-gateway`:

```
nimro ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed nimro-gateway*, /usr/bin/systemctl --no-ask-password start nimro-gateway*, /usr/bin/systemctl --no-ask-password restart nimro-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Nimro will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Nimro installations on the same machine (with different `NIMRO_HOME` directories), each gets its own systemd service name. The default `~/.nimro` uses `nimro-gateway`; other installations use `nimro-gateway-<hash>`. The `nimro gateway` commands automatically target the correct service for your current `NIMRO_HOME`.
:::

### macOS (launchd)

```bash
nimro gateway install               # Install as launchd agent
nimro gateway start                 # Start the service
nimro gateway stop                  # Stop the service
nimro gateway status                # Check status
tail -f ~/.nimro/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.nimro.gateway.plist`. It includes three environment variables:
...
- **NIMRO_HOME** — scopes the gateway to your Nimro installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `nimro gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `NIMRO_HOME` directory gets its own launchd label. The default `~/.nimro` uses `ai.nimro.gateway`; other installations use `ai.nimro.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `nimro-cli` | Full access |
| Telegram | `nimro-telegram` | Full tools including terminal |
| Discord | `nimro-discord` | Full tools including terminal |
| WhatsApp | `nimro-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `nimro-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `nimro-slack` | Full tools including terminal |
| Google Chat | `nimro-google_chat` | Full tools including terminal |
| Signal | `nimro-signal` | Full tools including terminal |
| SMS | `nimro-sms` | Full tools including terminal |
| Email | `nimro-email` | Full tools including terminal |
| Home Assistant | `nimro-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `nimro-mattermost` | Full tools including terminal |
| Matrix | `nimro-matrix` | Full tools including terminal |
| DingTalk | `nimro-dingtalk` | Full tools including terminal |
| Feishu/Lark | `nimro-feishu` | Full tools including terminal |
| WeCom | `nimro-wecom` | Full tools including terminal |
| WeCom Callback | `nimro-wecom-callback` | Full tools including terminal |
| Weixin | `nimro-weixin` | Full tools including terminal |
| BlueBubbles | `nimro-bluebubbles` | Full tools including terminal |
| QQBot | `nimro-qqbot` | Full tools including terminal |
| Yuanbao | `nimro-yuanbao` | Full tools including terminal |
| Microsoft Teams | `nimro-teams` | Full tools including terminal |
| API Server | `nimro-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `nimro-webhook` | Full tools including terminal |
| Raft | `nimro-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.nimro/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
