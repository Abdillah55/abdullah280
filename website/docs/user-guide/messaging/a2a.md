...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The Nimro A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your Nimro over HTTP.

It interoperates with any A2A-compliant peer — another Nimro, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **Nimro ↔ Nimro across machines** — let your desktop agent hand tasks to a Nimro on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your Nimro so other frameworks' agents can send it tasks.
...
The outbound client tools ship as the `a2a` toolset, **off by default** — enable it with `nimro tools`.
...
With the platform enabled, Nimro serves:
...
- **Audit log** — every exchange appends to `~/.nimro/a2a_audit.jsonl`.
