...
sidebar_position: 12
title: "Google Chat"
description: "Set up Nimro Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Nimro Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Nimro
...
> Run `nimro gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Nimro can read (e.g.,
`~/.nimro/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a nimro restart)
...
- Principal: `nimro-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Nimro calls
...
- **App name**: whatever you want users to see ("Nimro" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/nimro-chat-events`.
...
`ADDED_TO_SPACE` event that Nimro uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Nimro

Add the Google Chat section to `~/.nimro/.env`:
...
Send "hola" in the test DM. The bot posts a "Nimro is thinking…" marker, then
...
`~/.nimro/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Nimro is thinking…").
...
Thread support: when a user replies inside a thread, Nimro detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Nimro session.
...
3. Download the JSON. Move it onto the host that runs Nimro.
4. Register the client with Nimro (run under the profile you want it scoped to):
...
nimro -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Nimro home (e.g.
`~/.nimro/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.nimro/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.nimro/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Nimro isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `nimro gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Nimro will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.nimro/google_chat_user_tokens/<sanitized_email>.json` (filesystem
