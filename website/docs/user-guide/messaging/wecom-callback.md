...
Connect Nimro to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Nimro supports two WeCom integration modes:
...
> Run `nimro gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Nimro decrypts the message, queues it for the agent
...
(Use `nimro gateway start` only after `nimro gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Nimro and the token the
...
in `~/.nimro/.env` values around `=` will also break signature checks. After
fixing, restart `nimro gateway run`.
...
Check `nimro gateway run` logs for the bound host/port. If the adapter bound to
