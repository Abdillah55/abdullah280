...
sidebar_position: 30
title: "Nimro Relay"
description: "Connect Nimro to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Nimro Relay (Connector)
...
Nimro Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.nimro/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.nimro/.env`:
...
`nimro gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.nimro/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `nimro auth add nous` (or `nimro setup`) and retry.
...
startup. Restart it (`nimro gateway restart`).
