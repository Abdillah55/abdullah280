...
Nimro Agent can drive your desktop — clicking, typing, scrolling,
...
```
nimro computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `nimro computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `nimro tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Nimro app). `nimro computer-use doctor` will tell you which permission is missing. |
...
```
nimro -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.nimro/config.yaml`.

## Permission modes and logged-in browser profiles

Nimro maps its existing approval UX onto cua-driver 0.10's immutable daemon
modes. There is no second permission toggle to keep in sync:

| Nimro session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Nimro approvals; Cua stops at its protected boundary | Refuses unless a certified protected host is available; Nimro does not claim one today |
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Nimro risk acceptance; no runtime Cua prompts | Allowed within Cua's built-in, managed, and user policy ceilings |

The unrestricted daemon is private to that Nimro session. Turning `/yolo`
...
daemon's mode or grants another Nimro conversation the same authority.
...
## `nimro computer-use doctor` — your first triage stop

`nimro computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ nimro computer-use doctor
...
agent is acting here." Each Nimro run declares its own cua-driver
**session id** (something like `nimro-3a7b9c14d2e8`); the cursor's
...
Nimro intentionally keeps its skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Nimro-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Nimro skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Nimro
...
Nimro applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.nimro/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Nimro applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Nimro agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Nimro agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Nimro warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Nimro from inside the RDP / console
...
```
NIMRO_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
NIMRO_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Nimro disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Nimro sets
...
When it's on, `nimro computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Nimro at a binary you built from source instead
of the published release. Nimro resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`NIMRO_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Nimro testing (see notes).
...
### Option B — point Nimro straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`NIMRO_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
NIMRO_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
NIMRO_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Nimro is using your build

- `nimro computer-use status` prints the resolved binary path and
  version.
- `nimro computer-use doctor` confirms the binary is reachable and
...
- **Nimro spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Nimro YOLO, Nimro instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Nimro
  change needed either way.
- **Local builds skip the version check.** Nimro warns when the
...
**First action when anything's off: run `nimro computer-use doctor`.**
...
Run `nimro computer-use install` to fetch the cua-driver binary, or
run `nimro tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `nimro computer-use doctor` will
...
- **Nimro-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Nimro `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Nimro (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Nimro among them)
