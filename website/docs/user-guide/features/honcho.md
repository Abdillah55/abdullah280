...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Nimro's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Nimro instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
nimro memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `nimro` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Nimro syncs them back at session init.
...
The `nimro honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `nimro memory setup honcho` (or run `nimro memory setup` and pick it from the list); the `nimro honcho` subcommand then appears on the next invocation.

```bash
nimro memory setup honcho    # Configure Honcho directly (works before activation)
nimro honcho status          # Connection status, config, and key settings
nimro honcho setup           # Redirects to `nimro memory setup` (post-activation alias)
nimro honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
nimro honcho peer            # Show or update peer names + dialectic reasoning level
nimro honcho mode            # Show or set recall mode (hybrid/context/tools)
nimro honcho tokens          # Show or set token budget for context and dialectic
nimro honcho identity        # Seed or show the AI peer's Honcho identity
nimro honcho sync            # Sync Honcho config to all existing profiles
nimro honcho peers           # Show peer identities across all profiles
nimro honcho sessions        # List known Honcho session mappings
nimro honcho map             # Map current directory to a Honcho session name
nimro honcho enable          # Enable Honcho for the active profile
nimro honcho disable         # Disable Honcho for the active profile
nimro honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `nimro honcho`

If you previously used the standalone `nimro honcho setup`:
...
No re-login or re-setup needed. Run `nimro memory setup` and select "honcho" — the wizard detects your existing config.
