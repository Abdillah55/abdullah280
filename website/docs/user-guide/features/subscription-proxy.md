...
chat completions — use your Nimro-managed provider subscription as their
...
| Use case | "Use Nimro as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Nimro stores
the refresh token in `~/.nimro/auth.json` — the same place all Nimro
...
Model:      Nimro-4-70B    # or Nimro-4.3-36B, Nimro-4-405B
```
...
interface in `nimro_cli/proxy/adapters/`.
...
If you see `not logged in`, run `nimro portal`. If you see
...
`nimro_cli/proxy/adapters/<provider>.py` and registering it in
