...
Schedule tasks to run automatically with natural language or cron expressions. Nimro exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Nimro itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the global default from `nimro model`.

- **Per-job pin** — set by *you* via the dashboard, `nimro cron create/edit --model … --provider …`, or by editing `~/.nimro/cron/jobs.json`. Once set, it sticks until you change it. The agent's `cronjob` tool cannot set or change per-job models — inference pins are user-owned.
- **`cron.model` / `cron.model_provider`** — a cron-fleet default: every unpinned job runs on this model, independent of your chat model. Set it once (`nimro config set cron.model <name>`) and switching your chat model with `nimro model` or `/model` never touches your cron fleet.
- **Global default** — only when neither of the above is set does a job follow `nimro model`. In this case Nimro **snapshots** the provider and model at creation, and if the global default later changes the job **fails closed**: it skips the run, makes no inference call, and alerts you to pin the provider/model explicitly (#44585). This prevents an unattended job from silently inheriting a switch to a paid provider/model. Setting `cron.model` (or a per-job pin) is the deliberate way to route cron spend, and the drift guard does not engage for an axis covered by it. Operators who instead want unpinned jobs to track the changing global default can [disable the drift guard](#letting-unpinned-jobs-track-global-defaults).

`nimro setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Nimro disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
nimro cron create "every 2h" "Check server status"
nimro cron create "every 1h" "Summarize new feed items" --skill blogwatcher
nimro cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Nimro will use the unified `cronjob` tool internally.
...
```bash
nimro config set cron.model_drift_guard false
```
...
nimro cron create "every 1d at 09:00" \
...
```bash
nimro cron edit <job_id> --schedule "every 4h"
nimro cron edit <job_id> --prompt "Use the revised task"
nimro cron edit <job_id> --skill blogwatcher --skill maps
nimro cron edit <job_id> --add-skill maps
nimro cron edit <job_id> --remove-skill blogwatcher
nimro cron edit <job_id> --clear-skills
```
...
```bash
nimro cron list
nimro cron pause <job_id_or_name>
nimro cron resume <job_id_or_name>
nimro cron run <job_id_or_name>
nimro cron remove <job_id_or_name>
nimro cron edit <job_id_or_name> [...flags]
nimro cron status
nimro cron tick
```
...
```bash
nimro gateway install     # Install as a user service
sudo nimro gateway install --system   # Linux: boot-time system service for servers
nimro gateway             # Or run in foreground
...
1. loads jobs from `~/.nimro/cron/jobs.json`
...
A file lock at `~/.nimro/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.

### Execution history

Nimro records each claimed cron attempt in the profile-local
`~/.nimro/cron/executions.db` before executor or provider dispatch. Attempts
move through `claimed`, `running`, and one immutable terminal state:
`completed`, `failed`, or `unknown`. After restart, Nimro marks an abandoned
...
Inspect recent attempts with `nimro cron runs [job-id] --limit 20` (alias:
...
| `"local"` | Save to local files only (`~/.nimro/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.nimro/cron/output/`), but no message is sent to the delivery target.
...
Or set the `NIMRO_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
`.sh` / `.bash` files run under `bash` from `PATH` when available, otherwise `/bin/bash` (important on Windows Git Bash). Anything else runs under the current Python interpreter (`sys.executable`). Scripts must resolve inside `$NIMRO_HOME/scripts/` — relative names, absolute paths, and `~`-prefixed paths are accepted when the resolved target stays in that directory; paths that escape it are rejected. Subprocess env is sanitized (`_sanitize_subprocess_env`): provider API credentials and other Nimro-managed secrets are **not** inherited by cron scripts.

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Nimro directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Nimro will write the check script to `~/.nimro/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.nimro/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.nimro/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.nimro/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.nimro/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Nimro reads Job 1's most recent output from `~/.nimro/cron/output/{job1_id}/*.md`
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `nimro tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `nimro tools` cron-platform config wins; otherwise Nimro falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Nimro should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.nimro/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.nimro/scripts/.feed-changed.last"
...
:::tip
Nimro's own `~/.nimro/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/NousResearch/nimro-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.nimro/cron/jobs.json`. Output from job runs is saved to `~/.nimro/cron/output/{job_id}/{timestamp}.md`.

:::tip
Ask the agent to manage jobs through the `cronjob` tool, `nimro cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `NIMRO_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Nimro resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
