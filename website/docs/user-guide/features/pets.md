...
Nimro can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<NIMRO_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Nimro renders the sprite at full fidelity when your
...
nimro pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `nimro pets list [query] [--limit N]` |
| List installed pets | `nimro pets list --installed` |
| Install a pet | `nimro pets install <slug> [--select] [--force]` |
| Set the active pet | `nimro pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `nimro pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `nimro pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `nimro pets off` |
| Remove an installed pet | `nimro pets remove <slug>` |
| Diagnose setup | `nimro pets doctor` |

`nimro pets show` flags:
...
Beyond installing pre-made pets from the gallery, Nimro can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `nimro pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `nimro tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `NIMRO_PET_IMAGE_PROVIDER` env var (e.g. `NIMRO_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Nimro is
...
Run `nimro pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Nimro uses its own
  profile-scoped `<NIMRO_HOME>/pets/` instead — install through `nimro pets`.

## See also

- The [`nimro-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-nimro-agent.md)
