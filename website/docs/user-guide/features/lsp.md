...
Nimro runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Nimro
...
1. Nimro captures a baseline of current diagnostics for the file.
...
…). Nimro auto-detects the binary on PATH or in
`<NIMRO_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Nimro at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<NIMRO_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`nimro lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Nimro installs both packages together when you
run `nimro lsp install typescript` or auto-install fires on first
use.

## CLI

```
nimro lsp status          # service state + per-server install status
nimro lsp list            # registry, optionally --installed-only
nimro lsp install <id>    # eagerly install one server
nimro lsp install-all     # try every server with a known recipe
nimro lsp restart         # tear down running clients
nimro lsp which <id>      # print resolved binary path
```

`nimro lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <NIMRO_HOME>/lsp/bin
...
When `install_strategy: auto`, Nimro installs binaries into
`<NIMRO_HOME>/lsp/bin/`. NPM packages land in
`<NIMRO_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Nimro-owned and is
...
**`nimro lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<NIMRO_HOME>/lsp/bin/`. Run
`nimro lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `nimro lsp status`**
...
When `nimro lsp status` shows a `Backend warnings` section, install
...
`~/.nimro/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.nimro/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `nimro lsp restart` to clear the set;
