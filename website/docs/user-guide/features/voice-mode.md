...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Nimro Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Nimro Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Nimro](/guides/use-voice-mode-with-nimro).

For hands-free session start — saying "hey nimro" (or any phrase) to open a fresh voice session on the CLI, TUI, or desktop app — see [Wake Word](/user-guide/features/wake-word).

## Prerequisites

Before using voice features, make sure you have:

1. **Nimro Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `nimro model` or set your preferred provider credentials in `~/.nimro/.env`
3. **A working base setup** — run `nimro` to verify the agent responds to text before enabling voice

:::tip
The `~/.nimro/` directory and default `config.yaml` are created automatically the first time you run `nimro`. You only need to create `~/.nimro/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `nimro setup --portal` wires both up at once.
:::
...
cd ~/.nimro/nimro-agent && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.nimro/nimro-agent && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.nimro/nimro-agent && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.nimro/nimro-agent && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`nimro chat`) and the **TUI** (`nimro --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.nimro/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
nimro                # Start the interactive CLI
```
...
1. Start the CLI with `nimro` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.nimro/config.yaml` (default: `ctrl+b`).
:::
...
```bash
nimro gateway        # Start the gateway (connects to configured platforms)
nimro gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@nimrobyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.nimro/.env`:
```bash
...
```bash
nimro gateway        # Start with existing configuration
```
...
If you are running Nimro inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.nimro/.env`
...
- Review gateway logs: `tail -f ~/.nimro/logs/gateway.log`
