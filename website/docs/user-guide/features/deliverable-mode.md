...
When Nimro Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.nimro/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.nimro/config.yaml` (switchable per session
via `/personality`).
...
If you use Nimro' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.nimro/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Nimro Agent's deliverable
