...
All skills live in **`~/.nimro/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Nimro at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `nimro update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.nimro` profile):

```bash
curl -fsSL https://nimro-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
nimro profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
nimro skills opt-out            # stop future seeding — nothing on disk is touched
nimro skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
nimro skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `nimro update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `nimro skills opt-in`) to re-enable.

:::note Safe by default
`nimro skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Nimro installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Nimro to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.nimro/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
nimro chat --toolsets skills -q "What skills do you have?"
nimro chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Nimro-tool framing, no invented
commands).
...
/home/user/.nimro/cache/chart-q4-2025.png
...
When a missing value is encountered, Nimro asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `nimro setup` or `~/.nimro/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `nimro config migrate` prompts for unconfigured settings, and `nimro config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.nimro/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Nimro scans the
...
If you maintain skills outside of Nimro — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Nimro to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.nimro/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.nimro/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Nimro process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Nimro ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.nimro/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.nimro/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `nimro bundles list`.
...
nimro bundles create research

# Overwrite an existing bundle
nimro bundles create backend-dev --skill ... --force

# Delete a bundle
nimro bundles delete backend-dev

# Re-scan ~/.nimro/skill-bundles/ and report changes
nimro bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.nimro/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.nimro/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.nimro/pending/skills/` and are
...
```bash
nimro skills browse                              # Browse all hub skills (official first)
nimro skills browse --source official            # Browse only official optional skills
nimro skills search kubernetes                   # Search all sources
nimro skills search react --source skills-sh     # Search the skills.sh directory
nimro skills search https://mintlify.com/docs --source well-known
nimro skills inspect openai/skills/k8s           # Preview before installing
nimro skills install openai/skills/k8s           # Install with security scan
nimro skills install official/security/1password
nimro skills install skills-sh/vercel-labs/json-render/json-render-react --force
nimro skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
nimro skills list --source hub                   # List hub-installed skills
nimro skills check                               # Check installed hub skills for upstream updates
nimro skills update                              # Reinstall hub skills with upstream changes when needed
nimro skills audit                               # Re-scan all hub skills for security
nimro skills uninstall k8s                       # Remove a hub skill
nimro skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
nimro skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
nimro skills publish skills/my-skill --to github --repo owner/repo
nimro skills snapshot export setup.json          # Export skill config
nimro skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Nimro. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `nimro skills search <query> --source skills-sh`. Nimro resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Nimro currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Nimro repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Nimro can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
nimro skills search https://mintlify.com/docs --source well-known
nimro skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
nimro skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Nimro can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
nimro skills install openai/skills/k8s
nimro skills tap add myorg/skills-repo
```
...
[Skills Hub](https://nimro-agent.nousresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Nimro-side changes required.
...
- Nimro source id: `clawhub`

#### 6. LobeHub (`lobehub`)

Nimro can search and convert agent entries from LobeHub's public catalog into installable Nimro skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Nimro source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

Nimro integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Nimro' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Nimro source id: `browse-sh`
- Trust level: `community`

```bash
nimro skills search airbnb --source browse-sh
nimro skills inspect browse-sh/airbnb.com/search-listings-ddgioa
nimro skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Nimro also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Nimro source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Nimro does not enumerate or copy unrelated files from the host.

```bash
nimro skills install https://sharethis.chat/SKILL.md
nimro skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
nimro skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `nimro skills update` re-fetches from the same URL automatically when you want to refresh.
...
`nimro skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Nimro | Always trusted |
...
```bash
nimro skills check          # Report which installed hub skills changed upstream
nimro skills update         # Reinstall only the skills with updates available
nimro skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Nimro users add with `nimro skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.nimro.tags`, `version`, `author`, `platforms`, `metadata.nimro.config`).
...
Nimro discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Nimro user can subscribe and install:

```bash
nimro skills tap add my-org/nimro-skills
nimro skills search deploy
nimro skills install my-org/nimro-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.nimro/skills/.hub/taps.json`:
...
The `nimro skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `nimro skills tap list` shows the effective path per tap.
...
```bash
nimro skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.nimro/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`nimro skills reset`)

Nimro ships with a set of bundled skills in `skills/` inside the repo. On install and on every `nimro update`, a sync pass copies those into `~/.nimro/skills/` and records a manifest at `~/.nimro/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Nimro recomputes the hash of your local copy and compares it to the origin hash:
...
nimro skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `NIMRO_HOME`, so `nimro -p coder skills reset <name>` only affects that profile.
:::
