...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Nimro' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Nimro")

The wake word turns Nimro into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Nimro listens in the background for a spoken
trigger phrase. Say it, and Nimro starts a fresh session, opens the microphone,
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey nimro"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey nimro"** — a model for it ships with Nimro, so
...
```bash
cd ~/.nimro/nimro-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `nimro` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.nimro/config.yaml`,
...
wake_word:
  ...
  phrase: "hey nimro"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_nimro         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey nimro" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey nimro" utterances are missed.
...
default) to let Nimro choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Nimro installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Nimro surfaces, and `surface` picks which one
...
| `cli` | Only the classic `nimro` CLI. |
| `tui` | Only `nimro --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Nimro surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Nimro does not silently fail over to another open surface.
...
"Hey Nimro" works out of the box — the bundled openWakeWord model
(`model: hey_nimro`) is the default. To wake on something else, the easiest
...
command (`nimro -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.nimro/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("nimro" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Nimro" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey nimro"
  porcupine:
    keyword: ~/.nimro/wakewords/hey_nimro.ppn
```

Set your access key in `~/.nimro/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `nimro tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `nimro-agent[wake]`).
...
but the phrase never fires. Nimro detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Nimro
backend (it may appear as your terminal, `python`, or Nimro), then toggle the
...
```bash
nimro config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
nimro config set wake_word.input_device null
```
