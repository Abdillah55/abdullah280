...
title: "Git Worktrees"
description: "Run multiple Nimro agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Nimro Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Nimro so each session has a clean, isolated working directory.

## Why Use Worktrees with Nimro?

Nimro treats the **current working directory** as the project root:

- CLI: the directory where you run `nimro` or `nimro chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.nimro/config.yaml`
...
git worktree add ../repo-feature feature/nimro-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/nimro-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Nimro there:

```bash
cd ../repo-feature

# Start Nimro in the worktree
nimro
```
...
git worktree add ../repo-experiment-a feature/nimro-a
git worktree add ../repo-experiment-b feature/nimro-b
```
...
- Works on its own branch (`feature/nimro-a` vs `feature/nimro-b`).
...
- Nimro checkpoint data under `~/.nimro/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Nimro experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/nimro-checkpoints-docs`, `feature/nimro-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Nimro from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `nimro -w` (Automatic Worktree Mode)

Nimro has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `nimro/nimro-<hash>`).
...
```bash
nimro -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `nimro -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Nimro session its own clean checkout.
