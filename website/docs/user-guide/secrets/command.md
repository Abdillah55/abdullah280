...
secrets:
  command:
    ...
    # command: "pass show nimro/env"
    # command: "secret-tool lookup service nimro-env"
```
...
- The helper's **stderr is discarded** — vault CLI diagnostics can carry secret material, so they never reach Nimro' output. Failures log structured fields only (exit code / signal / errno), never the command string.
...
| `helper command failed` | Non-zero exit, timeout, spawn failure | Run the helper manually in a shell to see its real error (Nimro discards its stderr on purpose) |
