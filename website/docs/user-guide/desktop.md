...
sidebar_position: 3
title: "Desktop App"
description: "The native Nimro desktop app — a polished experience for chatting with Nimro, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Nimro desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Nimro Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `nimro` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Nimro has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`nimro`) and **[TUI](./tui.md)** (`nimro --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`nimro dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Nimro Desktop](../getting-started/installation.md).

If you already have Nimro installed, simply run
...
- **The same conversation history** as every other Nimro surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a Nimro instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Nimro Desktop to a remote backend](./features/web-dashboard.md#connecting-nimro-desktop-to-a-remote-backend).

#### Repository discovery

Nimro Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional Nimro sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `nimro desktop --cwd <path>` (or the `NIMRO_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask Nimro to open PR**. You can also create and switch branches from here.
...
Talk to Nimro and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `nimro model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Nimro management surface so you don't have to drop to a terminal:
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#nimro-cron).
- **Profiles** — switch between [Nimro profiles](./profiles.md) (isolated config/skills/sessions).
- **Messaging** — set up gateway channels.
- **Agents** and **Command Center** — orchestration surfaces for multi-agent work.

### Keyboard & navigation

- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update Nimro, and more.
...
The [manual update process](https://nimro-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Nimro agent, your config, and your chats stay. (Same as `nimro uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `nimro uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `nimro uninstall --full`.)
...
You can do the same from the terminal — `nimro uninstall --gui` for the GUI alone, or `nimro uninstall` / `nimro uninstall --full` for the agent too.

:::note
Running `nimro uninstall --gui` from a **source checkout** (a `nimro desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `nimro desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `nimro desktop`

To launch via the CLI, simply run `nimro desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `nimro update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `NIMRO_DESKTOP_CWD`)           |
| `--nimro-root PATH` | Override the Nimro source root the app uses (sets `NIMRO_DESKTOP_NIMRO_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `nimro` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Nimro Agent runtime into `NIMRO_HOME` (`~/.nimro`, or `%LOCALAPPDATA%\nimro` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `NIMRO_DESKTOP_NIMRO_ROOT`, then a completed managed install, then a probed `nimro` on `PATH` (unless `--ignore-existing` / `NIMRO_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `NIMRO_DESKTOP_NIMRO` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `nimro serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `nimro --tui`. The desktop app is **self-contained**: it runs its own `nimro serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Nimro backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

**Settings → Gateway → Connection mode** offers the alternatives to the local gateway:

- **Remote gateway** — enter the URL of a `nimro serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
- **Nimro Cloud** — sign in once to Nimro Cloud and pick from the agents on your account; no URL to paste. The app discovers your agents (with an organization picker if your account spans several orgs), and connecting to one switches the session over automatically. The status bar shows the cloud connection while it's active.
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.nimro/.env` (the secrets file, mode 0600):
...
nimro serve --host 0.0.0.0 --port 9119
```

Keep that `nimro serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `nimro serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `NIMRO_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.nimro/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/nimro` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `NIMRO_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `NIMRO_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateway settings panel.
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `NIMRO_DASHBOARD_BASIC_AUTH_USERNAME` / `NIMRO_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for an unknown user and a wrong password (no enumeration oracle), so double-check both. Confirm the gate is on with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.nimro/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `NIMRO_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Nimro Desktop to a remote backend](./features/web-dashboard.md#connecting-nimro-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Nimro Desktop](../reference/environment-variables.md#web-dashboard--nimro-desktop).
...
`$NIMRO_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
Boot logs land in `NIMRO_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.nimro/nimro-agent/.nimro-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.nimro/nimro-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.nousresearch.nimro
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.nimro/nimro-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
```bash
NIMRO_DESKTOP_NIMRO_ROOT=/path/to/clone npm run dev
NIMRO_HOME=/tmp/throwaway npm run dev
...
certificate once and tell Nimro to use it:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `Nimro Local Signing`, Identity Type: *Self-Signed Root*,
   Certificate Type: **Code Signing**.
3. `nimro config set desktop.macos_signing_identity "Nimro Local Signing"`
...
`tccutil reset All com.nousresearch.nimro` and re-grant.

## See also

- [CLI Guide](./cli.md) — the terminal interface
- [TUI](./tui.md) — the modern terminal UI used by `nimro --tui` and the dashboard chat tab
