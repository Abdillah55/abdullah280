...
If you only run one Nimro agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Nimro agents that should all
...
(`ai.nimro.gateway-<name>.plist`) or systemd user service
(`nimro-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
nimro profile create coder
nimro profile create personal-bot
nimro profile create research
...
```bash
nimro config set gateway.multiplex_profiles true
nimro gateway restart
```

Equivalently, in the default profile's `~/.nimro/config.yaml`:
...
You do **not** run `nimro gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `nimro gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.nimro/profiles/coder/.env`; the default listener key is rejected.
...
home). `nimro status` reports the multiplexer and the profiles it serves;
`nimro status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/nimro-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: nimro-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    nimro gateway "$action"
  else
    nimro -p "$profile" gateway "$action"
  fi
}
...
```bash
nimro-gateways start      # start every configured profile
nimro-gateways stop       # stop every configured profile
nimro-gateways restart    # restart all
nimro-gateways status     # status across all
nimro-gateways list       # delegates to `nimro gateway list`
```

:::tip
The `default` profile is targeted with `nimro gateway <action>` (no `-p`),
not `nimro -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `nimro -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.nimro.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/nimro-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.nimro.gateway.plist` /
`nimro-gateway.service`.
...
```bash
# Default profile
tail -f ~/.nimro/logs/gateway.log
tail -f ~/.nimro/logs/gateway.error.log

# Named profile
tail -f ~/.nimro/profiles/<name>/logs/gateway.log
tail -f ~/.nimro/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.nimro/logs/gateway.log ~/.nimro/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
nimro logs -f                  # follow default profile
nimro -p coder logs -f         # follow one profile
nimro logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
nimro profile list             # profiles + model + gateway state
nimro-gateways status          # full status across every profile
launchctl list | grep nimro    # macOS — PIDs and labels
systemctl --user list-units 'nimro-gateway-*'   # Linux — units
```
...
The default profile uses `~/.nimro/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
nimro config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.nimro/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=nimro --why="gateways running" \
  sleep infinity &
...
`nimro-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.nimro/.env ~/.nimro/profiles/*/.env
```

## Updating the code

`nimro update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `nimro gateway start` after a previous `nimro gateway stop`. The
...
```bash
ps -ef | grep "nimro_cli.*-p <profile>"
cat ~/.nimro/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.nimro.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.nimro.gateway-<profile>.plist

# Linux
systemctl --user restart nimro-gateway-<profile>.service
```

### Health check

```bash
nimro doctor                  # default profile
nimro -p <profile> doctor     # one profile
```
