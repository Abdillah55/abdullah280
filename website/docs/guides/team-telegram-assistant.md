...
This tutorial walks you through setting up a Telegram bot powered by Nimro Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Nimro Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.nimro/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Nimro itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Nimro Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_nimro_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Nimro Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Nimro Gateway...
...
# View live logs
journalctl --user -u nimro-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `nimro gateway install` to update the plist.
:::
...
```bash
nimro gateway stop && nimro gateway start
```
...
nimro pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.nimro/.env`:
...
Control how much detail the bot shows when using tools. In `~/.nimro/config.yaml`:
...
Customize how the bot communicates by editing `~/.nimro/SOUL.md`:

For a full guide, see [Use SOUL.md with Nimro](/guides/use-soul-with-nimro).
...
```bash
# From the CLI
nimro cron list          # View all scheduled jobs
nimro cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u nimro-gateway` (Linux) or `~/.nimro/logs/gateway.log` (macOS) |
| Cron job output | `~/.nimro/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.nimro/cron/jobs.json` |
| Pairing data | `~/.nimro/pairing/` |
| Session history | `~/.nimro/sessions/` |
