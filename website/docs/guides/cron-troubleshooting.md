...
description: "Diagnose and fix common Nimro cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`nimro gateway` for foreground, or `nimro gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `nimro cron tick`.
...
```bash
date
nimro cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.nimro/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.nimro/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.nimro/.env` |
...
| `local` | Write access to `~/.nimro/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `nimro cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `nimro skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `nimro skills list`.
...
2. `~/.nimro/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `nimro cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Nimro config directory). Verify:
```bash
ls ~/.nimro/scripts/your-script.py   # Must exist
nimro cron edit <job_id> --script ~/.nimro/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `nimro skills install <skill-name>`.
...
Jobs are stored in `~/.nimro/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.nimro/cron/jobs.json
chmod 600 ~/.nimro/cron/jobs.json   # Your user should own it
```
...
```bash
nimro cron list                    # Show all jobs, states, next_run times
nimro cron run <job_id>            # Schedule for next tick (for testing)
nimro cron edit <job_id>           # Fix configuration issues
nimro logs                         # View recent Nimro logs
nimro skills list                  # Verify installed skills
```
...
1. Run the job with `nimro cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.nimro/logs/agent.log` for scheduler messages and `~/.nimro/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/nimro-agent](https://github.com/NousResearch/nimro-agent) with:
