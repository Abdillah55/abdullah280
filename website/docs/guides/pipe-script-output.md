...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `nimro send`."
---

# Pipe Script Output to Messaging Platforms

`nimro send` is a small, scriptable CLI that pushes a message to any
messaging platform Nimro is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | nimro send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `nimro
...
nimro send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
nimro send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
nimro send --list

# Filter by platform
nimro send --list telegram
```
...
Any platform Nimro ships adapters for works as a target:
...
`nimro send` resolves the message body in this order:

1. **Positional argument** — `nimro send --to telegram "hi"`
2. **`--file PATH`** — `nimro send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | nimro send --to telegram`

When stdin is a TTY (no pipe), Nimro does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  nimro send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `nimro send` reuses your Nimro config, the same script works on
any host where Nimro is installed — no need to export bot tokens into
...
`nimro send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  nimro send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | nimro send \
...
```bash
./train.py --epochs 200 && \
  nimro send --to telegram "training done" || \
  nimro send --to telegram "training failed (exit $?)"
```
...
nimro send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(nimro send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `nimro send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `nimro send` calls
the platform's REST endpoint directly using credentials from
`~/.nimro/.env` and `~/.nimro/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `nimro gateway start` and retry.
...
# Machine-readable
nimro send --list --json
```

The listing is built from `~/.nimro/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`nimro gateway
...
| Approach | Multi-platform | Reuses Nimro creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `nimro send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`nimro send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `nimro send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `nimro send` shares with cron delivery.
