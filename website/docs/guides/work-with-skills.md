...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Nimro new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Nimro how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Nimro installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Nimro to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Nimro but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
nimro skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.nimro/skills/`
...
For how to ship skills in your own plugin, see [Build a Nimro Plugin → Bundle skills](/developer-guide/plugins#bundle-skills).
...
When a skill with config is first loaded, Nimro prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
nimro skills config gif-search

# View all skill config
nimro config get skills.config --json
```
...
```bash
mkdir -p ~/.nimro/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.nimro/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
nimro chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.nimro/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Nimro may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Nimro will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.nimro/skills/devops/`, `~/.nimro/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Nimro to update the skill with what you learned. Skills that aren't maintained become liabilities.
