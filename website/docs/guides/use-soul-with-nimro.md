---
sidebar_position: 7
title: "Use SOUL.md with Nimro"
description: "How to use SOUL.md to shape Nimro Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Nimro

`SOUL.md` is the **primary identity** for your Nimro instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Nimro to feel like the same assistant every time you talk to it — or if you want to replace the Nimro persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Nimro should be
- what Nimro should avoid stylistically
- how Nimro should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Nimro is and how Nimro speaks
...
Nimro now uses only the global SOUL file for the current instance:

```text
~/.nimro/SOUL.md
```

If you run Nimro with a custom home directory, it becomes:
...
Nimro automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Nimro does not overwrite it
- if the file exists but is empty, Nimro adds nothing from it to the prompt

## How Nimro uses it

When Nimro starts a session, it reads `SOUL.md` from `NIMRO_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Nimro feels.
...
Nimro already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Nimro should sound.

# Avoid
What Nimro should not do.

# Defaults
How Nimro should behave when ambiguity appears.
```
...
Then restart Nimro or start a new session.
...
### I edited SOUL.md but Nimro still sounds the same

Check:
- you edited `~/.nimro/SOUL.md` or `$NIMRO_HOME/SOUL.md`
...
### Nimro is ignoring parts of my SOUL.md
