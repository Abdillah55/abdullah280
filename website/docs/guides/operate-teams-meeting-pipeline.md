...
```bash
nimro teams-pipeline token-health
nimro teams-pipeline token-health --force-refresh
```
...
```bash
nimro teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
nimro teams-pipeline maintain-subscriptions
nimro teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Nimro cron (recommended if you already run the Nimro gateway)

Nimro ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.nimro/scripts/`. First create the script:

```bash
mkdir -p ~/.nimro/scripts
cat > ~/.nimro/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec nimro teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.nimro/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
nimro cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/nimro-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Nimro Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=nimro
EnvironmentFile=/etc/nimro/env
ExecStart=/usr/local/bin/nimro teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/nimro-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Nimro Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now nimro-teams-pipeline-maintain.timer
systemctl list-timers nimro-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/nimro teams-pipeline maintain-subscriptions >> /var/log/nimro/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.nimro/.env` at the top of a wrapper script that crontab calls.
...
```bash
nimro teams-pipeline subscriptions   # should show expirationDateTime advanced
nimro teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
nimro teams-pipeline list
nimro teams-pipeline list --status failed
nimro teams-pipeline show <job-id>
```

### Replay a stored job

```bash
nimro teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
nimro teams-pipeline fetch --meeting-id <meeting-id>
nimro teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `nimro teams-pipeline maintain-subscriptions --dry-run`
- inspect `nimro teams-pipeline list --status failed`
...
- run `nimro teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `nimro teams-pipeline run`
...
- [ ] `nimro teams-pipeline validate` returns an OK snapshot
- [ ] `nimro teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Nimro cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
