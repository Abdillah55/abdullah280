...
Copy-paste blueprints for common automation patterns. Each blueprint uses Nimro's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`nimro webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `nimro webhook subscribe`) |
...
```bash
nimro cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/nimro-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/nimro-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
nimro webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
nimro cron create "0 9 * * 1" \
  "Scan the NousResearch/nimro-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/nimro-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (nimro_cli/commands.py, nimro_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (nimro_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
nimro cron create "0 6 * * *" \
  "Run a dependency security audit on the nimro-agent project.

1. cd ~/.nimro/nimro-agent && source .venv/bin/activate
...
```bash
nimro webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
nimro webhook subscribe alert-triage \
...
```python title="~/.nimro/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Nimro-Monitor/1.0"})
...
nimro cron create "every 30m" \
  ...
  --script ~/.nimro/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
nimro cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Nimro Agent development." \
...
```bash
nimro webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
nimro webhook subscribe stripe-payments \
...
```bash
nimro cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the nimro-agent codebase.
