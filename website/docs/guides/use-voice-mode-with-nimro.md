---
sidebar_position: 8
title: "Use Voice Mode with Nimro"
description: "A practical guide to setting up and using Nimro voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Nimro
...
- you want Nimro sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Nimro.
...
## Step 1: make sure normal Nimro works first
...
Nimro supports both local and cloud speech stacks.
...
### If you use `nimro setup`

If you choose NeuTTS in the setup wizard, Nimro checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Nimro transcribes and responds
...
- asking Nimro to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Nimro loop.

## Tuning CLI behavior

### Silence threshold

If Nimro starts/stops too aggressively, tune:
...
Nimro stays a normal chat bot, but can speak replies.
...
- you want Nimro to function like a portable research or ops assistant
...
Nimro joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Nimro detects speech boundaries
- transcripts are posted in the associated text channel
- Nimro responds in text and audio
...
1. get text Nimro working
2. run `nimro setup tts` to enable voice support
