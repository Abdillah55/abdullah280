---
sidebar_position: 6
title: "Use MCP with Nimro"
description: "A practical guide to connecting MCP servers to Nimro Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Nimro

This guide shows how to actually use MCP with Nimro Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Nimro tool
- you want Nimro to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Nimro to internal APIs, databases, or company systems without modifying Nimro core

Do not use MCP when:
- a built-in Nimro tool already solves the job well
...
- Nimro remains the agent
- MCP servers contribute tools
- Nimro discovers those tools at startup or reload time
...
If you installed Nimro with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Nimro banner/status should show MCP integration when configured
- ask Nimro what tools it has available
...
## WSL2: bridge Nimro in WSL to Windows Chrome

This is the practical setup when:

- Nimro runs inside WSL2
...
In this setup, Nimro does **not** connect to Chrome directly. Instead:

- Nimro runs in WSL
- Nimro starts a local stdio MCP server
...
```text
Nimro (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Nimro stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Nimro core browser transport
...
```bash
nimro mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
nimro mcp test chrome-devtools-win
```

Then start a fresh Nimro session or run:
...
Once loaded, Nimro can use the MCP-prefixed browser tools directly. For example:
...
If Nimro runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Nimro from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Nimro from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Nimro:
...
2. Nimro-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Nimro will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Nimro to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Nimro to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Nimro remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
nimro mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
nimro mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `nimro mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Nimro to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Nimro to merge, publish, deploy, or spawn runtimes.
...
Now Nimro can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Nimro core.
...
Because Nimro now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
