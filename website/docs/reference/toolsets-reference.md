...
description: "Reference for Nimro core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `nimro-cli` is the default for interactive CLI sessions)
...
```bash
nimro chat --toolsets web,file,terminal
nimro chat --toolsets debugging        # composite — expands to file + terminal + web
nimro chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - nimro-cli          # default for CLI
  # - nimro-telegram   # override for Telegram gateway
```

### Interactive management

```bash
nimro tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Nimro tools programmatically. |
...
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `nimro-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `nimro-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `nimro-cli` or other messaging toolsets. |
...
| `x_search` | `x_search` | Read-only public X discovery via xAI's built-in `x_search` Responses tool. Use the `xurl` skill for authenticated X API reads and account actions. Off by default; opt in via `nimro tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `nimro-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `nimro-cli`:

| Toolset | Differences from `nimro-cli` |
|---------|-------------------------------|
| `nimro-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal (plus the desktop-GUI pane tools `read_terminal`, `close_terminal`, `open_preview`, `focus_pane`), web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, clarify, computer_use, Home Assistant, and the kanban tools (all check_fn-gated at runtime). |
| `nimro-acp` | Drops `clarify`, `cronjob`, `image_generate`, `text_to_speech`, `computer_use`, all four Home Assistant tools, the kanban tools, and the desktop-GUI pane tools. Focused on coding tasks in IDE context. |
| `nimro-api-server` | Drops `clarify`, `text_to_speech`, `computer_use`, the kanban tools, and the desktop-GUI pane tools. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `nimro-cron` | Same as `nimro-cli`. |
| `nimro-telegram` | Same as `nimro-cli`. |
| `nimro-discord` | Adds `discord` and `discord_admin` on top of `nimro-cli`. |
| `nimro-slack` | Same as `nimro-cli`. |
| `nimro-whatsapp` | Same as `nimro-cli`. |
| `nimro-signal` | Same as `nimro-cli`. |
| `nimro-matrix` | Same as `nimro-cli`. |
| `nimro-mattermost` | Same as `nimro-cli`. |
| `nimro-email` | Same as `nimro-cli`. |
| `nimro-sms` | Same as `nimro-cli`. |
| `nimro-bluebubbles` | Same as `nimro-cli`. |
| `nimro-dingtalk` | Same as `nimro-cli`. |
| `nimro-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `nimro-qqbot` | Same as `nimro-cli`. |
| `nimro-wecom` | Same as `nimro-cli`. |
| `nimro-wecom-callback` | Same as `nimro-cli`. |
| `nimro-weixin` | Same as `nimro-cli`. |
| `nimro-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `nimro-cli`. |
| `nimro-homeassistant` | Same as `nimro-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `nimro-webhook` | Restricted safe subset — only `web_search`, `web_extract`, `vision_analyze`, and `clarify`. Webhook-triggered runs get no terminal, file, or browser access. |
| `nimro-gateway` | Internal gateway orchestrator toolset — union of every `nimro-<platform>` toolset; used when the gateway needs to accept any message source. |
...
- **Workflow-gated** tools — the `kanban` toolset — are deliberately opt-in. `all`/`*` does **not** enable kanban; you must list `kanban` explicitly (or be a dispatcher-spawned worker with `NIMRO_KANBAN_TASK` set). Kanban tools mutate shared board state, so they stay off by default even under `all`.

## Relationship to `nimro tools`

The `nimro tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
