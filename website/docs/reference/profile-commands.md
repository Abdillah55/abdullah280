...
This page covers all commands related to [Nimro profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `nimro profile` without a subcommand shows help.
...
```bash
nimro profile use <name>
```

Sets `<name>` as the active profile. All subsequent `nimro` commands (without `-p`) will use this profile.
...
nimro profile use default
```

## `nimro profile create`

```bash
nimro profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `nimro profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `nimro update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.nimro`), use `nimro skills opt-out` / `nimro skills opt-in`. |
...
nimro profile create work2 --clone-from work

# Clone everything from a specific profile
nimro profile create work2-backup --clone-from work --clone-all
```

## `nimro profile describe`

```bash
nimro profile describe [<name>] [options]
```
...
nimro profile describe researcher

# Set it explicitly
nimro profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
nimro profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
nimro profile describe --all --auto
```

## `nimro profile delete`

```bash
nimro profile delete <name> [options]
```
...
```bash
nimro profile delete mybot
nimro profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.nimro`) cannot be deleted — use `nimro uninstall` to remove everything.
:::

## `nimro profile show`

```bash
nimro profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Nimro home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ nimro profile show work
Profile: work
Path:    ~/.nimro/profiles/work
...
```bash
nimro profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Nimro installation.
...
```bash
nimro profile alias work
# Creates/updates ~/.local/bin/work

nimro profile alias work --name mywork
# Creates ~/.local/bin/mywork

nimro profile alias work --remove
# Removes the wrapper script
```

## `nimro profile rename`

```bash
nimro profile rename <old-name> <new-name>
```
...
```bash
nimro profile rename mybot assistant
# ~/.nimro/profiles/mybot → ~/.nimro/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `nimro profile export`

```bash
nimro profile export <name> [options]
```
...
```bash
nimro profile export work
# Creates work.tar.gz in the current directory

nimro profile export work -o ./work-2026-03-29.tar.gz
```

## `nimro profile import`

```bash
nimro profile import <archive> [options]
```
...
```bash
nimro profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

nimro profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`nimro profile export` / `import` are still the right commands for
...
### `nimro profile install`

```bash
nimro profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `nimro -p telemetry`). |
...
nimro profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
nimro profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
nimro profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
nimro profile install ./telemetry/
```

### `nimro profile update`

```bash
nimro profile update <name> [--force-config] [--yes]
```
...
```bash
nimro profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Nimro version, author, env var requirements, the source URL/path, and
...
`nimro profile list` also shows the distribution name and version in a
`Distribution` column, and `nimro profile show <name>` / `delete <name>`
...
nimro profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
nimro profile install https://github.com/your-org/internal-assistant.git
```
...
`nimro_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Nimro version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Nimro can clone from.
3. Tell recipients to run `nimro profile install <your-repo-url>`.
...
## `nimro -p` / `nimro --profile`

```bash
nimro -p <name> <command> [options]
nimro --profile <name> <command> [options]
```

Global flag to run any Nimro command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
nimro -p work chat -q "Check the server status"
nimro --profile dev gateway start
nimro -p personal skills list
nimro -p work config edit
```

## `nimro completion`

```bash
nimro completion <shell>
```
...
- `nimro profile <TAB>` — subcommands (list, use, create, etc.)
- `nimro profile use <TAB>` — profile names
- `nimro -p <TAB>` — profile names
