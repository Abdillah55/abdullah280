...
The ACP adapter wraps Nimro' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
nimro acp / nimro-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.nimro/.env
  -> configure stderr logging
  -> construct NimroACPAgent
...
- `allow_once` -> Nimro `once`
- `allow_always` -> Nimro `always`
- reject options -> Nimro `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Nimro tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["nimro-acp"])
...
Instead it reuses Nimro' runtime resolver:

- `acp_adapter/auth.py`
- `nimro_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Nimro provider/credentials. It also always advertises a terminal setup auth method (`nimro-setup`, args `--setup`) so first-run ACP clients can open Nimro' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.nimro/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `nimro-acp` toolset definition
- `nimro_cli/main.py` — `nimro acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `nimro-acp` script
