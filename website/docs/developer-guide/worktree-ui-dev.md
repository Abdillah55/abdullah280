...
| Variable | Meaning |
|----------|---------|
| `NIMRO_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `NIMRO_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `NIMRO_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Nimro itself — they're private to these helpers. The variables Nimro *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `nimro --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_nimro_root)" || { echo "htui: not in a Nimro checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$NIMRO_MAIN_CHECKOUT/.venv/bin/python" -m nimro_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `NIMRO_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_nimro_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `NIMRO_TUI_DIR` are mutually exclusive
`NIMRO_TUI_DIR` points Nimro at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `nimro --tui --dev` exits with an error. Run `unset NIMRO_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_nimro_root)" || { echo "hgui: not in a Nimro checkout" >&2; return 1; }
  deps="${NIMRO_GUI_DEPS_CHECKOUT:-$NIMRO_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _nimro_link_deps "$desktop" "$deps/apps/desktop"
    _nimro_link_deps "$root" "$deps"
  else
  ...
  trap '_nimro_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    NIMRO_DESKTOP_NIMRO_ROOT="$root" \
    NIMRO_DESKTOP_PYTHON="$NIMRO_MAIN_CHECKOUT/.venv/bin/python" \
    NIMRO_DESKTOP_IGNORE_EXISTING=1 \
    NIMRO_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `NIMRO_DESKTOP_NIMRO_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `nimro`. |
| `NIMRO_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `NIMRO_DESKTOP_IGNORE_EXISTING` | Ignores any `nimro` on `PATH` so it can't shadow the worktree. |
| `NIMRO_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Nimro checkout.
_nimro_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/nimro_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_nimro_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'nimro_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `nimro --tui --dev` and the `NIMRO_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/NousResearch/nimro-agent/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `NIMRO_*` variable Nimro reads
