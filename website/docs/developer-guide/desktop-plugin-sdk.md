...
title: "Desktop Plugin SDK (@nimro/plugin-sdk)"
description: "Extend the native Nimro Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Nimro Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `NimroPlugin`.
It imports one module — `@nimro/plugin-sdk` — and gets everything: the app's
...
`$NIMRO_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Nimro. This page is the **native
desktop app** (`nimro desktop`) SDK — the `@nimro/plugin-sdk` module and
`$NIMRO_HOME/desktop-plugins/`. The **web dashboard** (`nimro dashboard`) has
its own, unrelated plugin system on `window.__NIMRO_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Nimro Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$NIMRO_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

Both take the same `NimroPlugin` contract, appear in **Settings → Plugins**, and
...
Create `$NIMRO_HOME/desktop-plugins/hello/plugin.js` (that's `~/.nimro/...`
by default, or `~/.nimro/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.nimro/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@nimro/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Nimro' }),
      jsx('div', {
...
The only importable specifiers are `@nimro/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `NimroPlugin`:

```ts
interface NimroPlugin {
...
interface PluginContext {
  ...
  /** Plugin-scoped JSON persistence (keys live under `nimro.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@nimro/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@nimro/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@nimro/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@nimro/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@nimro/plugin-sdk'
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@nimro/plugin-sdk'
...
```javascript
import { queryClient } from '@nimro/plugin-sdk'
...
`dashboard/` subfolder of a regular Nimro plugin and declare it in a
`manifest.json`:
...
nimro-agent codebase directly (`nimro_state`, `nimro_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.nimro/`)
...
(`nimro.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `NimroPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@nimro/plugin-sdk` alias.
2. It's still lint-fenced to `@nimro/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@nimro/plugin-sdk`, `react`,
...
| Plugin contract | `NimroPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginRestOptions`, `Contribution` |
...
### Agents: the `nimro-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`nimro-desktop-plugins`** skill — it carries the same contract as this page in
...
`$NIMRO_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `nimro logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@nimro/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.nimro/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.nimro/logs/errors.log` for
