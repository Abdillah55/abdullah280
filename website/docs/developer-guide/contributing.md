...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Nimro Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Nimro Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Nimro core? Start with [Build a Nimro Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Nimro itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Nimro instance (throwaway NIMRO_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m nimro_cli.main
scripts/dev-sandbox.sh --persistent python -m nimro_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.nimro/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.nimro` — use `get_nimro_home()` from `nimro_constants` for code paths and `display_nimro_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/nimro-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/nimro-agent/blob/main/LICENSE).
