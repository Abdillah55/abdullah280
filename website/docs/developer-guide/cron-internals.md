...
description: "How Nimro stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `nimro_cli/cron.py` | CLI `nimro cron` subcommands |
...
Jobs are stored in `~/.nimro/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `nimro cron` commands are run or during active CLI sessions.
...
```python
# ~/.nimro/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.nimro/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `nimro cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `nimro cron` CLI provides direct job management:

```bash
nimro cron list                    # Show all jobs
nimro cron create                  # Interactive job creation (alias: add)
nimro cron edit <job_id>           # Edit job configuration
nimro cron pause <job_id>          # Pause a running job
nimro cron resume <job_id>         # Resume a paused job
nimro cron run <job_id>            # Trigger immediate execution
nimro cron remove <job_id>         # Delete a job
```
