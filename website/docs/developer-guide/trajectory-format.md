...
Nimro Agent saves conversation trajectories in ShareGPT-compatible JSONL format
...
It follows the Nimro function-calling prompt template with:
...
Trajectory saving is a `run_agent.py` / library-level switch — the `nimro` CLI
