...
Plugins can launch and supervise fresh Nimro child sessions without importing
...
active agent turn fails closed with `No active Nimro parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Nimro can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Nimro's existing unsafe-tool block remains enforced.
