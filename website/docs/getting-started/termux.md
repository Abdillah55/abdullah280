...
description: "Run Nimro Agent directly on an Android phone with Termux"
---

# Nimro on Android with Termux
...
Nimro Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Nimro from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Nimro now ships a Termux-aware installer path:
...
- links `nimro` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `nimro` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/nimro" "$PREFIX/bin/nimro"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `nimro` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.nimro/.env`.
...
### `nimro doctor` says ripgrep or Node is missing
