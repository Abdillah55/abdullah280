...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/nimro-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    nimro_meta = (fm.get("metadata") or {}).get("nimro") or {}
    tags = nimro_meta.get("tags") or []
    related = nimro_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `nimro skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Nimro Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Nimro ships with a large built-in skill library copied into `~/.nimro/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Nimro also syncs bundled skills on `nimro update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.nimro/skills/` tree, it is still shipped with Nimro; restore it with `nimro skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with nimro-agent — install via nimro skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with nimro-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
