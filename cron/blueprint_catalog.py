...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``nimro://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``nimro://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"nimro://blueprint/{quote(blueprint.key)}{qs}"
