...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global NIMRO_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `nimro cron
        ...
        # Heartbeat once before the first sleep so `nimro cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `nimro cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_nimro_home_override()`` + ``use_cron_store()``
        ...
        for entry in profile_homes:
            ...
            home_token = set_nimro_home_override(str(home))
            try:
            ...
            for entry in profile_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `nimro cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:
