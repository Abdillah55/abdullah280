...
``nimro gateway restart`` (or ``launchctl kickstart ai.nimro.gateway``
or ``systemctl restart nimro-gateway``).  When the cron fires, the
...
``nimro cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``nimro gateway``, ``launchctl ... nimro-gateway``,
``systemctl ... nimro-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_NIMRO_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``nimro gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `nimro gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:nimro\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a nimro-gateway label. macOS launchd
    # labels look like `ai.nimro.gateway` / `nimro-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated nimro services (e.g.
    # `launchctl unload ai.nimro.update-checker.plist`).
    # `submit` and `bootstrap` are included alongside the direct verbs
    # (kickstart/etc.): `launchctl submit -l ai.nimro.gateway-<suffix> --
    ...
    # Branch C: systemctl ops on a nimro-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bnimro[.\-]?gateway)"
    # Branch D: pkill / kill targeting the nimro gateway process. Both
...
# lines (e.g. `launchctl submit \` / `  -l ai.nimro.gateway-... \` / `  -- ...`,
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.nimro.svc-reload-tmp``)
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".nimro"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<NIMRO_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.nimro/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_nimro_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `nimro gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `nimro gateway restart` from a shell outside "
