...
# 5. Symlinks the 'nimro' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Nimro Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/nimro-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink nimro into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up nimro command..."

NIMRO_BIN="$SCRIPT_DIR/venv/bin/nimro"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$NIMRO_BIN" "$COMMAND_LINK_DIR/nimro"
echo -e "${GREEN}✓${NC} Symlinked nimro → $COMMAND_LINK_DISPLAY_DIR/nimro"
...
# ============================================================================
# Seed bundled skills into ~/.nimro/skills/
# ============================================================================

NIMRO_SKILLS_DIR="${NIMRO_HOME:-$HOME/.nimro}/skills"
mkdir -p "$NIMRO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.nimro/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$NIMRO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  nimro status        # Check configuration"
if is_termux; then
    echo "  nimro gateway       # Run gateway in foreground"
else
    echo "  nimro gateway install # Install gateway service (messaging + cron)"
fi
echo "  nimro cron list     # View scheduled jobs"
echo "  nimro doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m nimro_cli.main setup
fi
