# Nimro Agent - Development Guide

Instructions for AI coding assistants and developers working on the nimro-agent codebase.

**Never give up on the right solution.**

## What Nimro Is

Nimro is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Nimro ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`nimro tools`, `nimro setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `NIMRO_HOME`. Mocks hide integration bugs.
...
- **New `NIMRO_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `nimro tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.nimro/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `nimro <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `nimro webhook`, `nimro cron`, `nimro tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.nimro/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.nimro/nimro-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _NIMRO_CORE_TOOLS list
├── cli.py                # NimroCLI class — interactive CLI orchestrator (~11k LOC)
├── nimro_state.py       # SessionDB — SQLite session store (FTS5 search)
├── nimro_constants.py   # get_nimro_home(), display_nimro_home() — profile-aware paths
├── nimro_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── nimro_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── nimro-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `nimro --tui`
...
**User config:** `~/.nimro/config.yaml` (settings), `~/.nimro/.env` (API keys only).
**Logs:** `~/.nimro/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_nimro_home()`.
Browse with `nimro logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Nimro: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`nimro_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `NimroCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.nimro/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`nimro_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/nimro` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `nimro_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `NimroCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `nimro --tui` or `NIMRO_TUI=1`.
...
npm run dev       # watch mode (rebuilds nimro-ink + tsx --watch)
npm start         # production
npm run build     # full build (nimro-ink + tsc)
...
### TUI in the Dashboard (`nimro dashboard` → `/chat`)

The dashboard embeds the real `nimro --tui` — **not** a rewrite.  See `nimro_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `nimro_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `nimro --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `nimro --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@nimro/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `nimro serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `NIMRO_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `nimro` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `nimro --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Nimro built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Nimro core.
Use the plugin route instead: create `~/.nimro/plugins/<name>/plugin.yaml`
and `~/.nimro/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Nimro tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_NIMRO_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_NIMRO_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_nimro_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `NIMRO_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_nimro_home()` for the base directory — never `Path.home() / ".nimro"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `nimro_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `nimro_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `nimro tools`, `nimro setup`, most CLI subcommands | `nimro_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`nimro_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
nimro_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.nimro/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Nimro gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `nimro_cli/skin_engine.py`:
...
Users create `~/.nimro/skins/<name>.yaml`:
...
Nimro has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.nimro/plugins/` and pip-installed entry points.

### General plugins (`nimro_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.nimro/plugins/`, `./.nimro/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `nimro` at startup so
  `nimro <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(nimro_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `nimro <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `nimro --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `nimro_cli/main.py`, etc.).
...
into `~/.nimro/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `nimro memory setup` / `post_setup()` without
...
`~/.nimro/plugins/` (or via pip entry points). They register through
...
2. User: `$NIMRO_HOME/plugins/model-providers/<name>/`
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `nimro skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.nimro.tags`, `metadata.nimro.category`,
`metadata.nimro.related_skills`, `metadata.nimro.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.nimro.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Nimro tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Nimro Agent" is the secondary collaborator. If the
   contributor's commit shows "Nimro Agent" as author (because they
   used Nimro to draft the skill), replace it with their actual name
...
lives in the `nimro-agent-dev` skill at
...
`"messaging"`); `_NIMRO_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `nimro tools` (the curses UI) or the
...
Nimro returns a delegation id immediately and the result re-enters the
...
go to `~/.nimro/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `nimro_cli/curator.py` wires `nimro curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.nimro/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `nimro cron <verb>`
...
- File lock at `~/.nimro/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `nimro kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `nimro_cli/kanban.py` wires `nimro kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`nimro-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `NIMRO_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Nimro-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `NIMRO_BACKGROUND_NOTIFICATIONS` env var):
...
Nimro supports **profiles** — multiple fully isolated instances, each with its own
`NIMRO_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `nimro_cli/main.py` sets
`NIMRO_HOME` before any module imports. All `get_nimro_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_nimro_home()` for all NIMRO_HOME paths.** Import from `nimro_constants`.
   NEVER hardcode `~/.nimro` or `Path.home() / ".nimro"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".nimro" / "config.yaml"
   ```

2. **Use `display_nimro_home()` for user-facing messages.** Import from `nimro_constants`.
   This returns `~/.nimro` for default or `~/.nimro/profiles/<name>` for profiles.
   ```python
   # GOOD
   from nimro_constants import display_nimro_home
   print(f"Config saved to {display_nimro_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.nimro/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_nimro_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_nimro_home()`,
   not `Path.home() / ".nimro"`.

4. **Tests that mock `Path.home()` must also set `NIMRO_HOME`** — since code now uses
   `get_nimro_home()` (reads env var), not `Path.home() / ".nimro"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"NIMRO_HOME": str(tmp_path / ".nimro")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not NIMRO_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".nimro" / "profiles"`, NOT `get_nimro_home() / "profiles"`.
   This is intentional — it lets `nimro -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.nimro` paths
Use `get_nimro_home()` from `nimro_constants` for code paths. Use `display_nimro_home()`
for user-facing print/log messages. Hardcoding `~/.nimro` breaks profiles — each profile
has its own `NIMRO_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `nimro_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `nimro_cli/curses_ui.py` — see
`nimro_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `NIMRO_HOME`.

### Tests must not write to `~/.nimro/`
The `_isolate_nimro_home` autouse fixture in `tests/conftest.py` redirects `NIMRO_HOME` to a temp dir. Never hardcode `~/.nimro/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_nimro_home()` resolve within the temp dir.
Use the pattern from `tests/nimro_cli/test_profiles.py`:
```python
@pytest.fixture
...
subprocess (`--file-retries`, default 1; `NIMRO_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.nimro/` | Your real config+auth.json                  | Temp dir per test                         |
