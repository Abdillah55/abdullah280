...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'nimro:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.nimroDesktop?.saveImageFromUrl) {
        if (await window.nimroDesktop.saveImageFromUrl(src)) {
