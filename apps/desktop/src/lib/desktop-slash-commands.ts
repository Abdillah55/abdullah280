...
/** Why a known Nimro command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Nimro profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Nimro Agent version', surface: exec() },
...
function isKnownNimroSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Nimro' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownNimroSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Nimro and were never asked for. It is
