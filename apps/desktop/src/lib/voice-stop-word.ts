...
//
// When someone is in a hands-free "Hey Nimro" voice chat, the natural way to
...
// phrase (optionally addressed to Nimro), so a real turn that merely contains
...
// Optional address prefixes so "nimro stop" / "ok stop" / "hey nimro, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey nimro', 'hey nimro,', 'nimro', 'nimro,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("nimro") is not a stop command on its own.
      continue
    }
 ...
 * to Nimro). Returns false for anything that merely contains "stop" as part of
