...
const SKIP_PROTO_RE = /^(?:file|data|mailto|javascript|blob|chrome|about|nimro):/i
...
export function fetchLinkTitle(url: string): Promise<string> {
  ...
  const bridge = typeof window === 'undefined' ? undefined : window.nimroDesktop?.fetchLinkTitle
...
export function openExternalLink(href: string): void {
  if (href) {
    void window.nimroDesktop?.openExternal?.(href)
  }
}
