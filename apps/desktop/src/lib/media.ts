...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.nimroDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.nimroDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.nimroDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.nimroDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaStreamUrl(path: string): string {
  return `nimro-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.nimro/cache, etc.); /api/media is intentionally
