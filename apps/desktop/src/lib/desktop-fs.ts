import type {
  NimroConnection,
  NimroReadDirResult,
  NimroReadFileTextResult,
  NimroSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: NimroConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<NimroReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<NimroReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<NimroReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<NimroReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: NimroSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
