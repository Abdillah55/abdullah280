...
const desktopWindow = window as unknown as { nimroDesktop?: Window['nimroDesktop'] }
const initialNimroDesktop = desktopWindow.nimroDesktop

function installDesktopBridge(partial: Partial<Window['nimroDesktop']> = {}) {
  desktopWindow.nimroDesktop = {
    fetchLinkTitle: vi.fn().mockResolvedValue(''),
    openExternal: vi.fn().mockResolvedValue(undefined),
    ...partial
  } as unknown as Window['nimroDesktop']
}
...
function installTitleBridge(title: string) {
  ...
  installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['nimroDesktop']['fetchLinkTitle'] })
...
afterEach(() => {
  ...
  if (initialNimroDesktop) {
    desktopWindow.nimroDesktop = initialNimroDesktop
  } else {
    delete desktopWindow.nimroDesktop
  }
})
...
describe('external link helpers', () => {
  ...
  it('opens links via the desktop bridge', () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['nimroDesktop']['openExternal'] })
  ...
  it('prefixes a pretty link to a known host with its brand glyph', () => {
    ...
    const url = 'https://github.com/NousResearch/nimro-agent/pull/123'
