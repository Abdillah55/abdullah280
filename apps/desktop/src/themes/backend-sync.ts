/**
 * Live skin sync from the Nimro backend.
 *
 * The backend resolves the active skin (built-in or `$NIMRO_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Nimro
 ...
 */
export function ingestBackendSkin(skin: NimroSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as NimroSkin)
