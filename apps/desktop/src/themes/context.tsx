...
const SKIN_KEY = 'nimro-desktop-theme-v2'
const MODE_KEY = 'nimro-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'nimro-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'nimro-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'nimro-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.nimroTheme = skinName
  root.dataset.nimroMode = rendered
  ...
  window.nimroDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('nimro-boot-background', chromeBg)
    window.localStorage.setItem('nimro-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.nimroThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.nimroDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Nimro authoring/activating a skin from a
