...
const NIMRO_PATHS_MIME = 'application/x-nimro-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(NIMRO_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(NIMRO_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.nimroDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (nimro --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.nimroDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[nimro-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.nimroDesktop?.terminal?.write(sessionId, `${command}\r`)
