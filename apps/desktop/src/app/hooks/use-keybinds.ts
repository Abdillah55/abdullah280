...
export function useKeybinds(deps: KeybindRuntimeDeps): void {
  ...
  handlersRef.current = {
    ...
    // Target resolution + the "nothing here is a repo" warning both live in
    // openWorktreeDialog, so there's no pre-gate here: the key works from a
    // detached session that sits inside a project, not just from a repo-backed
    // one.
