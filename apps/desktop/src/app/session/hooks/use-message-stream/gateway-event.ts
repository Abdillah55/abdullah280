import type { BillingBlock } from '@nimro/shared'
 ...
 * billing-specific toast — never the generic "Nimro error" — with a smart CTA
...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  ...
  // refreshNimroConfig — two REST calls (config + defaults) per event, per
  ...
  return useCallback(
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        ingestBackendSkin((payload as { skin?: NimroSkin } | undefined)?.skin, { apply: false })
        ...
        // A runtime skin switch (Nimro activating an authored skin, or `/skin`
        ...
        if (fromActiveProfile) {
          ingestBackendSkin(payload as NimroSkin | undefined, { apply: true })
        }
        ...
        if (apply) {
          ...
          // everything refreshNimroConfig applies is either active-session
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'Nimro reported an error',
        ...
        // native OS notification when Nimro is backgrounded; dispatch is gated
        ...
        const errorMessage = payload?.message || 'Nimro reported an error'
