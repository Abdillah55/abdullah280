...
import { getNimroConfig } from '@/nimro'
...
import { useNimroConfig } from './use-nimro-config'

vi.mock('@/nimro', () => ({
  getNimroConfig: vi.fn(),
  getNimroConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'nimro.desktop.workspace-cwd'
...
const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getNimroConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getNimroConfig>>)

describe('useNimroConfig refreshNimroConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Nimro' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useNimroConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshNimroConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useNimroConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getNimroConfig>>>()
    vi.mocked(getNimroConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useNimroConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshNimroConfig(true)
    })
    expect(getNimroConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getNimroConfig>>)
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getNimroConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getNimroConfig>>>()
    vi.mocked(getNimroConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshNimroConfig(true)
      refreshC = result.current.refreshNimroConfig(true)
    })
