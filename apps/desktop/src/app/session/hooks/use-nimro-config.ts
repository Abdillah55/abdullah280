...
import { getNimroConfig, getNimroConfigDefaults } from '@/nimro'
...
interface NimroConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useNimroConfig({ activeSessionIdRef }: NimroConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshNimroConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([getNimroConfig(), getNimroConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Nimro' built-in medium over the user's config.
  ...
  return { refreshNimroConfig, sttEnabled, voiceMaxRecordingSeconds }
}
