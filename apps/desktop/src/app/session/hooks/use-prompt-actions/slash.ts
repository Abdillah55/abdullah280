import { skillInvocationText } from '@nimro/shared'
import { type MutableRefObject, useCallback, useRef } from 'react'

import { getProfiles } from '@/nimro'
...
// 120s RPC budget (NIMRO_TUI_RPC_TIMEOUT_MS default) instead.
...
const renderWakeStatus = (status: WakeStatusResponse): string => {
  const lines = [
    ...
    `Phrase: "${status.phrase?.trim() || 'hey nimro'}"`,
  ...
  return useCallback(
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      const actionHandlers: Record<DesktopActionId, (ctx: SlashActionCtx) => Promise<void>> = {
        ...
        // lease. Sending it through slash.exec creates a separate NimroCLI in
