        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.nimroDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Nimro to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.nimroDesktop?.watchPreviewFile ||
      !window.nimroDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.nimroDesktop.onPreviewFileChanged(payload => {
    ...
    void window.nimroDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.nimroDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.nimroDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:nimro-preview')
