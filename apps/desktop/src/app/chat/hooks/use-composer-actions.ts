...
export async function attachmentPreviewDataUrl(filePath: string): Promise<string> {
  try {
    const local = await window.nimroDesktop?.readFileDataUrl?.(filePath)
...
export const NIMRO_PATHS_MIME = 'application/x-nimro-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Nimro sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.nimroDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(NIMRO_PATHS_MIME)
...
export function useComposerActions({
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.nimroDesktop?.saveImageBuffer(data, blobExtension(blob))
  ...
  const pasteClipboardImage = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      try {
        const path = await window.nimroDesktop?.saveClipboardImage()
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.nimroDesktop?.getPathForFile ? window.nimroDesktop.getPathForFile(file) : ''
