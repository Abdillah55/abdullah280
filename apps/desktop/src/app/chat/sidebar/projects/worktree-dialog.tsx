...
import type { NimroGitBranch } from '@/global'
...
const branchActionLabel = (branch: NimroGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<NimroGitBranch[]>([])
  ...
  const convert = async (branch: NimroGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
