...
import type { SessionInfo } from '@/nimro'
...
// zero Nimro sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, NimroGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, NimroGitWorktree[]>>({})
