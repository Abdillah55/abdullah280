...
// Nimro Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.nimroDesktop
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.nimroDesktop?.sshConfigHosts) {
      return
    }

    let cancelled = false
    void window.nimroDesktop
      .sshConfigHosts()
      .then(result => {
        if (!cancelled) {
  ...
  const payload = () => ({
    ...
    sshRemoteNimroPath: state.sshRemoteNimroPath.trim(),
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.nimroDesktop.applyConnectionConfig(payload())
        : await window.nimroDesktop.saveConnectionConfig(payload())
      ...
      const errors = {
        ...
        'nimro-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.nimroDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.nimroDesktop.oauthLoginConnectionConfig(trimmedUrl)
      ...
      if (result.connected) {
        const refreshed = await window.nimroDesktop.getConnectionConfig(scope)
  ...
  const signOut = async () => {
    ...
    try {
      await window.nimroDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.nimroDesktop.getConnectionConfig(scope)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.nimroDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.nimroDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.nimroDesktop.testConnectionConfig(payload())
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.nimroDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.nimroDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Nimro Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/nimro"
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteNimroPath: event.target.value }))}
                placeholder={g.sshNimroPathPlaceholder}
                value={state.sshRemoteNimroPath}
              />
            }
            description={g.sshNimroPathDesc}
            title={g.sshNimroPathTitle}
          />
          {scope !== null ? (
            <ListRow
              action={
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          <ListRow
            action={
              <Button onClick={() => void window.nimroDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
