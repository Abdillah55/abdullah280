...
import type { ToolsetConfig } from '@/types/nimro'
...
const getNimroConfigSchema = vi.fn()
...
vi.mock('@/nimro', () => ({
  ...
  getNimroConfigSchema: () => getNimroConfigSchema(),
...
beforeEach(() => {
  ...
  getNimroConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getNimroConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveNimroConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveNimroConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
