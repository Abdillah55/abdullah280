...
function reveal(file: string) {
  void window.nimroDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // nimro_home breaks against a remote backend (#66899).
    const dir = await window.nimroDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.nimroDesktop?.openDir?.(dir)
