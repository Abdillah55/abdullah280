...
import { disconnectOAuthProvider, listOAuthProviders } from '@/nimro'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/nimro'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.nimroDesktop?.terminal)
...
//      in nimro_cli/provider_catalog.py) — the SAME provider identity
//      `nimro model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Nimro can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Nimro can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Nimro never deletes creds another tool owns behind a silent API call.
