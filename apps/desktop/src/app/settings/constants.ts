...
export const EMPTY_SELECT_VALUE = '__nimro_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Nimro & Nous-trained models',
  ...
  { prefix: 'NIMRO_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // nimro_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  agent: {
    ...
    maxTurns: 'Upper bound for tool-calling turns before Nimro stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Nimro can read from one file request.',
  approvals: {
    mode: 'How Nimro handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Nimro updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
