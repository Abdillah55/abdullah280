...
import { getElevenLabsVoices, getNimroConfigSchema, saveNimroConfig } from '@/nimro'
...
import type { ConfigFieldSchema, NimroConfigRecord } from '@/types/nimro'

import { setNimroConfigCache, useNimroConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: NimroConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<NimroConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useNimroConfigRecord()
  ...
  const {
    ...
    queryKey: ['nimro-config-schema'],
    queryFn: getNimroConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveNimroConfig(config)
  ...
  const updateConfig = (next: NimroConfigRecord) => {
