...
import { invalidateNimroConfig, setNimroConfigCache, useNimroConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `nimro model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in nimro_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useNimroConfigRecord()
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `nimro model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Nimro runs the flow for you.`}
          </p>
        )}
