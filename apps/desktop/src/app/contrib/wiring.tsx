...
import { getSessionMessages, triggerCronJob } from '@/nimro'
...
import { useNimroConfig } from '../session/hooks/use-nimro-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshNimroConfig, sttEnabled, voiceMaxRecordingSeconds } = useNimroConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('nimro:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('nimro:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshNimroConfig(true)
    void refreshActiveProfile()
  }, [activeGatewayProfile, refreshCurrentModel, refreshNimroConfig])
