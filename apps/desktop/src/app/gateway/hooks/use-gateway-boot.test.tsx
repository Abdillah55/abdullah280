...
// bug that drives the REAL useGatewayBoot hook + REAL NimroGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshNimroConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { nimroDesktop?: unknown }).nimroDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { nimroDesktop?: unknown }).nimroDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForNimro) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startNimro()'s remote branch awaits waitForNimro() for 45s before it
    ...
    ;(window as { nimroDesktop?: unknown }).nimroDesktop = desktop
    ...
    // After ~45s waitForNimro gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Nimro backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Nimro couldn't start" overlay — the socket is open
