import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@nimro/shared'
import { useEffect, useRef } from 'react'

import type { NimroConnection } from '@/global'
import { NimroGateway } from '@/nimro'
...
import type { RpcEvent } from '@/types/nimro'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.nimroDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: NimroGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: NimroConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Nimro...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Nimro…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Nimro gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshNimroConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshNimroConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startNimro re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new NimroGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshNimroConfig(),
          ...
          // retry it), not brick boot behind the "Nimro couldn't start"
