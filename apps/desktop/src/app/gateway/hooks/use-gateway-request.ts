...
import type { NimroGateway } from '@/nimro'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as NimroGateway | null
  const gatewayRef = useRef<NimroGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.nimroDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<NimroGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as NimroGateway | null
      }),
    []
  )
