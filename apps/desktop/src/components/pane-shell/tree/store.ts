...
const STORAGE_KEY = 'nimro.desktop.layoutTree.v2'

writeKey('nimro.desktop.layoutTree.v1', null)
 ...
 */
export const $activePresetId = atom<string>(readKey('nimro.desktop.layoutPreset.active') ?? 'default')

export function markActivePreset(id: string) {
  $activePresetId.set(id)
  writeKey('nimro.desktop.layoutPreset.active', id)
}
 ...
 */
const DISMISSED_KEY = 'nimro.desktop.dismissedPanes.v1'
...
const USER_PLACED_KEY = 'nimro.desktop.userPlacedPanes.v1'
...
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__NIMRO_LAYOUT_TREE__ = {
    close: closeTreePane,
