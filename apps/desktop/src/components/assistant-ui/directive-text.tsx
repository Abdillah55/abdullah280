...
const NIMRO_REF_TYPES = WIRE_REFERENCE_KINDS
type NimroRefType = (typeof NIMRO_REF_TYPES)[number]
...
const NIMRO_DIRECTIVE_RE = referenceRe()
...
export const nimroDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(NIMRO_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...nimroDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Nimro directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.nimroDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.nimroDesktop?.readFileDataUrl(id)
