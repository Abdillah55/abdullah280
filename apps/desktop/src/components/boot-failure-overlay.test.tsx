...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Nimro gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.nimroDesktop
  Object.defineProperty(window, 'nimroDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'nimroDesktop', { configurable: true, value: original })
}
