...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Nimro doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in nimro_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `nimro model` uses — instead of
...
export type NimroConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `nimro model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors nimro_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Nimro, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `nimro tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Nimro); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/nimro/update/check` — the backend's own update state.
