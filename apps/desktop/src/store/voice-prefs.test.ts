...
vi.mock('@/nimro', () => ({
  getNimroConfigRecord: vi.fn(async () => ({})),
  saveNimroConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye nimro', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye nimro')
  })
