import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@nimro/shared'
...
const isOpen = (gateway: NimroGateway | null): boolean => gateway?.connectionState === 'open'
...
interface GatewayRegistryState {
  ...
  primaryGateway: NimroGateway | null
  primaryProfile: string
  activeKey: string
  secondaries: Map<string, Secondary>
  $gateway: ReturnType<typeof atom<NimroGateway | null>>
}

const STATE_KEY = Symbol.for('nimro.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<NimroGateway | null>(null)
  }
}
...
export function setPrimaryGateway(gateway: NimroGateway | null, profile = 'default'): void {
...
export function activeGateway(): NimroGateway | null {
...
function createSecondary(profile: string): Secondary {
  const gateway = new NimroGateway()
...
export async function ensureActiveGatewayOpen(): Promise<NimroGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
