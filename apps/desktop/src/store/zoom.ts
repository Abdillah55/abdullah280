...
export function setZoomPercent(percent: number): void {
  window.nimroDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.nimroDesktop?.zoom) {
  void window.nimroDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.nimroDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
