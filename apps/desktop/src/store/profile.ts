...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/nimro'
...
import type { ProfileInfo } from '@/types/nimro'
...
// /api/profiles/active `current`). "default" is the root ~/.nimro. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'nimro.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'nimro.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.nimroDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new NIMRO_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.nimroDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.nimroDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'nimro.desktop.showAllProfiles'
...
// Switch to the default (root ~/.nimro) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.nimroDesktop?.touchBackend?.(target).catch(() => undefined)
}
