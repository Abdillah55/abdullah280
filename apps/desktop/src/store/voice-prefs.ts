...
import { getNimroConfigRecord, saveNimroConfig } from '@/nimro'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getNimroConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveNimroConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
