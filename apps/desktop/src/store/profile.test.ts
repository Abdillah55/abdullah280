...
const { getProfiles } = await import('@/nimro')
...
const remoteConn = (over: Partial<NimroConnection> = {}): NimroConnection =>
  ({ baseUrl: 'https://nimro-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as NimroConnection

const localConn = (over: Partial<NimroConnection> = {}): NimroConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as NimroConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<NimroConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { nimroDesktop: { getConnection } })
