...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.nimroDesktop?.openSessionWindow === 'function'
}

// True when the shell can open a full peer app window (⌘⇧N / "New Window").
export function canOpenNewWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.nimroDesktop?.openWindow === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await runWindowOpen(
    () => window.nimroDesktop.openSessionWindow(sessionId, opts),
...
export async function openNewWindow(): Promise<void> {
  ...
  await runWindowOpen(() => window.nimroDesktop.openWindow(), 'Could not open a new window')
}
