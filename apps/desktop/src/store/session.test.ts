...
describe('workspaceCwdForNewSession', () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem('nimro.desktop.workspace-cwd')
    window.localStorage.removeItem('nimro.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-a.default')
    window.localStorage.removeItem('nimro.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default')
  })

  it('prefers the configured default over the sticky remembered workspace', () => {
    window.localStorage.setItem('nimro.desktop.workspace-cwd', '/home/user/sticky')
  ...
  it('keeps remote workspace memory separate from local and other remotes', () => {
    window.localStorage.setItem('nimro.desktop.workspace-cwd', '/local/project')
    $currentCwd.set('/live/session/path')
...
describe('remembered session id (per profile)', () => {
  ...
  it('keeps the default profile on the legacy unsuffixed key for back-compat', () => {
    ...
    localStorage.setItem('nimro.desktop.lastSessionId', 'legacy-session')
...
describe('remembered route (per profile)', () => {
  ...
  it('keeps the default profile on the legacy unsuffixed key for back-compat', () => {
    localStorage.setItem('nimro.desktop.lastRoute', '/skills')
