import type { ConnectionState } from '@nimro/shared'
import { atom, computed } from 'nanostores'

import { lastVisibleMessageIsUser } from '@/app/chat/thread-loading'
import type { ContextSuggestion } from '@/app/types'
import type { NimroConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/nimro'
...
const COMPOSER_MODEL_KEY = 'nimro.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'nimro.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'nimro.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'nimro.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'nimro.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'nimro.desktop.lastSessionId'
...
const LAST_ROUTE_KEY = 'nimro.desktop.lastRoute'
...
function workspaceCwdKey(connection: NimroConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.nimroDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.nimroDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<NimroConnection | null>(null)
...
export const setConnection = (next: Updater<NimroConnection | null>) => updateAtom($connection, next)
...
// until config loads, and re-seeded on every profile switch by useNimroConfig.
