 ...
 * app's bounds and stays visible while Nimro is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'nimro.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'nimro.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.nimroDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.nimroDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.nimroDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.nimroDesktop?.petOverlay?.close()
}
