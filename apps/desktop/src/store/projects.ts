...
import type { NimroGitBaseBranch, NimroGitBranch } from '@/global'
import { getNimroConfig, type NimroGateway } from '@/nimro'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/nimro'
...
const PROJECT_SCOPE_KEY = 'nimro.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Nimro gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active Nimro profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: NimroGateway): Promise<void> {
...
const repoScanStates = new WeakMap<NimroGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<NimroGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getNimroConfig(context.profile))
...
// repo, returning where Nimro should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<NimroGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<NimroGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.nimroDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.nimroDesktop?.writeClipboard?.(path)
  }
}
