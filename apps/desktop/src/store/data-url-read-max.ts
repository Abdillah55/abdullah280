 ...
 * (`nimro:data-url-read-max:*`) and the default/clamp in electron/hardening.ts.
 */
...
export async function refreshDataUrlReadMaxMb(): Promise<number> {
  const api = window.nimroDesktop?.dataUrlReadMax
...
if (typeof window !== 'undefined' && window.nimroDesktop?.dataUrlReadMax) {
