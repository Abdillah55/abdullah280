...
export function closeFindBar(): void {
  ...
  void window.nimroDesktop?.stopFindInPage()
}
...
export function setFindQuery(query: string): void {
  ...
  void window.nimroDesktop?.findInPage(query, { forward: true, findNext: false })
}
...
export function findNext(): void {
  ...
  if (query) {
    void window.nimroDesktop?.findInPage(query, { forward: true, findNext: true })
  }
}
...
export function findPrevious(): void {
  ...
  if (query) {
    void window.nimroDesktop?.findInPage(query, { forward: false, findNext: true })
  }
}
...
export function initFindInPageListener(): () => void {
  ...
  if (listenerRefs === 1) {
    detachListener = window.nimroDesktop?.onFoundInPage?.(result => {
