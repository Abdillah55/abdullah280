...
import type { NimroReviewFile, NimroReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Nimro' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'nimro.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'nimro.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'nimro.desktop.reviewTreeMode'
const SELECTED_KEY = 'nimro.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<NimroReviewFile[]>([])
...
export const $reviewShipInfo = atom<NimroReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['nimroDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: NimroReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly NimroReviewFile[], path: string): NimroReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.nimroDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.nimroDesktop?.openExternal?.(url)
    }
