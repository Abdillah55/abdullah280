...
const STORAGE_KEY = 'nimro:native-notifications'
...
// "Backgrounded" = the user isn't on Nimro. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.nimroDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.nimroDesktop
