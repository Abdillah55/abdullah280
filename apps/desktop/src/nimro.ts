import { JsonRpcGatewayClient } from '@nimro/shared'
...
// "Timed out connecting to Nimro backend" that hangs the UI (#48504).
//
...
export class NimroGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Nimro gateway connection closed',
      connectErrorMessage: 'Could not connect to Nimro gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Nimro gateway is not connected',
...
// the call; each pooled backend already has its own NIMRO_HOME, so no backend
...
/** Options for a plugin REST call — mirrors the app's own `nimroDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see NimroApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.nimroDesktop?.api) {
    throw new Error('Nimro desktop bridge unavailable')
  }

  const suffix = pluginPathSuffix('pluginRest', path)

  return window.nimroDesktop.api<T>({
...
export function pluginSocket(pluginId: string, path: string, onMessage: (data: unknown) => void): () => void {
  ...
  const connect = async () => {
    const connection = await window.nimroDesktop.getConnection().catch(() => null)
...
export async function listSessions(
  ...
  const result = await window.nimroDesktop.api<PaginatedSessions>({
    path:
...
// "Error invoking remote method 'nimro:api': Error: 404: ..." through the
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await window.nimroDesktop.api<SidebarSessionsResponse>({
...
export function setSessionArchived(id: string, archived: boolean, profile?: string | null): Promise<{ ok: boolean }> {
  return window.nimroDesktop.api<{ ok: boolean }>({
    ...(profile ? { profile } : {}),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return window.nimroDesktop.api<SessionSearchResponse>({
...
export function getSession(id: string, profile?: string | null): Promise<SessionInfo> {
  ...
  return window.nimroDesktop.api<SessionInfo>({
    ...(profile ? { profile } : {}),
...
export function getSessionMessages(id: string, profile?: string | null): Promise<SessionMessagesResponse> {
  ...
  return window.nimroDesktop.api<SessionMessagesResponse>({
    ...(profile ? { profile } : {}),
...
export function renameSession(
  ...
  return window.nimroDesktop.api<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
...
export function getGlobalModelInfo(): Promise<ModelInfoResponse> {
  return window.nimroDesktop.api<ModelInfoResponse>({
    ...profileScoped(),
    path: '/api/model/info',
...
export function getStatus(): Promise<StatusResponse> {
  return window.nimroDesktop.api<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return window.nimroDesktop.api<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getNimroConfig(profile?: string): Promise<NimroConfig> {
  return window.nimroDesktop.api<NimroConfig>({
...
export function getNimroConfigRecord(): Promise<NimroConfigRecord> {
  return window.nimroDesktop.api<NimroConfigRecord>({
    ...profileScoped(),
    path: '/api/config'
  })
}

export function getNimroConfigDefaults(): Promise<NimroConfigRecord> {
  return window.nimroDesktop.api<NimroConfigRecord>({
    ...profileScoped(),
...
export function getNimroConfigSchema(): Promise<ConfigSchemaResponse> {
  return window.nimroDesktop.api<ConfigSchemaResponse>({
    ...profileScoped(),
    path: '/api/config/schema'
  })
}

export function saveNimroConfig(config: NimroConfigRecord): Promise<{ ok: boolean }> {
  return window.nimroDesktop.api<{ ok: boolean }>({
    ...profileScoped(),
    path: '/api/config',
    method: 'PUT',
...
export function getMemoryProviderConfig(provider: string): Promise<MemoryProviderConfig> {
  return window.nimroDesktop.api<MemoryProviderConfig>({
    ...profileScoped(),
...
export function getEnvVars(): Promise<Record<string, EnvVarInfo>> {
  return window.nimroDesktop.api<Record<string, EnvVarInfo>>({
    ...profileScoped(),
    path: '/api/env'
  })
}
...
export function validateProviderCredential(
  ...
  return window.nimroDesktop.api<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return window.nimroDesktop.api<CustomEndpointsResponse>({
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return window.nimroDesktop.api<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return window.nimroDesktop.api<{ ok: boolean; provider: string; model: string }>({
...
export function revealEnvVar(key: string): Promise<{ key: string; value: string }> {
  return window.nimroDesktop.api<{ key: string; value: string }>({
    ...profileScoped(),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return window.nimroDesktop.api<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return window.nimroDesktop.api<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string): Promise<OAuthStartResponse> {
  return window.nimroDesktop.api<OAuthStartResponse>({
    ...profileScoped(),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return window.nimroDesktop.api<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(providerId: string, sessionId: string): Promise<OAuthPollResponse> {
  return window.nimroDesktop.api<OAuthPollResponse>({
    ...profileScoped(),
...
export function startMemoryProviderOAuth(provider: string): Promise<MemoryProviderOAuthStatus> {
  return window.nimroDesktop.api<MemoryProviderOAuthStatus>({
    ...profileScoped(),
...
export function getSkills(): Promise<SkillInfo[]> {
  return window.nimroDesktop.api<SkillInfo[]>({
    ...profileScoped(),
    path: '/api/skills'
  })
}
...
export function getStarmapGraph(): Promise<StarmapGraph> {
  return window.nimroDesktop.api<StarmapGraph>({
    ...profileScoped(),
...
export function getLearningNode(id: string): Promise<LearningNodeDetail> {
  return window.nimroDesktop.api<LearningNodeDetail>({
    ...profileScoped(),
...
export function deleteLearningNode(id: string): Promise<{ message: string; ok: boolean }> {
  return window.nimroDesktop.api<{ message: string; ok: boolean }>({
    ...profileScoped(),
...
export function setSkillEnabled(
  ...
  return window.nimroDesktop.api<{ ok: boolean; name: string; enabled: boolean }>({
    ...profileScoped(),
...
export function testMcpServer(name: string): Promise<McpTestResult> {
  return window.nimroDesktop.api<McpTestResult>({
    ...profileScoped(),
 ...
 *  `saveNimroConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function authMcpServer(name: string): Promise<McpOAuthFlow> {
  return window.nimroDesktop.api<McpOAuthFlow>({
    ...profileScoped(),
...
export function getToolsets(): Promise<ToolsetInfo[]> {
  return window.nimroDesktop.api<ToolsetInfo[]>({
    ...profileScoped(),
...
export function getToolsetConfig(name: string): Promise<ToolsetConfig> {
  return window.nimroDesktop.api<ToolsetConfig>({
    ...profileScoped(),
...
export function getToolsetModels(name: string, provider?: string): Promise<ToolsetModelsResponse> {
  ...
  return window.nimroDesktop.api<ToolsetModelsResponse>({
    ...profileScoped(),
...
export function selectToolsetModel(
  ...
  return window.nimroDesktop.api<{ ok: boolean; name: string; model: string }>({
    ...profileScoped(),
...
export function selectToolsetProvider(
  ...
  return window.nimroDesktop.api<SelectToolsetProviderResponse>({
    ...profileScoped(),
...
export function runToolsetPostSetup(name: string, key: string): Promise<ActionResponse & { key: string }> {
  return window.nimroDesktop.api<ActionResponse & { key: string }>({
    ...profileScoped(),
...
export function getTerminalBackends(): Promise<TerminalBackendsResponse> {
  return window.nimroDesktop.api<TerminalBackendsResponse>({
    ...profileScoped(),
...
export function selectTerminalBackend(backend: string): Promise<{ ok: boolean; backend: string }> {
  return window.nimroDesktop.api<{ ok: boolean; backend: string }>({
    ...profileScoped(),
...
export function getComputerUseStatus(): Promise<ComputerUseStatus> {
  return window.nimroDesktop.api<ComputerUseStatus>({
    ...profileScoped(),
...
export function grantComputerUsePermissions(): Promise<ActionResponse> {
  return window.nimroDesktop.api<ActionResponse>({
    ...profileScoped(),
...
export function getMessagingPlatforms(): Promise<MessagingPlatformsResponse> {
  return window.nimroDesktop.api<MessagingPlatformsResponse>({
...
export function updateMessagingPlatform(
  ...
  return window.nimroDesktop.api<{ ok: boolean; platform: string }>({
...
export function testMessagingPlatform(platformId: string): Promise<MessagingPlatformTestResponse> {
  return window.nimroDesktop.api<MessagingPlatformTestResponse>({
...
export function getPairing(): Promise<PairingResponse> {
  return window.nimroDesktop.api<PairingResponse>({
    ...profileScoped(),
    path: '/api/pairing'
  })
}
...
export function approvePairing(platform: string, requestId: string): Promise<{ ok: boolean; user: PairingUser }> {
  return window.nimroDesktop.api<{ ok: boolean; user: PairingUser }>({
    ...profileScoped(),
...
export function getWebhooks(): Promise<WebhooksResponse> {
  return window.nimroDesktop.api<WebhooksResponse>({
    ...profileScoped(),
    path: '/api/webhooks'
  })
}
...
export function enableWebhooks(): Promise<WebhookEnableResponse> {
  return window.nimroDesktop.api<WebhookEnableResponse>({
    ...profileScoped(),
...
export function createWebhook(body: WebhookCreatePayload): Promise<WebhookCreateResponse> {
  return window.nimroDesktop.api<WebhookCreateResponse>({
    ...profileScoped(),
    path: '/api/webhooks',
    method: 'POST',
    body
...
export function setWebhookEnabled(
  ...
  return window.nimroDesktop.api<{ enabled: boolean; name: string; ok: boolean }>({
    ...profileScoped(),
...
// Cron jobs are stored per-profile (<NIMRO_HOME>/cron/jobs.json), and the
...
export function getCronJobs(profile?: string): Promise<CronJob[]> {
  ...
  return window.nimroDesktop.api<CronJob[]>({
    ...profileScoped(),
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await window.nimroDesktop.api<{ runs: SessionInfo[] }>({
    ...profileScoped(),
...
export async function getCronDeliveryTargets(): Promise<CronDeliveryTarget[]> {
  const { targets } = await window.nimroDesktop.api<{ targets: CronDeliveryTarget[] }>({
    ...profileScoped(),
...
export function getAutomationBlueprints(): Promise<{ blueprints: AutomationBlueprint[] }> {
  return window.nimroDesktop.api<{ blueprints: AutomationBlueprint[] }>({
    ...profileScoped(),
...
export function getProfiles(): Promise<ProfilesResponse> {
  return window.nimroDesktop.api<ProfilesResponse>({
    path: '/api/profiles',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
}
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return window.nimroDesktop.api<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string): Promise<{ ok: boolean; path: string }> {
  return window.nimroDesktop.api<{ ok: boolean; path: string }>({
    path: `/api/profiles/${encodeURIComponent(name)}`,
    method: 'DELETE'
  })
}
...
export function getProfileSoul(name: string): Promise<ProfileSoul> {
  return window.nimroDesktop.api<ProfileSoul>({
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return window.nimroDesktop.api<ProfileSetupCommand>({
    path: `/api/profiles/${encodeURIComponent(name)}/setup-command`
  })
}

export function getUsageAnalytics(days = 30): Promise<AnalyticsResponse> {
  return window.nimroDesktop.api<AnalyticsResponse>({
    ...profileScoped(),
...
export function getGlobalModelOptions(opts?: {
  ...
  return window.nimroDesktop.api<ModelOptionsResponse>({
    ...profileScoped(),
...
// curation `nimro model` does — for Nous it honors the free/paid tier so a
// free user gets a free model instead of a paid default.
export function getRecommendedDefaultModel(provider: string): Promise<RecommendedDefaultModel> {
  return window.nimroDesktop.api<RecommendedDefaultModel>({
    ...profileScoped(),
...
export function getAuxiliaryModels(): Promise<AuxiliaryModelsResponse> {
  return window.nimroDesktop.api<AuxiliaryModelsResponse>({
    ...profileScoped(),
...
export function getMoaModels(): Promise<MoaConfigResponse> {
  return window.nimroDesktop.api<MoaConfigResponse>({
    ...profileScoped(),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(body: MoaConfigResponse): Promise<MoaConfigResponse & { ok: boolean }> {
  return window.nimroDesktop.api<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function setModelAssignment(body: ModelAssignmentRequest): Promise<ModelAssignmentResponse> {
  return window.nimroDesktop.api<ModelAssignmentResponse>({
    ...profileScoped(),
    path: '/api/model/set',
    method: 'POST',
    body
...
export function updateNimro(): Promise<ActionResponse> {
  return window.nimroDesktop.api<ActionResponse>({
    ...profileScoped(),
    path: '/api/nimro/update',
    method: 'POST'
  })
}
...
export function checkNimroUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return window.nimroDesktop.api<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/nimro/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200): Promise<ActionStatusResponse> {
  return window.nimroDesktop.api<ActionStatusResponse>({
    ...profileScoped(),
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return window.nimroDesktop.api<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return window.nimroDesktop.api<AudioSpeakResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(): Promise<ElevenLabsVoicesResponse> {
  return window.nimroDesktop.api<ElevenLabsVoicesResponse>({
...
// ---------------------------------------------------------------------------
// Skills hub — search / preview / scan / install (parity with `nimro skills`
...
export function getSkillHubSources(): Promise<SkillHubSourcesResponse> {
  return window.nimroDesktop.api<SkillHubSourcesResponse>({
    ...profileScoped(),
...
export function searchSkillsHub(query: string, source = 'all', limit = 20): Promise<SkillHubSearchResponse> {
  ...
  return window.nimroDesktop.api<SkillHubSearchResponse>({
    ...profileScoped(),
...
export function previewSkillHub(identifier: string): Promise<SkillHubPreview> {
  return window.nimroDesktop.api<SkillHubPreview>({
    ...profileScoped(),
...
export function scanSkillHub(identifier: string): Promise<SkillHubScanResult> {
  return window.nimroDesktop.api<SkillHubScanResult>({
    ...profileScoped(),
...
// `nimro mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveNimroConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return window.nimroDesktop.api<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function getMcpCatalog(): Promise<McpCatalogResponse> {
  return window.nimroDesktop.api<McpCatalogResponse>({
    ...profileScoped(),
...
export function installMcpCatalogEntry(
  ...
  return window.nimroDesktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `nimro memory` / `nimro curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return window.nimroDesktop.api<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return window.nimroDesktop.api<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return window.nimroDesktop.api<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return window.nimroDesktop.api<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `nimro doctor` / `nimro security
// audit` / `nimro backup` / `nimro debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return window.nimroDesktop.api<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return window.nimroDesktop.api<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return window.nimroDesktop.api<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return window.nimroDesktop.api<DebugShareResponse>({
