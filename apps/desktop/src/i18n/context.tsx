...
import { getNimroConfigRecord, type NimroConfigRecord, saveNimroConfig } from '@/nimro'
...
export interface I18nConfigClient {
  getConfig: () => Promise<NimroConfigRecord>
  saveConfig: (config: NimroConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.nimroDesktop?.api) {
      return Promise.resolve({})
    }

    return getNimroConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.nimroDesktop?.api) {
...
export function getConfigDisplayLanguage(config: NimroConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: NimroConfigRecord, locale: Locale): NimroConfigRecord {
