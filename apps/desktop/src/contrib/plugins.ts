 ...
 *    `NimroPlugin` registers automatically (vite glob — drop a folder in).
 *    None ship in-tree today; reference/demo plugins live in the companion
 *    `nimro-example-plugins` repo.
 *  - RUNTIME: the on-disk door (`<nimro home>/desktop-plugins/<name>/plugin.js`)
...
const modules = import.meta.glob<{ default: NimroPlugin }>('../plugins/*/plugin.{ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default NimroPlugin export — skipped`)
