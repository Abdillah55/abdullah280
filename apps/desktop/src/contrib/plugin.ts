 ...
 * `NimroPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/nimro'
...
export type { PluginRestOptions } from '@/nimro'
 ...
 *  under `nimro.plugin.<id>.` — plugins can't read or clobber each other. */
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `nimro.plugin.${pluginId}.${key}`
