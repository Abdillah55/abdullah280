 ...
 *   (`@nimro/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default NimroPlugin -> register(ctx)
 *
 ...
 * and `<nimro home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { createPluginContext, type NimroPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@nimro/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @nimro/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: NimroPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default NimroPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<nimro home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    ...
    // connected backend — a remote backend's nimro_home is a remote path and
...
export function watchRuntimePlugins(): void {
  ...
  const startDirWatch = async (): Promise<boolean> => {
    ...
    try {
      ...
      // nimro_home, which is a remote path in remote mode (#66899).
      const root = await desktop.desktopPluginsRoot?.()
