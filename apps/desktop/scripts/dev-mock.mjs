...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `nimro-dev-mock-${Date.now()}`))
  ...
  return { root, nimroHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(nimroHome, mockUrl) {
  fs.writeFileSync(
    path.join(nimroHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(nimroHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.nimroHome, mock.url)
  console.log(`  NIMRO_HOME: ${sandbox.nimroHome}`)
  ...
  const env = {
    ...
    NIMRO_DESKTOP_APP_NAME: `NimroDevMock-${Date.now()}`,
  }
