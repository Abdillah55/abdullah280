 ...
 * product name (`Nimro`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Nimro'
 *
 * This is a hard failure with no obvious cause for the user — `nimro desktop`
 ...
 * (the mac-unpacked Nimro.app bundle) and Windows (win-unpacked), so we
 ...
 * Nimro.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `nimro desktop --build-only` (nimro_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Nimro.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Nimro'}.exe`
