...
//   · its own NIMRO_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · NIMRO_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated NIMRO_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate NIMRO_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = nimroHome ?? mkTemp('nimro-perf-home-')
  const userData = userDataDir ?? mkTemp('nimro-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !nimroHome) {
    seedConfigFrom(join(homedir(), '.nimro'), home)
  }
  ...
  try {
    ...
    // NIMRO_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set NIMRO_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    if (devUrl) {
      env.NIMRO_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'nimro-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'nimro-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.nimro'), home)
