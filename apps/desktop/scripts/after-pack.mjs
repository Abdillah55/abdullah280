 ...
 *
 * Stamps the Nimro icon + identity onto the packed Windows Nimro.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `nimro desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Nimro')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Nimro'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Nimro.exe keeps the stock Electron icon`)
  }
}
