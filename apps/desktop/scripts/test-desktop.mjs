...
// no longer bundles the Nimro Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Nimro.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Nimro'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Nimro.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Nimro'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default NIMRO_HOME for non-sandboxed runs -- matches main.ts's
// resolveNimroHome(). On Windows it's %LOCALAPPDATA%\nimro; elsewhere
// it's ~/.nimro. The fresh-install sandbox launchFresh() sets its own
// NIMRO_HOME and never touches this.
const DEFAULT_NIMRO_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'nimro')
  }
  return path.join(os.homedir(), '.nimro')
})()
const VENV_ROOT = path.join(DEFAULT_NIMRO_HOME, 'nimro-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'nimro-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.NIMRO_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Nimro-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Nimro-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Nimro-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Nimro-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.NIMRO_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.NIMRO_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const nimroHome = path.join(sandbox, 'nimro-home')
  ...
  env.NIMRO_DESKTOP_CWD = cwd
  env.NIMRO_DESKTOP_IGNORE_EXISTING = '1'
  env.NIMRO_DESKTOP_TEST_MODE = 'fresh-install'
  env.NIMRO_DESKTOP_USER_DATA_DIR = userDataDir
  env.NIMRO_HOME = nimroHome
  delete env.NIMRO_DESKTOP_NIMRO
  delete env.NIMRO_DESKTOP_NIMRO_ROOT
  ...
  console.log(`  NIMRO_HOME: ${nimroHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(nimroHome, 'nimro-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Nimro Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of nimro_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'nimro-agent', 'nimro_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Nimro
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + NIMRO_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  NIMRO_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
