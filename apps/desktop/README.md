...
<p align="center">
  <a href="https://github.com/NousResearch/nimro-agent/releases"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Nimro Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Nimro surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Nimro and hear it back.</td></tr>
...
### Install with Nimro (recommended)

Already have the Nimro CLI? Just run:

```bash
nimro desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Nimro gateway or install Nimro locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Nimro Desktop website.](https://nimro-agent.nousresearch.com/).
...
```bash
# throwaway NIMRO_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Nimro Agent runtime into `NIMRO_HOME`
(`~/.nimro`, or `%LOCALAPPDATA%\nimro` on Windows), using the same layout as a
CLI install.
...
- **Nimro Agent** runs as a headless `nimro serve` process and exposes the
...
1. `NIMRO_DESKTOP_NIMRO_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `NIMRO_DESKTOP_NIMRO`, or `nimro` on `PATH`
5. a system Python that can import the Nimro runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Nimro
...
screen offers **Connect to existing Nimro** before starting the local installer.
...
terminal commands, and file operations run against the remote Nimro host, not
...
Boot logs land in `NIMRO_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\nimro\nimro-agent\.nimro-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\nimro\nimro-agent\venv"
```

> The default Nimro home on Windows is `%LOCALAPPDATA%\nimro`. Set the `NIMRO_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://nimro-agent.nousresearch.com/docs/)
