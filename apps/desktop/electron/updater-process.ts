 ...
 *
 * The Tauri installer self-copies into NIMRO_HOME on *every* platform
 * (`nimro-setup.exe` on Windows, `nimro-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_nimro_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `nimro update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(nimroHome, 'nimro-setup.exe')
 ...
 *
 * `copy_self_to_nimro_home` deliberately no-ops during `--update`
 ...
 * "Another Nimro update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `nimro update`, which
 ...
 * the installer is written to NIMRO_HOME at install/repair time, making mtime
