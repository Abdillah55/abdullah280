 ...
 * the three user-facing uninstall modes to the `nimro uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m nimro_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `nimro_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'nimro_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Nimro.app/Contents/MacOS/Nimro  → …/Nimro.app
 *   Windows: …\Nimro\Nimro.exe                 → …\Nimro  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Nimro.app/Contents/MacOS/Nimro → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Nimro.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Nimro.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Nimro$/i.test(dir) || /[\\/]nimro-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/nimro
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import nimro_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, nimroHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import nimro_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Nimro install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "NIMRO_HOME=${String(nimroHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
