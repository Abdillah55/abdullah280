...
const READY_RE = /^NIMRO_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitNimroPath = '') {
  const explicit = psLiteral(explicitNimroPath)

  const script = [
    '$ErrorActionPreference="Stop"',
    `$explicit=${explicit}`,
    '$nimroHome=$env:NIMRO_HOME',
    'if(-not $nimroHome){$nimroHome=Join-Path $env:LOCALAPPDATA "nimro"}',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command nimro.exe -ErrorAction SilentlyContinue',
    'if($cmd){$candidates+=$cmd.Source}',
    '$candidates+=(Join-Path $nimroHome "nimro-agent\\venv\\Scripts\\nimro.exe")',
    '$candidates+=(Join-Path $HOME "nimro-agent\\.venv\\Scripts\\nimro.exe")',
    '$nimro=$candidates|Where-Object{Test-Path -LiteralPath $_ -PathType Leaf}|Select-Object -First 1',
    'if(-not $nimro){throw "Nimro is not installed on the remote Windows host."}',
    'if($explicit -and $nimro -ne $explicit){throw "The configured Nimro path is not an executable file."}',
    '$python=Join-Path (Split-Path $nimro) "python.exe"',
    'if(-not (Test-Path -LiteralPath $python -PathType Leaf)){throw "The remote Nimro Python runtime was not found."}',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;nimroHome=$nimroHome;nimroPath=$nimro;python=$python}|ConvertTo-Json -Compress'
  ].join(';')
...
async function detectRemotePlatform(ssh, explicitNimroPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitNimroPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'nimro_cli.windows_ssh_runtime', operation, ...args]
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.nimroPath === 'string' &&
    typeof lock.nimroHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.nimroPath === runtime.nimroPath &&
    lock.nimroHome === runtime.nimroHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertCurrent(signal)
  const runtime = await probeWindowsRemote(ssh, remoteNimroPath)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.nimroPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Nimro on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.nimroPath = inspection.path
  const nimroVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located nimro at ${runtime.nimroPath}`)
  ...
  try {
    spawned = await helper(
      ...
      'spawn',
      [],
      JSON.stringify({ ownershipId, spawnNonce, profile, nimroPath: runtime.nimroPath })
    )
  } catch (error) {
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForNimro(baseUrl, token)
    assertCurrent(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Nimro SSH"', 'powershell.exe -NoLogo')
