...
//   NIMRO_WIN_SSH_HOST   ssh alias/host of the Windows box
//   NIMRO_WIN_SSH_USER   remote user
//   NIMRO_WIN_SSH_NIMRO absolute path to the remote nimro.exe under test
const liveHost = process.env.NIMRO_WIN_SSH_HOST || ''
const liveUser = process.env.NIMRO_WIN_SSH_USER || ''
const configuredNimro = process.env.NIMRO_WIN_SSH_NIMRO || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Nimro-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredNimro)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredNimro.replace('nimro.exe', 'python.exe')}' -m nimro_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredNimro.replace('nimro.exe', 'python.exe')
        const terminate = `& '${python}' -m nimro_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.nimroPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m nimro_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m nimro_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
