// Unit tests for the pure Windows `nimro` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvNimroCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `nimro` shim shadows
//      the real nimro.cmd/nimro.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the nimro.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvNimroCommand() — must probe the venv python via
//      canImportNimroCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvNimroCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportNimroCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    nimroHome: '/fake/nimro-home',
...
test('resolveVenvNimroCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvNimroCommand('/root/venv/Scripts/nimro.exe', [], deps), null)
})

test('resolveVenvNimroCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvNimroCommand('/root/venv/Scripts/nimro.cmd', [], deps), null)
})

test('resolveVenvNimroCommand: returns null when the basename is not nimro/nimro.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvNimroCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvNimroCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvNimroCommand('/root/venv/bin/nimro.exe', [], deps), null)
})

test('resolveVenvNimroCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvNimroCommand('/root/venv/Scripts/nimro.exe', [], deps), null)
})

test('resolveVenvNimroCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportNimroCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvNimroCommand('/root/venv/Scripts/nimro.exe', ['serve'], deps)
...
test('resolveVenvNimroCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvNimroCommand('/root/venv/Scripts/nimro.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'nimro_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvNimroCommand: is case-insensitive on nimro.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvNimroCommand('/root/venv/Scripts/NIMRO.EXE', [], deps))
  assert.ok(resolveVenvNimroCommand('/root/venv/SCRIPTS/nimro.exe', [], deps))
})
