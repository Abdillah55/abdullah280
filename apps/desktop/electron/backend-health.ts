...
export interface NimroReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForNimroReady`).
   */
...
export async function waitForNimroReady(baseUrl: string, options: NimroReadyOptions): Promise<void> {
  ...
  throw new Error(`Nimro backend did not become ready: ${detail}`)
}
