...
export const ZOOM_STORAGE_KEY = 'nimro:desktop:zoomLevel'
 ...
 * but forgot to emit 'nimro:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('nimro:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
