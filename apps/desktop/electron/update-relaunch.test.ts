 ...
 *      internals; NIMRO_HOME / NIMRO_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.nimro/nimro-agent'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'nimro')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_Nimro12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/nimro/nimro', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/Nimro/nimro', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.nimro/nimro-agent/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'nimro'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'nimro')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'nimro://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'nimro://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves NIMRO_HOME + NIMRO_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    NIMRO_HOME: '/home/u/.nimro',
    NIMRO_DESKTOP_REMOTE_URL: 'http://box:9119',
    NIMRO_DESKTOP_REMOTE_TOKEN: 'secret',
    NIMRO_DESKTOP_NIMRO_ROOT: '/home/u/dev/nimro',
    NIMRO_DESKTOP_APP_NAME: 'NimroSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.nimro/nimro-agent/apps/desktop/release/linux-unpacked/Nimro',
    args: ['nimro://open/agent/42', "--note=it's fine"],
    env: { NIMRO_HOME: '/home/u/.nimro', NIMRO_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export NIMRO_HOME='\/home\/u\/\.nimro'/)
  assert.match(script, /export NIMRO_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(script, /exec '.*\/linux-unpacked\/Nimro' 'nimro:\/\/open\/agent\/42' '--note=it'\\''s fine'/)

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `nimro-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/Nimro/Nimro',
    args: [],
    env: {},
    cwd: ''
  })

  const tmp = path.join(os.tmpdir(), `nimro-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/Nimro\/Nimro'\n/)
})
