 ...
 *   1. NIMRO_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\nimro\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect NIMRO_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.NIMRO_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'nimro', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'nimro', 'git', 'usr', 'bin', 'bash.exe'))
  }
