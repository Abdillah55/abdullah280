...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'nimro-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/nimro-runner-test',
    sourceRepoRoot: null,
    nimroHome: '/tmp/nimro-runner-test',
    logRoot: '/tmp/nimro-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'nimro-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'nimro-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/nimro-agent',
      nimroHome: '/tmp/nimro'
    }),
    ['--dir', '/tmp/nimro-agent', '--nimro-home', '/tmp/nimro', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/nimro-agent', '--nimro-home', '/tmp/nimro', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/nimro',
      nimroHome: '/tmp/home'
    }),
    ['--dir', '/tmp/nimro', '--nimro-home', '/tmp/home', '--branch', 'main']
  )
})
