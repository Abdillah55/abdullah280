...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(NIMRO_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Nimro-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.nimro/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'nimro-desktop-ssh')
  }

  return path.join(os.homedir(), '.nimro', 'desktop-ssh')
}
