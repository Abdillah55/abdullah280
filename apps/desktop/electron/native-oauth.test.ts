...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/nimro', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/nimro/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/nimro'), 'https://gw.example.com/nimro/auth/native/refresh')
})
