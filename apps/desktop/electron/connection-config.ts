 ...
 *     `X-Nimro-Session-Token` header; WS uses `?token=`.
...
// nimro_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - nimro_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - nimro_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / nimro #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-nimro_session_at', '__Secure-nimro_session_at', 'nimro_session_at']
const RT_COOKIE_VARIANTS = ['__Host-nimro_session_rt', '__Secure-nimro_session_rt', 'nimro_session_rt']

// The Nous portal (NAS) does NOT use Nimro gateway session cookies — it is a
...
// Keep this aligned with nimro_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['nimro', 'test', 'tmp', 'root', 'sudo'])
...
// Nimro Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteNimroPath = String(entry.remoteNimroPath || '').trim()

  if (remoteNimroPath) {
    out.remoteNimroPath = remoteNimroPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Nimro installation. Preserve an explicit mapping
  // when it is a valid Nimro profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `NIMRO_HOME` scopes it.
 *
...
function pathWithGlobalRemoteProfile(path, profile, opts: ProfileRouteOptions = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://nimro.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a nimro session ACCESS-token cookie
 ...
 * the Nimro gateway session cookies, so cloud sign-in / discovery liveness
