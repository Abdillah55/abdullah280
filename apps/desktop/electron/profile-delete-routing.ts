// Profile-delete routing logic for the `nimro:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_nimro_home() and
...
/**
 * Parse a `nimro:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `nimro:api` request away from the primary/window backend
 ...
 * ensure_nimro_home() recreates the directory the delete just removed.
 */
