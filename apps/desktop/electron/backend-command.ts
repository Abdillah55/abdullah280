// Backend subcommand routing for the desktop-managed Nimro process.
//
// The desktop app launches its own headless backend via `nimro serve` — it
...
// app hasn't updated yet, or an older `nimro` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Nimro profile to pin via `--profile`.
 */
 ...
 * `-m nimro_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `nimro_cli/subcommands/dashboard.py` source registers
