 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Nimro dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Nimro install on the remote (login-shell probe),
 ...
 *     `NIMRO_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForNimro in.
 *
 * The minted NIMRO_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const READY_RE = /^NIMRO_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.nimro/desktop-ssh'
...
// Resolve the remote nimro executable. An EXPLICIT path is honored strictly
...
async function locateNimro(ssh, remoteNimroPath) {
  ...
  if (remoteNimroPath) {
    if (await isExecutable(remoteNimroPath)) {
      return resolveLauncher(remoteNimroPath)
    }

    const err: any = new Error(
      `The Nimro path you set is not an executable on the remote host: "${remoteNimroPath}". ` +
        'Check the path (it must be the full path to the `nimro` binary on the remote, e.g. ' +
        '~/nimro-agent/.venv/bin/nimro), or clear it to auto-detect.'
    )

    err.kind = 'nimro-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v nimro')}`)).trim()
  ...
  candidates.push('~/.local/bin/nimro')
  candidates.push('/usr/local/bin/nimro')
  candidates.push('~/.nimro/nimro-agent/venv/bin/nimro')
  ...
  const err: any = new Error(
    'Nimro is not installed on the remote host (could not find a `nimro` executable). ' +
      'Install it on the remote with:  curl -fsSL https://nimro-agent.nousresearch.com/install.sh | sh  ' +
      '— or set the Nimro path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<nimro> --version`,
// e.g. "Nimro Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH nimro a
// connection uses, so a stale/unexpected install is visible.
async function probeNimroVersion(ssh, nimroPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(nimroPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Nimro Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The NIMRO_HOME the remote dashboard will use (explicit env wins, else
// ~/.nimro). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteNimroHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${NIMRO_HOME:-$HOME/.nimro}"')).trim().split('\n').pop()

    return out || '~/.nimro'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Nimro home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'nimroPath', 'nimroHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(ssh, pid, spawnNonce, nimroPath = '') {
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !nimroPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(nimroPath)})\n` +
...
async function cleanupStale(ssh, ownershipId, lock, pidAlive = true) {
  if (pidAlive && lock && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.nimroPath))) {
    try {
      const result = (
        await ssh.exec(
...
function buildSpawnCommand(nimroPath, profile, opts: any = {}) {
  const nimro = expandRemotePath(nimroPath)
  ...
  const dashCmd = `env NIMRO_DESKTOP=1 ${nimro} ${profileArgs}${subCmd}`
...
async function remoteSupportsSshOwnership(ssh, nimroPath) {
  const nimro = expandRemotePath(nimroPath)

  const out = await ssh.exec(
    `help="$(${nimro} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(ssh, { nimroPath, profile, token, ownershipId }) {
  if (!(await remoteSupportsSshOwnership(ssh, nimroPath))) {
    const err: any = new Error(
      'The remote Nimro install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Nimro on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    out = await ssh.exec(buildSpawnCommand(nimroPath, profile, { spawnNonce, tokenFilePath, logPath }))
  } catch (error) {
    try {
      await ssh.exec(`rm -f ${expandRemotePath(tokenFilePath)}`)
    } catch {
 ...
 * opened SshConnection, forward/pickLocalPort/waitForNimro, a token-gated
...
async function connect(deps) {
  ...
  const nimroPath = await locateNimro(ssh, remoteNimroPath)
  log(`located nimro at ${nimroPath}`)
  const nimroVersion = await probeNimroVersion(ssh, nimroPath)

  if (nimroVersion) {
    log(`remote nimro version: ${nimroVersion}`)
  }

  const reuseToken = deps.reuseToken || ''
  const nimroHome = await probeRemoteNimroHome(ssh)
  const lock = await readLockfile(ssh, ownershipId)

  if (lock) {
    const pidAlive = await remotePidAlive(ssh, lock.pid)
    const owned = pidAlive && (await pidIsOurDashboard(ssh, lock.pid, lock.spawnNonce, lock.nimroPath))
    ...
    const reusable =
      ...
      lock.nimroPath === nimroPath &&
      lock.nimroHome === nimroHome
  ...
  try {
    ...
    assertNotAborted(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForNimro(baseUrl, spawnToken)
    assertNotAborted(signal)
