...
const SCAN_MODULE = 'nimro_cli._scan_venv_blockers'
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Nimro process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Nimro installation is free.\n' +
    '\n' +
    'Close other Nimro windows and terminals, then retry.  If the problem\n' +
    'persists, run `nimro update` in a terminal for detailed diagnostics.'
  )
}
