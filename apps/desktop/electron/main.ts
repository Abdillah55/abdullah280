...
import { buildDesktopBackendEnv, nimroManagedNodePathEntries, normalizeNimroHomeRoot } from './backend-env'
import { isReauthRequiredError, waitForNimroReady } from './backend-health'
import {
  canImportNimroCli,
  execProbeSync,
  PROBE_TIMEOUT_MS,
...
import {
  ...
  resolveVenvNimroCommand
} from './windows-nimro-path'
import {
...
const USER_DATA_OVERRIDE = process.env.NIMRO_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.NIMRO_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.NIMRO_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with NIMRO_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[nimro] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[nimro] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. NIMRO_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[nimro] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[nimro] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// never go through `nimro desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Nimro sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[nimro] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[nimro] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[nimro] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[nimro] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[nimro] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('nimro:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Nimro burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[nimro] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[nimro] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[nimro] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[nimro] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// NIMRO_HOME — the user-facing root for everything Nimro-related. Mirrors
// scripts/install.ps1's $NimroHome and scripts/install.sh's $NIMRO_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\nimro (matches install.ps1)
//   macOS / Linux: ~/.nimro (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.nimro directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\nimro yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// NIMRO_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// NIMRO_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.nimro / %LOCALAPPDATA%\nimro.
function resolveNimroHome() {
  if (process.env.NIMRO_HOME) {
    return normalizeNimroHomeRoot(process.env.NIMRO_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'nimro-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a NIMRO_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\nimro and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('NIMRO_HOME')

    if (fromRegistry) {
      return normalizeNimroHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'nimro')
    const legacy = path.join(app.getPath('home'), '.nimro')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.nimro setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.nimro')
}

const NIMRO_HOME = resolveNimroHome()

function pathWithNimroManagedNode(...entries) {
  const managed = nimroManagedNodePathEntries(NIMRO_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_NIMRO_ROOT — the canonical mutable Nimro install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_NIMRO_ROOT = path.join(NIMRO_HOME, 'nimro-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_NIMRO_ROOT, 'venv')
...
// boots, going straight to `resolveNimroBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_NIMRO_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_NIMRO_ROOT, '.nimro-bootstrap-complete')
...
// active-profile.json records which Nimro profile the desktop launches its
// local backend as. When set, startNimro() passes `nimro --profile <name>
// dashboard …`, which deterministically pins NIMRO_HOME (see
// _apply_profile_override in nimro_cli/main.py) and bypasses the sticky
// ~/.nimro/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors nimro_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// nimroDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under NIMRO_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by nimro_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(NIMRO_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (nimro_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.NIMRO_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.NIMRO_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.NIMRO_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.NIMRO_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.NIMRO_DESKTOP_APP_NAME || 'Nimro'
...
// 'nimro:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.nimro) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.nimro')
}

// Seed the native About panel with the live Nimro version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `nimro update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveNimroVersion(),
...
const MEDIA_PROTOCOL = 'nimro-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(nimro-media://…)`.
...
// startNimro(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.NIMRO_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.NIMRO_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startNimro() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning nimro serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Nimro backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (nimroLog.length > 300) {
    nimroLog.splice(0, nimroLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-nimro-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('nimro:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the nimro:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('nimro:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_NIMRO_ROOT
  })
}
...
// The Tauri updater writes NIMRO_HOME/.nimro-update-in-progress for the whole
...
// (`force_kill_other_nimro`, taskkill /IM nimro.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(NIMRO_HOME)),
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Nimro will start automatically when it completes…',
        12
      )
    },
...
function findOnPath(command) {
  ...
  // shell-script shim named `nimro` — must not shadow `nimro.cmd`/`nimro.exe`.
...
function unwrapWindowsVenvNimroCommand(command, backendArgs) {
  return resolveVenvNimroCommand(command, backendArgs, {
...
// spawns `nimro serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `nimro` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'nimro_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `nimro --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, NIMRO_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveNimroBackend step 4); execFileSync of a .cmd without
...
function isNimroSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'nimro_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.NIMRO_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Nimro deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the nimro-import
...
// findGitBash — locate bash.exe on Windows. Resolves NIMRO_GIT_BASH_PATH
...
// normal NIMRO_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'nimro', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'nimro', 'git', 'bin', 'git.exe'))
  }
...
function recentNimroLog() {
  return nimroLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's nimro root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_NIMRO_ROOT.
// NIMRO_DESKTOP_NIMRO_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.NIMRO_DESKTOP_NIMRO_ROOT && path.resolve(process.env.NIMRO_DESKTOP_NIMRO_ROOT),
    !IS_PACKAGED && isNimroSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isNimroSourceRoot(ACTIVE_NIMRO_ROOT) ? ACTIVE_NIMRO_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_NIMRO_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('nimro:updates:progress', merged)
  }
}
...
// Windows that binary owns ALL repo mutation — running `nimro update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(NIMRO_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `nimro update` can write
// fresh entry points. On Windows this is the file the running backend
// `nimro.exe` holds open; on POSIX it's never mandatory-locked.
function venvNimroShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'nimro.exe')
    : path.join(updateRoot, 'venv', 'bin', 'nimro')
}
...
// only signals the direct child, so on Windows a backend `nimro.exe` that
// spawned its own grandchildren (a `nimro` REPL, a pty terminal session, the
...
// `nimroProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `nimro.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a nimro
// REPL, a pty terminal, the gateway) keep `nimro.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  const nimroProcess = backendConnectionState.getProcess()

  if (nimroProcess && Number.isInteger(nimroProcess.pid)) {
    pids.push(nimroProcess.pid)
  }
  ...
  if (nimroProcess && !nimroProcess.killed) {
    try {
      nimroProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvNimroShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    const currentNimroProcess = backendConnectionState.getProcess()

    if (currentNimroProcess && Number.isInteger(currentNimroProcess.pid)) {
      stragglers.push(currentNimroProcess.pid)
    }
...
// `nimro update`). Instead we spawn the staged Nimro-Setup binary with
// --update and quit, so it can run `nimro update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux: never hand off, staged nimro-setup or not — the resolver
      ...
      // `nimro update` (backend) + `nimro desktop --build-only` (OS-aware GUI
    ...
    if (!updater) {
      ...
      // `nimro desktop`, never the Tauri installer that self-copies
      // nimro-setup.exe into NIMRO_HOME). They DO have a working `nimro`
      ...
      // checkout they're on — bare `nimro update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `nimro update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `nimro update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, nimroRoot: updateRoot }
    }

    const handoffConflict = updateHandoffConflict(NIMRO_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Nimro — this window will close and the updater will open. Don’t reopen Nimro yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(NIMRO_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // nimro.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running nimro, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Nimro install open ' +
        '(a second Nimro window or a terminal running nimro?). Close it and retry.'
    ...
    // Nimro processes running from this venv.  The updater normally refuses
    ...
    // `nimro update` will run (the venv shim is locked while we live).
    const child = spawnUpdaterProcess(updater, updaterArgs, {
      cwd: NIMRO_HOME,
      env: {
        ...process.env,
        NIMRO_HOME,
        PATH: pathWithNimroManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore'
    })
    ...
    // abort with "Another Nimro update is already running (PID <itself>)" —
    ...
    if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
      writeUpdateMarker(NIMRO_HOME, child.pid)
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvNimro = path.join(venvBin, IS_WINDOWS ? 'nimro.exe' : 'nimro')
  ...
  // not just the `nimro.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvNimro) || fileExists(path.join(updateRoot, '.nimro-bootstrap-complete'))
...
// Resolve the nimro CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `nimro` on PATH.
function resolveNimroCliBinary(updateRoot) {
  const venvNimro = path.join(updateRoot, 'venv', 'bin', 'nimro')

  if (fileExists(venvNimro)) {
    return venvNimro
  }

  return findOnPath('nimro') || null
}
...
// Python-level pre-update snapshot inside `nimro update`.
function preflightStateDb(nimroHome, rememberLog) {
  const stateDbPath = path.join(nimroHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(nimroHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(nimroHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(nimroHome, old))
            } catch {
              void 0
            }
          }
...
// macOS/Linux in-app update: backend (`nimro update`) + OS-aware GUI rebuild
// (`nimro desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const nimro = resolveNimroCliBinary(updateRoot)

  if (!nimro) {
    emitUpdateProgress({ stage: 'manual', message: 'nimro update', percent: null })

    return { ok: true, manual: true, command: 'nimro update', nimroRoot: updateRoot }
  }

  // ── Pre-flight state.db integrity guard (#68474) ──
  preflightStateDb(NIMRO_HOME, rememberLog)

  // Put the Nimro-managed Node and the venv on PATH so `nimro desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\\nimro\\node, not node\\bin.
  // PYTHONUNBUFFERED: `nimro update` writes to a pipe here, so CPython
  ...
  const env: Record<string, string> = {
    NIMRO_HOME,
    PYTHONUNBUFFERED: '1',
    PATH: pathWithNimroManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `nimro update` reaps stale `nimro serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads NIMRO_DESKTOP_CHILD_PID and excludes
  ...
  if (nimroProcess && Number.isInteger(nimroProcess.pid)) {
    desktopChildPids.push(nimroProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.NIMRO_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Nimro (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(nimro, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'nimro update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'nimro update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(nimro, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Nimro to retry.',
  ...
  // Linux in-app update terminal state (#45205). `nimro desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Nimro…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `nimro-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Nimro to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Nimro desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Nimro to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Nimro.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Nimro.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Nimro to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.nimro-update-new"; then
    rm -rf "$DST.nimro-update-old" 2>/dev/null || true
    mv "$DST" "$DST.nimro-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.nimro-update-new" "$DST"
    rm -rf "$DST.nimro-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Nimro to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_NIMRO_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isNimroSourceRoot(ACTIVE_NIMRO_ROOT) &&
    fileExists(venvPython) &&
    canImportNimroCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_NIMRO_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `nimro
...
function resolveWebDist() {
  const override = process.env.NIMRO_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set NIMRO_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: nimro desktop --force-build`
  )
...
function resolveNimroCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Nimro.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveNimroCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'nimro_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_NIMRO_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Nimro at ${ACTIVE_NIMRO_ROOT}`,
    command,
    args: ['-m', 'nimro_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      nimroHome: NIMRO_HOME,
      pythonPathEntries: [ACTIVE_NIMRO_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_NIMRO_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveNimroBackend(backendArgs) {
  // 1. Explicit override -- NIMRO_DESKTOP_NIMRO_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.NIMRO_DESKTOP_NIMRO_ROOT && path.resolve(process.env.NIMRO_DESKTOP_NIMRO_ROOT)

  if (overrideRoot && isNimroSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Nimro source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `nimro` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isNimroSourceRoot.)
  if (!IS_PACKAGED && isNimroSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Nimro source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_NIMRO_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\nimro\\nimro-agent (Windows) or ~/.nimro/nimro-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Nimro runtime at ${ACTIVE_NIMRO_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `nimro` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    NIMRO_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.NIMRO_DESKTOP_IGNORE_EXISTING !== '1') {
    let nimroCommand = null
    const nimroOverride = process.env.NIMRO_DESKTOP_NIMRO

    if (nimroOverride) {
      const resolvedOverride = findOnPath(nimroOverride)

      if (resolvedOverride) {
        nimroCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(nimroOverride, { isWsl: IS_WSL })) {
        nimroCommand = nimroOverride
      } else {
        rememberLog(`Ignoring Windows Nimro override under WSL: ${nimroOverride}`)
      }
    } else {
      nimroCommand = findOnPath('nimro')
    }

    if (nimroCommand) {
      if (looksLikeDesktopAppBinary(nimroCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Nimro CLI: ${nimroCommand}`)
        nimroCommand = null
      }
    }

    if (nimroCommand) {
      const unwrapped = unwrapWindowsVenvNimroCommand(nimroCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `nimro` shim
      ...
      const shellForProbe = isCommandScript(nimroCommand)

      // NIMRO_DESKTOP_NIMRO is an explicit deployment override (used by
      ...
      if (shouldTrustNimroOverride(nimroOverride) || verifyNimroCli(nimroCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvNimroCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Nimro CLI at ${nimroCommand}`,
          command: nimroCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Nimro CLI at ${nimroCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed nimro_cli module via system Python.
  ...
  if (python) {
    ...
    // having nimro_cli installed -- common on dev boxes that have
    ...
    if (canImportNimroCli(python)) {
      return {
        kind: 'python',
        label: `installed nimro_cli module via ${python}`,
        command: python,
        args: ['-m', 'nimro_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: nimro_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveNimroBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Nimro Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveNimroBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Nimro install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Nimro recovery was handed off to Nimro Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      nimroHome: NIMRO_HOME,
      logRoot: path.join(NIMRO_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Nimro install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Nimro bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(NIMRO_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startNimro() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // nimro:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveNimroBackend(backend.args))
  }
  ...
  if (!isNimroSourceRoot(ACTIVE_NIMRO_ROOT)) {
    throw new Error(
      `Nimro install at ${ACTIVE_NIMRO_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Nimro' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\nimro\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `nimro` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Nimro on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isNimroSourceRoot()
    // plus an importable nimro_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Nimro venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Nimro at ${ACTIVE_NIMRO_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Nimro runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----nimro-${crypto.randomBytes(12).toString('hex')}`
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Nimro backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Nimro backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Nimro backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Nimro-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('nimro:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveNimroCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('nimro:preview-file-changed', payload)
}
...
// waitForNimroReady how to read a 401 — rejected session vs gated route.
...
function sendBackendExit(payload) {
  ...
  webContents.send('nimro:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('nimro:close-preview-requested')
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('nimro:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Nimro...".
function sendPowerResume() {
  ...
  webContents.send('nimro:power-resume')
}
...
// 'nimro:power-battery' push below.
ipcMain.handle('nimro:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('nimro:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('nimro:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('nimro:window-state-changed', state)
}
...
//
// Hosted Nimro gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``nimro_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / nimro #37247).
//     Both are set as HttpOnly cookies (``nimro_session_at`` ~15 min,
//     ``nimro_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:nimro-remote-oauth'
...
// "not signed in", which on the initial boot path (startNimro → the renderer's
// single-shot boot() with no retry) surfaced as the "Nimro couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Nimro
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Nimro Cloud agent…' : 'Sign in to Nimro gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Nimro backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in nimro_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Nimro Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:nimro-remote-oauth`:
...
// convention (nimro_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Nimro surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.NIMRO_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Nimro gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for nimro_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Nimro Cloud sign-in.'))
...
// Discover the hosted (Nimro Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Nimro Cloud. Open Settings → Gateway, choose Nimro Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Nimro Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Nimro Cloud session has expired. Sign in to Nimro Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Nimro Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root NIMRO_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.NIMRO_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.NIMRO_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Nimro Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteNimroPath: (ssh || savedSsh)?.remoteNimroPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by NIMRO_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Nimro Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Nimro Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteNimroPath: input.sshRemoteNimroPath ?? existingBlock.remoteNimroPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (nimro #37247), and the gateway middleware transparently
    ...
    if (
      ...
      const err = new Error(
        'Remote Nimro gateway uses OAuth, but you are not signed in. ' +
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Nimro gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Nimro gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget() {
  ...
  if (process.env.NIMRO_DESKTOP_REMOTE_URL) {
    return null
  }
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  try {
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteNimroPath || '')
    ...
    result = await lifecycle({
      ...
      remoteNimroPath: sshConfig.remoteNimroPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForNimro: (baseUrl, token) => waitForNimro(baseUrl, token, lease.signal, 'token'),
  ...
  sshConnections.set(scope, {
    ...
    host: sshConfig.host,
    hostLabel,
    nimroVersion: result.nimroVersion || '',
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.nimroVersion || 'nimro (version unknown)'} at ${result.nimroPath || '?'}`
  )
  ...
  return { ...connection, remoteNimroVersion: result.nimroVersion || '' }
}
...
//   2. env override (NIMRO_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.NIMRO_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.NIMRO_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'NIMRO_DESKTOP_REMOTE_URL is set but NIMRO_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Nimro backend.'
      )
    }
...
// env → global) so a startNimro() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Nimro
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteNimroPath: input.sshRemoteNimroPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteNimroPath || '')
          let nimroPath
          let nimroVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            nimroPath = runtime.nimroPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.nimroPath])
            nimroVersion = inspection.version
            supported = inspection.supported
          } else {
            nimroPath = await remoteLifecycle.locateNimro(ssh, sshConfig.remoteNimroPath || '')
            nimroVersion = await remoteLifecycle.probeNimroVersion(ssh, nimroPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, nimroPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Nimro on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteNimroPath: nimroPath,
            remoteNimroVersion: nimroVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startNimro())
  ...
  // connect to Nimro gateway". Mirror the renderer's connect here so the test
...
function resetNimroConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const nimroProcess = backendConnectionState.invalidate()
  stopBackendChild(nimroProcess)
...
// startNimro() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetNimroConnection() invalidates it.
  const nimroProcess = backendConnectionState.getProcess()
  const dying = nimroProcess && !nimroProcess.killed ? nimroProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('nimro:connection:applied')
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startNimro()
...
// local-spawn portion of startNimro() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForNimro(remote.baseUrl, remote.token, undefined, remote.authMode)
  ...
  // --profile wins over the inherited NIMRO_HOME env (see _apply_profile_override
  // step 3 in nimro_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveNimroBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const nimroCwd = resolveNimroCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Nimro backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: nimroCwd,
        NIMRO_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        NIMRO_DESKTOP: '1',
        NIMRO_WEB_DIST: webDist,
        ...(readyFile ? { NIMRO_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Nimro backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Nimro backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Nimro backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForNimro(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Nimro backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Nimro backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_nimro_home() recreates the deleted profile directory.
//
...
async function startNimro() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startNimro() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Nimro backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      await advanceBootProgress('backend.remote', `Connecting to remote Nimro backend at ${remote.baseUrl}`, 24)
      await waitForNimro(remote.baseUrl, remote.token, undefined, remote.authMode)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Nimro backend is ready',
        progress: 94,
        running: true,
        error: null
      })
      ...
      return {
        ...
        remoteNimroVersion: remote.remoteNimroVersion,
        token: remote.token,
        wsUrl: remote.wsUrl,
        logs: nimroLog.slice(-80),
        ...getWindowState()
    ...
    // deterministic (it wins over the sticky ~/.nimro/active_profile file) and
    // resolves NIMRO_HOME the same way `nimro -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Nimro runtime', 28)

        return resolveNimroBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Nimro backend via ${backend.label}`, 84)
    rememberLog(`Starting Nimro backend via ${backend.label}`)

    const nimroProcess = spawn(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: nimroCwd,
        env: {
          ...process.env,
          // Explicitly pin NIMRO_HOME for the child so Python's get_nimro_home()
          // resolves to the SAME location our resolveNimroHome() picked. Without
          // this pin, Python falls back to ~/.nimro on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\nimro, which differs from C:\Users\<u>\.nimro.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets NIMRO_HOME via setx; the desktop
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, nimroProcess)

    if (!processOwner) {
      stopBackendChild(nimroProcess)
      throw new Error('Nimro backend start was superseded by a newer connection attempt.')
    }

    nimroProcess.stdout.on('data', rememberLog)
    nimroProcess.stderr.on('data', rememberLog)
    ...
    nimroProcess.once('error', error => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Nimro backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Nimro backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Nimro backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Nimro backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    nimroProcess.once('exit', (code, signal) => {
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Nimro backend exit (${signal || code})`)
      ...
      rememberLog(`Nimro backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Nimro backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Nimro backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentNimroLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Nimro backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(nimroProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Nimro backend to become ready', 90)
    await Promise.race([waitForNimro(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => nimroProcess.exitCode === null && !nimroProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Nimro backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Nimro backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Nimro is
...
// pushes pet state over IPC (nimro:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via nimro:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Nimro app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Nimro icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see nimro:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Nimro in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('nimro:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('nimro:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('nimro:quick-entry:shown')
}
...
function createWindow() {
  ...
  startNimro().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('nimro:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Nimro…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('nimro:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetNimroConnection,
...
ipcMain.handle('nimro:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('nimro:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('nimro:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('nimro:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('nimro:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('nimro:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('nimro:zoom:get', event => {
...
ipcMain.on('nimro:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('nimro:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('nimro:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('nimro:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('nimro:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('nimro:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('nimro:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('nimro:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('nimro:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('nimro:pet-overlay:control', payload)
})
ipcMain.handle('nimro:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startNimro() call restarts the
...
ipcMain.handle('nimro:bootstrap:repair', async () => {
  // Forceful repair: force the next startNimro() through the full installer
  ...
  // "bypass active runtime" path inside startNimro() and fall through
...
ipcMain.handle('nimro:boot-progress:get', async () => bootProgressState)
ipcMain.handle('nimro:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('nimro:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('nimro:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('nimro:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('nimro:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('nimro:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('nimro:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startNimro() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('nimro:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Nimro Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('nimro:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('nimro:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('nimro:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('nimro:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('nimro:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new NIMRO_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('nimro:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('nimro:requestMicrophoneAccess', async () => {
...
ipcMain.handle('nimro:api', async (_event, request) => {
  ...
  // backend calls ensure_nimro_home() which recreates the profile directory,
...
ipcMain.handle('nimro:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('nimro:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('nimro:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('nimro:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('nimro:data-url-read-max:get', () => ({
...
ipcMain.handle('nimro:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('nimro:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('nimro:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('nimro:readFileText', async (_event, filePath) => {
...
ipcMain.handle('nimro:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('nimro:writeClipboard', (_event, text) => {
...
ipcMain.handle('nimro:readClipboard', () => clipboard.readText())

ipcMain.handle('nimro:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('nimro:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('nimro:saveClipboardImage', async () => {
...
ipcMain.handle('nimro:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('nimro:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('nimro:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('nimro:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('nimro:active-work', (event, payload) => {
...
ipcMain.on('nimro:titlebar-theme', (_event, payload) => {
...
ipcMain.on('nimro:native-theme', (_event, mode) => {
...
ipcMain.on('nimro:translucency', (_event, payload) => {
...
ipcMain.on('nimro:keep-awake', (_event, on) => {
...
ipcMain.handle('nimro:quick-entry:settings:get', async () => {
...
ipcMain.handle('nimro:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('nimro:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('nimro:quick-entry:submit', {
...
ipcMain.on('nimro:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('nimro:quick-entry:state', payload)
  }
})

ipcMain.on('nimro:quick-entry:dismiss', () => hideQuickEntryWindow())

ipcMain.handle('nimro:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `nimro:window:openSession`).
...
ipcMain.handle('nimro:find-in-page', (event, query, options) => {
...
ipcMain.handle('nimro:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('nimro:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveNimroCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('nimro:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveNimroCwd()
}))

ipcMain.handle('nimro:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('nimro:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('nimro:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('nimro:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('nimro:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('nimro:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: nimroLog.slice(-200) }))
...
function terminalShellCommand() {
  // NIMRO_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.NIMRO_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Nimro' TUI into
  ...
  env.TERM_PROGRAM = 'Nimro'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a nimro/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from NIMRO_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.NIMRO_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `nimro:terminal:${id}:${suffix}`
}
...
ipcMain.handle('nimro:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('nimro:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('nimro:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('nimro:fs:openDir', async (_event, dirPath) => {
...
// The LOCAL Desktop runtime-plugin root: `<NIMRO_HOME>/desktop-plugins`,
// resolved from the main-process NIMRO_HOME (see resolveNimroHome) — NOT from
// the connected backend. A remote backend reports its own `nimro_home` over
...
ipcMain.handle('nimro:fs:desktopPluginsRoot', async () => {
  // Profile-aware: a named Desktop profile gets its own plugin root under
  // profiles/<name>/, matching the profile-scoped nimro_home the backend
  // reported before this resolver existed. 'default'/unset pins the global root.
  const profile = readActiveDesktopProfile()
  const base = profile && profile !== 'default' ? path.join(NIMRO_HOME, 'profiles', profile) : NIMRO_HOME
...
ipcMain.handle('nimro:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('nimro:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('nimro:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('nimro:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('nimro:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('nimro:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('nimro:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('nimro:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('nimro:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('nimro:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('nimro:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('nimro:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('nimro:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('nimro:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('nimro:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('nimro:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('nimro:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('nimro:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('nimro:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('nimro:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('nimro:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('nimro:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('nimro:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('nimro:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('nimro:terminal:write', (_event, id, data) => {
...
ipcMain.handle('nimro:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('nimro:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('nimro:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('nimro:updates:check', async () =>
...
ipcMain.handle('nimro:updates:apply', async (_event, payload) =>
...
ipcMain.handle('nimro:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('nimro:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Nimro version (the one `release.py` bumps in
// nimro_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Nimro version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveNimroVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'nimro_cli', '__init__.py')
...
// Re-resolve the live Nimro version and push it into the native About panel
// just before showing it, so an in-place `nimro update` is reflected without
...
ipcMain.handle('nimro:version', async () => ({
  appVersion: resolveNimroVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  nimroRoot: resolveUpdateRoot()
}))
...
// the actual removal via `nimro uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (nimro_cli/uninstall.py + nimro_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_NIMRO_ROOT
  ...
  const fallback = () => ({
    nimro_home: NIMRO_HOME,
    agent_installed: isNimroSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'nimro_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, NIMRO_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Nimro agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import nimro_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_NIMRO_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `nimro-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `nimro-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('nimro:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('nimro:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('nimro:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('nimro:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// nimro:// deep links (e.g. nimro://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const NIMRO_PROTOCOL = 'nimro'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${NIMRO_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // nimro://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('nimro:deep-link', payload)
...
ipcMain.handle('nimro:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${NIMRO_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(NIMRO_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(NIMRO_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `nimro://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching nimro:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Nimro never keeps another app's chord hostage.
