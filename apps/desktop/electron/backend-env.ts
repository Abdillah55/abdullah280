...
/**
 * Nimro-managed Node.js directories, in preferred lookup order.
 *
 ...
 * `$NIMRO_HOME/node/bin`. Emit BOTH on every platform so mixed and migrated
 ...
 * `iter_nimro_node_dirs()` in nimro_constants.py, which the Electron main
 * process cannot import.
 */
function nimroManagedNodePathEntries(
  nimroHome,
  ...
  const root = pathModule.join(nimroHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const nimroNodeDirs = nimroManagedNodePathEntries(nimroHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([nimroNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeNimroHomeRoot(nimroHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!nimroHome) {
    return nimroHome
  }

  const resolved = pathModule.resolve(String(nimroHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). nimro_bootstrap sets this inside the child too, but only
