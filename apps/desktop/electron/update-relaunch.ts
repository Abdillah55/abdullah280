 ...
 * Background
 * ----------
 * After `nimro update` + `nimro desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything NIMRO_DESKTOP_* is preserved
// plus NIMRO_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['NIMRO_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['NIMRO_DESKTOP_']
