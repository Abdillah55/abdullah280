...
test('stagedUpdaterSupportsPrewrittenMarker rejects installers predating the self-adopt fix', () => {
  ...
  // refreshed because copy_self_to_nimro_home no-ops during --update.
  assert.equal(
    stagedUpdaterSupportsPrewrittenMarker('C:\\Nimro\\nimro-setup.exe', {
...
test('resolveStagedUpdaterBinary still returns a stale staged updater on Windows', () => {
  ...
  assert.equal(
    resolveStagedUpdaterBinary('C:\\Nimro', {
      fileExists: () => true,
      isWindows: true,
      stagedMtimeMs: () => MARKER_SELF_ADOPT_EPOCH_MS - 60 * DAY_MS
    }),
    path.join('C:\\Nimro', 'nimro-setup.exe')
  )
})
...
test('spawnUpdaterProcess hides the updater console and detaches the child on Windows', () => {
  ...
  const result = spawnUpdaterProcess(
    'nimro-setup.exe',
    ['--update', '--branch', 'main'],
    { cwd: 'C:\\Nimro', detached: true, stdio: 'ignore' },
    {
      isWindows: true,
  ...
  assert.deepEqual(calls, [
    {
      ...
      command: 'nimro-setup.exe',
      options: { cwd: 'C:\\Nimro', detached: true, stdio: 'ignore', windowsHide: true }
    }
  ])
})
...
test('resolveStagedUpdaterBinary hands Windows the staged installer it finds', () => {
  const home = 'C:\\Users\\nimro\\AppData\\Local\\nimro'
  const staged = path.join(home, 'nimro-setup.exe')
  const probed: string[] = []
...
test('resolveStagedUpdaterBinary returns null off Windows even when nimro-setup is staged (#74836)', () => {
  const home = '/Users/nimro/.nimro'
  let probes = 0

  const resolved = resolveStagedUpdaterBinary(home, {
    // The installer stages nimro-setup on macOS/Linux too, so "it exists" is
