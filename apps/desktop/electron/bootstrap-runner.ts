 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Nimro Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.nimro-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(nimroHome) {
  return path.join(nimroHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.nimro/nimro-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(nimroHome) {
  if (!nimroHome) {
    return null
  }

  const candidate = path.join(nimroHome, 'nimro-agent', 'scripts', installScriptName())
...
function cachedScriptPath(nimroHome, commit) {
  return path.join(bootstrapCacheDir(nimroHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/NousResearch/nimro-agent/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(nimroHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(nimroHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, nimroHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass NIMRO_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          NIMRO_HOME: nimroHome || process.env.NIMRO_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, nimroHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, nimroHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--nimro-home', nimroHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, nimroHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, nimroHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, nimroHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(nimroHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, nimroHome, emit })
