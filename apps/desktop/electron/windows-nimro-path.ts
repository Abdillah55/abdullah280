/**
 * windows-nimro-path.ts
 *
 * Pure, dependency-injected pieces of Windows `nimro` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvNimroCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `nimro` shim shadowed the real
 *      nimro.cmd/nimro.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\nimro.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvNimroCommand() — unwrapWindowsVenvNimroCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `nimro`) must
 * not shadow `nimro.cmd`/`nimro.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv nimro shim, bootstrap-complete
 * marker) — gating on just the nimro.exe console-script shim alone is the
...
export interface ResolveVenvNimroCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportNimroCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `nimro`/`nimro.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/nimro(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m nimro_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportNimroCli(). A
 * venv whose update died mid-`pip install` still has python.exe + nimro.exe
 ...
 *
 * Returns null when `command` is not a venv nimro shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvNimroCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvNimroCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^nimro(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Nimro at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Nimro Python at ${python}`,
    command: python,
