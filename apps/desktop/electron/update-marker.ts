...
export function markerPath(nimroHome) {
  return path.join(nimroHome, '.nimro-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(nimroHome)
  let raw
 ...
 *
 * The Tauri-based nimro-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(nimroHome, pid, { now = Date.now } = {}) {
  const file = markerPath(nimroHome)
 ...
 * Nimro to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(nimroHome, opts)
