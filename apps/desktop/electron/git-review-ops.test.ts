...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nimro-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'nimro-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Nimro Test'], { cwd: dir })
