...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'nimro_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'nimro_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'nimro_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Nimro.app/Contents/MacOS/Nimro', 'darwin'),
    '/Applications/Nimro.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Nimro.app/Contents/MacOS/Nimro', 'darwin'),
    '/Users/x/Applications/Nimro.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Nimro\\Nimro.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Nimro'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\nimro-desktop\\Nimro.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\nimro-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Nimro.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_NimroXXXX/nimro', 'linux', { APPIMAGE: '/home/x/Apps/Nimro.AppImage' }),
    '/home/x/Apps/Nimro.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/nimro/linux-unpacked/nimro', 'linux', {}), '/opt/nimro/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/nimro', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Nimro.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Nimro.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.nimro/nimro-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.nimro/nimro-agent',
    uninstallArgs: ['-m', 'nimro_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/nimro/linux-unpacked',
    nimroHome: '/home/x/.nimro'
  })
  ...
  assert.match(script, /'-m' 'nimro_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/nimro\/linux-unpacked'/)
  assert.match(script, /export NIMRO_HOME='\/home\/x\/\.nimro'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.nimro/nimro-agent',
    agentRoot: '/home/x/.nimro/nimro-agent',
    uninstallArgs: ['-m', 'nimro_cli.uninstall', '--mode', 'full'],
    appPath: null,
    nimroHome: '/home/x/.nimro'
  })

  // System python + source on PYTHONPATH so import nimro_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.nimro\/nimro-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'nimro_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'nimro_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    nimroHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'nimro_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\nimro',
    uninstallArgs: ['-m', 'nimro_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Nimro',
    nimroHome: 'C:\\Users\\x\\AppData\\Local\\nimro'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import nimro_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\nimro;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "nimro_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Nimro" >nul 2>&1/)
