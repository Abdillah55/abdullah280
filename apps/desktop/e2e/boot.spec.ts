 ...
 * packaged binary) with a real `nimro serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → nimro serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Nimro title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Nimro')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → nimro serve is spawned → WS connects → config
