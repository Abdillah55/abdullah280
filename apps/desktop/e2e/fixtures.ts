/**
 * Shared E2E fixtures for the Nimro desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → nimro serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `nimro desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `nimro-e2e-${prefix}-${Math.random()}`))
  const nimroHome = path.join(root, 'nimro-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(nimroHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(nimroHome, 'config.yaml')
 ...
 */
export function writeEnvFile(nimroHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(nimroHome, '.env')
 ...
 */
function writeEmptyConfig(nimroHome: string): void {
  const configPath = path.join(nimroHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - NIMRO_HOME → sandbox nimro-home (isolated config/sessions)
 *  - NIMRO_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - NIMRO_DESKTOP_IGNORE_EXISTING=1 → don't pick up `nimro` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - NIMRO_DESKTOP_NIMRO_ROOT → REPO_ROOT (dev checkout resolution)
 *  - NIMRO_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    NIMRO_HOME: sandbox.nimroHome,
    NIMRO_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    NIMRO_DESKTOP_IGNORE_EXISTING: '1',
    NIMRO_DESKTOP_NIMRO_ROOT: REPO_ROOT,
    NIMRO_DESKTOP_APP_NAME: `NimroE2E-${Date.now()}`,
    ...
    NIMRO_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated NIMRO_HOME + userData
 * @param env      - the process environment (already has NIMRO_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.nimroHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.nimroHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via NIMRO_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`nimro serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.nimroHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { NIMRO_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Nimro backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Nimro.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Nimro.app', 'Contents', 'MacOS', 'Nimro')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'nimro')
}
 ...
 * progress without spawning a real Nimro backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * NIMRO_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    NIMRO_DESKTOP_BOOT_FAKE: '1',
    NIMRO_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + nimro-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).NIMRO_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).NIMRO_DESKTOP_NIMRO
  delete (env as Record<string, string | undefined>).NIMRO_DESKTOP_NIMRO_ROOT
