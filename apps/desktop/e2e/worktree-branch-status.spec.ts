...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Nimro E2E'], { cwd: repo })
...
function configureRepoCwd(nimroHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(nimroHome, mockUrl)
  fs.appendFileSync(path.join(nimroHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(nimroHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.nimroHome, mock.url, repo)
