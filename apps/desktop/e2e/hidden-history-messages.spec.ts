...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.nimroHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.nimroHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.nimroHome)
  const builder = await RealSessionBuilder.start(sandbox.nimroHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.nimroHome, mock.url)
  fs.appendFileSync(path.join(sandbox.nimroHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.nimroHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
