...
How to build Nimro Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Nimro surface can also
...
- A **runtime home change** (switching the underlying `NIMRO_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Nimro — do not assume
