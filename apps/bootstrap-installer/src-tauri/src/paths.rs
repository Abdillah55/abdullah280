...
//!
//! Mirrors `nimro_constants.get_nimro_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\nimro
//!   macOS:   ~/.nimro
//!   Linux:   ~/.nimro  (override via $NIMRO_HOME)
//!
//! NOTE (macOS): Python's get_nimro_home(), scripts/install.sh, and the
//! Electron desktop's resolveNimroHome() ALL use ~/.nimro on macOS — there
...
/// Returns the canonical Nimro home directory, respecting $NIMRO_HOME if set.
pub fn nimro_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("NIMRO_HOME") {
    ...
    {
        // %LOCALAPPDATA%\nimro — matches scripts/install.ps1's $NimroHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("nimro");
        }
    }

    // macOS + Linux + fallback: ~/.nimro (matches Python get_nimro_home(),
    // install.sh, and the Electron desktop's resolveNimroHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".nimro");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    nimro_home().join("bootstrap-cache")
}
...
/// NIMRO_HOME so it survives repo checkout deletion (unlike anything under
/// nimro-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\nimro\nimro-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_nimro`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under NIMRO_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves NIMRO_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    nimro_home().join(".nimro-update-in-progress")
}
...
pub fn copy_self_to_nimro_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to NIMRO_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_NIMRO_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".nimro-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under NIMRO_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[nimro-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("NIMRO_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_nimro_home() -> String {
    nimro_home().to_string_lossy().into_owned()
}
