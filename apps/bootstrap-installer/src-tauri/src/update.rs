...
//!
//! Driven when the installer is launched as `Nimro-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Nimro desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `nimro update`
//!      or repair bootstrap can race locked files),
//!   2. run `nimro update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in nimro_cli/main.py),
//!   3. run `nimro desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `nimro update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_nimro) and
...
/// `nimro update` exit code meaning "another nimro process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// nimro_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `nimro update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `nimro update` claims
/// the same file (see `nimro_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `nimro update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in nimro_cli/update_lock.py — all three read
...
/// owner that is itself and aborts ("Another Nimro update is already
/// running"), then the desktop relaunches and retries forever. A foreign live
/// pid (e.g. a dashboard-spawned `nimro update`) still blocks.
fn live_marker_owner(path: &Path) -> Option<MarkerOwner> {
    ...
    // Desktop `writeUpdateMarker(nimroHome, child.pid)` races ahead of us;
...
async fn run_update(app: AppHandle) -> Result<()> {
    let nimro_home = crate::paths::nimro_home();
    let install_root = nimro_home.join("nimro-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_nimro`
    ...
    //
    // The same marker is the cross-process update lock (nimro_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `nimro update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Nimro update is already running (PID {}, started {} ago). \
    ...
    let nimro = resolve_nimro(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the nimro CLI under {}. Is Nimro installed? \
    ...
    // async on Windows. If it still holds the venv shim, `nimro update`
    ...
    // ---- stage 2: nimro update -----------------------------------------
    // Pass --branch so `nimro update` targets the branch this installer was
    ...
    // without --branch, `nimro update` switches the checkout to `main` (a
    ...
    // --force skips `nimro update`'s Windows running-exe guard (which would
    ...
    // time `nimro update` runs there is no legitimate nimro.exe to protect,
    // and the guard would only produce a false "Nimro is still running" stop.
    //
    ...
    // probe only checks the nimro.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Nimro windows" message.
    ...
    // Retry-once for the update-boundary crash. `nimro update` lazily imports
    ...
    // second `nimro update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Nimro" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Nimro is still running. Close all Nimro windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "nimro update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::nimro_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: nimro desktop --build-only ----------------------------
    // `nimro update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `nimro update` already does above, and `nimro update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `nimro desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Nimro manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_nimro_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Nimro to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend nimro.exe (or the desktop Nimro.exe
            ...
            // force-kill every Nimro.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Nimro still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_nimro();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_nimro(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Nimro.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Nimro.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `nimro.exe` other than this process. Windows-only; a no-op
...
/// Spawn `nimro <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv nimro shim under an install root, regardless of existence.
fn venv_nimro(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("nimro.exe")
    } else {
        install_root.join("venv").join("bin").join("nimro")
    }
}

/// Resolve the nimro CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `nimro` on PATH.
fn resolve_nimro(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_nimro(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "nimro.exe" } else { "nimro" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let nimro_home = crate::paths::nimro_home();
    let mut envs = vec![(
        "NIMRO_HOME".to_string(),
        nimro_home.as_os_str().to_os_string(),
    )];
    // `nimro update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `nimro update` child claims that SAME lock (nimro_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Nimro is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in nimro_cli/update_lock.py.
    envs.push((
        "NIMRO_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        nimro_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_nimro_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Nimro.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.nimro-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.nimro-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::nimro_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_nimro_is_under_install_root() {
        let root = Path::new("/x/nimro-agent");
        let shim = venv_nimro(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/nimro-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/nimro-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "NIMRO_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the nimro update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_nimro(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/nimro-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".nimro-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `nimro update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(nimroHome, child.pid) races ahead
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Nimro.app"]),
            Some(PathBuf::from("/Applications/Nimro.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Nimro.app");
        let tmp = base.join("Nimro.app.nimro-update-new");
        let old = base.join("Nimro.app.nimro-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Nimro.app");
        let tmp = base.join("Nimro.app.nimro-update-new"); // intentionally absent
        let old = base.join("Nimro.app.nimro-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Nimro.app");
        let tmp = base.join("Nimro.app.nimro-update-new"); // absent
        let old = base.join("Nimro.app.nimro-update-old");
        write_marker(&target, "OLD");
