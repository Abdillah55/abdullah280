...
export const $nimroHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.nimro/logs/bootstrap-installer.log')
    $nimroHome.set('~/.nimro')
  ...
  try {
    const [logPath, nimroHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_nimro_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $nimroHome.set(nimroHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Nimro" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Nimro-Setup.exe --update);
...
export async function launchNimroDesktop(): Promise<void> {
  ...
  await invoke('launch_nimro_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Nimro repository' },
