"""Centralized logging setup for Nimro Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.nimro/logs/`` (profile-aware via ``get_nimro_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``nimro_cli.web_server``,
    ``nimro_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Nimro (TUI, gateway, ``hy_memory`` server, MCP
...
from nimro_constants import get_config_path, get_nimro_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_nimro_session_injector", False):
    ...
    _session_record_factory._nimro_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``nimro logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``nimro logs --component gateway``.
    "gateway": ("gateway", "nimro_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("nimro_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "nimro_cli.web_server",
        "nimro_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Nimro logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    nimro_home
        Override for the Nimro home directory.  Falls back to
        ``get_nimro_home()`` (profile-aware).
    log_level
    ...
    home = nimro_home or get_nimro_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_nimro_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._nimro_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the nimro group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from nimro_cli.config import is_managed
...
# the module header): when several Nimro processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._nimro_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="nimro-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_nimro_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse nimro_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when nimro_cli.config isn't importable (bare
        # nimro_logging consumers).
        try:
            from nimro_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
