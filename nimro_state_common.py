...
Extracted verbatim from nimro_state.py so the SessionDB mixin modules
(nimro_state_search / nimro_state_schema / nimro_state_portability) can
reference them without importing nimro_state (which would be a cycle).
nimro_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``nimro sessions optimize-storage``. A legacy DB sits at
...
# `nimro sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `nimro db optimize`. This is
