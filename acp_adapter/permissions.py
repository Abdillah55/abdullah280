"""ACP permission bridging for Nimro dangerous-command approvals."""
...
# Maps ACP permission option ids to Nimro approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, smart_denied: bool = False,
    ...
    """Return ACP options that match Nimro approval semantics."""
    ...
    if not smart_denied:
        options.append(PermissionOption(
            ...
            # hint while keeping Nimro semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_nimro(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Nimro approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_NIMRO.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Nimro-compatible approval callback that bridges to ACP.
