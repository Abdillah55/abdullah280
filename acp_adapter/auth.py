"""ACP auth helpers — detect and advertise Nimro authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "nimro-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Nimro runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Nimro can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Nimro.
    ...
    not have Nimro provider credentials configured yet, so Nimro always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Nimro using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Nimro provider",
            description=(
                "Open Nimro' interactive model/provider setup in a terminal. "
                "Use this when Nimro has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
