"""ACP agent server — exposes Nimro Agent via the Agent Client Protocol."""
...
from tools.approval import (
    reset_nimro_interactive_context,
    set_nimro_interactive_context,
)
...
try:
    from nimro_cli import __version__ as NIMRO_VERSION
except Exception:
    NIMRO_VERSION = "0.0.0"
...
# already uses (`nimro_cli/moa_cmd.py`). This bounds each provider's row, not
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Nimro running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Nimro cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Nimro/OpenAI-compatible user content payload."""
...
class NimroACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Nimro AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Nimro version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Nimro maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Nimro inventory is also used by ``nimro model``, the TUI,
        ...
        try:
            ...
            from nimro_cli.models import normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from nimro_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Nimro estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_nimro_session_id: str,
        previous_nimro_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.nimro.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_nimro_session_id,
                previous_nimro_session_id=previous_nimro_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_nimro_session_id: Optional[str] = None,
        previous_nimro_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Nimro changes it.

        When the internal Nimro head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_nimro_session_id`` so the
        attached ``_meta.nimro.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # nimro_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_nimro_session_id or session_id,
            previous_nimro_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["nimro-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        """
        try:
            from nimro_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from nimro_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="nimro-agent", version=NIMRO_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Nimro' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Nimro setup/model selection out-of-band.
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.nimro`` (ACP's extensibility
        ...
        * ``containsCompactionSummary: true`` — a merged-tail message: real
        ...
        if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
            ...
            kind = "standalone"
        if kind == "standalone":
            return {"nimro": {"compactionSummary": True}}
        if kind == "merged":
            return {"nimro": {"containsCompactionSummary": True}}
        return None
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Nimro
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Nimro on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Nimro' local kawaii waiting/status
        ...
        # tools.approval (set_nimro_interactive_context) rather than
        # os.environ["NIMRO_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue NIMRO_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind NIMRO_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Without it the prompt advertises the global Nimro workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.nimro/workspace and the
                # edit silently lands outside the editor's workspace.
                # cron_session="" explicitly marks this as a non-cron context,
                # masking any leaked process-global NIMRO_CRON_SESSION (#37968).
            ...
            interactive_token = set_nimro_interactive_context(True)
            ...
            previous_session_id = os.environ.get("NIMRO_SESSION_ID")
            os.environ["NIMRO_SESSION_ID"] = session_id
            try:
                ...
                if interactive_token is not None:
                    reset_nimro_interactive_context(interactive_token)
                # Restore NIMRO_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("NIMRO_SESSION_ID", None)
                else:
                    os.environ["NIMRO_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Nimro DB session id before the turn so we
            ...
            pre_turn_nimro_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (NIMRO_SESSION_KEY in
        ...
        # _meta.nimro.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_nimro_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_nimro_id
            and pre_turn_nimro_id
            and post_turn_nimro_id != pre_turn_nimro_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_nimro_session_id=post_turn_nimro_id,
                    previous_nimro_session_id=pre_turn_nimro_id,
                )
            except Exception:
                logger.debug(
        ...
        # Nimro' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Nimro install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Nimro Agent v{NIMRO_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Nimro has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
