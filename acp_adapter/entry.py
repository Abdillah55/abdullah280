"""CLI entry point for the nimro-agent ACP adapter.

Loads environment variables from ``~/.nimro/.env``, configures logging
...
try:
    ...
    # shadowing Nimro's own modules — ``nimro acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from NIMRO_HOME (default ``~/.nimro``)."""
    from nimro_cli.env_loader import load_nimro_dotenv

    nimro_home = get_nimro_home()
    loaded = load_nimro_dotenv(nimro_home=nimro_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", nimro_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="nimro-acp",
        description="Run Nimro Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Nimro version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Nimro provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.nimro/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import NimroACPAgent  # noqa: F401

    print("Nimro ACP check OK")


def _run_setup() -> None:
    from nimro_cli.main import main as nimro_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "nimro", "model"]
        nimro_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting nimro-agent ACP adapter")
    ...
    import acp
    from .server import NimroACPAgent
    ...
    if os.environ.get("NIMRO_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
